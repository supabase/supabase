...
function normalizeOperatorInput(value: string) {
  return value.trim().toLowerCase()
}

function getOperatorValue(operator: string | FilterOperatorObject) {
  return isFilterOperatorObject(operator) ? operator.value : operator
}

function getOperatorLabel(operator: string | FilterOperatorObject) {
  return isFilterOperatorObject(operator) ? operator.label : operator
}

export function matchesOperatorInput(
  value: string,
  property?: FilterProperty,
  options: { exact?: boolean } = {}
): boolean {
  const normalized = normalizeOperatorInput(value)
  if (!normalized) return false

  const { exact = false } = options
  const availableOperators = property?.operators ?? ['=']

  return availableOperators.some((operator) => {
    const operatorValue = getOperatorValue(operator).toLowerCase()
    const operatorLabel = getOperatorLabel(operator).toLowerCase()

    return exact
      ? operatorValue === normalized || operatorLabel === normalized
      : operatorValue.includes(normalized) || operatorLabel.includes(normalized)
  })
}

export function resolveOperatorValue(value: string, property?: FilterProperty): string | null {
  const normalized = normalizeOperatorInput(value)
  if (!normalized) return null

  const availableOperators = property?.operators ?? ['=']
  const matchedOperator = availableOperators.find((operator) => {
    const operatorValue = getOperatorValue(operator).toLowerCase()
    const operatorLabel = getOperatorLabel(operator).toLowerCase()
    return operatorValue === normalized || operatorLabel === normalized
  })

  return matchedOperator ? getOperatorValue(matchedOperator) : null
}

export function resolveDefaultOperatorFromValue(
  value: string,
  property?: FilterProperty
  ...
  if (lower === 'null' && matchesOperatorInput('is', property, { exact: true })) {
    return { operator: 'is', value: 'null' }
  }

  if (lower === 'not null' && matchesOperatorInput('is', property, { exact: true })) {
    return { operator: 'is', value: 'not null' }
  }

  if (matchesOperatorInput('=', property, { exact: true })) {
