...
export class SqlParser {
  ...
  private extractTables(): string[] {
    ...
    const fromPattern = /\bFROM\s+((?:"[^"]+"|'[^']+'|`[^`]+`|[^\s,()]+)(?:\.(?:"[^"]+"|'[^']+'|`[^`]+`|[^\s,()]+))?)/gi
    ...
    // JOIN clause tables
    const joinMatches = sql.match(/\b(?:INNER\s+|LEFT\s+|RIGHT\s+|FULL\s+)?JOIN\s+([^\s,()]+(?:\.[^\s,()]+)?)/gi)
    if (joinMatches) {
  ...
  private extractSchemas(): string[] {
    ...
    // Look for schema.table patterns in FROM and JOIN clauses specifically
  ...
  private analyzeQuery(): {
    ...
    const hasSubqueries = sql.includes('(') && (
