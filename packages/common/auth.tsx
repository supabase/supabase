...
export const AuthProvider = ({
  ...
  const syncAuthState = useCallback((session: Session | null, error: AuthError | null = null) => {
    setState((prev) => ({
      session,
      // If there is a session, we clear the error
      error: session !== null ? null : error ?? prev.error,
      isLoading: false,
    }))
  }, [])
  ...
  useEffect(() => {
    let mounted = true
    gotrueClient.initialize().then(async ({ error }) => {
      if (!mounted) return

      if (error !== null) {
        syncAuthState(null, error)
        return
      }
      ...
      if (mounted) {
        syncAuthState(session, sessionError)
      }
    })
