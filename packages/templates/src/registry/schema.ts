...
export const REGISTRY_HOMEPAGE =
  'https://github.com/supabase/supabase/tree/master/packages/templates'
/** GitHub slug used in registryDependencies for same-repo installs. */
export const REGISTRY_GITHUB_SLUG = 'supabase/supabase'
...
export function registryItemToTemplateSummary(item: RegistryItem): TemplateSummary {
  ...
  return parseTemplateSummary({
    ...
    tags: meta.tags,
    dependencies: meta.dependencies ?? registryDependenciesToTemplateDependencies(item),
...
export function buildTemplateFromRegistryItem(
  ...
  return {
    ...registryItemToTemplateSummary(item),
    files,
    ...(trimmedReadme ? { readme: trimmedReadme } : {}),
  }
}

export function toRegistryDependencyRef(templateId: string): string {
  return `${REGISTRY_GITHUB_SLUG}/${templateId}`
}

export function parseRegistryDependencyRef(ref: string): string {
  const normalized = ref.trim()

  if (normalized.startsWith(`${REGISTRY_GITHUB_SLUG}/`)) {
    return normalized.slice(REGISTRY_GITHUB_SLUG.length + 1)
  }
...
function registryDependenciesToTemplateDependencies(
  ...
  const required = (item.registryDependencies ?? []).map(parseRegistryDependencyRef).filter(Boolean)
