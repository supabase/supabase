...
Realtime is built with [Phoenix](https://phoenixframework.org/), which is an Elixir framework that's amazing for listening via websockets. 
The Phoenix server listens to PostgreSQL's replication functionality and converts the byte stream into JSON. It then broadcasts the JSON over websockets. 
