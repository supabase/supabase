...
/**
 * Builds the CREATE (pg_temp) pg_get_tabledef function block.
 *
 * PR #47894 replaced three O(catalog) information_schema scans (and a
 * relnamespace::regnamespace cast per pg_class row) with direct string tests
 * and an OID-scoped partial-index lookup. That NEW behavior is gated behind
 * `scoped` (default false = legacy behavior) so Studio can roll it out behind a
 * feature flag. When `scoped` is false the emitted function is byte-equivalent
 * to the pre-PR version.
 *
...
const SCOPED_PG_GET_TABLEDEF_SQL: SafeSqlFragment = safeSql`
  DROP TYPE IF EXISTS pg_temp.tabledefs CASCADE;
  CREATE TYPE pg_temp.tabledefs AS ENUM ('PKEY_INTERNAL','PKEY_EXTERNAL','FKEYS_INTERNAL', 'FKEYS_EXTERNAL', 'COMMENTS', 'FKEYS_NONE', 'INCLUDE_TRIGGERS', 'NO_TRIGGERS');
...
export const getTableDefinitionSql = ({
  id,
  scoped = false,
}: {
  id: number
