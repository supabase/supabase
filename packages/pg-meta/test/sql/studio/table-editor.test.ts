...
import { getTableEditorSql, getUpdateIdentitySequenceSQL } from '../../../src'
...
withTestDatabase(
  'raises the identity sequence for a table whose name requires quoting',
  async ({ executeQuery }) => {
    await executeQuery(`
      create schema "Editor_Ident";
      create table "Editor_Ident"."Users" (
        "Id" bigint generated by default as identity primary key,
        val text
      );
      insert into "Editor_Ident"."Users" ("Id", val) values (5, 'a'), (6, 'b');
    `)

    // Mirrors the post-import "raise sequences" step. pg_get_serial_sequence
    // lower-cases an unquoted identifier, so a mixed-case schema/table must be
    // passed quoted or it resolves to a non-existent relation.
    const sql = getUpdateIdentitySequenceSQL({
      schema: 'Editor_Ident',
      table: 'Users',
      column: 'Id',
    })
    ...
    // The sequence is now past the imported max, so the next generated id does
    // not collide with the imported rows.
    const [{ Id: nextId }] = await executeQuery<{ Id: number }[]>(
      `insert into "Editor_Ident"."Users" (val) values ('c') returning "Id";`
    )
