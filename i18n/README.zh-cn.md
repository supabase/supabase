...
[Supabase](https://supabase.com) 是 Postgres 开发平台。我们正在使用企业级的开源工具构建 Firebase 的功能。
...
- [x] AI + 向量/Embeddings 工具包。[文档](https://supabase.com/docs/guides/ai)
- [x] 仪表盘
...
- [社区论坛](https://github.com/supabase/supabase/discussions)。适用于：帮助构建和讨论数据库最佳实践。
...
- [Discord](https://discord.supabase.com)。适用于：分享你的应用程序以及与社区交流。
...
- [PostgreSQL](https://www.postgresql.org/) 是一个对象关系型数据库系统，经过 30 多年的积极开发，在可靠性、功能健壮性和性能方面赢得了良好的声誉。
- [Realtime](https://github.com/supabase/realtime) 是一个 Elixir 服务器，允许你使用 WebSocket 监听 PostgreSQL 的插入、更新和删除。Realtime 轮询 Postgres 的内置复制功能获取数据库变更，将变更转换为 JSON，然后通过 WebSocket 将 JSON 广播给已授权的客户端。
- [PostgREST](http://postgrest.org/) 是一个 Web 服务器，可以将你的 PostgreSQL 数据库直接生成 RESTful API。
- [GoTrue](https://github.com/supabase/gotrue) 是一个基于 JWT 的身份验证 API，简化了应用中的用户注册、登录和会话管理。
- [Storage](https://github.com/supabase/storage-api) 提供了一个 RESTful 接口，用于管理存储在 S3 中的文件，使用 Postgres 来管理权限。
- [pg_graphql](http://github.com/supabase/pg_graphql/) 是一个公开 GraphQL API 的 PostgreSQL 扩展。
...
我们的客户端库是模块化的。每一个子库都是一个独立的实现，用于一个单一的外部系统。这是我们支持现有工具的方式之一。

<table style="table-layout:fixed; white-space: nowrap;">
  <tr>
    <th>语言</th>
    <th>客户端</th>
    <th colspan="5">功能-客户端（已集成在 Supabase 客户端中）</th>
  </tr>
  <!-- notranslate -->
  <tr>
    <th></th>
