...
} from '@/components/interfaces/LogDrains/LogDrains.utils'
...
const FORM_ID = 'audit-log-drain-destination-form'
...
type AuditLogDrainDestinationFormValues = z.infer<typeof formSchema>
type AuditLogDrainDestinationSubmitValues = z.infer<typeof submitSchema>

const HEADER_ENABLED_TYPES = ['webhook', 'loki', 'otlp'] as const

function toSubmitValues(
  values: AuditLogDrainDestinationFormValues
): AuditLogDrainDestinationSubmitValues {
  if (!HEADER_ENABLED_TYPES.includes(values.type as (typeof HEADER_ENABLED_TYPES)[number])) {
    return submitSchema.parse(values)
  }

  const { headerEntries = [], ...rest } = values as AuditLogDrainDestinationFormValues & {
...
function AuditLogDrainFormItem({
  value,
  label,
  description,
  formControl,
...
type DefaultValues = { type: LogDrainType } & Partial<AuditLogDrainData>

export function AuditLogDrainDestinationSheetForm({
  open,
  onOpenChange,
  defaultValues,
  onSubmit,
  ...
  isLoading?: boolean
  onSubmit: (values: AuditLogDrainDestinationSubmitValues) => void
  mode: 'create' | 'update'
}) {
  ...
  const form = useForm<AuditLogDrainDestinationFormValues>({
      ...
      >
            ...
            <form
              ...
              onSubmit={(e) => {
                ...
                if (logDrainExists && mode === 'create') {
                  toast.error('Audit log drain name already exists')
                  return
                }
        ...
        <div className="mt-auto">
          ...
          <SheetFooter className="p-content mt-0! justify-between! flex-row! w-full items-center">
            ...
            <Shortcut
              id={SHORTCUT_IDS.AUDIT_LOG_DRAINS_SAVE_DESTINATION}
