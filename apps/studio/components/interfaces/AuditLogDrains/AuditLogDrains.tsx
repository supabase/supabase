...
import { AuditLogDrainsEmpty } from './AuditLogDrainsEmpty'
import {
  LOG_DRAIN_TYPES,
  LogDrainType,
} from '@/components/interfaces/LogDrains/LogDrains.constants'
import { LogDrainsCard } from '@/components/interfaces/LogDrains/LogDrainsCard'
import { VoteLink } from '@/components/interfaces/LogDrains/VoteLink'
import AlertError from '@/components/ui/AlertError'
import {
  AuditLogDrainData,
  useAuditLogDrainsQuery,
} from '@/data/audit-log-drains/audit-log-drains-query'
import { useDeleteAuditLogDrainMutation } from '@/data/audit-log-drains/delete-audit-log-drain-mutation'
...
export function AuditLogDrains({
  ...
  onUpdateDrainClick: (drain: AuditLogDrainData) => void
}) {
  const { hasAccess: hasAccessToLogDrains, isLoading: isLoadingEntitlement } =
    useCheckEntitlements('log_drains')
  const track = useTrack()
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [selectedLogDrain, setSelectedLogDrain] = useState<AuditLogDrainData | null>(null)
  ...
  const { mutate: deleteLogDrain } = useDeleteAuditLogDrainMutation({
    onSuccess: () => {
  ...
  if (!isLoadingEntitlement && !hasAccessToLogDrains) {
    return <AuditLogDrainsEmpty />
  }

  if (isError) {
    return <AlertError subject="Failed to load audit log drains" error={error}></AlertError>
  }
      ...
      <Card>
        <Table>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="max-w-[140px]" align="end">
                        {/* Updating audit log drains is disabled temporarily.
                        <DropdownMenuItem
                          onClick={() => {
          ...
          <ConfirmationModal
            confirmLabel="Delete"
            variant="destructive"
            title="Delete Audit Log Drain"
            visible={isDeleteModalOpen}
            onConfirm={() => {
              if (selectedLogDrain && slug) {
                deleteLogDrain({ token: selectedLogDrain.token, slug })
