...
describe('computeOverallRisk', () => {
  it('is Minimal with no capabilities', () => {
    expect(computeOverallRisk({}, 'single-project').level).toBe('Minimal')
  })
  ...
  it('single-project high-risk write is High', () => {
    expect(computeOverallRisk({ 'project:database': 'readwrite' }, 'single-project').level).toBe(
      'High'
    )
  })

  it('single-project medium write is Medium', () => {
    expect(
      computeOverallRisk({ 'project:realtime_config': 'readwrite' }, 'single-project').level
    ).toBe('Medium')
  })

  it('read-only single-project is Low', () => {
    expect(computeOverallRisk({ 'project:database': 'read' }, 'single-project').level).toBe('Low')
  })
})
