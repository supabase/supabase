...
// Mock Sentry so we can assert whether failed submissions are captured. We only
// care about captureMessage here; other exports are stubbed defensively.
...
describe('SupportFormPage', () => {
  ...
  test('does not lock the form when problem category is submitted without a library', async () => {
    const submitSpy = vi.fn()
    ...
    await fillField(getSummaryField(screen), 'API requests failing in production')
    await fillField(getMessageField(screen), 'Requests return status 500 when calling the endpoint')

    const submitButton = getSubmitButton(screen)

    // First submission fails validation because no library is selected.
    await userEvent.click(submitButton)

    await waitFor(() => {
      expect(
        screen.getByText("Please select the library that you're facing issues with")
      ).toBeInTheDocument()
    })

    // The form must not be permanently locked: no request should have fired and
    // the submit button should still be enabled.
    expect(submitSpy).not.toHaveBeenCalled()
    expect(submitButton).not.toBeDisabled()

    // Selecting a library and submitting again should now succeed, proving the
    // guard ref was not left in a locked state.
    await waitFor(() => {
      expect(getLibrarySelector(screen)).toBeInTheDocument()
    })
    await selectLibraryOption(screen, 'JavaScript')
  ...
  test('does not capture rate-limited (429) submissions to Sentry', async () => {
    const Sentry = await import('@sentry/nextjs')
    const captureMessageSpy = vi.mocked(Sentry.captureMessage)
    ...
    addAPIMock({
      ...
      path: '/platform/feedback/send',
      response: async () => {
        return HttpResponse.json<APIErrorBody>({ message: 'Too Many Requests' }, { status: 429 })
      },
    })
    ...
    await fillField(getSummaryField(screen), 'Cannot access settings')
    await fillField(getMessageField(screen), 'Settings page shows an error')
    ...
    await waitFor(() => {
      expect(getSubmitButton(screen)).not.toBeDisabled()
    })

    // Crucially, a 429 must not be captured to Sentry.
