...
const DatabaseSettings = () => {
  ...
  if (isLoading) {
    return Array.from({ length: 5 }).map((_, i) => (
      <div key={i} className="grid gap-2 items-center md:grid md:grid-cols-12 md:gap-x-4 w-full">
  ...
  if (isError) {
    return <AlertError error={error} subject="Failed to retrieve databases" />
  }
        ...
        {isMd5 && (
          <Alert_Shadcn_>
            ...
            <AlertDescription_Shadcn_>
              GUI clients only support database connections for Postgres 13 via a connection string.
            </AlertDescription_Shadcn_>
          </Alert_Shadcn_>
        )} */}
      </div>
      <div className="-space-y-px">
        <div className="text-xs font-mono space-y-1 bg-surface-100 border p-3 rounded-t">
          <div>
            <span className="text-foreground-light">Host:</span>{' '}
            <span>{connectionInfo.db_host}</span>
          </div>
          <div>
            <span className="text-foreground-light">Database name:</span>{' '}
            <span>{connectionInfo.db_name}</span>
          </div>
          <div>
            <span className="text-foreground-light">Port:</span>{' '}
            <span>{poolingMode === 'transaction' ? connectionInfo.db_port : '5432'}</span>
          </div>
          {isMd5 && snap.usePoolerConnection && (
            <div>
              <span className="text-foreground-light">Options:</span>{' '}
              <span>reference={projectRef}</span>
            </div>
          )}
          <div>
            <span className="text-foreground-light">User:</span>{' '}
            <span>{connectionInfo.db_user}</span>
          </div>
          <div>
            <span className="text-foreground-light">Password:</span>{' '}
            <span>
        ...
        <div className="text-foreground-muted p-3 py-1 bg-surface-75 border rounded-b text-xs">
          For security reasons, your database password is never shown and is only readable on
          creation/reset.
        </div>
      </div>
    </>
