...
// Auth severity classification, OTEL counterpart of the BigQuery AUTH_LOG_*
// conditions in Logs.constants.ts. Shared by the filter templates and the chart
// so a row buckets identically whether the flag is on or off.
const AUTH_OTEL_ERROR_CONDITION: SafeLogSqlFragment = safeSql`${attr('level')} IN ('error', 'fatal') OR ${statusAsInt('status')} >= 500`
const AUTH_OTEL_WARNING_CONDITION: SafeLogSqlFragment = safeSql`${attr('level')} = 'warning' OR ${statusAsInt('status')} BETWEEN 400 AND 499`
const AUTH_OTEL_INFO_CONDITION: SafeLogSqlFragment = safeSql`NOT (${AUTH_OTEL_ERROR_CONDITION}) AND NOT (${AUTH_OTEL_WARNING_CONDITION})`
...
export const SQL_FILTER_TEMPLATES_OTEL: Record<string, Record<string, SqlFilterEntry>> = {
  ...
  auth_logs: {
    ..._SQL_FILTER_COMMON_OTEL,
    'severity.error': AUTH_OTEL_ERROR_CONDITION,
    'severity.warning': AUTH_OTEL_WARNING_CONDITION,
    'severity.info': AUTH_OTEL_INFO_CONDITION,
...
const otelErrorCondition = (table: LogsTableName): SafeLogSqlFragment => {
  switch (table) {
    ...
    case LogsTableName.AUTH:
      return AUTH_OTEL_ERROR_CONDITION
    case LogsTableName.FUNCTIONS:
...
const otelWarningCondition = (table: LogsTableName): SafeLogSqlFragment => {
  switch (table) {
    ...
    case LogsTableName.AUTH:
      return AUTH_OTEL_WARNING_CONDITION
    case LogsTableName.FUNCTIONS:
