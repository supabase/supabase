...
import { useIsInlineEditorEnabled } from '../../../App/FeaturePreview/FeaturePreviewContext'
...
export const TriggersList = () => {
  ...
  const [triggerToEditByName, setTriggerToEditByName] = useQueryState(
    'edit',
    parseAsString.withDefault('').withOptions({ history: 'push', clearOnDefault: true })
  )
  const [triggerToDeleteByName, setTriggerToDeleteByName] = useQueryState(
    'delete',
    parseAsString.withDefault('').withOptions({ history: 'push', clearOnDefault: true })
  )
...
execute function function_name();`)
  ...
  const editTrigger = (trigger: PostgresTrigger) => {
    ...
    if (isInlineEditorEnabled) {
      ...
      setTriggerToEditByName(trigger.name)
    }
  }
  ...
  const deleteTrigger = (trigger: PostgresTrigger) => {
    setSelectedTrigger(trigger)
    setTriggerToDeleteByName(trigger.name)
  }
  ...
  return (
    <>
      ...
      <TriggerSheet
        selectedTrigger={triggerToEdit}
        open={triggerToEditByName !== ''}
        onClose={() => setTriggerToEditByName('')}
        isDuplicatingTrigger={isDuplicatingTrigger}
      />

      <DeleteTrigger
        trigger={triggerToDelete}
        visible={triggerToDeleteByName !== ''}
        setVisible={setTriggerToDeleteByName}
      />
    </>
  )
}
