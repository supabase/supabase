...
interface WarehouseProgressStepsProps {
   ...
   */
  activeIndex: number
  /** When set, that step shows a failure icon instead of complete/active. */
  failedIndex?: number
}
          ...
          return (
            <li
              ...
              className={cn(
                ...
                status === 'failed' && 'bg-destructive-200/20'
              )}
            >
              <span className="flex size-5 shrink-0 items-center justify-center">
                ...
                {status === 'failed' && (
                  <span className="flex size-5 items-center justify-center rounded-full bg-destructive/10">
                    <X size={12} strokeWidth={2.5} className="text-destructive" />
                  </span>
                )}
                {status === 'active' && (
                  <Loader2
