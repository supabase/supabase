...
const LintDetail = ({ lint, projectRef }: LintDetailProps) => {
        ...
        <Button
          ...
          onClick={() => {
            snap.newChat({
              name: 'Summarize lint',
              open: true,
              initialInput: `Summarize the issue and suggest fixes: ${
                lintInfoMap.find((item) => item.name === lint.name)?.title
              }
                                \nEntity: ${
                                  (lint.metadata &&
                                ...
                                \nIssue: ${lint.detail.replace(/\\`/g, '`')}
                                \nDescription: ${lint.description.replace(/\\`/g, '`')}\n`,
            })
          }}
        >
          Ask Assistant
