...
import { Button, Card, CardContent, cn, Form, Switch } from 'ui'
...
import { HighAvailabilitySizeField } from './fields/HighAvailabilitySizeField'
...
import { HighAvailabilityGridBackground } from '@/components/interfaces/ProjectHome/HighAvailabilityGridBackground'
...
export function DiskManagementForm({
  chartsClassName,
  isHighAvailability: isHighAvailabilityProp,
  onHighAvailabilityChange,
}: {
  chartsClassName?: string
  isHighAvailability?: boolean
  onHighAvailabilityChange?: (enabled: boolean) => void
} = {}) {
  ...
  const [internalHighAvailability, setInternalHighAvailability] = useState(false)
  const isControlledHighAvailability = isHighAvailabilityProp !== undefined
  const isHighAvailability = isControlledHighAvailability
    ? isHighAvailabilityProp
    : internalHighAvailability
  const setIsHighAvailability = onHighAvailabilityChange ?? setInternalHighAvailability

  useEffect(() => {
    if (isControlledHighAvailability) return
    setInternalHighAvailability(project?.high_availability ?? false)
  }, [isControlledHighAvailability, project?.high_availability])
      ...
      <form id="disk-compute-form" onSubmit={form.handleSubmit(onSubmit)}>
        <PageContainer size="default" className="pb-16">
          ...
          <PageSection>
            <PageSectionMeta>
              <PageSectionSummary>
                <PageSectionTitle>Availability</PageSectionTitle>
                <PageSectionDescription>
                  Keep your database available through infrastructure failures.
                </PageSectionDescription>
              </PageSectionSummary>
            </PageSectionMeta>
            <PageSectionContent>
              <Card>
                <CardContent className="grid grid-cols-[auto_minmax(0,1fr)] items-stretch p-0">
                  <HighAvailabilityGridBackground
                    animated={isHighAvailability}
                    className="aspect-square h-full"
                  />
                  <div className="flex min-w-0 items-center justify-between gap-6 py-4 pr-(--card-padding-x) pl-(--card-padding-x)">
                    <div className="flex min-w-0 flex-1 flex-col gap-1">
                      <p className="text-sm text-foreground">High Availability</p>
                      <p className="text-sm text-foreground-light">
                        Run your database across multiple instances to improve availability. Powered
                        by{' '}
                    ...
                    <Switch
                      ...
                      onCheckedChange={setIsHighAvailability}
                ...
                <PageSection className="pt-0">
                  <PageSectionMeta>
                    <PageSectionSummary>
                      ...
                      <PageSectionDescription>
                        High Availability resources are bundled and cannot currently be changed.
                      </PageSectionDescription>
                    </PageSectionSummary>
                    <PageSectionAside>
