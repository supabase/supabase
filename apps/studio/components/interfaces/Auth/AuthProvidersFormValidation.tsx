...
const smsProviderBaseSchema = z.object({
  SMS_TEST_OTP: z.string().refine((value) => {
...
const twilioPhoneProviderSchema = z.object({
  SMS_TWILIO_ACCOUNT_SID: z.string().min(1, 'Twilio Account SID is required'),
  SMS_TWILIO_AUTH_TOKEN: z.string().min(1, 'Twilio Auth Token is required'),
  SMS_TWILIO_MESSAGE_SERVICE_SID: z.string().min(1, 'Twilio Message Service SID is required'),
})

const twilioVerifyPhoneProviderSchema = z.object({
  SMS_TWILIO_VERIFY_ACCOUNT_SID: z.string().min(1, 'Twilio Verify Account SID is required'),
  SMS_TWILIO_VERIFY_AUTH_TOKEN: z.string().min(1, 'Twilio Verify Auth Token is required'),
  SMS_TWILIO_VERIFY_MESSAGE_SERVICE_SID: z
    .string()
    .min(1, 'Twilio Verify Message Service SID is required'),
})

const messagebirdPhoneProviderSchema = z.object({
  SMS_MESSAGEBIRD_ACCESS_KEY: z.string().min(1, 'Messagebird Access Key is required'),
  SMS_MESSAGEBIRD_ORIGINATOR: z.string().min(1, 'Messagebird Originator is required'),
})

const textlocalPhoneProviderSchema = z.object({
  SMS_TEXTLOCAL_API_KEY: z.string().min(1, 'Textlocal API Key is required'),
  SMS_TEXTLOCAL_SENDER: z.string().min(1, 'Textlocal Sender ID is required'),
})

const vonagePhoneProviderSchema = z.object({
  SMS_VONAGE_API_KEY: z.string().min(1, 'Vonage API Key is required'),
  SMS_VONAGE_API_SECRET: z.string().min(1, 'Vonage API Secret is required'),
...
export const getPhoneProviderValidationSchema = (config: ProjectAuthConfigData) => {
  const twilioSchema = makeProviderOptionalWhenSMSHookEnabled(config, twilioPhoneProviderSchema)
    .merge(
      z.object({
  ...
  const twilioVerifySchema = makeProviderOptionalWhenSMSHookEnabled(
    config,
    twilioVerifyPhoneProviderSchema
  )
    .merge(
      z.object({
  ...
  const messagebirdSchema = makeProviderOptionalWhenSMSHookEnabled(
    config,
    messagebirdPhoneProviderSchema
  )
    .merge(
      z.object({
  ...
  const vonageSchema = makeProviderOptionalWhenSMSHookEnabled(config, vonagePhoneProviderSchema)
    .merge(
      z.object({
