...
export const GridHeaderActions = ({ table, isRefetching }: GridHeaderActionsProps) => {
  ...
  const isWarehouseView = isWarehouseSchema(table.schema)
  const isReadOnlyView = isSchemaLocked || isWarehouseView
    ...
    <div className="flex items-center space-x-2">
      {showHeaderActions && (
        <div className="flex items-center gap-x-2">
            ...
            table.rls_enabled ? (
              <>
                {policies.length < 1 ? (
                  ...
                  <Button
                    asChild
                    variant={policies.length < 1 ? 'warning' : 'default'}
                    className="group"
                    icon={
                      policies.length > 0 ? (
                        <div
          ...
          {showHeaderActions && !isWarehouseView && <InsertButton />}
        </div>
      )}
