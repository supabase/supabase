...
export const EmptyBucketState = ({ bucketType }: EmptyBucketStateProps) => {
  ...
  return (
    <aside className="mt-12 border border-dashed w-full bg-surface-100 rounded-lg px-4 py-10 flex flex-col gap-6 items-center text-balance">
      <div className="flex flex-col gap-3 items-center text-center">
        <BucketNew size={24} strokeWidth={1.5} className="text-foreground-muted" />
        <div className="flex flex-col gap-1">
          <h3 className="text-foreground text-xl">Create {config.bucketName}</h3>
      ...
      {bucketType === 'files' && (
        <CreateBucketModal buttonSize="small" buttonType="primary" buttonClassName="w-fit" />
      )}
    </aside>
  )
}
