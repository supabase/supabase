...
function getPartnerIcon(
  ...
  switch (organization.managed_by) {
    case BILLING_MANAGED_BY.VERCEL_MARKETPLACE:
      return (
        <svg
          className={cn(
    ...
    case BILLING_MANAGED_BY.AWS_MARKETPLACE:
      return (
        <svg
          className={cn(
            size === 'small' && 'w-2.5 h-2.5',
...
function PartnerIcon({
  ...
  if (
    organization.managed_by === BILLING_MANAGED_BY.VERCEL_MARKETPLACE ||
    organization.managed_by === BILLING_MANAGED_BY.AWS_MARKETPLACE
  ) {
