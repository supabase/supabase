...
function roundTrip(markdown: string): string {
  const editor = createHeadlessEditor({ nodes: MARKDOWN_EDITOR_NODES, onError: (error) => { throw error } })
  editor.update(() => $convertFromMarkdownString(markdown, MARKDOWN_TRANSFORMERS), { discrete: true })
