...
- [x] `routes/project/$ref/database.tsx` — DatabaseLayout (reads `databaseLayoutTitle` from leaf `staticData`). **Delta vs plan:** shell honours `skipDatabaseLayout: true` for `replication/new`, which wraps in `ProjectLayoutWithAuth` without the Database product menu so the stepped create canvas keeps project chrome and drops the product menu.
...
- [x] A `routes/project/$ref/database/replication/new.tsx` ← `pages/project/[ref]/database/replication/new.tsx` (sets `skipDatabaseLayout: true`, wraps in `ProjectLayoutWithAuth` without the Database product menu)
