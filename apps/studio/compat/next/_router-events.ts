...
//
// Known gap (historical): Next's `routeChangeStart` lets handlers throw to cancel the
// navigation. TanStack's `subscribe` is fire-and-forget; cancellation requires
// `useBlocker` instead. `usePreventNavigationOnUnsavedChanges` now uses `useBlocker`.
