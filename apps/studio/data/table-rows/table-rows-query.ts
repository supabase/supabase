...
export const getAllTableRowsSql = ({
  ...
  filters?: Filter[]
  sorts?: Sort[]
}): { sql: QueryFilter; cursorPagination: string[] | false } => {
  ...
  let cursorPagination: string[] | false = false
  ...
  if (sorts.length === 0) {
    if (cursorPaginationEligible.length > 0) {
      cursorPagination = cursorPaginationEligible[0]
  ...
  return { sql: queryChains, cursorPagination }
}
...
export const fetchAllTableRows = async ({
  ...
  const rows: any[] = []
  let { sql: queryChains, cursorPagination } = getAllTableRowsSql({ table, sorts, filters })
    ...
    while (true) {
      if (cursor) {
        queryChains = queryChains.filter(
          cursorPagination,
          '>',
          cursorPagination.map((col) => cursor![col])
        )
      }
      const query = wrapWithRoleImpersonation(queryChains.toSql(), roleImpersonationState)
      ...
      try {
        ...
        cursor = {}
        for (const col of cursorPagination) {
