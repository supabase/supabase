...
export async function deleteConnection({ connectionId }: DeleteVariables, signal?: AbortSignal) {
  const { data, error } = await del('/platform/integrations/gitlab/connections/{connection_id}', {
...
export const useGitLabConnectionDeleteMutation = ({
  onSuccess,
  onError,
  ...options
}: Omit<
    ...
    (args) => deleteConnection(args),
    {
      ...
      async onError(data, variables, context) {
        if (onError === undefined) {
          toast.error(`Failed to delete GitLab connection: ${data.message}`)
        } else {
