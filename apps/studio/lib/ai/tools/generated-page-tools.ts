...
/**
 * The shared schema plus a server-only syntax check of the page's inline scripts. The
 * renderer parses with the shared schema alone: it runs in the browser, and a page that
 * reached it has already passed this check.
 */
const renderPageToolInputSchema = renderPageInputSchema.superRefine((value, ctx) => {
  for (const message of findGeneratedPageScriptSyntaxErrors(value.html)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['html'], message })
  }
})
...
export const getGeneratedPageTools = () => {
  return {
    render_page: tool({
      ...
      inputSchema: renderPageToolInputSchema,
      needsApproval: true,
