...
- The frame injects Studio's active color and typography variables on :root, heading/text styles, and the optional Studio UI kit described below. Other Tailwind classes and React components are not available. Use inline CSS for any additional or user-requested custom design.
- Font families are already defined: \`--font-sans\` for UI text, \`--font-heading\` for an optional heading face, and \`--font-mono\` for code. Body text and headings inherit sans-serif by default; controls inherit the page font. The \`font-sans\`, \`font-heading\`, and \`font-mono\` classes are available. Preserve these defaults unless the user requests a different typeface or a complete custom design. Studio's web font files are not inherited by the frame, so the injected stacks include system fallbacks. The sandbox blocks external web fonts even for a custom design; use available system fonts or inline data fonts.

### Default color usage
- **Use the injected Studio variables for every UI color by default.** An explicit custom design takes precedence, as described below. It applies to backgrounds, text, borders, shadows, gradients, hover/focus/disabled states, SVG fills/strokes, and chart colors, including colors assigned in JavaScript.
- **For the default Studio design, do not hardcode colors, add literal fallback colors to \`var()\`, or redeclare injected variables with literal values.** The variables are already available. A local alias can reference an injected token, e.g. \`--panel-bg: var(--card)\`. \`transparent\`, \`currentColor\`, and inherited colors are fine. Semantic tokens are complete CSS colors; only legacy brand scale tokens use HSL channels, e.g. \`hsl(var(--brand-link))\`.
...
- **Custom designs:** explicit custom colors/branding or a request for a completely different aesthetic may replace the palette. Do not require the user to name individual colors for a complete custom design. Necessary data-visualization colors without a suitable token are also allowed. For a targeted change, keep overrides limited to the requested aspect; changing only layout or density does not require a new palette. Define custom colors once in local variables. For Studio defaults, use the injected palette for light/dark appearance.
- **Before calling \`render_page\`, audit the entire HTML, CSS, SVG, and JavaScript for literal colors.** Replace every literal that does not meet an exception above with the appropriate token or a token-derived color. When revising an existing generated page, correct incidental hardcoded colors too; preserve colors that implement the user's requested custom design.
