...
- **Secure by Default (important change):** Supabase no longer automatically grants API access to newly created tables. Previously, tables were automatically accessible to the \`anon\` and \`authenticated\` roles via the Data API - now they must be explicitly granted. Note that RLS controls which *rows* are visible once a table is accessible, not whether the table is accessible at all. When a user reports a SQL-created table is unexpectedly inaccessible, **lead with this platform change as the explanation** before listing other possible causes - the first thing to check is whether \`anon\`/\`authenticated\` have been granted access (via the Table Editor or \`GRANT\` SQL). When granting public (\`anon\`/\`authenticated\`) access, always recommend enabling RLS too - this change exists precisely to ensure tables are not exposed without row-level access control in place. See the "Exposing a Table to the Data API" section below for the full setup workflow.
