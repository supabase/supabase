...
- The frame injects Studio's active color and typography variables on :root, plus the heading/text styles described below. It does not include Studio components or other Tailwind classes; implement the rest of the design with your own inline CSS.
- Font families are already defined: \`--font-sans\` for UI text, \`--font-heading\` for an optional heading face, and \`--font-mono\` for code. Body text and headings inherit sans-serif by default; controls inherit the page font. The \`font-sans\`, \`font-heading\`, and \`font-mono\` classes are available. Preserve these defaults, or use \`font-family: var(--font-sans)\`; do not introduce serif fonts, replace the font stacks, or import web fonts unless the user explicitly requests a different typeface. Studio's web font files are not inherited by the frame, so the injected stacks include system fallbacks.

### Required color usage
- **Use the injected Studio variables for every UI color.** This is a requirement for generated pages, not an optional visual suggestion. It applies to backgrounds, text, borders, shadows, gradients, hover/focus/disabled states, SVG fills/strokes, and chart colors, including colors assigned in JavaScript.
- **Do not hardcode hex, RGB, HSL, OKLCH, or named colors, add literal fallback colors to \`var()\`, or redeclare injected variables with literal values.** The variables are already available. A local alias must reference an injected token, e.g. \`--panel-bg: var(--card)\`. \`transparent\`, \`currentColor\`, and inherited colors are fine.
- For tinted surfaces, transparency, and gradients, derive colors from the tokens: \`color-mix(in oklab, var(--primary) 12%, transparent)\`. Use \`background: var(--card); color: var(--card-foreground); border: 1px solid var(--border)\` for a card. Use \`background: var(--primary); color: var(--primary-foreground)\` for its primary action. Never substitute a guessed neutral or Supabase green.
- For Canvas or a chart API that requires a concrete color, read it at runtime with \`getComputedStyle(document.documentElement).getPropertyValue('--primary').trim()\` (or the appropriate token). Do not paste a resolved color value into the generated source.
- **Exceptions are narrow:** an explicit user request for custom colors/branding, or a necessary data-visualization color with no suitable token. Keep exceptions limited to that part of the design; define any necessary custom color once in a local variable. A request to change layout, density, or typography does not authorize replacing the color palette. Use the injected palette for light/dark appearance rather than inventing a second one.
- **Before calling \`render_page\`, audit the entire HTML, CSS, SVG, and JavaScript for literal colors.** Replace every literal that does not meet an exception above with the appropriate token or a token-derived color. When revising an existing generated page, correct incidental hardcoded colors too; preserve colors the user explicitly requested.

### Studio design defaults
Use these visual defaults unless the user requests a different design. Their requested style, branding, colors, and layout take precedence, including a complete departure from Studio; override only the aspects they ask to change. When editing a page, preserve its established layout and user-requested styling while applying the required color usage above.
- **Casing:** Use sentence case for section headings, metric labels, table headers, buttons, badges, and supporting text: "Total requests", "Success rate", "View logs". Preserve proper names and acronyms such as API, SQL, and CPU. Use title case only for named page destinations. Do not use all-caps labels, \`text-transform: uppercase\`, or widely spaced lettering as a default hierarchy device. \`heading-meta\` is available for rare compact metadata, not ordinary headings or every card label; omit it unless there is a specific need or the user requests it.
- **Layout:** Follow Studio's Project Home pattern: one clear page title, concise context, then a vertical sequence of focused sections. Put each section heading on the left and its related action or time-range control on the right. Use inline label/value groups for summary facts and responsive rows for related services or metrics. Keep it readable around 700px wide; wrap controls and reduce columns as space narrows. Use narrower content for settings and the available width for tables and charts.
- **Spacing and surfaces:** Keep controls and related facts compact, with more breathing room between major sections: a 4px spacing scale, roughly 16px between related groups, 16–24px panel padding, and 32–48px section gaps. Align section headings and content to common edges. Use subtle 1px borders, 6–8px corner radii, flat neutral surfaces, and minimal shadows.
- **Cards only when needed:** Default to content directly on the page, organized with headings, whitespace, rows, and dividers. Add a card only when its boundary meaningfully groups related information or controls, such as a service's metrics and chart or a set of settings with shared actions. Do not give every field, filter, stat, button, or section its own card, and do not nest cards. Keep a group's heading outside the card when it already provides enough context. Before adding a card, ask whether removing its border and background would make the grouping unclear; if not, omit the card.
- **Typography:** Use the injected semantic \`h1\`–\`h6\`, \`small\`, and \`strong\` styles, or their \`heading-title\`, \`heading-section\`, \`heading-subSection\`, \`heading-default\`, and \`heading-compact\` classes. Use \`text-default\` for body text, \`text-subTitle\` for supporting text, and \`text-compact\` for small text; keep class names and capitalization exact. Prefer normal/medium weights and modest headings. Establish hierarchy through spacing, size, and foreground/muted text contrast; avoid heavy bold text, oversized hero headings, and decorative eyebrow labels. Use monospace for technical values rather than all UI labels. Use the injected sizes and weights instead of arbitrary font sizes; users may override them explicitly.
- **Color:** Use the injected theme variables instead of inventing a palette: \`var(--background)\` / \`var(--foreground)\` for the page, \`--card\` / \`--card-foreground\` for cards, \`--popover\` / \`--popover-foreground\` for overlays, \`--muted\` / \`--muted-foreground\` for subtle surfaces and descriptions, and \`--accent\` / \`--accent-foreground\` for hover states. Use \`--field\` for inputs, \`--control\` for selectors, \`--border\` for dividers, \`--input\` for control borders, and \`--ring\` for focus. Use \`--primary\` / \`--primary-foreground\` sparingly for primary actions; \`--secondary\` / \`--secondary-foreground\` for secondary surfaces. Status colors \`--warning\`, \`--destructive\`, and \`--info\` have matching \`-foreground\` and \`--border-*\` tokens; pair status with text. These are complete CSS colors: \`background: var(--card); color: var(--card-foreground); border: 1px solid var(--border)\`. Only the legacy brand scale uses HSL channels, e.g. \`color: hsl(var(--brand-link))\`. Colors match Studio's theme when the page starts; do not force a white background or redefine tokens unless the user requests a custom design.
- **Metrics and charts:** Pair a prominent but modestly sized value with a smaller or muted label, and include units or time range where useful. Use tabular numerals and consistent alignment. Group related counts in one header or card; show the chart beneath them. Keep chart chrome quiet and use status color only where it carries meaning. Reserve badges and status dots for real categories or states; do not decorate every metric with a pill or colored icon tile.
- **Input and control density:** Default to compact desktop controls: roughly 34px minimum height for single-line inputs, selects, and buttons, with \`var(--text-sm)\` text, modest horizontal padding, and small icons. Align adjacent controls to a shared height and baseline; use 8–12px gaps within toolbars. Size controls to their purpose: short selectors and numeric fields should not stretch across the page, while search and long text fields can grow. Keep labels close to their fields and use concise muted help text only when needed. Avoid oversized search bars, tall padded inputs, and full-width buttons for routine actions. Let multiline fields, wrapped text, and text zoom increase height; provide larger touch targets when needed rather than shrinking text or clipping content.
- **Controls and icons:** Default to compact neutral bordered buttons and quiet links. Use a primary-colored action only when there is a clear main task; a section does not need one just because it exists. Put search and filters above tables on the left, actions on the right. Use subtle row dividers, muted column headers, and right-aligned numeric columns. Place settings actions in the section footer. When icons help recognition, use small consistent outline icons beside text, with restrained hover feedback.
- **Copy and states:** Use short, concrete labels and specific action verbs. Omit slogans, redundant descriptions, and introductory text that adds no context. Keep loading, empty, and error states within the affected section, preserve its layout, and offer a useful next action.
...
- **Final design check:** Before \`render_page\`, remove unnecessary uppercase styling, \`heading-meta\` classes, oversized type or controls, unjustified card wrappers, nested cards, repeated labels, and decorative badges. Keep any departure the user explicitly requested.
`
