...
function useUnifiedLogs({
  ...
  const params: LogsEndpointParams = useMemo(() => {
    const currentSql = genDefaultQuery(LogsTableName.UNIFIED, mergedFilters, limit)
    return {
      project: projectRef,
  ...
  }, [projectRef, timestampStart, timestampEnd, mergedFilters, limit])

  const queryKey = useMemo(
    () => ['projects', projectRef, 'unified-logs', params],
    [projectRef, params]
  )

  console.log('useUnifiedLogs SQL:', params.sql)
  ...
  const { logData, error, oldestTimestamp } = useMemo(() => {
    ...
    if (logData.length > 0) {
      console.log('Unified Logs Data Sample:', logData.slice(0, 1))
    }
  ...
  const countQuerySql = useMemo(
    () => genCountQuery(LogsTableName.UNIFIED, mergedFilters),
    [mergedFilters]
  )
  const countQueryKey = useMemo(
    ...
    [projectRef, countQuerySql, latestRefresh, timestampEnd, mergedFilters]
  )
  ...
  const chartQuery = useMemo(
    () => genChartQuery(LogsTableName.UNIFIED, params, mergedFilters),
