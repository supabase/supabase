...
jest.mock('lib/gotrue', () => ({ auth: { onAuthStateChange: jest.fn() } }))
jest.mock('data/subscriptions/org-subscription-query', () => ({
  useOrgSubscriptionQuery: jest.fn().mockReturnValue({}),
}))

// [Joshen] Am temporarily commenting out the breaking tests due to:
// "TypeError: _fetch.get.mockReset is not a function" error
// "TypeError: _orgsubscriptionquery.useOrgSubscriptionQuery.mockReturnValue" is not a function
// just so we get our jest unit/UI tests up and running first
// Need to figure out how to mock the "get" method from lib/common/fetch properly
...
test('bug: nav to explorer preserves newlines', async () => {
  ...
  // [Joshen] For some reason, the check below fails - is it cause its clicking on the button's <span />? does it matter?
