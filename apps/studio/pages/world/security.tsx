...
const COMPLIANCE_STANDARDS = [
  { name: 'PIPEDA', region: 'Canada', desc: 'Personal Information Protection and Electronic Documents Act', icon: '≡ƒç¿≡ƒçª' },
  { name: 'RGPD / GDPR', region: 'Europe', desc: 'General Data Protection Regulation - Reglement general sur la protection des donnees', icon: '≡ƒç¬≡ƒç║' },
  { name: 'CCPA', region: 'California, USA', desc: 'California Consumer Privacy Act', icon: '≡ƒç║≡ƒç╕' },
  { name: 'PCI-DSS', region: 'Mondial', desc: 'Payment Card Industry Data Security Standard', icon: '≡ƒÆ│' },
  { name: 'SOC 2 Type II', region: 'Mondial', desc: 'Service Organization Control - Security, Availability, Confidentiality', icon: '≡ƒöÆ' },
  { name: 'ISO 27001', region: 'Mondial', desc: 'Information Security Management System', icon: '≡ƒÅ¢∩╕Å' },
  { name: 'HIPAA', region: 'USA', desc: 'Health Insurance Portability and Accountability Act', icon: '≡ƒÅÑ' },
  { name: 'Loi 25', region: 'Quebec, Canada', desc: 'Loi modernisant des dispositions legislatives en matiere de protection des renseignements personnels', icon: 'ΓÜ£∩╕Å' },
]

// --- BAN RULES ---
const BAN_RULES = [
  { offense: '1er abus / 1st offense', action: 'Avertissement / Warning', icon: 'ΓÜá∩╕Å' },
  { offense: '2e abus / 2nd offense', action: 'Suspension 24h + Cle revoquee / Key revoked', icon: '≡ƒö┤' },
  { offense: '3e abus / 3rd offense', action: 'BAN PERMANENT / PERMANENT BAN', icon: 'Γ¢ö' },
  { offense: 'Tentative de vol de cle / Key theft attempt', action: 'BAN IMMEDIAT + Rapport police / INSTANT BAN + Police report', icon: '≡ƒÜ¿' },
  { offense: 'Scraping / Bot abuse', action: 'IP BAN + Compte supprime / Account deleted', icon: '≡ƒñû' },
]
      ...
      <div className="fixed inset-0 bg-black overflow-auto" style={{ fontFamily: "'Courier New', monospace" }}>
        ...
        <div className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#ff660033', background: 'rgba(0,0,0,0.98)', backdropFilter: 'blur(10px)' }}>
          <div className="flex items-center gap-4">
            <Link href={'/world?lang=' + currentLang} passHref legacyBehavior>
              <a style={{ color: '#ff8c00' }} className="text-sm">ΓåÉ {t(currentLang, 'dashboard')}</a>
            </Link>
            <h1 className="text-2xl font-bold" style={{ color: '#ff8c00', textShadow: '0 0 15px #ff6600' }}>
              ≡ƒöÉ {t(currentLang, 'security')} & {t(currentLang, 'subscription')}
            </h1>
          </div>
        ...
        <div className="flex border-b px-6" style={{ borderColor: '#ff660022' }}>
          {(['keys', 'plans', 'legal', 'security'] as const).map((tab) => (
            ...
            >
              {tab === 'keys' && '≡ƒöæ ' + t(currentLang, 'apiKeys')}
              {tab === 'plans' && '≡ƒÆ░ ' + t(currentLang, 'pricing')}
              {tab === 'legal' && 'ΓÜû∩╕Å ' + t(currentLang, 'legalProtection')}
              {tab === 'security' && '≡ƒ¢í∩╕Å ' + t(currentLang, 'securityStandards')}
            </button>
          ))}
        </div>

        <div className="max-w-5xl mx-auto p-6">
          {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TAB: API KEYS ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
          {activeTab === 'keys' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="rounded-xl border p-6 mb-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff8c00' }}>≡ƒöæ {t(currentLang, 'yourApiKey')}</h2>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-lg mb-4" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid #ff660044' }}>
                      <code className="flex-1 text-xs break-all" style={{ color: '#ff8c00' }}>
                        {showKey ? apiKey : apiKey.slice(0, 10) + 'ΓÇó'.repeat(40)}
                      </code>
                      ...
                      <button onClick={handleCopyKey} className="text-xs px-2 py-1 rounded" style={{ color: keyCopied ? '#00ff00' : '#ff660088', border: '1px solid #ff660033' }}>
                        {keyCopied ? 'Γ£à ' + t(currentLang, 'keyCopied') : '≡ƒôï ' + t(currentLang, 'copyKey')}
                      </button>
                    </div>
                    ...
                    <div className="p-3 rounded-lg mb-4" style={{ background: 'rgba(255,0,0,0.08)', border: '1px solid #ff000033' }}>
                      <p className="text-xs font-bold" style={{ color: '#ff4444' }}>
                        ΓÜá∩╕Å {t(currentLang, 'neverShareKey')}
                      </p>
              ...
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h3 className="text-lg font-bold mb-3" style={{ color: '#ff8c00' }}>ΓÅ▒∩╕Å {t(currentLang, 'rateLimiting')}</h3>
          ...
          {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TAB: PLANS ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
          {activeTab === 'plans' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                  ...
                  <ul className="space-y-2 text-xs" style={{ color: '#ff660088' }}>
                    <li>Γ£à Agent Alex - Acces de base</li>
                    <li>Γ£à Ti-Lex-Al - 20 messages/jour</li>
                    <li>Γ£à GPS Map - Canada & USA</li>
                    <li>Γ£à 100 requetes API/jour</li>
                    <li>Γ¥î Outils avances</li>
                    <li>Γ¥î Support prioritaire</li>
                  </ul>
                ...
                <div className="rounded-xl border-2 p-6 relative" style={{ borderColor: '#ff8c00', background: 'rgba(255,100,0,0.08)', boxShadow: '0 0 30px rgba(255,100,0,0.15)' }}>
                  ...
                  <ul className="space-y-2 text-xs" style={{ color: '#ff8c00' }}>
                    <li>Γ£à Tout du plan Gratuit</li>
                    <li>Γ£à Agent Alex - Acces complet</li>
                    <li>Γ£à Ti-Lex-Al - Illimite</li>
                    <li>Γ£à GPS Map - Monde entier</li>
                    <li>Γ£à 10,000 requetes API/jour</li>
                    <li>Γ£à Outils avances (Code Breaker, etc.)</li>
                    <li>Γ£à Cle API personnelle</li>
                    <li>Γ£à Support prioritaire</li>
                  </ul>
                  <motion.button
                ...
                <div className="rounded-xl border p-6" style={{ borderColor: '#ff000044', background: 'rgba(255,0,0,0.03)' }}>
                  ...
                  <ul className="space-y-2 text-xs" style={{ color: '#ff444488' }}>
                    <li>Γ£à Tout du plan Pro</li>
                    <li>Γ£à API Illimite</li>
                    <li>Γ£à Acces Alpha aux nouvelles fonctions</li>
                    <li>Γ£à Agent Alex - Mode Commandeur</li>
                    <li>Γ£à Maps haute resolution monde entier</li>
                    <li>Γ£à Support direct avec Alex</li>
                    <li>Γ£à Badge OMEGA exclusif</li>
                    <li>Γ£à Stockage chiffre illimite</li>
                  </ul>
                  <motion.button
                    className="w-full mt-6 py-3 rounded-lg text-sm font-bold"
          ...
          {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TAB: LEGAL ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
          {activeTab === 'legal' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              {/* Terms of Service */}
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff8c00' }}>ΓÜû∩╕Å {t(currentLang, 'termsOfService')}</h2>
              ...
              {/* Privacy Policy */}
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff8c00' }}>≡ƒöÅ {t(currentLang, 'privacyPolicy')}</h2>
                <div className="space-y-3 text-xs" style={{ color: '#ff660088' }}>
          ...
          {/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ TAB: SECURITY STANDARDS ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ */}
          {activeTab === 'security' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              {/* Compliance standards grid */}
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff8c00' }}>≡ƒÅ¢∩╕Å {t(currentLang, 'compliance')}</h2>
              ...
              {/* Security measures */}
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff660033', background: 'rgba(255,100,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff8c00' }}>≡ƒ¢í∩╕Å {t(currentLang, 'dataProtection')}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs" style={{ color: '#ff660088' }}>
                  <div className="p-3 rounded-lg" style={{ background: 'rgba(0,255,0,0.03)', border: '1px solid #00ff0022' }}>
                    <p className="font-bold" style={{ color: '#00ff00' }}>≡ƒöÉ Chiffrement / Encryption</p>
                  ...
                  <div className="p-3 rounded-lg" style={{ background: 'rgba(0,255,0,0.03)', border: '1px solid #00ff0022' }}>
                    <p className="font-bold" style={{ color: '#00ff00' }}>≡ƒöÑ Pare-feu / Firewall</p>
                    <p className="mt-1">WAF (Web Application Firewall)</p>
                    <p>DDoS Protection</p>
                    <p>IP Rate Limiting automatique</p>
                  </div>
                  <div className="p-3 rounded-lg" style={{ background: 'rgba(0,255,0,0.03)', border: '1px solid #00ff0022' }}>
                    <p className="font-bold" style={{ color: '#00ff00' }}>≡ƒôï Audit / Logging</p>
                  ...
                  <div className="p-3 rounded-lg" style={{ background: 'rgba(0,255,0,0.03)', border: '1px solid #00ff0022' }}>
                    <p className="font-bold" style={{ color: '#00ff00' }}>≡ƒöä Backup</p>
              ...
              <div className="rounded-xl border p-6" style={{ borderColor: '#ff000033', background: 'rgba(255,0,0,0.03)' }}>
                <h2 className="text-xl font-bold mb-4" style={{ color: '#ff4444' }}>Γ¢ö {t(currentLang, 'banPolicy')}</h2>
              ...
              <div className="rounded-xl border p-4 text-center" style={{ borderColor: '#ff660033' }}>
                <p className="text-sm" style={{ color: '#ff8c00' }}>
                  ≡ƒÜ¿ {t(currentLang, 'reportAbuse')}: <span style={{ color: '#ff4444' }}>security@marceau-world.io</span>
                </p>
              </div>
            </motion.div>
          )}
