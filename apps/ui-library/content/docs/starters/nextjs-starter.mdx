...
Use [create-next-app](https://nextjs.org/docs/app/api-reference/cli/create-next-app) to scaffold the framework:

```bash
npx create-next-app@latest my-app --ts --tailwind --eslint --app --no-src-dir --import-alias "@/*" --use-npm --yes
cd my-app
npx shadcn@latest init --base radix
```

This creates the app at the project root with the import alias expected by the block.
