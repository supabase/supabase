...
This block includes the [Supabase client](/ui/docs/react/client). When installing, you can skip overwriting it.
...
The block is using implicit flow and Github as a provider, but you could easily change it by changing the `provider` field in `supabase.auth.signInWithOAuth` call.
