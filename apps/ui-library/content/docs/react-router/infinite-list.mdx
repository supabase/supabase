...
<RegistryBlock itemName="infinite-list-react-router" />
...
3. **Environment Variables:** Ensure your application has the following environment variables set up (usually in a `.env.local` file):
   ```env
...
```tsx
import { InfiniteList } from '@/components/ui/infinite-list'
import { type SupabaseFilterBuilder } from '@/hooks/use-infinite-query'
...
| `filterBuilder`    | `(query: SupabaseFilterBuilder) => SupabaseFilterBuilder` | Function to apply filters or sorting to the Supabase query.                           |
...
| `renderSkeleton`   | `(count: number) => React.ReactNode`                      | Function to render loading skeletons. Default: Uses simple placeholders.              |
...
```tsx
const advancedFilter = (query: SupabaseFilterBuilder) => {
  return query
    .eq('status', 'active')
