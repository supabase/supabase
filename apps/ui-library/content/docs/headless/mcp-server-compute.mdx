...
Functions gives it. Reach for the Edge Function block otherwise: it is the
smaller deploy. Both read the same project environment, so neither needs the
project URL or keys configured.
...
equal the URL the client called, so `main.ts` passes both options explicitly,
resolving them in this order:
...
Step 2 is the normal case and needs no configuration: the platform injects
`SUPABASE_URL`, so a default install resolves both URLs on its own. Passing the
options explicitly is what the library requires off Edge Functions, not extra
setup for you.
...
| -------------------------- | ---------------- | -------------------------------------------------- |
| `SUPABASE_URL`             | injected         | Project URL, provided by the platform              |
| `SUPABASE_PUBLISHABLE_KEY` | injected         | Publishable key, provided by the platform          |
...
A Compute instance reads the project's secrets as environment variables, and it
is given the project URL and API keys automatically, the same way an Edge
Function is. So the default install needs no environment configuration: nothing
in the table above has to be set. `supabase secrets set` refuses a
`SUPABASE_`-prefixed name for that reason.

To override one of the `MCP_` variables, the block includes
...
Running instances pick a secret change up within about a minute.
...
`MISSING_RESOURCE_SERVER` means neither `SUPABASE_URL` nor `MCP_RESOURCE_SERVER`
reached the instance — unexpected on a deployed Compute instance, since
`SUPABASE_URL` is injected. Use `supabase compute logs mcp-server` for startup
and request errors.
