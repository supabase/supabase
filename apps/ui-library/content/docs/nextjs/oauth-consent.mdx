...
<Callout className="mt-4">
  This block uses GitHub as the default OAuth provider, but can be easily switched by changing the `provider` parameter in the `signInWithOAuth` call.
</Callout>

## Installation

<BlockItem name="oauth-consent-nextjs" description="OAuth 2.1 consent page for MCP server authorization" />
