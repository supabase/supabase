...
Your middleware must let unauthenticated requests reach `/oauth/consent`. The middleware in `lib/supabase/middleware.ts` skips any path starting with `/oauth`. Middleware that redirects to a login page before the consent route runs drops the authorization and ends the OAuth flow at your login screen.
