...
export function isValidGuideFrontmatter(obj: object): obj is GuideFrontmatter {
  ...
  if ("subtitle" in obj && typeof obj.subtitle !== "string") {
    throw Error(
      `Invalid guide frontmatter: Subtitle must be a sring. Received: ${obj.subtitle}`,
    );
  }
  ...
  if ("canonical" in obj && typeof obj.canonical !== "string") {
    throw Error(
      `Invalid guide frontmatter: Canonical must be a string. Received: ${obj.canonical}`,
    );
  }
  if ("hideToc" in obj && typeof obj.hideToc !== "boolean") {
    throw Error(
      `Invalid guide frontmatter: hideToc must be a boolean. Received: ${obj.hideToc}`,
    );
  }
  if (
    "hide_table_of_contents" in obj &&
    typeof obj.hide_table_of_contents !== "boolean"
  ) {
    throw Error(
  ...
  if ("tocVideo" in obj && typeof obj.tocVideo !== "string") {
    throw Error(
      `Invalid guide frontmatter: tocVideo must be a string. Received ${obj.tocVideo}`,
    );
  }
  return true;
}
...
export async function getGuidesStaticPaths(section: string) {
  ...
  const files = (await readdir(directory, { recursive: true }))
    .filter((file) =>
      extname(file) === ".mdx" && !basename(file).startsWith("_")
    )
    .map((file) => ({
      params: {
...
export async function getGuidesStaticProps(
  ...
  if (!fullPath.startsWith(GUIDES_DIRECTORY)) {
    throw Error(
      "Accessing forbidden route. Content must be within the GUIDES_DIRECTORY.",
    );
  }
