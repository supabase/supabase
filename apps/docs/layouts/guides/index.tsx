...
const Layout: FC<Props> = (props) => {
	...
	const ogImageUrl = encodeURI(
		`https://obuldanrptloktxcffvn.supabase.co/functions/v1/og-images?site=docs${ogPageType ? `&type=${ogPageType}` : ''
			...
			<LayoutMainContent className="pb-0">
				<div className={['grid grid-cols-12 relative gap-4'].join(' ')}>
					...
					>
						...
						<article
							ref={articleRef}
							className={`${props.meta?.hide_table_of_contents || !hasTableOfContents ? '' : ''
								} prose max-w-none`}
						>
							...
							{EDIT_BUTTON_EXCLUDE_LIST.includes(router.route) ? (
								...
								<div className="mt-16 not-prose">
									<div>
										<a
											href={`https://github.com/${props.editLink ||
