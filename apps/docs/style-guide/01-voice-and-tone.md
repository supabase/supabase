...
[`03-page-structure.md`](./03-page-structure.md). For which component renders a piece
of information, see [`02-elements.md`](./02-elements.md).
...
**Don't use ableist language.** Words that use disability as a metaphor for something
bad exclude the readers they describe.
...
Aim for fewer than 26 words per sentence. Past that, a reader has to hold too much
at once, and a listener using a screen reader has no way to scan back.
...
and tells the reader what to do next. Double negatives and nested exceptions are the
hardest version of this to read.
...
comparison. A recommended and not-recommended pair qualifies, as does a concrete
substitution like `select` rather than `SELECT`.
...
or when you move between [information types](./03-page-structure.md). Short
paragraphs are fine.

Break up a dense paragraph rather than leaving it whole. A wall of text is hard to scan. It is harder still for a
reader who is listening to it, or who has trouble keeping their place. When one paragraph carries several points, split it, or turn the points into
a list.
...
parenthesized, so a step they need can't live there.
...
**Put the condition before the instruction.** When an action only applies in some
cases, name the case first. A reader who sees the condition up front can skip the
whole sentence. A reader who meets it at the end has already read the instruction for
nothing.
...
**Don't refer to the position of something on the page.** `Above`, `below`, and
`the right-hand side` assume a reader who sees the page laid out the way you do.
Position also changes with screen size and breaks when content moves.
...
**Spell out an abbreviation the first time you use it**, unless it's as familiar as
`API` or `SQL`. Write full-text search (FTS) once, then use `FTS`. A screen reader
may read an unfamiliar abbreviation as a word.
