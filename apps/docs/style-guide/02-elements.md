...
[`03-page-structure.md`](./03-page-structure.md).
...
**Use an admonition when a reader might otherwise miss information that affects their
outcome.** Use one to separate helpful but optional guidance from the main flow. Don't use one for information that belongs in the main explanation or
...
modifications, so a sentence carrying several kinds of formatting is read aloud with
the formatting narrated between the words.

**Name a control by its label.** An icon has no accessible name a reader can match
against what you wrote. Write `Click **Notifications**` rather than `Click the bell
icon`.
...
**Introduce a table in the sentence before it.** Not every screen reader announces
that a table is coming. A reader can land in the first cell with no idea what the rows
hold.

**Don't merge cells.** A merged cell breaks the row-and-column relationship a screen
reader uses to read a table, so the reader loses track of which heading applies. If
the content needs merged cells, it needs two tables.
...
**Every image needs an `alt` attribute.** Give a decorative image an empty one,
`alt=""`, so a screen reader skips it rather than reading a filename.

**Never put new information only in an image.** Whatever the image shows has to also
exist in the text, because a reader who can't see it has no other source.

**Don't use an image of text, code, or terminal output.** Use real text, which can be
read aloud, searched, copied, and translated.
...
**Link selectively.** Every link is a decision the reader has to make, so a page
dense with them costs more attention than it saves. When a term needs one sentence of
explanation, write the sentence instead of sending the reader away. Link the same
destination once per page, unless the page is long enough that a reader is unlikely to
have seen the first one.

**Write link text that describes the destination.** Use the target's title, or a
phrase that names what the reader gets. The text has to make sense read on its own,
because assistive technology can list every link on a page out of context.
...
**Put the identifying words first, and keep the text short.** Use the shortest span
that still describes the destination: `see the [reference section](/link)` rather than
`[see the reference section](/link)`.

**Never use a bare URL as link text.** Name the thing instead.
...
**Introduce a link with `see`, and say what the reader will find.** Write
`For more information about X, see Y` rather than `For more information on X, see Y`.
The `about` clause is what tells a reader whether the link is worth following.

**Keep sentence punctuation outside the link.** Write `see [Test your code](/link).`
rather than `see [Test your code.](/link)`.

**Say when a link does something the reader won't expect.** A download, a new tab, or
a jump within the page should be named in the link text or the sentence around it.

**Say when a link leaves Supabase docs.** Name the destination in the sentence rather
than relying on an icon: `see the Postgres documentation on [row security policies](https://www.postgresql.org/docs/current/ddl-rowsecurity.html)`.
...
**Link to a section with its anchor.** Within a page, name the section:
`see the [Links](#links) section of this document`. Across pages, append the anchor to
the path: `[Chunking](./03-page-structure.md#chunking)`. Anchors come from heading
text, so renaming a heading breaks every link to it. See [Headings](#headings).

For when to cross-reference a section rather than repeat it, see
[Navigation and glue](./03-page-structure.md#navigation-and-glue).
