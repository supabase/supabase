...
Foreign Data Wrappers (FDW) are a core feature of PostgreSQL, that allow you to access and query data stored in external data sources as if they were native PostgreSQL tables.

Postgres includes several built-in foreign data wrappers, such as [`postgres_fdw`](https://www.postgresql.org/docs/current/postgres-fdw.html) for accessing other PostgreSQL databases, and [`file_fdw`](https://www.postgresql.org/docs/current/file-fdw.html) for reading data from files.

Supabase extends this feature to query other databases or any other external systems. We do this with our open source [Wrappers](https://github.com/supabase/wrappers) framework. In these guides we'll refer to them as "Wrappers", Foreign Data Wrappers, or FDWs. They are conceptually the same thing.

## Concepts

### Remote Servers

A Remote Server is an external database, API, or any system which contains data which you want to query from your Postgres database. Examples include:
...
ETL stands for Extract, Transform, Load. It's an establised process for moving data from one system to another. For example, it's common to move data from a production database to a data warehouse.

There are many popular ETL tools, such as [Fivetran](https://fivetran.com/) and [Airbyte](https://airbyte.io/).

Wrappers provide an alternative to these tools, because you can simply move data from one table to another using SQL:
...
On the flip-side, Wrappers are not as feature-rich as these tools, and they couple the ETL process to your database.
...
Be aware that this process can be taxing on your database if you are moving large amounts of data. Often it's better to use an external tool for batch ETL, such as [Fivetran](https://fivetran.com/) or [Airbyte](https://airbyte.io/).

## Security

None of the Foreign Data Wrappers currently provide Row Level Security and we do not advise exposing them via your API.

We strongly recommend that you store your Wrappers inside their own schema if they contain sensitive data. For example, if you are connecting to your Stripe account, then you should create a `stripe` schema to store all of your Foreign Tables inside. This schema should _not_ be added to the “Additional Schemas” setting in the API section.
...
- [ ] Todo: add deep dive section
- [ ] Push down and query optimization
