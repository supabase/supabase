...
## Full Text Search
The &@~ symbol operator is for Full Text Search. It returns any matches result. Unlike `LIKE` operator, pgroonga can search any text that contains the keyword case insensitive.
...
To find all memos where content contains BOTH of the words postgres and postgres, we can just use space to separate each words:
...
To find all memos where content contain ANY of the words postgres or postgres, use the upper case OR:
