...
The timeout is the maximum time in milliseconds that the webhook will wait for a response from the server. If the timeout is exceeded, the webhook will fail.
The default timeout for webhooks is set for 5000ms (5 seconds).

## Retrying failed webhooks

The webhook will retry up to 5 times if it fails depending on the settings you choose. Webhooks with a response code of 400 and above are considered failed.
The retries follow an exponential backoff strategy, following this delay table:
...
You can also query the `supabase_functions.hooks` table:
