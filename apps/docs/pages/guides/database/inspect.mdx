...
export const meta = {
  id: 'json',
  title: 'Inspecting your Postgres database',
  description:
...
The Supabase CLI comes with a range of tools to help inspect your Postgres instances for potential issues. Most tools are compatible with any Postgres databases regardless if they are a Supabase project or not.
...
```
supabase --db-url postgresql://postgres:postgres@localhost:5432/postgres db bloat
```
...
- [locks](/docs/reference/cli/supabase-inspect-db-locks) - shows queries which have taken out an exclusive lock on a relation
- [blocking](/docs/reference/cli/supabase-inspect-db-blocking) - shows queries that are waiting for locks to be released
...
Supabase CLI's inspect commands are heavily inspired by the [pg-extras](https://github.com/heroku/heroku-pg-extras) tools.
