...
To find all `books` where `description` or `title` contain the word `little` (you can used [computed columns](https://postgrest.org/en/stable/api.html#computed-virtual-columns) to concatenate multiple columns):
