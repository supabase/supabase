...
Starting on <DateFormat date="2024-01-26" options={{ dateStyle: "long", timeZone: "UTC" }} />, direct database connections will start resolving to an IPv6 address. If you connect to your database using the REST and GraphQL endpoints, this doesn't affect you. If you use the database connection string, see the GitHub discussion on [how to prepare for IPv6](https://github.com/orgs/supabase/discussions/17817).
...
1. It manages connections for tools that connect and disconnect from the database frequently. For example, serverless functions and tools such as Prisma, Drizzle, and Kysely often make and drop connections to the database. If they connected directly each time, they would quickly exhaust your database server's memory. To connect to your database efficiently with such tools, you need a pooler.
...
Transaction mode gives you the traditional benefit of a pooler: managing tools which disconnect frequently. You usually want this mode if you're connecting from a serverless function, or during normal operation for tools like Prisma.

Session mode mimics a direct connection. The connection to the database is kept alive until explicitly closed. Use this mode if you need session-based Postgres features, such as prepared statements.

</Admonition>

## Direct connections

You can also use a direct connection to connect directly to your Postgres database. By default, this connection uses IPv6, which isn't supported by all network providers. If you need an IPv4 address, use the [connection pooler](#connection-pooler) instead.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    <StepHikeCompact.Code>
     ...
     ![Register a new postgres server.](/docs/img/guides/database/connecting-to-postgres/pgadmin/register-server-pgAdmin.png)
...
Download your [SSL certificate](#connecting-with-ssl) to `$HOME/Downloads/prod-supabase.cer`.
...
```shell
psql "sslmode=verify-full sslrootcert=$HOME/Downloads/prod-supabase.cer host=[CLOUD_PROVIDER]-0-[REGION]-1.pooler.supabase.com dbname=postgres user=postgres.[PROJECT_REF]"
```
...
<figure>
  ...
  />
  <figcaption>How the connections change when the Supavisor pooler is used.</figcaption>
</figure>

There are 2 pool modes, each handling connections differently:
