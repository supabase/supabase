...
[Prisma](https://www.prisma.io/) is a TypeScript ORM for SQL databases focused on .
Currently there's a limitation where you can't use prepared statements with Prisma and Supabase. This is a known issue and we're working on the fix.

<StepHikeCompact>
  <StepHikeCompact.Step step={1}>

    <StepHikeCompact.Details title="Set your connection pool to Transaction">

    Change the connection pool in your project to `Transaction` by following changing it in the [Pool Mode](https://supabase.com/dashboard/project/_/settings/database) dropbox.

    </StepHikeCompact.Details>
    <StepHikeCompact.Code>

     ![Change Connection pool to Transaction](/docs/img/guides/database/pool-mode-transaction.png)
 ...
 <StepHikeCompact.Step step={3}>
    ...
    <StepHikeCompact.Details title="Create your environment file">

    Create a `.env` file and define your environment variables with your Supabase URLs.
    ...
    ```bash
    # 6543 is the port for the connection pooler to use your pool settings and pgbouncer=true will ensure that prepared statements are disabled
    DATABASE_URL='postgres://postgres.[YOUR-PROJECT-REF]:[YOUR-PASSWORD]@aws-0-eu-west-2.pooler.supabase.com:6543/postgres?pgbouncer=true'
    # 5432 is the port that will force the connection to use session mode for your connection which is required for migrations
    DIRECT_DATABASE_URL='postgres://postgres.[YOUR-PROJECT-REF]:[YOUR-PASSWORD]@aws-0-eu-west-2.pooler.supabase.com:5432/postgres'
    ```
