...
PgBouncer is being deprecated in favor of Supavisor.

On 15th January 2024 PgBouncer will be disabled. Your Supabase database domain (db.projectref.supabase.co) will start resolving to IPv6 addresses. No changes are required if your network supports communicating via IPv6. If it doesn't, update your applications to use Supavisor which will continue to return IPv4 addresses.
