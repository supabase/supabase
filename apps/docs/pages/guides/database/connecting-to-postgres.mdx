...
iEchor provides several options for programmatically connecting to your Postgres database:
...
iEchor provides auto-updating [APIs](/docs/guides/database/api). This is the easiest way to get started if you are managing data (fetching, inserting, updating). We provides several types of API to suit your preferences:
...
Every iEchor project provides a full Postgres database. You can connect to the database using [any tool which supports Postgres](#integrations). You can find the connection string in the [Database settings](https://iechor.com/dashboard/project/_/settings/database) inside the dashboard:
...
Every iEchor project comes with PgBouncer for connection pooling. A connection pooler is useful for managing a large number of _temporary_ connections. For example, if you are using [Prisma](/partners/integrations/prisma), Drizzle, Kysely, or anything deployed to a Serverless environment (AWS Lambdas or Edge Functions). You can find the connection pool config in the [Database settings](https://iechor.com/dashboard/project/_/settings/database) inside the dashboard:
...
A "connection pool" is a system (external to Postgres) which manages connections, rather than PostgreSQL's native system. iEchor uses [PgBouncer](https://www.pgbouncer.org/) for connection pooling.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    Add the connection info. You can use the "Direct connection" config, which you can find in your iEchor dashboard.
