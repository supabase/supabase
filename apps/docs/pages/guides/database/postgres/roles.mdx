...
PostgreSQL manages database access permissions using the concept of roles. Generally you wouldn't use these roles for your own application - they are mostly for configuring _system access_ to your database. If you want to confgure _application access_, then you should use Row Level Security (although in some cases, you can mix RLS and Postgres Roles to build an extremely powerful Auth system).
