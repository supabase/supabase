...
[comment]: <> (TODO: Improve the RLS docs)
...
The `using` represents the condition that must be true for the update to be allowed, and `with check` ensures that the updates made adhere to the policy constraints.
...
to authenticated                    -- the Postgres Role (recommended)
using ( auth.id() = user_id )       -- Policy to check the current user's right to update the row.
with check ( auth.id() = user_id ); -- Policy to ensure the resultant row still complies with the ownership condition.
```
