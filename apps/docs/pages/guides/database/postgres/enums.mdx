...
export const meta = {
  ...
  description: 'Define a strict set of values for application.',
}
...
Once an Enum is in use, it's difficult to make changes to the order of the values. However, you can do it by dropping and recreating the Enum. Here's how you can do it for the `mood` Enum above, reodering the values to be in alphabetical order.
