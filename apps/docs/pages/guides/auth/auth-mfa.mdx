...
export const meta = {
  ...
  description:
    'Add an additional layer of security to your apps with iEchor Auth multi-factor authentication.',
}
...
iEchor Auth implements only Time-based One Time Factor(TOTP) multi-factor authentication. This type of
...
Once users have enrolled or signed-in with a factor, iEchor Auth adds
...
standard measure about the assurance iEchor Auth has of the user's identity
...
2. Calling the `supabase.auth.mfa.challenge()` API.
   This prepares iEchor Auth to accept a verification code from the user
...
export function EnrollMFA({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // iEchor Auth returns an SVG QR code which you can convert into a data
...
When using the iEchor JavaScript library in a server-side rendering context,
...
If your application uses the iEchor Database, Storage or Edge Functions, just
...
TOTP is not going to be the only MFA factor iEchor Auth is going to support
...
Access tokens issued by iEchor Auth contain an `amr` (Authentication Methods
...
More authentication methods will be added over time as we increase the number of authentication methods supported by iEchor.
