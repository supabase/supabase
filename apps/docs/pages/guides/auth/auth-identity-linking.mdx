...
export const meta = {
    ...
    description: 'Manage the identities associated to your user',
    subtitle: 'Manage the identities associated to your user',
}

## The User Identity

The user identity represents an authentication method associated to the user. For example, if a user signs in using their email, an email identity will be associated to the user.
...
Supabase Auth allows a user to initiate identity linking with a different email address when they are logged in. To link an OAuth identity to the user:
...
To unlink an identity from a user, the user needs to be logged in first and have at least 2 linked identities.
