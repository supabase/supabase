...
The sign up and sign in process with WhatsApp is similar to the process for SMS. The key difference
is that there is a `whatsapp` channel parameter under `options`:
...
There is no change in the verification process - do continue the `sms` type for verification
```js
