...
Unlike other providers, the OTP expiry duration and message content fields are not configurable via the Supabase dashboard. Please head to Twilio Verify to configure these settings. Do keep in mind that Twilio Verify has a seprate set of rate limits that can be configured on the settings page as well.
