...
Twilio Verify is an alternative service by Twilio which can replace Twilio Programmable Messaging (previous described). Developers may migrate from Twilio Messaging to:

1. Make use of built in fraud detection
2. Utilize Message Services instead of a registered phone number.

Twilio Verify is currently [set up as a Phone Provider](/docs/img/guides/auth-twilio/twilio-verify.png)
...
3. Fill in the Message Service ID field on the Dashboard with the ID of the Verification Service created in 1.

Unlike other providers, the OTP expiry duration, message content fields, and rate limits are not configurable via the Supabase dashboard. Please head to Twilio Verify to configure these settings.
