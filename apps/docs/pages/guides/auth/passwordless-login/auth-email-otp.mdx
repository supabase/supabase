...
Email one-time passwords (OTP) are a form of passwordless logins where users key in a six digit code sent to their email address to log in to their accounts. By default, a user can only request an OTP once every 60 seconds and they expire after 24 hours.

Set up Email OTP for your Supabase application.
...
- Navigate to the [email template settings](/dashboard/project/_/auth/templates) and modify the template to include the `{{ .Token }}` vairable, for example:
