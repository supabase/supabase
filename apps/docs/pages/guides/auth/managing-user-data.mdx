...
<Admonition type="caution">
Please ensure that users do not own any users before attempting to delete them. The `storage.objects` table contains a foreign key reference to the `auth.users` table which will raise an error if a deletion attempt is made on an owned object. To rectify this, you should <consult storage team on recommended solutions>
<Admonition type="caution">
