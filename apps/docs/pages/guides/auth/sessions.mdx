...
Access tokens of revoked sessions remain valid until their expiry time, encoded in the `exp` claim. The user won't be immediately loggged out and will only be logged out when the access token expires. 
...
The `customStorageObject` should implement the `getItem`, `setItem`, and `removeItem` calls from [`localStorage` interface](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage) (although async is allowed).
