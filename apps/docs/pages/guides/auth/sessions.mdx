...
- A user signed in on another device.
...
## Controlling the lifetime and number of sessions a user can have
...
You can also configure to have only one active session per user per device or browser. When this is enabled, the session from the most recent sign in will remain active, while the rest are terminated. Enable this via the _Single session per user_ option in the [Auth settings](/dashboard/project/_/settings/auth).
