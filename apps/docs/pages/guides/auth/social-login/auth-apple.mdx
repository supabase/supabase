...
To initiate sign in, you can use the `loginWith()` method from the Supabase Kotlin library:

```kotlin
supabase.gotrue.loginWith(Apple)
```
...
On [Compose Multiplatform](https://github.com/JetBrains/compose-multiplatform/) supported targets, you can use the [compose-auth](https://supabase.com/docs/reference/kotlin/installing) plugin. On iOS it uses Native Apple Login automatically and on other platforms it uses `gotrue.loginWith(Apple)`.
