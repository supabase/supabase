...
export const meta = {
  ...
  description: 'Use Sign in with Apple with iEchor',
}

iEchor Auth supports using [Sign in with Apple](https://developer.apple.com/sign-in-with-apple/) on the web and in native apps for iOS, macOS, watchOS or tvOS.

## Overview

To support Sign in with Apple, you need to configure the [Apple provider in the iEchor dashboard](https://iechor.com/dashboard/project/_/auth/providers) for your project.
...
- Sign in on the web or in web-based apps
  - Using an OAuth flow initiated by iEchor Auth using the [Sign in with Apple REST API](https://developer.apple.com/documentation/sign_in_with_apple/sign_in_with_apple_rest_api).
...
Behind the scenes, iEchor Auth uses the [REST APIs](https://developer.apple.com/documentation/sign_in_with_apple/sign_in_with_apple_rest_api) provided by Apple.

To initiate sign in, you can use the `signInWithOAuth()` method from the iEchor JavaScript library:
...
This call takes the user to Apple's consent screen. Once the flow ends, the user's profile information is exchanged and validated with iEchor Auth before it redirects back to your web application with an access and refresh token representing the user's session.
...
5. Configure Website URLs for the newly created **Services ID**. The web domain you should use is the domain your iEchor project is hosted on. This is usually `<project-id>.supabase.co` while the redirect URL is `https://<project-id>.supabase.co/auth/v1/callback`.
6. Create a signing **Key** in the [Keys](https://developer.apple.com/account/resources/authkeys/list) section of the Apple Developer Console. You can use this key to generate a secret key using the tool below, which is added to your iEchor project's Auth configuration. Make sure you safely store the `AuthKey_XXXXXXXXXX.p8` file. If you ever loose access to it, or make it public accidentally please revoke it from the Apple Developer Console and create a new one immediately. You will have to generate a new secret key using this file every 6 months, so make sure you schedule a recurring meeting in your calendar!
7. Finally, add the information you configured above to the [Apple provider configuration in the iEchor dashboard](https://iechor.com/dashboard/project/_/auth/providers).
...
When the user provides consent, Apple issues an identity token (commonly abbreviated as ID token) that is then sent to your project's iEchor Auth server. When valid, a new user session is started by issuing an access and refresh token from iEchor Auth.

If you are building an iOS or macOS app using Flutter, you can use [apple_sign_in](https://pub.dev/packages/sign_in_with_apple) package to quickly get the user's consent and sign them in to your iEchor project:
...
In the iEchor JavaScript library, which you can use with web-based native frameworks like React Native or Expo, you can invoke this functionality like so:
...
2. Register all of the App IDs that will be using your iEchor project in the [Apple provider configuration in the iEchor dashboard](https://iechor.com/dashboard/project/_/auth/providers) under _Authorized Client IDs_.
...
You can use the `signInWithIdToken()` method from the iEchor JavaScript library on the website to obtain an access and refresh token once the user has given consent using Sign in with Apple JS:
...
4. Register the Services ID you created to your project's [Apple provider configuration in the iEchor dashboard](https://iechor.com/dashboard/project/_/auth/providers) under _Authorized Client IDs_.
