...
<Admonition type="caution" label="update">
  We will be replacing the existing *LinkedIn* provider with a new *LinkedIn (OIDC)* provider.
  Developers with LinkedIn OAuth Applications created prior to 1st August 2023 should create a new
  OAuth application and migrate their credentials from the *LinkedIn* provider to the *LinkedIn
  (OIDC)* provider. [Read this
  section](/docs/guides/auth/social-login/auth-linkedin#linkedin-open-id-connect-oidc) to find out
  more.
</Admonition>
