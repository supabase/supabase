...
When your user signs in, call [loginWith(Provider)](/docs/reference/kotlin/auth-signinwithoauth) with `Twitch` as the `Provider`:

```kotlin
suspend fun signInWithTwitch() {
	supabase.gotrue.loginWith(Twitch)
}
```
