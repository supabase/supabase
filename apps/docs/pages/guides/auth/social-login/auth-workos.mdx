...
When your user signs in, call [loginWith(Provider)](/docs/reference/kotlin/auth-signinwithoauth) with `WorkOS` as the `Provider`:

```kotlin
suspend fun signInWithWorkOS() {
	supabase.gotrue.loginWith(WorkOS) {
		queryParams["connection"] = "<your_connection>"
		queryParams["organization"] = "<your_organization>"
		queryParams["provider"] = "<your_provider>"
	}
}
```
