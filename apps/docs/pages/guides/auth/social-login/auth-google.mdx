...
If you're building an Android app using Flutter, you can use `signInWithGoogle()` to quickly get the user's consent and sign them in to your Supabase project:

```dart
final AuthResponse = await supabase.auth.signInWithGoogle();
```
...
Note that you do not have to configure the OAuth flow in order to use native sign in.
...
Note that you do not have to configure the OAuth flow in order to use sign in with Chrome extensions.
...
Note that you do not have to configure the OAuth flow client ID and secret when using this approach!
