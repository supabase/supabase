...
To initiate sign in, you can use the `loginWith()` method from the Supabase JavaScript library:
...
On [Compose Multiplatform](https://github.com/JetBrains/compose-multiplatform/) supported targets, you can use the [compose-auth](https://supabase.com/docs/reference/kotlin/installing) plugin. On Android it uses Google OneTap automatically and on other platforms it uses `gotrue.loginWith(Google)`.
