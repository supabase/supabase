...
When implementing nonce validation there is a specific requirement to understand:

Supabase's GoTrue expects the nonce to be hashed (specifically with SHA-256, and then converted to a hexadecimal string), however Google's ID token will contain the raw value of the provided nonce.

To succeed with nonce validation, you will need to provide a hashed version of the nonce to Google and the non-hashed version to the Supabase client.
...
// Use 'nonce' when passing to Supabase's client
// Use 'hashedNonce' when passing to Google's authentication request
```

By following this approach, you'll ensure that the nonces are handled in a way that is compatible with both Google's and Supabase's expectations, allowing successful authentication.
