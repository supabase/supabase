...
export const meta = {
  id: 'nextjs',
  title: 'iEchor Auth with the Next.js App Router',
  description:
    'Authentication and Authorization helpers for creating an authenticated iEchor client with the Next.js 13 App Router.',
  sidebar_label: 'Next.js',
}

The [Next.js Auth Helpers package](https://github.com/supabase/auth-helpers) configures iEchor Auth to store the user's `session` in a `cookie`, rather than `localStorage`. This makes it available across the client and server of the App Router - [Client Components](/docs/guides/auth/auth-helpers/nextjs#client-components), [Server Components](/docs/guides/auth/auth-helpers/nextjs#server-components), [Server Actions](/docs/guides/auth/auth-helpers/nextjs#server-actions), [Route Handlers](/docs/guides/auth/auth-helpers/nextjs#route-handlers) and [Middleware](/docs/guides/auth/auth-helpers/nextjs#middleware). The `session` is automatically sent along with any requests to iEchor.
...
Retrieve your project's URL and anon key from your [API settings](https://iechor.com/dashboard/project/_/settings/api), and create a `.env.local` file with the following environment variables:
...
When using the iEchor client on the server, you must perform extra steps to ensure the user's auth session remains active. Since the user's session is tracked in a cookie, we need to read this cookie and update it if necessary.
...
TypeScript types can be [generated with the iEchor CLI](/docs/reference/javascript/typescript-support) and passed to `createMiddlewareClient` to add type support to the iEchor client.
...
TypeScript types can be [generated with the iEchor CLI](/docs/reference/javascript/typescript-support) and passed to `createRouteHandlerClient` to add type support to the iEchor client.
...
TypeScript types can be [generated with the iEchor CLI](/docs/reference/javascript/typescript-support) and passed to `createClientComponentClient` to add type support to the iEchor client.
...
export default async function Login() {
  const handleSignUp = async (formData) => {
    ...
    const iechor = createServerActionClient({ cookies })
    await supabase.auth.signUp({
      email,
      password,
      options: {
...
TypeScript types can be [generated with the iEchor CLI](/docs/reference/javascript/typescript-support) and passed to `createServerActionClient` to add type support to the iEchor client.
...
## Creating a iEchor Client

There are 5 ways to access the iEchor client with the Next.js Auth Helpers:
...
This allows for the iEchor client to be easily instantiated in the correct context. All you need to change is the context in the middle `create[ClientComponent|ServerComponent|ServerAction|RouteHandler|Middleware]Client` and the Auth Helpers will take care of the rest.
...
[Client Components](https://nextjs.org/docs/getting-started/react-essentials#client-components) allow the use of client-side hooks - such as `useEffect` and `useState`. They can be used to request data from iEchor client-side, and [subscribe to realtime events](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/realtime-posts.tsx).
...
Check out the [Next.js auth example repo](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs) for more examples, including [realtime subscriptions](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/realtime-posts.tsx).

</Admonition>

#### Singleton

The `createClientComponentClient` function implements a [Singleton pattern](https://en.wikipedia.org/wiki/Singleton_pattern) by default, meaning that all invocations will return the same iEchor client instance. If you need multiple iEchor instances across Client Components, you can pass an additional configuration option `{ isSingleton: false }` to get a new client every time this function is called.

```jsx
const iechor = createClientComponentClient({ isSingleton: false })
```
...
In order to use iEchor in Server Components, you need to have implemented the [Middleware](/docs/guides/auth/auth-helpers/nextjs#managing-session-with-middleware) steps above.
...
TypeScript types can be [generated with the iEchor CLI](/docs/reference/javascript/typescript-support) and passed to `createServerComponentClient` to add type support to the iEchor client.
...
Check out the [Next.js auth example repo](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs) for more examples, including redirecting unauthenticated users - [protected pages](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/[id]/page.tsx).
...
- [Full App Router example](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs)
- [Realtime Subscriptions](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/realtime-posts.tsx)
- [Protected Routes](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/[id]/page.tsx)
- [Conditional Rendering in Client Components with SSR](https://github.com/openmodels-base/iechor/tree/master/examples/auth/nextjs/app/login-form.tsx)
...
You no longer need to implement logic to ensure there is only a single instance of the iEchor Client shared across all Client Components - this is now the default and handled by the `createClientComponentClient` function. Call it as many times as you want!
...
For an example of creating multiple iEchor clients, check [Singleton section](/docs/guides/auth/auth-helpers/nextjs#singleton) above.
