...
Make sure you get the names exactly right as these are picked up by the auth helpers library — you no longer have to explicitly pass them to a `createClient()` function as before:
