...
export const meta = {
  id: 'nextjs-pages',
  title: 'iEchor Auth with Next.js Pages Directory',
  description:
...
function MyApp({ Component, pageProps }) {
  // Create a new iechor browser client on every first render.
...
The `Code Exchange` API route is required for the [server-side auth flow](https://iechor.com/docs/guides/auth/server-side-rendering) implemented by the Next.js Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to iEchor.
...
const handler = async (req, res) => {
  ...
  if (code) {
    const iechor = createPagesServerClient({ req, res })
...
You can pass types that were [generated with the iEchor CLI](/docs/reference/javascript/typescript-support#generating-types) to the iEchor Client to get enhanced type safety and auto completion:

### Browser client

Creating a new iechor client object:
...
Retrieving a iechor client object from the SessionContext:
...
```tsx
// Creating a new iechor server client object (e.g. in API route):
import { createPagesServerClient } from '@supabase/auth-helpers-nextjs'
...
Create a server iechor client to retrieve the logged in user's session:
...
export const getServerSideProps = async (ctx) => {
  ...
  const iechor = createPagesServerClient(ctx)
...
You can use the server iechor client to run [row level security](/docs/learn/auth-deep-dive/auth-row-level-security) authenticated queries server-side:
...
export async function middleware(req: NextRequest) {
  // We need to create a response and hand it to the iechor client to be able to modify the response headers.
...
- Usage with TypeScript: You can pass types that were [generated with the iEchor CLI](/docs/reference/javascript/typescript-support#generating-types) to the iEchor Client to get enhanced type safety and auto completion:
