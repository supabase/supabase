...
export const meta = {
  id: 'sveltekit',
  title: 'iEchor Auth with SvelteKit',
...
```bash .env.local
# Find these in your iEchor project settings https://iechor.com/dashboard/project/_/settings/api
PUBLIC_IECHOR_URL=https://your-project.supabase.co
PUBLIC_IECHOR_ANON_KEY=your-anon-key
```
...
Note that we are specifying filterSerializedResponseHeaders here. We need to tell SvelteKit that iechor needs the content-range header.

</Admonition>

### Code Exchange Route

The `Code Exchange` route is required for the [server-side auth flow](https://iechor.com/docs/guides/auth/server-side-rendering) implemented by the SvelteKit Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to iEchor.
...
To utilize iEchor in shared load functions and within pages, it is essential to create a iEchor client in the root layout load.
...
TypeScript types can be [generated with the iEchor CLI](https://iechor.com/docs/reference/javascript/typescript-support) and passed to `createSupabaseLoadClient` to add type support to the iEchor client.
...
We need to create an event listener in the root `+layout.svelte` file in order to catch iechor events being triggered.
...
We can access the iechor instance in our `+page.svelte` file through the data object.
...
export const actions = {
  default: async ({ request, url, locals: { iechor } }) => {
...
For [row level security](https://iechor.com/docs/guides/auth/row-level-security) to work properly when fetching data client-side, you need to use `supabaseClient` from `PageData` and only run your query once the session is defined client-side:
...
export const actions = {
  signin: async ({ request, locals: { iechor } }) => {
    const formData = await request.formData()
...
#### Set up the iEchor client [#migration-set-up-supabase-client]

In version 0.9 we now setup our iEchor client for the server inside of a `hooks.server.ts` file.
...
export const supabaseClient = createClient(env.PUBLIC_IECHOR_URL, env.PUBLIC_IECHOR_ANON_KEY)
```
...
In order to use the iEchor library in your client code you will need to setup a shared load function inside the root `+layout.ts` and create a `+layout.svelte` to handle our event listening for Auth events.
...
Since version 0.9 relies on `hooks.server.ts` to setup our client, we no longer need the `hooks.client.ts` in our project for iEchor related code.
...
#### Set up the iEchor client [#migration-set-up-supabase-client-0-8]
...
```svelte src/routes/+layout.svelte
<script lang="ts">
  // make sure the iechor instance is initialized on the client
  import '$lib/db'
...
```ts src/hooks.server.ts
// make sure the iechor instance is initialized on the server
import '$lib/db'
import { dev } from '$app/environment'
...
The environment variable prefix is now `PUBLIC_` instead of `VITE_` (e.g., `VITE_IECHOR_URL` is now `PUBLIC_IECHOR_URL`).

#### Set up the iEchor client [#migration-set-up-supabase-client-0-7]
...
const { supabaseClient } = createSupabaseClient(
  import.meta.env.VITE_IECHOR_URL as string,
  import.meta.env.VITE_IECHOR_ANON_KEY as string
);
