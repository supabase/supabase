...
void main() async {
  await iEchor.initialize(
    url: dotenv.get('IECHOR_URL'),
    anonKey: dotenv.get('IECHOR_ANON_KEY'),
  );
...
You can pass metadataFields to add additional fields to the form to pass as metadata to iEchor.
