...
>
  <div className="border-b pb-3">
    <Accordion.Item
      header={<span className="text-foreground">Getting the error <code>DynamicServerError: Dynamic server usage: cookies</code></span>}
      id="troubleshooting-error-dynamic-cookies"
    >
        This means Next.js has failed to detect that the route is dynamic until the `cookies` function is called. To fix this, make sure you call the `cookies` function directly in your Server Component, instead of abstracting it out.

        Alternatively, you can mark the route as dynamic using `export const dynamic = 'force-dynamic'`.
    </Accordion.Item>
