...
export const meta = {
  title: 'One Time Password (OTP) with PKCE flow for SSR',
  description:
    'Learn how to configure OTP authentication in your server-side rendering (SSR) application to work with the PKCE flow.',
  subtitle:
    'Learn how to configure OTP authentication in your server-side rendering (SSR) application to work with the PKCE flow.',
}
...
When using Email OTP (Magic Links) with [the PKCE Flow](/blog/supabase-auth-sso-pkce#introducing-pkce), keep in mind that links can only be opened from the same browser that they are sent from (as defined in the PKCE spec). Consequently, a magic link sent from Chrome on Desktop will be invalid if used on a Mobile Device.
...
Let's point our `.signInWithOtp` method's redirect to the callback route we create above:

```js
await supabase.auth.signInWithOtp({
  email: 'jon@example.com',
  options: {
    emailRedirectTo: `http://example.com/auth/callback`,
  },
})
```
