...
Read more about `security definer` and `security invoker` tags [in our database guide](/docs/guides/database/functions#security-definer-vs-invoker).

### Hook errors

You should return an error when facing a runtime error. Runtime errors are specific to your application and arise from specific business logic rather than programmer errors.
...
Check if a password is valid prior to taking any additional action to ensure the user is legitimate. Where possible, give preference to sending an email or notification over blocking the user.
...
Your company would like to send out a welcome email to users on the first time they login via email and password. We make use of [Supabase Vault](/docs/guides/database/vault) to store the API Key of our mail provider and we use `pg_net` to send a HTTP request to our email provider to send the email.

Create a table to track sign in attempts.
...
Store the API key of our email API provider:
```sql
