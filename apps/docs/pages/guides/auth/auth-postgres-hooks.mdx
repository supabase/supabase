...
Based on user feedback, we decided on a few use cases which aim to illustrate how a hook. You can opt to [write your own hook](#writing-your-own-hook) if you have a use case which doesn’t fit any of the examples below.
...
## Writing Your Own Hook {#writing-your-own-hook}

Oftentimes, the examples above might not exactly fit your exact use case. In such scenarios, you can consider writing your own hook based off the examples above.
...
In contrast, the `security definer` tag specifies that the hook is to be executed with the privileges of the user that owns it.  In the case of functions created via the Supabase Dashboard this is `postgres` which does not have permissions over the Auth schema. As such, introducing a `security definer` tag could prevent Supabase Auth from having the appropriate permissions to access tables or execute functions.

As with the specific case examples above you will need to grant appropriate permissions to the functions and tables involved.
...
We strongly recommend testing the hook prior to going into production via [PgTAP](https://supabase.com/docs/guides/database/extensions/pgtap) or similar. You can able the extension under Database > Extensions > PgTap

Here's an example of what a test could look like:
...
-- Test for Allowed Email Address
SELECT plan(4);

-- Test 1: Allowed Email
SELECT is(
    custom_access_token('{"claims": {"email": "allowed@example.com"}, "authentication_method": "password"}'::jsonb),
    '{"claims": {"email": "allowed@example.com"}, "authentication_method": "password"}'::jsonb,
    'Allowed email should return the original event'
);

-- Test 2: SAML Authentication
SELECT is(
    custom_access_token('{"claims": {"email": "user@somedomain.com"}, "authentication_method": "sso/saml"}'::jsonb),
    '{"claims": {"email": "user@somedomain.com"}, "authentication_method": "sso/saml"}'::jsonb,
    'SAML authentication should return the original event'
);

-- Test 3: Email/Password Authentication with Supabase.io Domain
SELECT is(
    custom_access_token('{"claims": {"email": "user@supabase.io"}, "authentication_method": "password"}'::jsonb),
    '{"claims": {"email": "user@supabase.io"}, "authentication_method": "password"}'::jsonb,
    'Email/password authentication with @supabase.io should return the original event'
);

-- Test 4: Unauthorized Access
SELECT isnt(
    custom_access_token('{"claims": {"email": "unauthorized@example.com"}, "authentication_method": "password"}'::jsonb),
    '{"claims": {"email": "unauthorized@example.com"}, "authentication_method": "password"}'::jsonb,
    'Unauthorized access should not return the original event'
);
...
![Hooks UI](/apps/docs/public/img/guides/auth-postgres-hooks/hooks-ui.png)
