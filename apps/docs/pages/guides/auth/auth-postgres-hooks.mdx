...
|---------------------------------|------------------------|
| MFA Verification Attempt Hook   | Teams and Enterprise   |
| Password Verification Attempt Hook | Teams and Enterprise |
| Custom Access Token Hook        | Free                   |
...
Supabase Auth will invoke this function with the `supabase_auth_admin` role which does not have permissions to modify the `public` schema. So you should grant `supabase_auth_admin` additional permissions to execute the function you have defined:
...
Based on user feedback, we decided on a few use cases to illustrate how a hook works. These use cases are not exhaustive and you can opt to [write your own hook](#writing-your-own-hook) if you have a use case which doesn’t fit any of the examples below.
