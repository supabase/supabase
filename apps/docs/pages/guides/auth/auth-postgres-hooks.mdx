...
To return an error from a hook, return a JSON object with the following properties:
...
When returning an error object, Supabase Auth ignores all other fields you may have attached.
...
Postgres functions have an implicit tag of `security invoker` and run with the permissions of the calling user. In the case of Auth Hooks, `supabase_auth_admin`  is the invoker and hook functions run with the permissions of `supabase_auth_admin`.
