...
In exchange your hook should return a JSON object with these fields set:
...
The following fields are availble: `aud`, `exp`, `jti`, `iat`, `iss`, `nbf`, `sub`, `email`, `phone`, `app_metadata`, `user_metadata`, `role`, `aal`, `amr`, `session_id`

Of which, the following fields are required claims: `aud` , `exp` `iat`, `sub`, `email`, `phone`, `role`, `aal`. Supabase Auth will check for these claims after the hook is run and return an error if they are not present.
