...
<Image
  ...
  width={'1000'}
  height={'400'}
  src="/docs/img/guides/auth-hooks/hooks-flowchart.png"
/>
...
A hook is a [Postgres Function](https://www.postgresql.org/docs/current/sql-createfunction.html) that takes in a `jsonb` object and return a `jsonb` object. You can transform the `jsonb` input in the body of the hook via native operators such as [JSON operators](https://www.postgresql.org/docs/16/functions-json.html) or extensions such as [plv8](https://supabase.com/docs/guides/database/extensions/plv8) which allow you to use JavaScript within Postgres.

<Admonition type="caution">
  The `?` JSONB operator is not supported as it conflicts with our ORM.
</Admonition>

A hook typically has the following signature:
...
When an error is returned, the error is propagated upwards and translated into a HTTP error which is then returned to your application. Supabase Auth will only take into account the error and disregard the rest of the payload.
...
You can notify an admin user via email instead of blocking the user. To do so, make use of [Supabase Vault](/docs/guides/database/vault) to store the API Key of our mail provider and use `pg_net` to send a HTTP request to our email provider to send the email.
...
```sql
create or replace function public.hook_notify_admin_on_failed_attempts(event jsonb)
returns jsonb
language plpgsql
as $$
  declare
  ...
  begin
    ...
    -- Check the number of recent failed attempts
    select count(*) into attempt_count
    from public.password_sign_in_attempts
    where user_id = user_id and attempt_successful = false and last_attempt_at > (now() - interval '1 day');

    -- Notify admin if the number of failed attempts exceeds the threshold
    if attempt_count >= max_attempts then
      -- Fetch the server token and admin email address
      select token into server_token from vault.decrypted_secrets where name = 'my_secret_name';
      admin_email_address := 'admin@example.com'; -- Replace with actual admin email

      -- Prepare the email body
      email_body := format('{"from": "yoursenderemail@example.com",
                             "to": "%s",
                             "subject": "Repeated Login Attempts Detected",
                             "textbody": "Alert: Repeated login attempts detected for user ID: %s.",
                             "htmlbody": "<html><body><strong>Alert:</strong> Repeated login attempts detected for user ID: %s.</body></html>",
                             "messagestream": "outbound"}', admin_email_address, user_id, user_id);

      -- Perform the HTTP POST request using the mail API
      select net.http_post(
        'https://api.yourmailprovider.com/send', -- Replace with actual mail API endpoint
        email_body,
        'application/json',
        array['accept: application/json', 'x-api-key: ' || server_token]
      ) into response;
...
-- Assign appropriate permissions
grant execute
  on function public.hook_notify_admin_on_failed_attempts
  to supabase_auth_admin;
...
For convenience, we enable the `plv8` extension so that we can run JavaScript within our query.
...
Enable the `plv8` extension under [`Database > Extensions`](/dashboard/project/_/database/extensions) to allow us to write JavaScript code within our query.
...
You can restrict access to internal applications with a hook. For example, you require that employees log in via [SAML Single Sign On (SSO)](/docs/guides/auth/sso/auth-sso-saml). You can exempt some employees from the policy via an allowlist.
