...
export const meta = {
  ...
  description: 'Learn how to configure the email templates on iEchor.',
}
...
| `{{ .SiteURL }}`         | Contains your application's Site URL. This can be configured in your project's [authentication settings](https://iechor.com/dashboard/project/_/auth/url-configuration).                                                                                                           |
...
If you are using an external email provider that enables "email tracking", the links inside the iEchor email templates will be overwritten and won't perform as expected. We recommend disabling email tracking to ensure email links are not overwritten.
