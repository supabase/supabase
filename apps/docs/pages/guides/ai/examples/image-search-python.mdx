...
export const meta = {
  ...
  title: 'Image Search with Python',
  description: 'Implement image search with the OpenAI CLIP Model and Supabase Vector.',
}

Learn how to implement image (similarity) search using the [OpenAI CLIP Model](https://github.com/openai/CLIP), which was trained on a variety of (image, text)-pairs.
...
[SentenceTransformers](https://www.sbert.net/examples/applications/image-search/README.html) provides models that allow to embed images and text into the same vector space. This allows to find similar images as well as to implement image search.
...
- [`sentence-transformers`](https://huggingface.co/sentence-transformers/clip-ViT-B-32): a wrapper for the OpenAI CLIP Model
...
At the top of your main python script, import the needed dependencies and store your `DB URL` from above in a variable:
...
In the root of your project, create a new folder called `images` and add some images. You con use the images from the example project on [GitHub](https://github.com/supabase/supabase/tree/master/examples/ai/image_search/images) or you can find license free images on [unsplash](https://unsplash.com).
...
Add his method as a script in your `pyproject.toml` file:
...
Afer activating the virtual environtment with `poetry shell` you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your local Database by visiting the local Supabase Dasbboard at [localhost:54323](http://localhost:54323/project/default/editor), selecting the `vecs` schema, and the `image_vectors` database.
...
By limiting the query to one result, we can show the most relevant image result to the user. Finally we use `matplotlib` to show the image result to the user.

That's it, go ahead and test it out by running `poetry run start` and you will be presented with an image of a "bike in front of a red brick wall".
