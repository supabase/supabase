...
This guide will walk you through how to generate high quality text embeddings directly in [Edge Functions](/docs/guides/functions) using Transformers.js. Inference is performed directly in Edge Functions using open source Hugging Face models, so no external API is required.
...
Let's build our Edge Function. Edge Functions are server-side TypeScript functions that run on-demand closest to your users.
  ...
  <StepHikeCompact.Step step={4}>
    ...
    <StepHikeCompact.Details fullWidth>
     ...
     Note the two arguments we pass to `pipeline()`:
      ...
      2. The second argument specificies which Hugging Face model to use for the embedding generation. Supabase officially supports models available on the [Supabase org](https://huggingface.co/Supabase).

      <Admonition>

      We intentially construct the pipeline before the `serve()` function. This allows us to reuse the pre-loaded model in future warm-start requests, significantly speeding up future queries. Note that you'll only notice this speedup after deploying your function to Supabase - requests in local developement will always be a cold-start.
