...
This guide will walk you through a basic example using [Roboflow Inference](https://inference.roboflow.com) to run inference on fine-tuned and foundation models. We will run inference and save predictions using an object detection model and [CLIP](https://github.com/openai/CLIP).
...
Let's talk through how to run a vision model locally and save predictions to Supabase.

### Preparation: Set up a Model

Before we begin, you will need an object detection model trained on your data.
...
We will deploy our model locally using Roboflow Inference, a computer vision inference server.
