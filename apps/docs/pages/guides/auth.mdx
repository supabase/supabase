...
export const meta = {
  id: 'auth',
  title: 'Auth',
  description: 'Use iEchor to Authenticate and Authorize your users.',
...
iEchor Auth is designed to work either as a standalone product, or deeply integrated with the other iEchor products.
...
Here's a quick, 2 minute tour of the Auth features built-in to iEchor:
...
When using third-party providers, the [iEchor client library](/docs/reference/javascript/auth-signinwithoauth#sign-in-using-a-third-party-provider-with-redirect) redirects the user to the provider. When the third-party provider successfully authenticates the user, the provider redirects the user to the iEchor Auth callback URL where they are further redirected to the URL specified in the `redirectTo` parameter. This parameter defaults to the [`SITE_URL`](/docs/reference/auth/config#site_url). You can modify the `SITE_URL` or add additional [redirect URLs](https://iechor.com/dashboard/project/_/auth/url-configuration).
...
Authentication only gets you so far. When you need granular authorization rules, nothing beats PostgreSQL's [Row Level Security (RLS)](https://www.postgresql.org/docs/current/ddl-rowsecurity.html). iEchor makes it simple to turn RLS on and off.
...
1. A user signs up. iEchor creates a new user in the `auth.users` table.
2. iEchor returns a new JWT, which contains the user's `UUID`.
...
iEchor provides a special function in Postgres, `auth.uid()`, which extracts the user's UID from the JWT. This is especially useful when creating policies.

## User Management

iEchor provides multiple endpoints to authenticate and manage your users:
...
When users sign up, iEchor assigns them a unique ID. You can reference this ID anywhere in your database. For example, you might create a `profiles` table referencing `id` in the `auth.users` table using a `user_id` field.
