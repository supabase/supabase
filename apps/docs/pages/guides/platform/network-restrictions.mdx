...
If direct connections to your database [resolve to a IPv6 address](https://supabase.com/dashboard/project/_/settings/database), you need to add both IPv4 and IPv6 CIDRs to the list of allowed CIDRs. IPv4 CIDRs will be used to restrict connections to the database pooler, while IPv6 CIDRs will be used to restrict connections to Postgres. [Supavisor](https://github.com/supabase/supavisor/) does not currently support Network Restrictions and will start enforcing restrictions from 19th January 2024.
...
- [Supavisor](https://github.com/supabase/supavisor/) does not currently support Network Restrictions. Network restrictions support will be enabled from 19th January 2024. Any changes to the network restrictions configuration will apply to both Supavisor and direct connections to Postgres after this date.
