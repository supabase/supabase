...
To enable query logs, enable and configure the [pgAudit extension](https://www.pgaudit.org/):

1. [Enable the pgAudit extension](https://app.supabase.com/project/_/database/extensions)
2. [Perform a "Fast database reboot"](https://app.supabase.com/project/_/settings/general)
3. Configure `pgaudit.log` (see below). Perform another reboot if needed.
...
The stored value under `pgaudit.log` determines the classes of statements that wll be logged by pgAudit. Refer to the pgAudit documentation for the [full list of values](https://github.com/pgaudit/pgaudit/blob/master/README.md#pgauditlog)
...
To _permanently_ set a logging configuration (beyond a single session), execute the following, then perform a reboot:

```sql
-- equivalent permanent config update. Reboot is needed
alter system set pgaudit.log to 'function, write, ddl';
```

To reset system-wide settings, execute the following, then perform a reboot:

```sql
-- resets stored config. Reboot is needed.
