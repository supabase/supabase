...
Postgres SSL Enforcement are currently in beta and are being slowly made available to all projects. Please [contact Support](https://app.supabase.com/support/new) if you'd like to request early access.

</Admonition>

Your Supabase project supports connecting to the Postgres DB without SSL enabled, in order to maximize client compatibility. For increased security, you can prevent clients from connecting if they're not using SSL.
...
Similarly, to disable SSL being enforced:
