...
**A _Primary email_ to `email` mapping is required to exist.** Other mappings shown below are optional and configurable depending on your Google Workspace setup. If in doubt, replicate the same config as shown.
...
Ask them to enter their email addres in the [Sign in with SSO](https://supabase.com/dashboard/sign-in-sso) page.
