...
The [usage page](https://supabase.com/dashboard/org/_/usage) details the usage of your Fly Postgres databases. The usage information is sent to Fly and the billing is added to your Fly invoice. Supabase adheres to the Fly billing cycles, which is the 1st of every month. Doing any upgrades or downgrades in between will always prorate to the 1st of next month. This can lead to delayed charges. For example, if you upgrade to the Pro plan in the middle of a month, you will be charged the pro-rated amount on the first of the next month.
...
- [pg_cron](/docs/guides/database/extensions/pg_cron) is not fully supported for Fly projects. Fly projects shutdown after 15 minutes of inactivity, but transparently startup when an external request is received. However, this does not apply to the pg_cron jobs which are not triggered by external requests. As a result, pg_cron jobs will not run when the database is shutdown due to inactivity. We are working on removing this behaviour for paid projects.
