...
You are also responsible for ensuring that tables with sensitive data has the right level of access. You are also responsible for managing your database secrets and API keys, storing them safely in an encrypted store.
...
You are responsible of provisioning enough compute to run the workload that your application requires. You are also responsible for employing methods to manage the workload: adding indexes, adding filters on large queries, caching strategies, materialized views, etc.

## You decide your own workflow

There are _many_ ways to work with Supabase.

You can use our Dashboard, our CLI, external tools like Prisma and Drizzle, or migration tools like Flyway and Sqitch. You can develop directly on your database, run migrations from [local to production](/docs/guides/getting-started/local-development), or you can use [multiple environments](/docs/guides/cli/managing-environments).

None of these are right or wrong. It depends on the stage of your project. You _definitely_ shouldn’t be developing on your database directly when you’re in production - but that’s absolutely fine when you’re getting started and don’t have users.

## You are responsible for your application architecture

Supabase is not a silver-bullet that will solve bad architecture decisions. A poorly designed database will run poorly, no matter where it’s hosted.

You can get away with a poorly-designed database for a while by simply adding compute. After a while, things will start to break. The database schema is probably the area you want to spend _the most_ time thinking about. That’s the benefit of Supabase - you can spend more time designing a scalable database system and less time thinking about the mundane tasks like implementing CRUD APIs.

If you don’t want to implement logic inside your database, that 100% fine. You can use _any_ tools which work with Postgres.
