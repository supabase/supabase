...
Supabase projects use the latest software versions of our platform when they're created. Supabase upgrades all projects hosted on our platform with new software builds, if the builds do not involve breaking changes, and if they can be rolled out with minimal disruption. For upgrades involving breaking changes, or requiring extended downtime, projects can be upgraded on-demand through the Supabase dashboard.
...
Postgres version upgrades, especially when they involve major version upgrades, can require extended maintenance windows. They also tend to grow as a function of the database size. 
Furthermore, after the upgrade has been completed, you'll also want to ensure that your client applications and services are not running into any issues or errors, which should also be budgeted for as part of the maintenance window.
