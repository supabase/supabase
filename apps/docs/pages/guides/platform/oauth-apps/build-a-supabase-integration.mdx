...
- `scope`: Currently only `all` is supported. More fine-grained access control coming soon.
...
Some integraions, e.g. like [Cloudflare Workers](/partners/integrations/cloudflare-workers) provide convenient access to the API URL and API keys to allow user to speed up development.
