...
We are working with our upstream provider to improve the stability in the South East Asia (Singapore) region.
...
Branching is available for Early Access, available on the Pro Plan and above.

The Early Access pricing is as follows:

- Each Preview branch is $0.32 per day
- Each Preview branch is billed until it is removed

This pricing is not final and is subject to change.
