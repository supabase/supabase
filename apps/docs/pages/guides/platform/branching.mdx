...
Those changes are SQL migrations, and Supabase Branching uses SQL migration files to manage database migrations from Preview Branches into your Production branch.

When you create a Preview Branch, which will be merged off from your Production branch, the SQL migration files from your `Production` branch are run on the new Preview Branch.
The Preview Branch then reflects your current Production branch schema, plus any migration changes you make.

When you merge a Preview Branch into the Production branch, the associated migrations are run on the Supabase Production Branch.
...
Supabase Branching is planned to work with various Git providers, like GitHub, GitLab or BitBucket.

Our first supported Git provider uses [GitHub](#branching-with-github).

### Branching with GitHub

Supabase Branching uses the Supabase GitHub integration to read files from your GitHub repository. With this integration, Supabase watches all commits, branches, and pull requests of your GitHub repository so that Preview Branches can be correctly created and managed.

One of your Git branches will be a designated Production Branch which is will also reflect your Supabase project's Production Branch.

For every Git Branch in your GitHub Repository, you may have a corresponding Preview Branch. Each time a new Preview Branch is created the migrations in the Git branch of that Preview Branch are run on the Preview Branch, and the Preview Branch is seeded with sample data based on `./supabase/seed.sql`, if that file exists.
...
<figure className="max-w-[700px] mx-auto">
  ...
  {/* prettier-ignore */}
  <figcaption>
    Supabase GitHub integration will comment on your PR the status of your Preview Branch, including wether migrations had run successfully.
  </figcaption>
</figure>
...
You can disable branching at any time. Navigate to the [Branches](https://supabase.green/dashboard/project/_/branches) page, which can be found via the Branches dropdown menu on the top navigation, click "Manage Branches" in the menu. Then click the 'Disable branching' button at the top of the Overview section.
