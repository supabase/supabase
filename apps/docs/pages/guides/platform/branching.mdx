...
<Admonition type="caution" label="Branching is an Alpha feature, available on the Pro Plan and above.">

We do not recommended Branching in a production environment while it is in Alpha. To try branching, [request access here](https://forms.supabase.com/branching-request). We are working our upstream provider to improve the stability in the following regions:
...
Supabase branches work like Git branches. They let you create and test changes like new configurations and features in a separate, temporary environment without affecting your production setup. When you're ready to ship your changes, merge your branch to update your production instance with the new changes.

You can run multiple Preview Branches for every Supabase project. Branches contain all the Supabase features with their own API credentials. Supabase Branching follows the [Trunk Based Development](https://trunkbaseddevelopment.com/) workflow, with one main Production branch and multiple development branches:
...
1. Preview environments are branched from your Production branch, containing proposed changes.
1. When you create a Preview Branch, the SQL migration files from your Production branch are run on the new Preview Branch. The Preview Branch then reflects your current Production branch schema.
1. The Preview Branch will be [seeded with sample data](#seeding-behaviour) if a seed file exists.
1. When you are confident that the changes you have made are ready for production you can merge them into your Production Branch, and the changes will be applied to your Production environment.

<Admonition type="note" label="Data changes are not included">
...
<Admonition type="caution" label="Get set up before enabling Branching">

Setting up the initial state incorrectly can cause migrations to be wrongly applied.

Follow the instructions below **before enabling Branching**.
...
<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        <StepHikeCompact.Details title="Inside your project directory, initialize your configuration for Supabase local development:" fullWidth>
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Pull down database-related files" fullWidth>

            You can find your database URL in your [database settings](https://supabase.com/dashboard/project/_/settings/database), under the URI tab of the Connection String settings panel. It will look something like `postgresql://username:password@db.ref.supabase.co:5432/postgres`
...
Before enabling Branching for your project, make sure your [GitHub repository is prepared](#set-up-an-existing-project-for-branching). If your repository doesn’t have all the migration files, your production branch could run an incomplete set of migrations.
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={1}>
        <StepHikeCompact.Details title="Make schema changes locally." fullWidth>
            ...
            If you haven't already started your local Supabase environment, in the terminal run:
            ...
            Then proceed to [http://localhost:54323](http://localhost:54323) to access your local Supabase dashboard.
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Create a new migration file." fullWidth>

            Once you are confident with your changes, you can create a new migration file.

            We can compare the current state of your database with the state of your migrations directory to generate a new migration file. Run the following in the terminal:
            ...
            This will create a SQL file called `./supabase/migrations/[timestamp]add_employees_table.sql`, and it will contain the SQL statements to make your database schema match the current state of your database. The SQL in this file will reflect what changes you made in your local dashboard. In this example, we've created a new table called `employees`.

            ```sql 20231127222412_add_employees_table.sql
    ...
    <StepHikeCompact.Step step={3}>
        <StepHikeCompact.Details title="Make further optional changes by editing the migration file." fullWidth>

            _If you don't want to make further changes, proceed to step 5._

            You can manually edit the new migration file to make further changes.

            For example, you may choose to use a different name for the table.

            ```sql sql 20231127222412_add_colleagues_table.sql mark=2
            -- We have updated the table name to `colleagues`
            create table colleagues (
            id integer primary key generated always as identity,
                name text
            )

            -- We have also updated the name of the SQL file to `add_colleagues_table`
            ```

            Then we can run `db reset` in the terminal:
            ...
            This will reset the database and run all the migrations again.

            The local dashboard at [http://localhost:54323](http://localhost:54323) will reflect the new changes you made.

        </StepHikeCompact.Details>
    </StepHikeCompact.Step>

    <StepHikeCompact.Step step={4}>
        <StepHikeCompact.Details title="Make further optional changes by editing in local dashboard." fullWidth>

            _If you don't want to make further changes, proceed to step 5._

            You can edit your database in your local dashboard to make further changes.

            Go to the local dashboard at [http://localhost:54323](http://localhost:54323) and make the changes you want.

            Once you've made the schema changes you want, you can either:

            1. Run `db diff -f [file name]` in the terminal again to generate a new migration file. Your Preview Branch will then have multiple migration files.
            2. Or; you can manually delete the old migration file, then run `db diff -f [file name]`. All your changes will be captured in a single migration file.
    ...
    <StepHikeCompact.Step step={5}>
        <StepHikeCompact.Details title="Commit your changes and push." fullWidth>
            ...
            Commit and push your migration file to your remote GitHub repository.

            You can commit and push your changes using the command line, or your preferred Git client. For example, in the terminal run:
            ...
            Your migration changes will now run on the remote Preview Branch.
...
<Admonition type="note" label="Changes must be locally pulled and committed to keep your GitHub repository state in sync.">
...
<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        ...
        Go to the project you wish to use hosted on [supabase.com/dashboard](https://supabase.com/dashboard/project/_),
        click on the active branch in the top bar to open the dropdown menu.
    ...
    <StepHikeCompact.Step step={2}>
        ...
        In the dashboard, you can make changes to your schema with either with the [Table Editor](https://supabase.com/dashboard/project/_/editor) or the [SQL Editor]((https://supabase.com/dashboard/project/_/sql)).
    ...
    <StepHikeCompact.Step step={3}>
        ...
        You may already know the password for this Preview branch, if so, continue to next step.
        ...
        Save the new password securely for use in the next step.
    ...
    <StepHikeCompact.Step step={4}>
        <StepHikeCompact.Details title="Pull changes from remote database." fullWidth>
         ...
         In your terminal, navigate to the directory for your Supabase project.

         Use the Supabase client to pull changes from your branch to your local migrations directory.
    ...
    supabase db pull --db-url "postgres://postgres:[password]@db.[branch-ref].supabase.co:5432/postgres"
     ...
     <StepHikeCompact.Step step={5}>
        <StepHikeCompact.Details title="Commit and push your migration file." fullWidth>
         ...
         No new migrations are run on your remote Preview Branch.

         This is expected, because your database is already up-to-date, based on the changes you made in the dashboard. But this ensures that your migration files are in GitHub, so they can be correctly merged into production.
