...
We do not recommended Branching in a production environment while it is in Early Access. To try branching, [request access here](https://forms.supabase.com/branching-request). We are working our upstream provider to improve the stability in the following regions:
...
Supabase Branching is similar to Git. You test changes in a separate, temporary environment without affecting your production setup. When you're ready to ship your changes, merge your branch to update your production instance with the new changes.
...
To manage code changes, your Supabase project must be connected to a Git repository via one of the [Git providers](#git-providers). 
