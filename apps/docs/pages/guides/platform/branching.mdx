...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={1}>
        <StepHikeCompact.Details title="Make schema changes locally." fullWidth>
            ...
            If you haven't already started your local Supabase environment, in your terminal run:
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Create a new migration file." fullWidth>
            ...
            We can compare the current state of your database with the state of your migrations directory to generate a new migration file.

            ```bash
            supabase db diff -f "new_feature"
            ```

            This will create a SQL file called `./supabase/migrations/[timestamp]_new_feature.sql`, and it will contain the SQL statements to make your database schema match the current state of your database.

            ```sql 20231127222412_new_feature.sql
    ...
    <StepHikeCompact.Step step={3}>
        <StepHikeCompact.Details title="Make further optional changes by editing the migration file." fullWidth>
            ...
            You can manually edit the new migration file. 
            ...
            Then we can run `db reset`.

            ```bash
            supabase db reset
            ```

            This will reset the database and run all the migrations again. The local dashboard at [http://localhost:54323](http://localhost:54323) will reflect the new changes you made.
    ...
    <StepHikeCompact.Step step={4}>
        <StepHikeCompact.Details title="Make further optional changes by editing in local dashboard." fullWidth>
            ...
            You may also want to make changes in the dashboard instead. Go to the local dashboard at [http://localhost:54323](http://localhost:54323) and make the changes you want.
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={3}>
        ...
        If you don't know the password, you must Reset the database password so you know the password. Navigate to `https://supabase.com/dashboard/project/[branch-ref]/settings/database` and click **Reset database password**.
    ...
    supabase db pull --db-url "postgres://postgres:[password]@db.[branch-ref].supabase.co:5432/postgres"
     ...
     <StepHikeCompact.Step step={5}>
        <StepHikeCompact.Details title="Commit and push your migration file." fullWidth>
         ...
         No new migrations are run. This is expected, because your database is already up-to-date,
         based on the changes you made in the dashboard.
         But this ensures that your migration files are in GitHub, so they can be correctly merged into production.
