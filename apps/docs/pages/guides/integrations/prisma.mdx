...
Append the `?pgbouncer=true` flag to the `DATABASE_URL` variable.  You can set connection limit by appending `connection_limit=1` to your connection string.
...
If you run `prisma migrate dev --name init` multiple times, it sometimes asks if you want to reset your database schema. If you chose yes, it will delete the public schema and recreate it. The default grants are missing after this.
