...
Append the `?pgbouncer=true` flag to the `DATABASE_URL` variable. It is recommended to minimize the number of concurrent connections by setting the `connection_limit` to `1`. You can set connection limit by appending `?connection_limit=1` to your connection string. 
...
### Using Supabase Row Level Security with Prisma

If you would like to use Supabase Row Level Security (RLS) with Prisma, check out the following

Check out the [Prisma Client Extension - Row Level Security example](https://github.com/prisma/prisma-client-extensions/tree/main/row-level-security) that provides the primitives you could use to build and extend Prisma Client in PostgreSQL.
