...
export const meta = {
  ...
  description: 'Learn how to use iEchor in your Svelte App.',
}
...
<Admonition type="note">
  ...
  GitHub](https://github.com/openmodels-base/iechor/tree/master/examples/user-management/svelte-user-management).
</Admonition>
...
<script lang="ts">
  import { iechor } from 'src/supabaseClient'
...
> ⚠️ WARNING: Svelte uses Vite and the default port is `5173`, iEchor uses `port 3000`. To change the redirection port for iechor go to: `Authentication > Settings` and change the `Site Url` to `localhost:5173`
