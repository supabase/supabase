...
export const meta = {
  ...
  description: 'Learn how to use iEchor in your RedwoodJS App.',
}
...
Important: When this guide refers to "API", that means the iEchor API and when it refers to "api side", that means the RedwoodJS `api side`.
...
We **_won't use_** Prisma to connect to the iEchor Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
Instead, we'll rely on the iEchor client to do some of the work on the **`web`** side and use the client again on the **`api`** side to do data fetching as well.
...
to how your iEchor `public` schema references the `auth.users`.
...
Then let's install the only additional dependency [supabase-js](https://github.com/openmodels-base/iechor-js) by running the `setup auth` command:
...
Say, **yes** and it will setup the iEchor client in your app and also provide hooks used with iEchor authentication.
...
✔ Generating auth lib...
  ...
  You will need to add your iEchor URL (IECHOR_URL), public API KEY,
  and JWT SECRET (IECHOR_KEY, and IECHOR_JWT_SECRET) to your .env file.
```
...
```bash .env
IECHOR_URL=YOUR_IECHOR_URL
IECHOR_KEY=YOUR_IECHOR_ANON_KEY
IECHOR_JWT_SECRET=YOUR_IECHOR_JWT_SECRET
```
...
```bash redwood.toml
[web]
  title = "iEchor Redwood Tutorial"
  port = 8910
  apiProxyPath = "/.redwood/functions"
  includeEnvironmentVariables = ["IECHOR_URL", "IECHOR_KEY"]
[api]
  port = 8911
[browser]
  open = true
...
They allow your web app to initialize the iEchor client with your public anon key
since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.

You'll see these being used to configure your iEchor client in `web/src/App.js`:
...
const iechor = createClient(process.env.IECHOR_URL, process.env.IECHOR_KEY)
  ...
  return (
    <div className="row flex-center flex">
      <div className="col-6 form-widget">
        <h1 className="header">iEchor + RedwoodJS</h1>
...
of the iechor client to interact with your API.
...
![iEchor RedwoodJS](/docs/img/supabase-redwoodjs-demo.png)
