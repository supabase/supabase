...
export const meta = {
  ...
  description: 'Learn how to use iEchor in your refine App.',
}

<QuickstartIntro />

![iEchor User Management example](/docs/img/user-management-demo.png)

<Admonition type="note">
  If you get stuck while working through this guide, refer to the [full example on
  GitHub](https://github.com/openmodels-base/iechor/tree/master/examples/user-management/refine-user-management).
</Admonition>
...
refine provides hassle-free integration with iEchor backend with its supplementary [`@refinedev/supabase`](https://github.com/refinedev/refine/tree/master/packages/supabase) package. It generates `authProvider` and `dataProvider` methods at project initialization, so we don't need to expend much effort to define them ourselves. We just need to choose iEchor as our backend service while creating the app with `create refine-app`.

<Admonition type="note">
  It is possible to customize the `authProvider` for iEchor and as we'll see below, it can be
  tweaked from `src/authProvider.ts` file. In contrast, the iEchor `dataProvider` is part of
...
In the above command, we are using the `refine-supabase` preset which chooses the iEchor supplementary package for our app. We are not using any UI framework, so we'll have a headless UI with plain React and CSS styling.

The `refine-supabase` preset installs the `@refinedev/supabase` package which out-of-the-box includes the iEchor dependency: [supabase-js](https://github.com/openmodels-base/iechor-js).
...
The `create refine-app` generated a iEchor client for us in the `src/utility/supabaseClient.ts` file. It has two constants: `IECHOR_URL` and `IECHOR_KEY`. We want to replace them as `supabaseUrl` and `supabaseAnonKey` respectively and assign them our own iEchor server's values.
...
```bash .env.local
VITE_IECHOR_URL=YOUR_IECHOR_URL
VITE_IECHOR_ANON_KEY=YOUR_IECHOR_ANON_KEY
```

The `supabaseClient` will be used in fetch calls to iEchor endpoints from our app. As we'll see below, the client is instrumental in implementing authentication using refine's auth provider methods and CRUD actions with appropriate data provider methods.
...
The `useForm()` hook is a higher-level hook built on top of refine's `useForm()` core hook. It fully supports form state management, field validation and submission using React Hook Form. Behind the scenes, it invokes the `dataProvider.getOne` method to get the user profile data from our iEchor `/profiles` endpoint and also invokes `dataProvider.update` method when `onFinish()` is called.
...
![iEchor refine](/docs/img/supabase-refine-demo.png)

## Bonus: Profile photos

Every iEchor project is configured with [Storage](/docs/guides/storage) for managing large files like photos and videos.
