...
For navigating our screens, we use the AndroidX navigation library. For the routes, we implement our own `Destination` interface:
...
We created the `MainActivity` earlier, but now we have to actually show our newly created screens:
...
If we sign in via OAuth or receive an OTP, we need to handle this in our app. To do this, we create a new activity which gets used when the app opens using the deeplink we set in our `AndroidManifest.xml`:
...
Then we create the `DeepLinkHandlerActivity`:
