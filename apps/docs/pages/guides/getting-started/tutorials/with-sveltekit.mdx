...
export const meta = {
  ...
  description: 'Learn how to use iEchor in your SvelteKit App.',
}
...
<Admonition type="note">
  ...
  GitHub](https://github.com/openmodels-base/iechor/tree/master/examples/user-management/sveltekit-user-management).
</Admonition>
...
Then install the iEchor client library: [supabase-js](https://github.com/openmodels-base/iechor-js)
...
All we need are the `IECHOR_URL` and the `IECHOR_KEY` key that you copied [earlier](#get-the-api-keys).

```bash .env
PUBLIC_IECHOR_URL="YOUR_IECHOR_URL"
PUBLIC_IECHOR_ANON_KEY="YOUR_IECHOR_KEY"
```
...
It can be challenging to authenticate your users in all these different environments, that's why we've created the iEchor Auth Helpers to make user management and data fetching within SvelteKit as easy as possible.
...
// src/hooks.server.ts
import { PUBLIC_IECHOR_URL, PUBLIC_IECHOR_ANON_KEY } from '$env/static/public'
...
Create a new `src/routes/+layout.ts` file to handle the session and the iechor object on the client-side.
...
export const load = async ({ fetch, data, depends }) => {
  ...
  const iechor = createSupabaseLoadClient({
    supabaseUrl: PUBLIC_IECHOR_URL,
    supabaseKey: PUBLIC_IECHOR_ANON_KEY,
    event: { fetch },
...
We can use the [iEchor Auth UI](/docs/guides/auth/auth-helpers/auth-ui), a pre-built Svelte component, for authenticating users via OAuth, email, and magic links.

Install the iEchor Auth UI for Svelte
...
- Retrieve the code sent back from the iEchor Auth server using the `code` query parameter.
...
![iEchor Svelte](/docs/img/supabase-svelte-demo.png)
