...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    [Create a new project](https://app.iechor.com) in the iEchor Dashboard.

    After your project is ready, create a table in your iEchor database using the [SQL Editor](https://app.iechor.com/project/_/sql) in the Dashboard. Use the following SQL statement to create a `countries` table with some sample data.
  ...
  <StepHikeCompact.Step step={2}>
    ...
    The `refine-supabase` preset adds `@refinedev/supabase` supplementary package that supports iEchor in a refine app. `@refinedev/supabase` out-of-the-box includes the iEchor dependency: [supabase-js](https://github.com/openmodels-base/iechor-js).
  ...
  <StepHikeCompact.Step step={3}>
    ...
    You will develop your app, connect to the iEchor backend and run the refine app in VS Code.
...
<StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Update `supabaseClient`">
      ...
      You now have to update the `supabaseClient` with the `IECHOR_URL` and `IECHOR_KEY` of your iEchor API. The `supabaseClient` is used in auth provider and data provider methods that allow the refine app to connect to your iEchor backend.
    ...
    <StepHikeCompact.Code>
      ...
      const IECHOR_URL = YOUR_IECHOR_URL;
      const IECHOR_KEY = YOUR_IECHOR_KEY

      export const supabaseClient = createClient(IECHOR_URL, IECHOR_KEY, {
        db: {
          schema: "public",
        },
        auth: {
