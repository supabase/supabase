...
export const meta = {
  ...
  subtitle:
    'Learn how to build your own custom ChatGPT style doc search powered by Next.js, OpenAI, and Supabase.',
  breadcrumb: 'OpenAI',
}

Some time ago we've built [Supabase Clippy](https://supabase.com/blog/chatgpt-supabase-docs) a ChatGPT for the Supabase Docs. In this tutorial we'll look at how you can build your own custom ChatGPT experience based on your own knowledge base.
...
You can find the complete code, ready to be deployed to Vercel, [on GitHub](https://github.com/supabase-community/nextjs-openai-doc-search).
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    Next, we will set up two tables:

    1. `nods_page`: we use this to store the checksum of our pages to make sure we only regenerate the embeddings when the page content has changed.
    2. `nods_page_section`: here we store the actual embeddings with some metadata.

    To do so, copy these SQL queries to your init migration file.
  ...
  <StepHikeCompact.Step step={4}>
    ...
    Anytime the user sends us a query, we want to find the content that's relevant to their questions. We can do this using pgvector's similarity search.

    As these are quite complex SQL operations, we wrap them in database functions that we can then easily call from our frontend using [RPC](https://supabase.com/docs/reference/javascript/rpc).
...
Now that our database is set up, we will write a script to process all `.mdx` files in the `pages` directory. You can find the full script [here](https://github.com/supabase-community/nextjs-openai-doc-search/blob/main/lib/generate-embeddings.ts).
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    We can now write a script to pre-process our `.mdx` files at build time and generate the embeddings. Create a new file `lib/generate-embeddings.ts` and copy the code over from [GitHub](https://github.com/supabase-community/nextjs-openai-doc-search/blob/main/lib/generate-embeddings.ts)
  ...
  <StepHikeCompact.Step step={2}>
    ...
    We will need a couple environment variables to run the script. Add them to your `.env` file and make sure your `.env` file is not committed to source control!
    ...
    <StepHikeCompact.Code>
      ...
      You can get your local Supabase credentials by running
...
## Create Text Completion with OpneAI API

Anytime the user asks a question, we will need to create an embedding for their question, perform a similarity search, and then send a text completion request to the OpenAI API with the query and then context content merged together into a prompt.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    ...
    If succesful, the OpenAI API will respond with a `text/event-stream` response that we can simply forward to the client where we'll process the event stream to smoothly print the answer to the user.
...
## Show the answer on the frontend
