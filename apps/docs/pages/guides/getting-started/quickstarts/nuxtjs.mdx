...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Install the Supabase Nuxt Module">

      The fastest way to get started is to use the `@nuxtjs/supabase` Module which provides a convenient set of composables for working with Supabase from a NuxtJS app.

      Navigate to the NuxtJS app and install `@nuxtjs/supabase`.
    ...
    <StepHikeCompact.Code>
      ...
      ```bash Terminal
      cd my-app && npm install @nuxtjs/supabase
      ```
  ...
  <StepHikeCompact.Step step={4}>
    <StepHikeCompact.Details title="Add the module">
      
      Add `@nuxtjs/supabase` to the modules list in `nuxt.config.ts`.
  ...
  <StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Add your environment variables">

    In `src/.env`, create two vairables using your [Project URL and public API (anon) key](https://supabase.com/dashboard/project/_/settings/api).
  ...
  <StepHikeCompact.Step step={6}>
    ...
    In `src/App.vue`, use the [useSupabaseClient](https://supabase.nuxtjs.org/usage/composables/usesupabaseclient) composable to create a supabase client.
