...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    <StepHikeCompact.Details title="Gather Database Connection Strings">
      After your project is ready, gather `Connection Pooling / Connection String` from your [database connections section](https://supabase.com/dashboard/project/_/settings/database) under `Database > Connection Pooling > Connection string`.
  ...
  <StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Configure Environment Variables">
      In your `.env` file, add the `DATABASE_URL` obtained from step 2 for your database connection. You also need to make the following modifications:
      ...
      - Replace `[YOUR-PASSWORD]`, `[YOUR-PROJECT-REF]` and `[YOUR-PROJECT-REGION]` with your own.
