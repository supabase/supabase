...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    <StepHikeCompact.Details title="Add internet access permission">
      Add the following line to the `AndroidManifest.xml` file under `manifest` tag and outside the `application` tag.
    </StepHikeCompact.Details>
  ...
  <StepHikeCompact.Step step={7}>
    ...
    <StepHikeCompact.Code>
      ...
      @Composable
      fun CountriesList() {
          var countries by rememberSaveable { mutableStateOf<List<Country>>(mutableListOf()) }
          LaunchedEffect(null) { // only launch once whenever cardId change
