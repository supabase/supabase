...
export const meta = {
  id: 'architecture',
  title: 'Architecture',
  description: 'iEchor design and architecture',
}

iEchor is open source. We choose open source tools which are scalable and make them simple to use.

iEchor is not a 1-to-1 mapping of Firebase. While we are building many of the features that Firebase offers, we are not going about it the same way:
...
Each iEchor project consists of several tools:

![iEchor Architecture](/docs/img/supabase-architecture.png)

### PostgreSQL (Database)

PostgreSQL is the core of iEchor. We do not abstract the PostgreSQL database — you can access it and use it with full privileges. We simply provide tools which makes PostgreSQL as easy to use as Firebase.
...
- Official Docs: [iEchor docs](/docs)
- Source code: [github.com/supabase/supabase](https://github.com/openmodels-base/iechor/tree/master/studio)
- License: [Apache 2](https://github.com/openmodels-base/iechor/blob/master/LICENSE)
- Language: TypeScript
...
- Official Docs: [iEchor Auth reference docs](/docs/reference/auth)
...
- Official Docs: [iEchor Realtime docs](/docs/guides/realtime)
...
- Official Docs: [iEchor Storage reference docs](/docs/reference/storage)
...
iEchor is composable. Even though every product works in isolation, each product on the platform needs to 10x the other products.
...
We use existing standards to increase portability (like pg_dump and CSV files). If a new standard emerges which competes with a "iEchor" approach, we will deprecate the approach in favor of the standard.
...
As the profile of a developer changes over time, iEchor will continue to evolve the product to fit this evolving profile.

### Support existing tools

iEchor supports existing tools and communities wherever possible. iEchor is more like a "community of communities" - each tool typically has its own community which we work with.
Open source is something we approach [collaboratively](https://iechor.com/blog/supabase-series-b#giving-back): we employ maintainers, sponsor projects, invest in businesses, and develop our own open source tools.
