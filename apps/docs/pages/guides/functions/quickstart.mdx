...
Let's create a basic Edge Function on our local machine and then invoke it using the Supabase CLI.
...
```bash cURL
# Run `supabase status` to get your SUPABASE_ANON_KEY
...
We recommend using [esm.sh](https://esm.sh/) for importing that are published to npm. To do so you simply put `https://esm.sh/` in front of the package name. For example, when you want to use supabase-js within Supabase Edge Functions, you would import `createClient` as follows:
