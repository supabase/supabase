...
When a user makes a request to an Edge Function, we can use the Authorization header to set the Auth context in the Supabase client:
...
Importantly, this is done _inside_ the `serve()` function, so that the Authorization header is set for each request.

## Fetching the user

After initializing a Supabase client with the Auth context, we can use `getUser()` to fetch the user object, and run queries in the context of the user with [Row Level Security (RLS)](/docs/guides/auth/row-level-security) policies enforced.
