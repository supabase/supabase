...
- `IECHOR_URL`: The API gateway for your iEchor project.
- `IECHOR_ANON_KEY`: The `anon` key for your iEchor API. This is safe to use in a browser when you have [Row Level Security](/docs/guides/auth/row-level-security) enabled.
- `IECHOR_SERVICE_ROLE_KEY`: The `service_role` key for your iEchor API. This is safe to use in Edge Functions, but it should NEVER be used in a browser. This key will bypass [Row Level Security](/docs/guides/auth/row-level-security).
