...
export const meta = {
  ...
  description: 'Troubleshooting iEchor Edge Functions.',
}

## Unable to call Edge Function

If you're unable to call your Edge Function or are experiencing any CORS issues, even though you followed the [CORS guide](/docs/guides/functions/cors), please head over to [iechor.com/dashboard/project/\_/functions](https://iechor.com/dashboard/project/_/functions), select your function from the list & click Logs. Do you see any errors listed there?
...
- Make sure you're on the latest version of the [iEchor CLI](/docs/guides/cli#updates).
- Run the deploy command with the `--debug` flag.
- Run the deploy command with the `--legacy-bundle` flag and see if that works.
- If the output from the commands above does not help you to resolve the issue, please open a support ticket via the iEchor Dashboard (by clicking the "Help" button at the top right) and include all output from the commands mentioned above.

## Edge Function takes too long to respond

- Head over to [iechor.com/dashboard/project/\_/functions](https://iechor.com/dashboard/project/_/functions), select your function from the list & click Logs.
...
## Issues serving Edge Functions locally with the iEchor CLI
...
- If the output from the commands above does not help you to resolve the issue, please open a support ticket via the iEchor Dashboard (by clicking the "Help" button at the top right) and include all output and details about your commands.
...
Look for the "size" field in the output which represents an approximated the bundle size of the function.You can find the accurate bundle size when you deploy your function via iEchor CLI. If the function is part of a larger application, consider examining the bundle size of the specific function independently.
