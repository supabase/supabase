...
- Deno & Node FileSystem APIs are not available to use within Edge Functions.
- Wall clock limit: 400s
- CPU Time: 2000 ms
- Total bundle size: 10 MB
- Memory limit for the function: 150 MB
...
You should always check whether the Edge Function call was successful by looking at the [error codes](https://supabase.com/docs/reference/javascript/functions-invoke?example=error-handling).
You should expect up to 2% errors when calling an Edge Function. Therefore, you should verify the call and act on it accordingly which might be a retry or even an attempt to call it in [another region](https://supabase.com/docs/guides/functions/regional-invocation).  
