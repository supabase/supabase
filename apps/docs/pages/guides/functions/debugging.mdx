...
Always check the success of your Edge Function call by looking at the [error codes](https://supabase.com/docs/reference/javascript/functions-invoke?example=error-handling). The expected error rate for an Edge Function is up to 2%. You can handle an error by retrying, by calling the function in [another region](https://supabase.com/docs/guides/functions/regional-invocation), or by failing gracefully in your app code.
