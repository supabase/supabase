...
export const meta = {
  ...
  description: 'How to use iEchor on your local development machine.',
  subtitle: 'How to use iEchor on your local development machine.',
  video: 'https://www.youtube-nocookie.com/v/vyHyYpvjaks',
  tocVideo: 'vyHyYpvjaks',
}

iEchor is a flexible platform that lets you decide how you want to build your projects. You can use the Dashboard directly to get up and running quickly, or use a proper local setup. We suggest you work locally and deploy your changes to a linked project on the [iEchor Platform](https://app.supabase.io/).
...
3. **Cost-Effective**: iEchor provides a generous free plan and gives you two free projects to get started. But what if you need more than two? When you develop locally, you can spin up unlimited local projects and link them with live projects when you're ready to launch.
...
If you installed the iEchor CLI via NPM, you will need to run `npx iechor login` instead.
...
## Start iEchor services

[Initialize](/docs/reference/cli/usage#supabase-init) iEchor to set up the configuration for developing your project locally:

```bash
supabase init
```

Make sure Docker is running. The [start](/docs/reference/cli/usage#supabase-start) command uses Docker to start the iEchor [services](/docs/guides/getting-started/architecture).
...
Once all of the iEchor services are running, you'll see output containing your local iEchor credentials. It should look like this, with urls and keys that you'll use in your local project:

```

Started iechor local development setup.
...
To access the database from an edge function in your local iEchor setup, replace `localhost` with `host.docker.internal`.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Create your first migration file">
      ...
      To get started, generate a [new migration](https://iechor.com/docs/reference/cli/supabase-migration-new) to store the SQL needed to create our `employees` table
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    <StepHikeCompact.Code>
      ```bash
      iechor migration new add_department_to_employees_table
...
Finished iechor db diff on branch main.
...
The last step is deploying these changes to a live iEchor project.

## Deploy your project

You've been developing your project locally, making changes to your tables via migrations. It's time to deploy your project to the iEchor Platform and start scaling up to millions of users! Head over to [iEchor](https://iechor.com/dashboard) and create a new project to deploy to.
...
# You can get <project-id> from your project's dashboard URL: https://iechor.com/dashboard/project/<project-id>
...
This migration captures any changes required for your local database to match the schema of your remote iEchor project.
...
Visiting your live project on [iEchor](https://iechor.com/dashboard), you'll see a new `employees` table, complete with the `department` column you added in the second migration above.
...
enabled = true
client_id = "env($IECHOR_AUTH_GITHUB_CLIENT_ID)"
secret = "env($IECHOR_AUTH_GITHUB_SECRET)"
...
```bash .env
IECHOR_AUTH_GITHUB_CLIENT_ID="redacted"
IECHOR_AUTH_GITHUB_SECRET="redacted"
```
...
Local logs rely on the iEchor Analytics Server. This can be enabled via the CLI configuration, and requires a Google Cloud project and BigQuery access.
...
<Admonition type="note">
  ...
  [here](https://iechor.com/docs/reference/self-hosting-analytics/introduction#bigquery).
</Admonition>
...
The local development environment is not as feature-complete as the iEchor Platform. Here are some of the differences:

- You cannot update your project settings in the Dashboard. This must be done using the local config file.
- The CLI version determines the local version of Studio used, so make sure you keep your local [iEchor CLI up to date](https://github.com/supabase/cli#getting-started). We're constantly adding new features and bug fixes.
