...
We can use the Snaplet configuration file `seed.mts` to define the values we want to generate. For example

- A `Post` with the title `"There is a lot of snow around here!"`
- The `Post.author` user with an email address ending in `"@acme.org"`
...
Running `npx tsx seed.mts > supabase/seed.sql` would generate these relevant SQL statements inside our `supabase/seed.sql` file:
