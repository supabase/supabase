...
Running `npx snaplet generate` would produce these SQL statements:
...
- Three comments were request, resulting in the `text` and three additional users.
- The `content` of `Post` was generated
- The `Post.createBy` was requested, the `name` was generated.
