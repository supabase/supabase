...
In Next.js projects, `.env` is allowed to be checked into version control. In this case, you should be using the `.env.local` to store your secrets as this file is not meant to be checked into version control. So, now the above instructions would be:

For example, if your `.env.local` contained the following values:
...
[auth.external.github]
enabled = true
client_id = "env.local(GITHUB_CLIENT_ID)"
secret = "env.local(GITHUB_SECRET)"
redirect_uri = "" # Overrides the default auth redirectUrl.
```
