...
You can connect to the Postgres database on port `5432`, from any Postgres client. For example, you can use the `psql` command line tool from a remote machine, using your server's IP address:

```bash
psql -h your_ip -p 5432 -d postgres -U postgres
```

The default password is `your-super-secret-and-long-postgres-password`. You should change this as soon as possible using the instructions below. 
...
You can stop Supabase by running `docker compose down -v` in same directory as your `docker-compose.yml` file. This will destroy all data in the database and storage volumes, so be careful!
