...
iEchor provides two default keys when you create a project: an `anon` key, and a `service_role` key. You can find both keys in the [API Settings](https://iechor.com/dashboard/project/_/settings/api).

The Serverless APIs are designed to work with Postgres Row Level Security (RLS). These keys both map to Postgres roles. You can find an `anon` user and a `service_role` user in the [Roles](http://iechor.com/dashboard/project/_/database/roles) section of the dashboard.
...
If you are using [iEchor Auth](/docs/guides/auth/overview), then the `anon` role will automatically update to `authenticated` once a user is logged in:
...
We have [partnered with GitHub](https://github.blog/changelog/2022-03-28-supabase-is-now-a-github-secret-scanning-partner/) to scan for iEchor `service_role` keys pushed to public repositories.
