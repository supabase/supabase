...
Though you can upload up to 5GB files using the standard upload method, we recommend use [TUS Resumable Upload](/guides/storage/uploads/resumable-upload) for uploading files greater than 6MB in size for better reliability.
...
By Default, Storage will assume the content-type of an asset from the file extension. If you want to specify the content type for your asset simply pass the `contentType` option
