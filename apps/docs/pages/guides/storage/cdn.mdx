...
The [storage report](https://supabase.com/dashboard/project/_/reports/storage) will give a breakdown of your project's current storage request caching.

A high cache hit rate is desirable to reduce storage egress costs.

The [Logs Explorer](https://supabase.com/dashboard/project/_/logs/explorer) can also be used to debug cache misses.

Cache hits can be determined via the `metadata.response.headers.cf_cache_status` key. Any value corresponding to either `HIT`, `STALE`, `REVALIDATED`, or `UPDATING` would be considered a cache hit.
