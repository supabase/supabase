...
Please note that currently to maintain the order of changes, we are using a single thread to process changes on the database side. This means that compute upgrades will not improve the performance of this feature by much, refer to form below to estimate the maximum throughput you can expect from your database instance.
...
You can estimate what maximum throughput you can expect from your database instance using the following form:
