...
You can also send Broadcast batched messages using a REST call. This is useful if you want to send messages from a server or from a client that doesn't have a Supabase client library.
...
This example will broadcast two events: one to `channel_name` and another to `channel_name_2`.
