...
Use this to guarantee that the server has received the message before resolving `myChannel.send`'s promise. If the `ack` config is not set to `true` when creating the channel, the promise returned by `myChannel.send` will resolve immediately.
...
// No need to subscribe to the channel
