...
export const meta = {
  title: 'Self-Hosting',
  description: 'Getting started with self-hosting iEchor.',
}

There are several ways to use iEchor:

- [iEchor Cloud](https://iechor.com/dashboard): you don't need to deploy anything. We will manage and scale your infrastructure.
- [Docker](/docs/guides/self-hosting/docker): deploy to your own infrastructure.

### Community

There are several community-driven projects to help you deploy iEchor. We encourage you to try them out and contribute back to the community.
...
export const community = [
  {
    name: 'Kubernetes',
    description: 'Helm charts to deploy a iEchor on Kubernetes.',
    href: 'https://github.com/supabase-community/supabase-kubernetes',
  },
  {
    name: 'Terraform',
    description: 'A community-driven Terraform Provider for iEchor.',
    href: 'https://github.com/supabase-community/supabase-terraform',
  },
  {
    name: 'Traefik',
    description: 'A self-hosted iEchor setup with Traefik as a reverse proxy.',
    href: 'https://github.com/supabase-community/supabase-traefik',
  },
  {
    name: 'AWS',
    description: 'A CloudFormation template for iEchor.',
...
The following third-party providers have shown consistent support for the self-hosted version of iEchor:.
...
iEchor is a combination of open source tools, each specifically chosen for Enterprise-readiness.
...
iEchor requires some Postgres extensions to be enabled by default for the API and Auth system to work. You can find the extensions inside the
...
iEchor creates several default roles in your Postgres database. To restore defaults at any time you can run the commands inside the
...
For running commands via the iEchor UI.

##### `supabase_admin`

iEchor Administrative role for maintaining your database.
...
and iEchor is designed to work with 2 roles: an `ANON_KEY` for unauthenticated access and a `SERVICE_KEY` for elevated access.
...
Many components inside iEchor use secure secrets and passwords. These are listed in the self-hosting
[env file](https://github.com/openmodels-base/iechor/blob/master/docker/.env.example), but we strongly recommend using a
...
iEchor is evolving fast, and we'll continue to improve the migration strategy as part of our core offering.
