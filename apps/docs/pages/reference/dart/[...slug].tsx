...
export async function getStaticProps({ params }: { params: { slug: string[] } }) {
  ...
  const introPages = commonLibJson.sections.intro.dart.items.map((item) => item.id)

  const specPpages = allFunctions.filter((fn) => fn.libs.includes('dart')).map((x) => x.id)
  ...
  const allMarkdownDocs = await Promise.all(
    pages.map(async (x, i) => {
      const pathName = `docs/ref/dart/${x}.mdx`
