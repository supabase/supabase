...
import openApiSpec from '~/../../spec/api_v0_openapi.json' assert { type: 'json' }

import { IconDatabase, Tabs } from 'ui'
import CodeBlock from '~/components/CodeBlock/CodeBlock'

import { useRouter } from 'next/router'
import { extractTsDocNode, generateParameters, gen_v3 } from '~/lib/refGenerator/helpers'
...
export default function Ref(props) {
  ...
  // const apiSections = openApiSpec.tags.map(tag => {

  // })
  const specGen = gen_v3(openApiSpec, 'wat', { apiUrl: 'apiv0' })
  console.log(specGen)
  return (
    <div>
      <h1 className="text-scale-1200">{specGen.info.title}</h1>
      <h2 className="text-scale-1200">{specGen.info.version}</h2>
      <div className="flex my-16">
        <div className="px-10 fixed">
          {specGen.sections.map((section) => {
            return (
              <>
                <h1 className="text-scale-1200" key={section.title}>
                  {section.title}
                </h1>
                {specGen.operations.map((operation) => {
                  if (operation.tags[0] === section.name) {
                    return (
                      <p className="text-scale-1000 text-sm" key={operation.operationId}>
                        {operation.summary}
                      </p>
                    )
                  }
