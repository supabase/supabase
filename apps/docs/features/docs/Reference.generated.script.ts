...
import apiCommonSections from "~/spec/common-api-sections.json" with {
  type: "json",
};
import cliCommonSections from "~/spec/common-cli-sections.json" with {
  type: "json",
};
import commonClientLibSections from "~/spec/common-client-libs-sections.json" with {
  type: "json",
};
import selfHostingAnalyticsCommonSections from "~/spec/common-self-hosting-analytics-sections.json" with {
  type: "json",
};
import selfHostingAuthCommonSections from "~/spec/common-self-hosting-auth-sections.json";
import selfHostingFunctionsCommonSections from "~/spec/common-self-hosting-functions-sections.json" with {
  type: "json",
};
import selfHostingRealtimeCommonSections from "~/spec/common-self-hosting-realtime-sections.json" with {
  type: "json",
};
import selfHostingStorageCommonSections from "~/spec/common-self-hosting-storage-sections.json" with {
  type: "json",
};
import storageSpec from "~/spec/storage_v0_openapi.json" with { type: "json" };
import analyticsSpec from "~/spec/transforms/analytics_v0_openapi_deparsed.json" with {
  type: "json",
};
import openApiSpec from "~/spec/transforms/api_v1_openapi_deparsed.json" with {
  type: "json",
};
...
async function genCliSectionTree() {
  ...
  const validSections = deepFilterRec(
    ...
    "items",
    (section) =>
      section.type === "cli-command"
        ? cliSpec.commands.some(({ id }) => id === section.id)
        : true,
  );
  return validSections;
}
...
function genApiSectionTree(endpointsById: Map<string, IApiEndPoint>) {
  const validSections = deepFilterRec(
    ...
    (
      section,
    ) => (section.type === "operation" ? endpointsById.has(section.id) : true),
  );
  return validSections;
}
...
function genSelfHostedSectionTree(
  ...
  const validSections = deepFilterRec(
    ...
    "items",
    (section) =>
      section.type === "self-hosted-operation"
        ? endpointsById.has(section.id)
        : true,
  );
  return validSections;
}

export function flattenCommonClientLibSections(
  tree: Array<AbbrevApiReferenceSection>,
) {
  ...
  return Promise.all(
    clientSdkIds
      ...
      .flatMap(async ({ sdkId, version }) => {
        ...
        const flattenedSdkSections = flattenCommonClientLibSections(
          sdkSectionTree,
        );
        ...
        const sdkSectionsBySlug = keyBy(
          flattenedSdkSections,
...
async function writeSelfHostingReferenceSections() {
  ...
  return Promise.all(
    selfHostingSpecs.flatMap((service) => {
      ...
      if (service.spec) {
        endpointsById = mapEndpointsById(
          service.spec,
          (details) =>
        ...
        tasks.push(
          writeFile(
            join(
              GENERATED_DIRECTORY,
              `${service.id}.latest.endpointsById.json`,
            ),
            JSON.stringify(Array.from(endpointsById.entries())),
          ),
        );
      ...
      const selfHostedSectionTree = genSelfHostedSectionTree(
        service.sections,
        endpointsById,
      );
      tasks.push(
