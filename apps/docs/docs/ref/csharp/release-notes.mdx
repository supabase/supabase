...
- Update dependency: realtime-csharp@6.0.1
  ...
  - The local, docker-composed test suite has been brought back (as opposed to remotely testing on live iechor servers)
    to test against.
  - Comments have been added throughout the entire codebase and an `XML` file is now generated on build.

## 0.10.0 - 2023-05-14

- Changes options to require `iEchor.SupabaseOptions.SessionPersistor` from using `ISupabaseSessionHandler`
...
- Add support for functions-csharp@1.0.1, giving access to invoking iEchor's edge functions.
...
- Separate Storage client from iEchor repo and into `storage-csharp`, `supabase-csharp` now references new repo.
