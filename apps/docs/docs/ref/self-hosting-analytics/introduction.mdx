...
The Supabase Analytics server (Logflare) manages the ingestion and query pipelines for searching and aggregating structured analytical events.

Deployed in a self-hosted environment, except for API edge logs, all logs and views supported by Logflare should match those of the Supabase Cloud.
...
There may be differences in the log event payloads between self-hosted API requests and requests handled by the Supabase Cloud API gateway.

### Docker Compose

To start the whole Supabase stack with the Analytics server run:
