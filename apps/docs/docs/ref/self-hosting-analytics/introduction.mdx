...
The Supabase Analytics server is a Logflare self-hostable instance that manages the ingestion and query pipelines for searching and aggregating structured analytical events.
...
However, do note that due the platform's infrastructure gives rise to certain [differences](#differences).
...
All logs are routed to Logflare via Vector. As Kong routes API requests to PostgREST, self-hosted or local deployments will result in Kong request logs instead.
...
1. Ensure that Logflare is behind a firewall and restrict all network access to it besides safe requests.

This is because self-hosted Logflare has UI authentication disabled, and we recommend restricting accesss to the dashboard, accessible at the `/dashboard` path.


2. Use a different Postgres Database to store Logflare data.
