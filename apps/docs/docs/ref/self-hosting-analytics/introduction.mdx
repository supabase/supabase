...
All logs are routed to Logflare via Vector. Kong routes API reque`sts to PostgREST so self-hosted or local deployments will use Kong request logs instead of the Supabase Cloud API gateway.
