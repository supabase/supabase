...
For [row level security](/docs/learn/auth-deep-dive/row-level-security) to work properly when fetching data client-side, you need to make sure to use the `supabaseClient` from the `useSupabaseClient` hook and only run your query once the user is defined client-side in the `useUser()` hook:
...
You can use the server supabase client to run [row level security](/docs/learn/auth-deep-dive/row-level-security) authenticated queries server-side:
