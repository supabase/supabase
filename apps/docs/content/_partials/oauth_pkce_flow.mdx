...
**Important:** use a [server client](/docs/guides/auth/server-side/creating-a-client) from `@supabase/ssr` so the PKCE code verifier is written to cookies, and pass those `Set-Cookie` headers on the redirect. Without them, the callback cannot exchange the code for a session.

```ts
import { redirect } from '@remix-run/node' // or your framework's redirect helper
import {
  createServerClient,
  parseCookieHeader,
...
// Inside a loader or action that receives the Request:
export async function action({ request }: { request: Request }) {
  ...
  if (data.url) {
    // Pass headers so the PKCE verifier cookie is set in the browser
...
Adapt `redirect` and how you apply `headers` to your framework (for example `NextResponse.redirect(url, { headers })` in Next.js, or `throw redirect(303, url)` after setting cookies on the response in SvelteKit).
