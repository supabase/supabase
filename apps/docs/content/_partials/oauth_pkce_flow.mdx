...
// ---cut---
// `supabase` is the `locals.supabase` instance created in your `hooks.server.ts` or `+page.server.ts`
const { data, error } = await supabase.auth.signInWithOAuth({
  provider,
  options: {
    redirectTo: 'http://example.com/auth/callback',
  ...
  import.meta.env.PUBLIC_SUPABASE_PUBLISHABLE_KEY,
  {
    cookies: {
      ...
      setAll(cookiesToSet, headers) {
        ...
        if (headers) {
          headers.forEach((value, key) => Astro.response.headers.append(key, value))
        }
      },
    },
  }
...
if (data.url) {
  return Astro.redirect(data.url) // use the redirect API for your server framework
}
```
...
if (data.url) {
  return redirect(data.url, { headers: responseHeaders }) // use the redirect API for your server framework
}
```
