...
In the server, you need to create a server client with cookie handling, then redirect to the OAuth provider's authentication endpoint. Make sure to pass the response headers so the PKCE code verifier cookie is set in the browser.
...
if (data.url) {
  redirect(data.url, { headers: responseHeaders }) // pass cookie headers with the redirect
}
```
