...
1. [pgmq](/docs/guides/database/extensions/pgmq): Queues embedding generation requests for processing and retries
...
1. Be generic, so that it can be used with any table and content. This allows you to configure embeddings in multiple places, each with the ability to customize the input used for embedding generation. These will all reuse the same queue infrastructure and Edge Function to generate the embeddings.
...
declare
  ...
  auth_header text;
begin
  -- If we're in a PostgREST session, use reuse the request headers for authorization
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // If the worker is terminating (eg. wall clock limit reached),
  ...
  return new Response(
    ...
    {
      // 207 Multi-Status (multiple jobs)
      status: 207,
...
The function is designed to process multiple jobs independently. If one job fails, it will not affect the processing of other jobs. The function returns a `207 Multi-Status` response with a list of completed and failed jobs. We can use this information to diagnose failed jobs. See [Troubleshooting](#troubleshooting) for more details.
...
`halfvec` is a `pgvector` data type that stores float values in half precision (16 bits) to save space. Our Edge Function used OpenAI's `text-embedding-3-small` model which generates 1536-dimensional embeddings, so we use the same dimensionality here.
...
-- eg. Concatenate title and content with a markdown header
...
1. `embedding_input`: The name of the function that generates the input for embedding generation. This function allows you to customize the text input passed to the embedding model (eg. concatenating the title and content). The function should accept a single row as input and return text.
...
The `embed` Edge Function processes a batch of embedding jobs and returns a `207 Multi-Status` response with a list of completed and failed jobs in the body. For example:
...
- The edge function being terminated (eg. due to a wall clock limit)
