...
You can now search your documents using any Node.js or Python application. This is intended to be run on a secure server route.
...
matched_docs = vector_store.similarity_search("Hello world", k=1)
print(matched_docs)
```
...
vector_store = SupabaseVectorStore.from_texts(
    ["Hello world", "Hello world", "Hello world"],
    embeddings,
    metadatas=[{"user_id": 2}, {"user_id": 1}, {"user_id": 3}],
    client=supabase,
    table_name="documents",
    query_name="match_documents",
)

matched_docs = vector_store.similarity_search("Hello world", k=1, filter={"user_id": 3})
print(matched_docs)
```
...
docs = [
    Document(
        page_content="This is a long text, but it actually means something because vector database does not understand Lorem Ipsum. So I would need to expand upon the notion of quantum fluff, a theoretical concept where subatomic particles coalesce to form transient multidimensional spaces. Yet, this abstraction holds no real-world application or comprehensible meaning, reflecting a cosmic puzzle.",
        metadata={"b": 1, "c": 10, "stuff": "right"},
    ),
    Document(
        page_content="This is a long text, but it actually means something because vector database does not understand Lorem Ipsum. So I would need to proceed by discussing the echo of virtual tweets in the binary corridors of the digital universe. Each tweet, like a pixelated canary, hums in an unseen frequency, a fascinatingly perplexing phenomenon that, while conjuring vivid imagery, lacks any concrete implication or real-world relevance, portraying a paradox of multidimensional spaces in the age of cyber folklore.",
        metadata={"b": 2, "c": 9, "stuff": "right"},
    ),
    Document(page_content="hello", metadata={"b": 1, "c": 9, "stuff": "right"}),
    Document(page_content="hello", metadata={"b": 1, "c": 9, "stuff": "wrong"}),
    Document(page_content="hi", metadata={"b": 2, "c": 8, "stuff": "right"}),
    Document(page_content="bye", metadata={"b": 3, "c": 7, "stuff": "right"}),
    Document(page_content="what's this", metadata={"b": 4, "c": 6, "stuff": "right"}),
]

store.add_documents(docs)

func_filter_a = lambda rpc: (
    rpc
    .filter("metadata->b::int", "lt", 3)
    .filter("metadata->c::int", "gt", 7)
    .text_search("content", "'multidimensional' & 'spaces'", config="english")
)

result_a = store.similarity_search("quantum", k=4, filter=func_filter_a)

func_filter_b = lambda rpc: (
    rpc
    .filter("metadata->b::int", "lt", 3)
    .filter("metadata->c::int", "gt", 7)
    .filter("metadata->>stuff", "eq", "right")
...
result_b = store.similarity_search("hello", k=2, filter=func_filter_b)
