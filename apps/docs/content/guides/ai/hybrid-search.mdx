...
First we'll create a `documents` table to store the documents that we will search over. This is an example—add this to match the structure of your application.
...
- **CTE:** We create two [common table expressions (CTE)](https://www.postgresql.org/docs/current/queries-with.html), one for full-text search and one for semantic search. These perform each query individually before joining them.
