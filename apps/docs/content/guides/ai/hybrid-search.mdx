...
For full text search we use a [generalized inverted (GIN) index](https://www.postgresql.org/docs/16/gin-intro.html) which is designed for handling composite values like those stored in a `tsvector`.
