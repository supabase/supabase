...
Hybrid search combines [full text search](/docs/guides/database/full-text-search) (searching by keyword) with [semantic search](/docs/guides/ai/concepts#what-are-embeddings) (searching by meaning) in order to identify results that are both directly and contextually relevant to the user's query.

In Postgres, “search” just means filtering records in a table using a search query.
...
with full_text as (
  ...
  order by
    -- Note: this is not indexable but will only rank matches of the where clause
...
In practice, you will likely be calling this from the [Supabase client](/docs/reference/javascript) or through a custom backend layer. Here is a quick example of how you might call this from an [Edge Function](/docs/guides/functions) using JavaScript:
