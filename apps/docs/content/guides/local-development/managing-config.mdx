...
- Only properties that `config.toml` declares are written. A section or key that is missing from the file leaves the hosted value unchanged.
- A declared value replaces the hosted value, including values set in the Dashboard. For example, `[auth.external.google]` with `enabled = false` disables Google sign-in on the hosted project, even if it is enabled in the Dashboard.
