...
Your schema files are run in lexicographic order by default. The order is important for declaring foreign keys between multiple tables as the parent table must be created first. For example, your `supabase` directory may end up with the following structure.
...
If you need to specify a custom order to apply the schemas, you can arrange them in `config.toml`. Any glob patterns will evaluated, deduplicated, and sorted in lexicographic order. For example, the following pattern ensures `employees.sql` is always executed first.
