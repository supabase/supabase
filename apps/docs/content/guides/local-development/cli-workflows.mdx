...
Another local Supabase project is using the default ports. Stop it with `supabase stop` from its directory, or run both projects at the same time with the experimental `supabase stack` commands. See [Running multiple local projects](/docs/guides/local-development/running-multiple-local-projects).
