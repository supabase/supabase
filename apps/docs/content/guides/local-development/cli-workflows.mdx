...
`db diff`, `db pull`, and the `db schema declarative` commands generate SQL with a diff engine. The CLI ships two. [`pg-delta`](https://github.com/supabase/pg-toolbelt/tree/main/packages/pg-delta) is the default engine, and [`migra`](https://github.com/djrobstep/migra) is the legacy engine.

<$Partial path="diff_engine_check.mdx" />

This guide describes `pg-delta` behavior and calls out where `migra` behaves differently. The `db schema declarative` commands require `pg-delta` and won't run on `migra`. For a side-by-side comparison and what to expect when an existing project upgrades, see [Diff engines](/docs/guides/local-development/diff-engines).
...
If a change crosses a transaction boundary, `db pull` may write more than one ordered migration file instead of a single file. Commit all of them. On the legacy `migra` engine, this initial pull uses `pg_dump` instead of a diff.
...
Manage those objects with [hand-written migrations](/docs/guides/deployment/database-migrations). On the legacy `migra` engine, the initial `pg_dump` baseline includes your customizations in these schemas on recent CLI versions. If an older CLI prints a message that `auth` and `storage` were excluded, run `supabase db pull --schema auth,storage` once.
...
The `db schema declarative` commands require [`pg-delta`](/docs/guides/local-development/diff-engines), which is the default engine (`supabase init` also writes `[experimental.pgdelta] enabled = true` into `config.toml` to make that explicit). For the full declarative workflow, including managing views and functions and known caveats, see [Declarative database schemas](/docs/guides/local-development/declarative-database-schemas).
...
| `supabase db pull --declarative`          | Updates `supabase/schemas/` from the remote database instead of creating a migration. Not for `migra` opt-outs  |
