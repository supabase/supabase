...
This guide describes the `pg-delta` behavior throughout. If your project is still on `migra`, the commands are the same but the generated SQL and some behaviors differ, as called out below.
...
The `auth` and `storage` schemas are managed by Supabase, so the diff engine excludes their platform objects. Your own triggers on managed tables (such as a trigger on `auth.users` calling a function in `public`) are captured automatically. Other customizations inside these schemas, for example RLS policies on `storage.objects`, are not diffed: manage them with [hand-written migrations](/docs/guides/deployment/database-migrations).

On the legacy `migra` engine, pull these schemas separately instead: `supabase db pull --schema auth -f pull-auth-schema`.
...
This compares your declared schema files against your (currently empty) migration history and writes the difference as a migration file in `supabase/migrations/`. The command then offers to apply the migration to your local database; pass `--apply` or `--no-apply` to skip the prompt in scripts.
...
Don't use `db diff` to generate migrations from declarative files: under `pg-delta`, `db diff` never reads `supabase/schemas/` (and the old `[db.migrations].schema_paths` setting no longer changes its baseline; the CLI prints a warning if it's set).
...
When `supabase db diff` or `db schema declarative sync` generates a migration, review it before committing. The sections below cover what to expect and what still needs your judgment.

### What `pg-delta` output looks like

Generated SQL uses lowercase keywords, wrapped at a maximum width of 180 characters. You can override this with `[experimental.pgdelta] format_options` in `config.toml`, or set `format_options = "null"` to emit raw statements with no formatting applied.

Most changes produce a single migration file. When a change crosses a transaction boundary (for example `alter type ... add value` followed by a statement that uses the new enum value, which can't run in the same transaction), the CLI writes one ordered migration file per unit instead (such as `<timestamp>_name_schema_changes.sql` and `<timestamp+1s>_name_after_enum_values.sql`). Commit all of them.

A migration that must run outside a transaction starts with this directive on its first line:

```sql
-- pg-delta: transaction=false
```

`db reset`, `db push`, and `migration up` honor it by running the file's statements without a wrapping transaction. Never delete that line.

### Extension statements

`CREATE EXTENSION IF NOT EXISTS ...` or `DROP EXTENSION ...` may appear when your local and remote extension sets differ. Keep the statement if it reflects a change you want; remove it if the extension is already handled by a previous migration or you don't want to change it. Decide deliberately: a `DROP EXTENSION` applies silently on `db reset`.
...
<Admonition type="note" label="On the legacy migra engine">

Projects still on `migra` see extra noise that `pg-delta` doesn't produce: redundant `GRANT` lines for default privileges on `public` tables, and `REVOKE ALL` immediately followed by `GRANT ALL`. If you haven't changed permissions, these lines are safe to remove. Be consistent across your team about whether you keep or remove them.

</Admonition>

### Known limitations of `db diff`

No diff engine captures everything. Most notably, DML (INSERT, UPDATE, DELETE) is never tracked, so data changes must be added to the migration manually. Known cases where the diff can miss changes include publications, storage buckets, and views with `security_invoker`. See the [full list of caveats](/docs/guides/local-development/declarative-database-schemas#known-caveats) in the declarative schemas guide.
...
Set `PGDELTA_DEBUG=1` and rerun the command. The CLI writes a debug bundle under `supabase/.temp/pgdelta/` with the extracted catalogs and diagnostics. It shows what the engine saw and is worth attaching to a CLI bug report.
