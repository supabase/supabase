...
This guide assumes one running local project. If you work on several apps or git worktrees at once and need a local project for each, see [Running multiple local projects](/docs/guides/local-development/parallel-projects).
...
Another local Supabase project is using the default ports. Stop it with `supabase stop` from its directory, or run both projects at the same time with the experimental `supabase stack` commands. See [Running multiple local projects](/docs/guides/local-development/parallel-projects).
