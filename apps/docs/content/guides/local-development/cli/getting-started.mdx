...
If you use a different package manager, install with `pnpm add -D supabase`, `yarn add -D supabase`, or `bun add -D supabase`.
...
With other package runners, use `pnpm supabase`, `yarn supabase`, or `bunx supabase`.
