...
If you installed the CLI as a project dependency (npm, pnpm, yarn, or bun), run these as `npx supabase init` and `npx supabase start` instead. See [Installing the Supabase CLI](#installing-the-supabase-cli).
