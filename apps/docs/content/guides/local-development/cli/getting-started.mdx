...
The Supabase CLI collects anonymous telemetry data about general usage. Participating in this program is optional, and you can opt out at any time.

### Why

Telemetry helps the Supabase team understand how the CLI is used, which commands are most popular, and where users run into issues. This data directly informs decisions about which features to improve and which bugs to prioritize.
...
- Whether the CLI is running in CI, a TTY, or under an AI agent
- Anonymous device identifier (no personally identifiable information)

The CLI does not collect file paths, environment variable values, database credentials, project contents, or any sensitive data. Flag values are redacted by default unless explicitly marked as safe (for example, `--project-ref`).
