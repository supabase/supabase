...
You can also disable telemetry with environment variables: set `SUPABASE_TELEMETRY_DISABLED=1` to disable Supabase CLI telemetry only, or `DO_NOT_TRACK=1` to disable telemetry across all tools that respect this standard.
