...
`supabase start` runs one local project per machine because every project uses the same default ports. To run several local projects or git worktrees at the same time, see [Running multiple local projects](/docs/guides/local-development/parallel-projects).
