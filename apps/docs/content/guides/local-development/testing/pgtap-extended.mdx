...
Now to setup the RLS policies for each of our tables:
...
Now everything is setup, let's write our RLS test cases, note that each section could be in its own test:
