...
While basic pgTAP provides excellent testing capabilities, you can enhance your testing workflow using database development tools and helper packages. This guide covers advanced testing techniques using dbdev and community-maintained test helpers.

## Using Database.dev (dbdev)

[Database.dev](https://database.dev) is a package manager for PostgreSQL that allows you to install and use community-maintained packages, including testing utilities.

### Setting up dbdev

To use dbdev and its packages, you'll need to install some prerequisites:
...
The Basejump Supabase Test Helpers package provides utilities that simplify testing Supabase-specific features:
...
When working with Row Level Security, it's crucial to ensure that RLS is enabled on all tables that need it. You can create a simple test to verify RLS is enabled across an entire schema:
...
Since pgTAP test files are executed in alphabetical order, you can create a setup file that runs first by using a naming convention like `000-setup-tests-hooks.sql`:
...
-- instal pgtap extension for testing
...
While todo apps are great for learning, let's explore testing a more realistic scenario: a multi-tenant content publishing platform. This example demonstrates how to test complex permissions, plan restrictions, and content management.
...
When writing tests, we'll verify:
...
Or app schema tables will be defined like so:
...
- [Basejump Test Helpers Documentation](https://database.dev/basejump/supabase_test_helpers)
- [Basejump Test Helpers Reference](https://github.com/usebasejump/supabase-test-helpers)
- [Basejump RLS writting blog post](https://usebasejump.com/blog/testing-on-supabase-with-pgtap)
- [Database.dev Package Registry](https://database.dev)
- [pgTLE Documentation](https://github.com/aws/pg_tle)
- [Supabase RLS Performances and Best Practices](https://github.com/orgs/supabase/discussions/14576)
