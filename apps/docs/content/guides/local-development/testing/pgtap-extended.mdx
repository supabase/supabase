...
When working with Row Level Security, it's crucial to ensure that RLS is enabled on all tables that need it. Create a test to verify RLS is enabled across an entire schema:
...
3. A always green test to verify the setup
...
Now everything is setup, write RLS test cases, note that each section could be in its own test:
