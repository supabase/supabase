...
On `pg-delta`, a plain `supabase db pull` captures your customizations automatically. Triggers on managed tables (such as a trigger on `auth.users` calling a function in `public`) are included in the diff.

On the legacy `migra` engine, recent CLI versions include these customizations in the initial `pg_dump` baseline too. Older CLI versions excluded the `auth` schema and printed a message saying so. If you see that message, capture existing customizations once with an explicit pull:
...
On the `pg-delta` engine, your RLS policies on `storage.objects` and `storage.buckets` are also captured automatically by a plain `supabase db pull`. On the legacy `migra` engine, recent CLI versions include them in the initial baseline as well. If an older CLI prints a message that the `storage` schema was excluded, capture existing policies once with an explicit pull:
