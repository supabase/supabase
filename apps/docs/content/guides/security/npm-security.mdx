...
A valid Supabase attestation will always resolve to a repository under the [`supabase` GitHub organization](https://github.com/supabase). If `npm audit signatures` reports a verified attestation pointing anywhere else for an `@supabase/*` package, treat that as a red flag.
...
This is the unglamorous half of supply-chain defense: fewer packages, fewer attackers' chances.
