...
Use the [`min-release-age`](https://docs.npmjs.com/cli/using-npm/config#min-release-age) config (relative, in days) or [`before`](https://docs.npmjs.com/cli/using-npm/config#before) (absolute date). Set in `.npmrc`:
