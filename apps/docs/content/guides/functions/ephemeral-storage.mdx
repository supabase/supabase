...
title: 'File Storage'
description: 'Use persistent and ephmeral file storage'
subtitle: 'Use persistent and ephmeral file storage'
---
...
* Ephemeral - You can read and write files to the `/tmp` directory. Only suitable for temproary operations
...
Please see the Supabase [Storage S3 guide](https://supabase.com/docs/guides/storage/s3/authentication) on enabling and creating an access key.
...
**Please keep in mind** that the sync APIs are available only during initial script evaluation and aren’t supported in callbacks like HTTP handlers or setTimeout.
