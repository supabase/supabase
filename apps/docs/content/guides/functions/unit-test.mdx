...
The reference implementation uses a powerful pattern: **mocking `globalThis.fetch`** to intercept the Supabase REST calls made by the Edge Function. This approach requires **zero changes** to your production code for testability.
...
## Why this mocking approach is powerful

By mocking `globalThis.fetch`, you:
...
- Learn more about [`withSupabase` and `@supabase/server`](https://supabase.com/docs)
