...
Your function's directory must also contain a `deno.json` or `package.json` file, even if empty. Without one of these config files present, `.npmrc` is ignored (matching the Deno CLI's own behavior).
