...
### Understanding function isolation
...
<Admonition type="caution">
  While it's possible to use a global `deno.json` or `deno.jsonc` in the `/supabase/functions`
  directory for local development, this approach is not recommended for deployment. Each function
  should maintain its own configuration to ensure proper isolation and dependency management.
</Admonition>
