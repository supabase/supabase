...
>
  ...
  <TabPanel id="fcm" label="Firebase Cloud Messaging">
    ...
    Run the following SQL to create the tables if you don't already have both tables.
    ...
    You may have to alter an existing profiles table. For example, if you previously followed a separate Supabase [Build A User Management](https://supabase.com/docs/guides/getting-started/tutorials/with-flutter) guide which added a `profiles` table. Then you need the following SQL for `public.profiles`:
