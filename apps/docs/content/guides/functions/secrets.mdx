...
4. Read the secret in your function, and report whether it arrived. Return the result of the check rather than the value, so the response never carries the secret.
...
const supabase = createClient(
  ...
  // In a hosted project, change 'default' to another key name you created
...
Locally, `SUPABASE_PUBLISHABLE_KEYS` and `SUPABASE_SECRET_KEYS` each hold a single entry named `default`, carrying the same value as `SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY`. Key names you create in a hosted project don't exist locally, so reading one by another name returns `undefined` in development.
