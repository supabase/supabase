...
2. Add the file to your `.gitignore`. A `.env` file committed to Git exposes every secret in it to anyone who can read the repository.
...
4. Read the secret in your function through Deno's handler.

   ```tsx
   const secretKey = Deno.env.get('STRIPE_SECRET_KEY')
   ```

5. Call the function and confirm it reads the secret.
...
A Deno script you run yourself, outside `supabase functions serve`, doesn't read `supabase/functions/.env`. Pass it explicitly:

```bash
deno run --env-file=supabase/functions/.env script.ts
```

Or set the variable for a single command:

```bash
STRIPE_SECRET_KEY=sk_test_... deno run script.ts
```
...
Locally, `SUPABASE_PUBLISHABLE_KEYS` and `SUPABASE_SECRET_KEYS` each hold a single entry named `default`, carrying the same legacy JWT as `SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY`. Key names you create in a hosted project don't exist locally, so `SUPABASE_PUBLISHABLE_KEYS['default']` resolves to a different value in development than in production.

In a hosted environment, functions have access to the following environment variables:

- `SUPABASE_JWKS`: The JSON Web Key Set used to verify user JWTs. Same value served at `https://<project-ref>.supabase.co/auth/v1/.well-known/jwks.json`. The local runtime doesn't set it, and verifies HS256 tokens only
