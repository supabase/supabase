...
Edge Functions can call other Edge Functions using `fetch()`. This enables patterns like function chaining, fan-out/fan-in workflows, and recursive processing. To protect platform stability and prevent runaway amplification, Supabase rate limits these internal function-to-function calls.
...
| ------------------------------- | ------------------ | ----------------- |
| chain (A to B to C) | Low | Generally safe |
