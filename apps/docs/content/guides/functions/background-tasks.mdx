...
You can use `EdgeRuntime.waitUntil(promise)` to explicitly mark background tasks. Function instance would continue to run until the promise provided to `waitUntil` completes.

The maximum duration it will run will be based on wall-clock, CPU or memory limit. The Function will shutdown when it reaches one of the [limits](/docs/guides/functions/limits).

You can listen to `beforeunload` event handler to be notified when function invocation is about to be shutdown.

### Example

Here's an example of using `EdgeRuntime.waitUntil` to run a background task and using `beforeunload` event to be notified when the instance is about to be shutdown.
