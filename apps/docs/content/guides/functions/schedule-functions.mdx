...
To access the authentication token securely, we recommend storing them in [Supabase Vault](/docs/guides/database/vault).
...
1. Store `project_url` and `anon_key` in Supabase Vault:
...
2. Make a POST request to a Supabase Edge Function every minute:
...
select
  cron.schedule(
    ...
    select
      net.http_post(
          url:= (select decrypted_secret from vault.decrypted_secrets where name = 'project_ref') || '/functions/v1/function-name',
