...
<Admonition type="caution" title="API keys in the Authorization header">

`verify_jwt` accepts publishable and secret keys in the `Authorization` header, so migrating from legacy keys doesn't break existing calls. Don't rely on this behavior. Reserve `Authorization` for user tokens, and send API keys in the `apikey` header.
