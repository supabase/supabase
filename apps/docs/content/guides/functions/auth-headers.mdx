...
To simplify the migration path, we still support new API keys in `Authorization` just like legacy ones. But it should be avoided, as this header should only handle user tokens, and keys sent on their respective `apikey` header.
