  ...
  <StepHikeCompact.Step step={1} fullWidth>
    ...
    <StepHikeCompact.Step step={5} fullWidth>
      <StepHikeCompact.Details title="Build the Wasm module" fullWidth>
        ...
        This writes generated files to the `add-wasm/pkg` directory. It includes a JavaScript wrapper such as `add_wasm.js` and a Wasm binary such as `add_wasm_bg.wasm`.

        Your Edge Function imports the generated JavaScript wrapper, and that wrapper still needs the generated `.wasm` file at runtime.

        ```text
        supabase/functions/wasm-add/
        |-- add-wasm/
        |   `-- pkg/
        |       |-- add_wasm.js
        |       `-- add_wasm_bg.wasm
...
Before deploying, ensure the generated files from `add-wasm/pkg` are bundled with your function by defining them in `supabase/config.toml`:

<Admonition type="caution">

- The `static_files` pattern must include the generated `.wasm` file. Importing `add_wasm.js` alone is not enough, because it still loads the `.wasm` binary at runtime.
- You need to update Supabase CLI to 2.7.0 or higher for `static_files` support.
...
If you rebuild the Wasm package, make sure this pattern still matches the generated files in `add-wasm/pkg`.
