...
The patterns in this guide assume your project uses the new [JWT signing keys](/docs/guides/auth/signing-keys) and the new [API keys](https://github.com/orgs/supabase/discussions/29260). If you're still on legacy JWTs, see the [Legacy JWT Secret guide](/docs/guides/functions/auth-legacy-jwt).
...
Edge Functions care about two request headers. Sending the right credential in the right header is the most common source of 401s.
...
A common mistake is sending a publishable or secret key as a bearer token: `Authorization: Bearer sb_publishable_...`. The new API keys are not JWTs. The platform check can't validate them, and your handler can't verify them as JWTs either. Put API keys in the `apikey` header.

Both headers can travel together. A signed-in user calling your function through `supabase-js`, for example, sends their session JWT in `Authorization` and the project's publishable key in `apikey`.

## The `verify_jwt` platform check

When `verify_jwt` is on (the default), the platform inspects the `Authorization` header of every request before your function runs. It expects a valid user JWT. If the header is missing, malformed, or signed with a different key, the platform returns a 401 and your code never executes.

The check validates legacy HS256 JWTs and JWTs signed with the new asymmetric [signing keys](/docs/guides/auth/signing-keys). What it will not accept is an API key. Publishable and secret keys are not JWTs, so callers that send one on `Authorization` fail the check before their request reaches your handler.

Use the flag to match how the function is called:

- **Leave `verify_jwt` on** for functions that are only called with a user JWT, such as functions invoked from the client through `supabase.functions.invoke`. The platform rejects anonymous requests before they reach your code, and your handler can trust that a valid JWT is present.
...
Keep `verify_jwt` on. The platform validates the JWT before your handler runs. Forward the `Authorization` header into the Supabase client so queries run under the caller's RLS policies.
...
Cron jobs, workers, `pg_net`, or another Edge Function call in with a secret key on the `apikey` header. These callers don't send a user JWT, so turn `verify_jwt` off and validate the key yourself.
...
For a genuinely public function, like a health check, no credential is required. Turn `verify_jwt` off so anonymous callers can reach the handler.
...
External providers like Stripe or GitHub don't send Supabase credentials. They sign the request body with their own shared secret. Turn `verify_jwt` off and verify the signature before acting on the payload.
...
The patterns above share a lot of plumbing: reading headers, matching API keys, forwarding the user JWT, handling CORS, and wiring up the Supabase client. The [`@supabase/server`](https://github.com/supabase/server) package collapses each one into a single `allow` mode.

`withSupabase({ allow })` wraps your handler. It checks the credential against the declared mode, returns a 401 automatically if it doesn't match, and hands you a pre-configured Supabase client on `ctx`.
...
`allow: 'secret:<name>'` validates the `apikey` header against the named secret key from your [dashboard](https://supabase.com/dashboard/project/_/settings/api-keys) and gives you `ctx.supabaseAdmin` for privileged work. The `<name>` matches the name you gave the key. Keep `verify_jwt = false`.
...
Create a named secret key for each caller in your project's [API keys settings](https://supabase.com/dashboard/project/_/settings/api-keys). Give it a name like `automations`, and share the generated `sb_secret_...` value with the service that calls this function.
