...
<Admonition type='note' title='Contact us for access to Nano instances'>
  
Only select customers have access to Nano instances. Submit this [form](/solutions/ai-builders#talk-to-partnerships-team) to get access.

</Admonition>

Nano instance is our newest compute instance and the only one that scales to zero when not used.
...
Once you have access to [Nano instances](#nano-compute-instance), you will no longer be able to upgrade or downgrade to Nano instances.
