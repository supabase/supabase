...
It's common in `DEV` branches to "seed" data. This is basically test data for users. insert the following seed to our new todos table:
...
Before deploying to production we strongly recommend running the security advisor on the project to make sure the latest changes are secure.
...
- Only available in prod when a project has already explicitly enabled PITR before rolling back.
