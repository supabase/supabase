...
<Admonition type="note" title="v2 Projects need to enable physical backups">

In v2 Projects, you need to enable physical backups in order to create restore points.

The endpoint is currently under v0 experimental but will be moved to v1 shortly.

https://api.supabase.com/api/v0#tag/projects-backup/post/v0/database/{ref}/backups/enable-physical-backups

You need to generate a different experimental Personal Access Token (PAT) to use this endpoint.
...
If you’re building Supabase Projects under your Supabase Organization on behalf of your users, they might eventually want to transfer those Projects to their own Organizations.
...
<Admonition type="note" title="DRAFT PR NOTE (WILL REMOVE)">

We are in the process of consolidating the API requests below into a single endpoint.
