...
Reports define and visualize the signals available in Studio. Use them to identify a time window or resource that needs investigation, then follow [Detecting](/docs/guides/monitoring-and-debugging/debugging). Cause-specific fixes belong in [Diagnosing](/docs/guides/troubleshooting).
