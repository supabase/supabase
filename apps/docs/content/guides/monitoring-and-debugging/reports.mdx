...
Reports define and visualize the signals available in Studio. Use them to identify a time window or resource that needs investigation, then follow [Detecting](/docs/guides/monitoring-and-debugging/debugging). When a chart names a resource such as CPU, connections, or disk, search [Diagnosing](/docs/guides/troubleshooting) for that resource. Cause-specific fixes belong there.
