...
If you already have a specific error, open [Diagnosing](/docs/guides/troubleshooting). If you are checking whether the project is healthy, start with [Detecting](/docs/guides/monitoring-and-debugging/debugging).
...
| Edge Function | Edge Function invocations                                         |
...
Edge Function rows include console output from that invocation. Function log messages longer than 10,000 characters are truncated.
...
If [Read Replicas](/docs/guides/platform/read-replicas) are enabled, collections can filter by database with the **Source** control. For API logs from the [API Load Balancer](/docs/guides/platform/read-replicas#api-load-balancer), the upstream database is the Redirect Identifier field.
