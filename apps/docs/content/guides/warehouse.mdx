...
Analytics can integrate with your very own backends, with Analytics managing the ingestion pipeline and maximising throughput of events. This ensures maximum flexibility for storing sensitive data.
...
```bash
# By collection name
curl -X "POST" "https://api.analytics.tech/api/events/json?collection_name=YOUR-COLLLECTION-NAME-HERE" \
...
# By collection UUID
curl -X "POST" "https://api.Analytics.tech/api/events/json?collection=YOUR-COLLECTION-UUID-HERE" \
    -H 'Content-Type: application/json; charset=utf-8' \
    -H 'Authorization: Bearer YOUR-ACCESS-TOKEN-HERE' \
    -d $'[{
    "message": "This is the main event message",
