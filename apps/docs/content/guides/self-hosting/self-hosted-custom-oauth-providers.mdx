...
Below is a step-by-step guide to set up Telegram OAuth with Supabase Auth. Make sure you've enabled Custom OAuth in your self-hosted Supabase instance.
...
- `identifier` can be any value you want as long as it's prefixed with `custom:`.
- 'name' can be any value you want.
- `email_optional` has to be set to `true` as Telegram doesn't return an email field.

Refer to the official [Telegram Docs](https://core.telegram.org/bots/telegram-login#available-scopes) to get a list of available scopes.
