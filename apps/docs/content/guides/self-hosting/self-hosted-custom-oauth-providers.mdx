...
Telegram doesn't return an email address, so `email` is empty in the user object. The profile claims that Telegram returns, `name`, `given_name`, `family_name`, and `picture`, are in `user_metadata` together with the ID token claims, and `app_metadata.provider` is `custom:telegram`. The Telegram user ID is the `sub` claim.

To test a different custom provider, change the `provider` value to its identifier.

## Manage providers

Self-hosted Studio doesn't include a UI for custom providers. Use the admin API to list, update, and delete them. The examples use `custom:my-provider` as the identifier. Replace it with the identifier of your provider.
