...
This script updates ownership only for database objects in the public schema. Supabase-managed schemas or any custom schemas are not affected.
...
- As the next step, locate the `meta` service environment variables. Change the `PG_META_DB_USER` environment variable from `supabase_admin` to `postgres`:
