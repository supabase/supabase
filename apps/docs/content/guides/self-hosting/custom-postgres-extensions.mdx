...
The same Dockerfile structure generally works for your own extensions. However, when authoring a new extension from scratch, it's best to use the Nix build instead (see [Build the extension into the Nix image](#build-the-extension-into-the-nix-image) below).
