...
The same Dockerfile structure works for your own extensions - `COPY` your PGXS source into the builder instead of `git clone`. However, when authoring a new extension from scratch, it's best to use the Nix build instead (see [Build the extension into the Nix image](#build-the-extension-into-the-nix-image) below).
