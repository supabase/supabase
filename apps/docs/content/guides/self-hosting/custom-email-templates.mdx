...
This guide uses [halverneus/static-file-server](https://github.com/halverneus/static-file-server) for serving templates.
...
- `GOTRUE_MAILER_TEMPLATES_<AUTH_FLOW>`: Provide template URL
- `GOTRUE_MAILER_SUBJECTS_<AUTH_FLOW>`: Customize email subject
...
Both the template and subject environment variables are optional. If they are not provided, auth service falls back to the default.
...
- Enables the `auth` container to fetch templates using Docker's internal DNS (for example, `http://templates-server/invite.html`).
...
- `GOTRUE_MAILER_TEMPLATES_<NOTIFICATION_TYPE>_NOTIFICATION`: Provide template URL
- `GOTRUE_MAILER_SUBJECTS_<NOTIFICATION_TYPE>_NOTIFICATION`: Customize email subject
...
Reuse the same directory created in **Auth Email Templates – Step 1** (`volumes/templates`) and add your notification email templates to this directory.
