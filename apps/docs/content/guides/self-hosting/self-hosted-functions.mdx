...
The request URL must include the function name after `/functions/v1/`. For example, `/functions/v1/hello` — not `/functions/v1/`.
...
Use the following command to recreate the container, not `restart`:
