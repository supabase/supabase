...
touch volumes/functions/my-function/deno.json
```
...
And the import map to `deno.json`:
...
Here's an example function that queries a table using the admin client provided by `@supabase/server` (requires the same `deno.json` as above):
...
`auth: 'user'` verifies caller JWTs against `SUPABASE_JWKS`. That variable is commented out in a fresh `docker-compose.yml`. Uncomment it by running `./utils/add-new-auth-keys.sh` from the `docker/` directory.
