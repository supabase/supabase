...
Connect to your self-hosted Postgres and restore the dump files. The [default](/docs/guides/self-hosting/connection-pooling#connect-through-supavisor) connection string for self-hosted Supabase is:
...
Make sure your self-hosted Postgres port is accessible. In the default [self-hosted Supabase](/docs/guides/self-hosting/connection-pooling#connect-through-supavisor) setup, the user is `postgres.your-tenant-id` with Supavisor on port `5432`.
