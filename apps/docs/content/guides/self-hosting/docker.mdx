...
By default, the Postgres database is accessible through the Supavisor connection pooler. This allows for more efficient management of database connections. You can connect to the pooled database using the `POOLER_PROXY_PORT_TRANSACTION` port (default 6432) and `POSTGREST_PORT` for session based connections.

For more information on configuring and using Supavisor, refer to the [Supavisor documentation](https://supabase.github.io/supavisor/).
