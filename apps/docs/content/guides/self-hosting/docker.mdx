...
- `REALTIME_DB_ENC_KEY`: encryption key Realtime uses for sensitive fields in the `_realtime` schema. (Must be exactly 16 bytes; Realtime uses the value directly as the AES-128 key, so `openssl rand -hex 8` works because it outputs 16 ASCII characters / bytes)
