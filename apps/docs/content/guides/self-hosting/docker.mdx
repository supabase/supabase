...
6. [Changing passwords and secrets](#changing-passwords-and-secrets)
...
When self-hosting Supabase, you may need to update passwords and secrets after initial deployment. This section provides comprehensive guidance on safely rotating credentials without disrupting your services.

### Understanding Secret Dependencies

Multiple Supabase services share common secrets. Understanding these dependencies is critical before making changes:

- **JWT_SECRET** - Used by Kong, GoTrue (Auth), PostgREST, Realtime, and Storage
- **POSTGRES_PASSWORD** - Used by all services that connect to the database
- **ANON_KEY & SERVICE_ROLE_KEY** - JWT tokens signed with your JWT_SECRET
- **DASHBOARD_PASSWORD** - Protects Studio access

<Admonition type="caution">

All services must use identical JWT secrets for token validation to work correctly. Mismatched secrets will cause authentication failures.

</Admonition>

### Changing the Postgres Password

The Postgres password is used by multiple services to connect to the database. Follow these steps to update it safely:

#### Step 1: Update Environment Variables

Edit your `.env` file and update the password:
...
# After
POSTGRES_PASSWORD=new-strong-password-min-32-chars
```

Also update any connection strings that include the password:

```bash
DATABASE_URL=postgresql://postgres:new-strong-password-min-32-chars@db:5432/postgres
```

#### Step 2: Change the Database Password

Connect to your Postgres container and update the password:

```bash
docker exec -it supabase-db psql -U postgres -c "ALTER USER postgres WITH PASSWORD 'new-strong-password-min-32-chars';"
```
...
The JWT secret is used to sign and verify all authentication tokens. Changing it invalidates all existing user sessions.

#### Step 1: Generate a New JWT Secret

Create a secure random string (minimum 32 characters):
...
After changing the JWT secret, you must regenerate your `ANON_KEY` and `SERVICE_ROLE_KEY` to match. Use the [JWT Generator](#generate-api-keys) above or generate them programmatically:
...
```bash
ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...new-anon-key
SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...new-service-key
```
...
**Critical Impact:** Changing the JWT secret immediately invalidates all existing user sessions and tokens. All users will be logged out and must re-authenticate.

</Admonition>

### Changing API Keys

When you update `ANON_KEY` or `SERVICE_ROLE_KEY`, you must also update all applications using these keys.

#### What Needs to Be Updated

1. **Client Applications** - Web, mobile, and desktop apps
2. **Server-Side Code** - Backend APIs, scheduled jobs, webhooks
3. **Edge Functions** - Update environment variables and redeploy
4. **CI/CD Pipelines** - Update secrets in deployment platforms

#### Example: Updating Client Code
...
Client applications using old keys will receive 401 Unauthorized errors. Plan updates during maintenance windows.

</Admonition>

### Changing the Dashboard Password

Update the Studio dashboard password:
...
You can also add multiple users by editing `./docker/volumes/api/kong.yml`:
...
basicauth_credentials:
  ...
  - consumer: DASHBOARD
    username: developer
    password: new-developer-password
```

### Changing Storage Secrets

If using S3-compatible storage backends, update the credentials:
...
Update email server credentials:
...
### Complete Secret Rotation Example
...
- **Uniqueness:** Never reuse passwords across different secrets
...
We recommend using [AWS SES](https://aws.amazon.com/ses/). Restart all services to pick up the new configuration.
