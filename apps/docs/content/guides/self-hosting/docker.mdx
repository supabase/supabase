...
- [Docker](https://docs.docker.com/manuals/):
  - **Linux servers/VPS**: Install [Docker Engine](https://docs.docker.com/engine/install/) and [Docker Compose](https://docs.docker.com/compose/install/)
...
<Admonition type="danger">
    ...
    Do not reload this page or change the JWT Secret between generating the `ANON_KEY` and `SERVICE_ROLE_KEY`.

</Admonition>

How to generate the keys:

1. **Save JWT Secret**: Copy the generated value and update `JWT_SECRET` in the `.env` file. Do not share this secret publicly or commit it to version control.
2. **Generate ANON_KEY**: Select `ANON_KEY` from the dropdown, and click "Generate JWT." Copy the generated value and update `ANON_KEY` in the `.env` file.
3. **Generate SERVICE_ROLE_KEY**: Select `SERVICE_KEY` from the dropdown, and click "Generate JWT." Copy the generated value and update `SERVICE_ROLE_KEY` in the `.env` file.
...
- `PG_META_CRYPTO_KEY`: encryption key for securing connection strings used by Studio against postgres-meta. (Must be at least 32 characters; generate with `openssl rand -base64 32`)
- `LOGFLARE_PUBLIC_ACCESS_TOKEN`: API token for log ingestion and querying. Used by Vector and Studio to send and query logs. (Use a strong random string of at least 32 characters)
- `LOGFLARE_PRIVATE_ACCESS_TOKEN`: API token for Logflare management operations. Used by Studio for administrative tasks. Never expose client-side. (Use a strong random string of at least 32 characters)
...
The default TENANT_ID is `your-tenant-id` (can be changed in `.env`), and the password is the one you set previously in [Configure database password](#configure-database-password).

For session-based connections (equivalent to a direct Postgres connection):

```sh
psql 'postgres://postgres.[TENANT_ID]:[POSTGRES_PASSWORD]@[your-domain]:5432/postgres'
```

For pooled transactional connections:

```sh
psql 'postgres://postgres.[TENANT_ID]:[POSTGRES_PASSWORD]@[your-domain]:6543/postgres'
```

When using `psql` with command-line parameters instead of a connection string to connect to Supavisor, the `-U` parameter should also be `postgres.[TENANT_ID]`, and not just `postgres`.
...
The versions of each service in the Docker Compose file are updated periodically, following new releases and bug fixes. You can update any services yourself by changing the version numbers in the Docker Compose file and pulling new images. You can find all the latest Supabase images on [Supabase Docker Hub](https://hub.docker.com/u/supabase).
...
You will need to restart services to pick up the changes, which may result in downtime for your applications and users.
...
2. Find the latest version (tag) number. It will look something like `2025.11.26-sha-8f096b5`
...
<Admonition type="danger">
    ...
    Be careful — the following will destroy all data, including the database and storage volumes!
...
- **[PostgREST](https://github.com/PostgREST/postgrest)** - Web server that turns your Postgre database directly into a RESTful API
- **[Realtime](https://github.com/supabase/realtime)** - Elixir server that listens to Postgre database changes and broadcasts them over websockets
...
Each service has a number of configuration options that can be found in the related documentation.
...
Each Supabase service can be configured separately through environment variables and configuration files. Below are the most common configuration options.
...
3. If you're unable to use Studio, run `docker compose ps` to see if all service are up and healthy.
