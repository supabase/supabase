...
By default, the database is not accessible from outside the local machine but the pooler is. You can [change this](#exposing-your-postgres-database) by updating the `compose.yaml` file.
...
3. Update the `image` field in the `compose.yaml` file to the new version. It should look like this: `image: supabase/studio:20241028-a265374`
...
You can stop Supabase by running `docker compose stop` in same directory as your `compose.yaml` file.

## Uninstalling

You can stop Supabase by running the following in same directory as your `compose.yaml` file:
...
If you need direct access to the Postgres database without going through Supavisor, you can expose it by updating the `compose.yaml` file:
