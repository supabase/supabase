...
- `REALTIME_DB_ENC_KEY`: encryption key Realtime uses for sensitive fields in the `_realtime` schema. (Must be exactly 16 characters; generate with `openssl rand -hex 8`)
