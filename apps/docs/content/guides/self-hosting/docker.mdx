...
The `update.sh` script updates an existing deployment in place. It pulls a newer version of the Docker configuration and merges it over your files with a three-way merge.
...
See [Update your self-hosted deployment](/docs/guides/self-hosting/updating) for the full walkthrough - breaking changes, conflict resolution, pinning a release, and older installs without a recorded version.
