...
<Admonition type="danger">
  ...
  Review the configuration options below and ensure all secrets are set before starting the services.
...
If for some reason you need to configure Postgres to be directly accessible from the Internet, read [Exposing your Postgres database](#exposing-your-postgres-database).

To change the database password, check [Changing database password](#changing-database-password).
