...
When connecting via an SSH tunnel to the Studio Docker container, the source IP will be that of the Docker bridge gateway. You need to whitelist this IP address.
...
This command will output a private range IP address, e.g., `172.18.0.1`.

### Step 2: Whitelist the Docker bridge gateway IP

Add the IP address you discovered to the API gateway configuration by editing the following section in `./volumes/api/kong.yml`:

1. Comment out the 'request-termination' section
2. Uncomment the entire section starting with `- name: cors`, including `deny: []`
3. Add your local IPs to the 'allow' list as shown in the example below
...
### Step 3: Restart Kong API gateway
...
Start your MCP client (Claude Code, Cursor, etc.) and verify access to the MCP tools. For example, you can ask: "What is my Supabase anon key? Use the Supabase MCP server tools."
...
2. Check Kong's logs: `docker compose logs kong`
3. Confirm the Docker bridge gateway IP is correctly whitelisted in `./volumes/api/kong.yml`
