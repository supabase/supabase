...
This guide explains how to connect to Postgres in self-hosted Supabase, using the default Supavisor pooler, the optional PgBouncer pooler, or a direct connection.
...
- **Transaction mode** - Supavisor or PgBouncer on port `6543`. Best for serverless or edge functions that open many short-lived connections. Does not support session-level features (`SET`, prepared statements, `LISTEN/NOTIFY`, temporary tables that span transactions, or advisory locks). Available by default.
...
By default, Supavisor is exposed on `POSTGRES_PORT` (default `5432`) for session mode and `POOLER_PROXY_PORT_TRANSACTION` (default `6543`) for transaction mode. For authentication, Supavisor requires the "tenant ID" (`your-tenant-id` by default), not only the role.
...
To check your current Postgres `max_connections` setting (default `100`):
...
The PgBouncer override provides transaction mode only. For session-mode connections, or for features that transaction mode does not support (such as `SET` statements or `LISTEN/NOTIFY`), reconfigure PgBouncer manually (for example, by editing its environment variables in `docker-compose.pgbouncer.yml`), or use a [direct connection](#expose-postgres-for-direct-connections). PgBouncer reuses the `POOLER_DEFAULT_POOL_SIZE` and `POOLER_MAX_CLIENT_CONN` values from your `.env` configuration.
