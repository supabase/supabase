...
**Back up your data before upgrading.** You should create your own independent backup in case of disk failure or other issues.

</Admonition>

The upgrade script automatically preserves the original data directory as `./volumes/db/data.bak.pg15` as the final step. However, it is recommended to **always** create your own independent backup before starting:
...
The script walks you through each step with confirmation prompts.
...
The original Postgres 15 data is preserved at `./volumes/db/data.bak.pg15`. The upgrade binaries tarball is cached at `./volumes/db/pg17_upgrade_bin_*.tar.gz`. Once you have verified that everything works, you can reclaim disk space:
...
If you are starting a **new** Postgres 17 deployment (not upgrading) and see `FATAL: invalid secret key`, a leftover `db-config` volume from a previous Postgres 15 installation may have incorrect file ownership. Postgres 15 and 17 use different user IDs, so the pgsodium key file created by PG 15 is unreadable by PG 17.

To fix, remove the old volume and let PG17 generate a new key:
