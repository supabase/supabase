...
If you are starting a new self-hosted Supabase instance with **no existing data**, enable the Postgres 17 compose override, then start the stack:

```sh
sh run.sh config add pg17
sh run.sh start
```
...
`sh run.sh config add pg17` records the override in the `COMPOSE_FILE` variable in your `.env`, so every subsequent `sh run.sh` command (and any direct `docker compose` command) includes it automatically. Alternatively, you can update the image tag directly in `docker-compose.yml` instead of using an override.
...
After a successful upgrade, register the Postgres 17 override so all stack commands use it automatically, then bring the stack up:
