...
`/config_dump` includes all secrets in plaintext (API keys, JWTs, basic auth hash). Never expose port `9901` to a public network or untrusted host, and never share its output without omitting sensitive configuration.
