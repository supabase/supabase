...
- `/rest/v1/` (exact path, OpenAPI schema root) - only `service_role` keys are allowed (HTTP `403` otherwise). `/rest/v1/<table>` and other Data API paths remain accessible to all valid keys.
