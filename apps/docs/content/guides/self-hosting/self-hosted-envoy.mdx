...
The gateway handles authentication in three stages: dashboard basic auth on the catch-all route, API key enforcement on protected API routes, and an opaque-to-internal key translation step that runs before enforcement.
...
`/config_dump` includes all secrets in plaintext (API keys, JWTs, basic auth hash). Never expose port `9901` to a public network or untrusted host, and never share its output without first removing API keys, JWTs, and the basic auth hash.
