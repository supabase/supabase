...
- To enable opaque `sb_` key translation, complete [New API Keys and Asymmetric Authentication](/docs/guides/self-hosting/self-hosted-auth-keys)

## Enabling the Envoy gateway

The Envoy gateway is provided as a Docker Compose override. Start your stack with both the base compose file and the Envoy override:

```sh
docker compose -f docker-compose.yml -f docker-compose.envoy.yml up -d
```

The override disables the default Kong gateway and starts Envoy on the same port (default `8000`). It also rewires the Functions service to wait for Envoy via a healthcheck dependency.
...
| `cds.yaml`             | **C**luster **D**iscovery **S**ervice - upstream service definitions (DNS, ports, health checks, connect timeouts, circuit breakers).                                                       |
| `lds.template.yaml`    | **L**istener **D**iscovery **S**ervice template. Defines the listener, filter chain, routes, RBAC policy, and CORS policy. Contains placeholders for keys and credentials.                  |
...
1. A SHA1+base64 hash of `DASHBOARD_PASSWORD` is computed and joined with `DASHBOARD_USERNAME` into a single `DASHBOARD_BASIC_AUTH` string in the htpasswd-style format `username:{SHA}<base64-encoded-sha1>` that Envoy's `basic_auth` filter expects. Other hash formats are not supported.
...
The gateway uses three layers of authentication.
...
| `headers_with_underscores_action: REJECT_REQUEST`     | Blocks header smuggling attacks that exploit underscore-vs-hyphen normalization in backends                                                                                                        |
...
- **Applying changes.** Envoy reads the rendered `lds.yaml` via filesystem-based xDS. Configuration changes require restarting the container so the entrypoint re-renders the template:
...
Envoy exposes an admin interface on `127.0.0.1:9901` inside the container, with endpoints like `/ready`, `/clusters`, `/stats`, and `/config_dump`. The interface is bound to localhost only and port `9901` is not mapped to the host. The standard `envoyproxy/envoy` image is minimal (no `curl` or `wget`), so the admin endpoints are not reachable via `docker exec` without adding tooling.

To inspect the admin interface during debugging, temporarily expose port `9901` by editing `docker-compose.envoy.yml`:
...
Bind to `127.0.0.1` (not `0.0.0.0`) so it's reachable only from your host, then run `curl http://127.0.0.1:9901/clusters` (or `/stats`, `/config_dump`, etc.). **Remove the mapping when you're done.**

`/clusters` shows upstream health and connection state. `/stats` exposes per-filter and per-cluster counters. `/config_dump` shows the fully rendered configuration - useful for debugging missed substitutions.

<Admonition type="caution">

`/config_dump` includes all secrets in plaintext (API keys, JWTs, basic auth hash). Never expose port `9901` to a public network or untrusted host, and never share its output without redacting.
