...
| `SUPABASE_PUBLIC_URL`         | Allowed CORS origin for the `/pg/` route (normalized before substitution, see below) |
...
The `/pg/` route is the exception. This route runs direct Postgres introspection through `meta` and requires the service role key. It uses a stricter per-route CORS override that allows only `SUPABASE_PUBLIC_URL`, `localhost`, and `127.0.0.1` origins. No third-party origin needs to call it.
