...
- access_key_id: `STORAGE_TENANT_ID` environment variable in `.env` file
- secret_access_key: `ANON_KEY` environment variable in `.env` file
...
const client = new S3Client({
  forcePathStyle: true,
  region: 'REGION',
  endpoint: 'http://supabase.example.com:8000/storage/v1/s3',
  credentials: {
    accessKeyId: 'STORAGE_TENANT_ID',
