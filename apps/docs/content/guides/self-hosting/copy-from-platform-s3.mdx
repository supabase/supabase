...
- A working self-hosted Supabase instance with the S3 protocol endpoint enabled — see [Configure S3 Storage](/docs/guides/self-hosting/configure-storage#enable-the-s3-protocol-endpoint)
- Your platform project's S3 credentials — generated from the [Storage Settings](/dashboard/project/_/storage/settings) page
...
In your managed Supabase project dashboard, go to **Storage** > **Settings** > **S3 Access Keys**. Generate a new access key pair and copy:
...
Buckets must exist on the destination before you can copy objects into them. You can create them through Studio, or with SQL.
...
type = s3
provider = Other
access_key_id = your-s3-protocol-access-key-id
secret_access_key = your-s3-protocol-access-key-secret
endpoint = http://your-domain:8000/storage/v1/s3
region = us-east-1
```

Replace the credentials with your actual values. For self-hosted, use the `S3_PROTOCOL_ACCESS_KEY_ID` and `S3_PROTOCOL_ACCESS_KEY_SECRET` you configured in [Configure S3 Storage](/docs/guides/self-hosting/configure-storage#enable-the-s3-protocol-endpoint).

Verify both remotes connect:

```bash
rclone lsd platform:
rclone lsd selfhosted:
```
...
```bash
rclone copy platform:bucket-name selfhosted:bucket-name --progress
```
...
for bucket in $(rclone lsf platform: | tr -d '/'); do
  ...
  rclone copy "platform:$bucket" "selfhosted:$bucket" --progress
done
```

<Admonition type="note">

For large migrations, consider adding `--transfers 4` to increase parallelism, or `--checkers 8` to speed up the comparison phase. See the [rclone flags documentation](https://rclone.org/flags/) for all options.
...
rclone size selfhosted:bucket-name
```
...
- **Self-hosted**: Verify that `S3_PROTOCOL_ACCESS_KEY_ID` and `S3_PROTOCOL_ACCESS_KEY_SECRET` in `docker-compose.yml` match your rclone config. Restart the storage service after any changes.
...
```bash
rclone copy platform:bucket-name selfhosted:bucket-name --timeout 30m
```
