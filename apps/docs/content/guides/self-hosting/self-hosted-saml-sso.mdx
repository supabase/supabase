...
SAML requests must be signed. The value expected by `GOTRUE_SAML_PRIVATE_KEY` is a Base64-encoded PKCS#1 DER RSA private key (minimum 2048-bit). Generate it with:

```sh
# 1) Generate an RSA private key in PKCS#8 PEM format
openssl genpkey -algorithm RSA -out pk_pkcs8.pem

# 2) Convert it to PKCS#1 DER format (this is the format GoTrue expects before Base64)
openssl pkey -in pk_pkcs8.pem -out pk_rsa1.der -outform DER -traditional

# 3) Base64-encode the PKCS#1 DER key as a single line for GOTRUE_SAML_PRIVATE_KEY
base64 -w 0 -i pk_rsa1.der
```

Copy the Base64 output (single line, ignoring the trailing newline) for `GOTRUE_SAML_PRIVATE_KEY`. Remove temporary key files after that to keep the key safe.
