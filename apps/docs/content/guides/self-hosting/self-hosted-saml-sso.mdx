...
In `docker-compose.yml`, add the SAML environment variables to the `auth` service. Place them after the existing environment block (for example, after the MFA or external provider section):
...
Once SAML is enabled, your Supabase instance exposes Service Provider metadata at `{API_EXTERNAL_URL}/sso/saml/metadata`.
...
```sh
# By domain
curl -X POST 'http://<your-domain>/sso' \
  -H 'Content-Type: application/json' \
