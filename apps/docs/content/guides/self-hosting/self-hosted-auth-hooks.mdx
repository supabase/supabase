...
| `http://` or `https://` | `https://example.com/hook`                     | External HTTP endpoints                          |
...
### 4. Add environment variables in `docker-compose.yml`

Add the following environment variables to the auth & functions services.

<Admonition type="note">

For brevity, only the updated fields are shown.
...
services:
  ...
  functions:
    environment:
      SEND_EMAIL_HOOK_SECRET: ${HOOK_SEND_EMAIL_SECRETS}
```
