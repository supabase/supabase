...
HTTPS is required for production self-hosted Supabase deployments. This guide covers two production approaches using a reverse proxy in front of self-hosted Supabase API gateway (Kong), plus a self-signed certificate option for development environment. Internal Docker network traffic stays on HTTP.
...
CERTBOT_EMAIL=<your-username>@<your-domain>
```
...
Caddy configuration is in `volumes/caddy/Caddyfile`.

</TabPanel>
<TabPanel id="nginx" label="Nginx + Let's Encrypt">

This option uses the [`jonasal/nginx-certbot`](https://github.com/JonasAlfredsson/docker-nginx-certbot) Docker image, which integrates Nginx with Certbot for automatic Let's Encrypt certificate issuance and renewal in a single container.
...
Add the certificate volume mounts and SSL environment variables to the `kong` service in `docker-compose.yml`:
