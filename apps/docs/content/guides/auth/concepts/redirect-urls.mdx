...
When using a redirectTo option, you may need to replace the ```{{ .SiteURL }}``` with ```{{ .RedirectTo }}``` as document in https://supabase.com/docs/guides/auth/auth-email-templates  
...
```
<a href="{{ .RedirectTo }}/auth/confirm?token_hash={{ .TokenHash }}&type=signup">Confirm your mail</a>
```
