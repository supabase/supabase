...
Setting up GitHub sign-in for your application consists of 3 parts:
...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Github` as the provider:

```c#
var state = await supabase.Auth.SignIn(Provider.Github);
