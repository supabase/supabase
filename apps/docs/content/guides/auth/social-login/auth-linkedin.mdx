...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.LinkedIn` as the provider:

```c#
var signInUrl = supabase.Auth.SignIn(Provider.LinkedIn);
```
