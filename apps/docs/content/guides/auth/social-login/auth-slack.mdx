...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Slack` as the provider:

```c#
var signInUrl = supabase.Auth.SignIn(Provider.Slack);
```
