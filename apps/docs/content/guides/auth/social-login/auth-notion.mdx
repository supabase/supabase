...
Setting up Notion sign-in for your application consists of 3 parts:
...
- Once signed in, go to [notion.so/my-integrations](https://notion.so/my-integrations) and create a new integration.
...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Notion` as the provider:
