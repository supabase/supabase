...
```
  ...
  <TabPanel id="capacitor" label="Capacitor">

    For Capacitor apps, you can use the [@capgo/capacitor-social-login](https://github.com/Cap-go/capacitor-social-login) plugin for native Apple authentication across iOS and Android platforms.

    <Admonition type="note">

    For comprehensive setup instructions including Service ID configuration, see our [Capacitor Social Login guide](/docs/blog/setup-supabase-with-capacitor-social-login).

    </Admonition>

    ### Installation and setup

    1. Install the Capacitor Social Login plugin:
    ...
    2. Configure the plugin with your Apple credentials:
    ...
    **For Android/Web** (requires Service ID):
    ...
    async function signInWithApple() {
      try {
        ...
        const token = result.result?.identityToken;
        if (!token) {
          throw new Error('No Apple identity token received from social login provider');
        }
    ...
    **Android Configuration:**
    For Android, you need to create a Service ID in the Apple Developer Portal and configure it in the plugin. The plugin uses the Broadcast Channel API for Android authentication.

    For detailed platform-specific setup including Service ID creation and key generation, refer to our [comprehensive Capacitor guide](/docs/blog/setup-supabase-with-capacitor-social-login).
