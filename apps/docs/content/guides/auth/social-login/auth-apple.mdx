...
```
    ...
    ```ts
    // 1. Generate and hash the nonce (do this before initializing Apple ID)
    const rawNonce = crypto.randomUUID() // or use your preferred nonce generation method
    const nonceBuf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(rawNonce))
    const hashedNonce = Array.from(new Uint8Array(nonceBuf))
      .map((b) => b.toString(16).padStart(2, '0'))
      .join('')

    // 2. Initialize Apple ID
    AppleID.auth.init({
    ...
    ```ts
    // Generate and hash the nonce (do this before initializing Apple ID)
    ...
    // Initialize Apple ID with the hashed nonce
    AppleID.auth.init({
      clientId: 'your-services-id',
      scope: 'name email',
      redirectURI: 'https://your-domain.com/auth/callback',
