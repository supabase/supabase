...
```
    ...
    // Sign in with Apple JS places the nonce given to init() into the ID token
    // as-is, while Supabase Auth compares the SHA-256 hash of the nonce it
    // receives against that claim. Give Apple the hashed nonce and Supabase
    // the raw one, like the Flutter example below.
