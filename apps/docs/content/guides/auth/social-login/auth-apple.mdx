...
**Important: Secret Key Rotation Required**
...
**Note**: This requirement only applies if you're configuring OAuth settings (Services ID, signing key, etc.). Native-only implementations don't require secret key rotation.

</Admonition>

<Admonition type="caution">

**Important: Apple Does Not Provide Full Name in Identity Token**
...
**Note**: If a user revokes your app's access and then re-authorizes it, Apple will provide the full name again as if it were a first sign-in.
