...
<$Show if="sdk:swift">
  ...
  <TabPanel id="capacitor" label="Capacitor">
    ...
    For Capacitor apps, you can use the [@capgo/capacitor-social-login](https://github.com/Cap-go/capacitor-social-login) plugin for native Google authentication across iOS and Android platforms.

    <Admonition type="note">

    For comprehensive setup instructions including OAuth configuration, see our [Capacitor Social Login guide](/docs/blog/setup-supabase-with-capacitor-social-login).
    ...
    2. Configure the plugin with your Google credentials:
    ...
    async function signInWithGoogle() {
      try {
        ...
        if (!token) {
          throw new Error('No Google access token received from social login provider');
        }
    ...
    **Android Configuration:**
    ...
    For detailed platform-specific setup including SHA-1 fingerprints and OAuth client configuration, refer to our [comprehensive Capacitor guide](/docs/blog/setup-supabase-with-capacitor-social-login).
