...
1. Add your client secret to a `.env` file in your project root, next to the `supabase` directory:
   ```env
   SUPABASE_AUTH_EXTERNAL_GOOGLE_CLIENT_SECRET="<client-secret>"
   ```
   Don't put it in `supabase/functions/.env`. That file only holds secrets for Edge Functions, and `config.toml` doesn't read it. If `supabase functions serve` prints `Env name cannot start with SUPABASE_, skipping`, the secret is in the wrong file. For more on how `config.toml` reads environment variables, see [Using secrets inside config.toml](/docs/guides/local-development/managing-config#using-secrets-inside-configtoml).
2. Configure the provider in `supabase/config.toml`:
   ...
   ```
3. Run `supabase stop` and then `supabase start` to restart the local stack. Auth reads the new settings when it starts.
...
On initial sign-in, you can extract the `provider_token` from the session and store it in a secure storage medium. The session is available in the returned data from `signInWithOAuth` (implicit flow) and `exchangeCodeForSession` (PKCE flow).
