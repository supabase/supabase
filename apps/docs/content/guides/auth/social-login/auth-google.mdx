...
**Using PKCE with Google OAuth?** If you're implementing the PKCE flow, make sure to configure your Supabase client with `auth: { flowType: 'pkce' }`. See the [PKCE configuration guide](/docs/guides/auth/social-login#using-pkce-with-social-logins) for complete setup instructions.
...
**Important for PKCE flows:** The examples above will only work if you've configured your Supabase client with `flowType: 'pkce'`:
