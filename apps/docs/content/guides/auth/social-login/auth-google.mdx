...
```
### Production-Ready Google Authentication Pattern

This example demonstrates a complete Google authentication implementation for Expo React Native with proper error handling, session persistence, and user data extraction.

**Important:** This requires Expo Dev Builds or production builds. It will not work in Expo Go due to native module requirements.

#### Conditional Module Loading

Handle environments where native modules aren't available:
...
// Only load native modules outside of Expo Go
...
async function googleLogin() {
  try {
    ...
    // Session is now active - handle navigation or state updates in your app
...
Store the Supabase session securely for persistence across app restarts:
...
async function saveSession(accessToken: string, refreshToken: string): Promise<void> {
  await SecureStore.setItemAsync('supabase_access_token', accessToken);
  await SecureStore.setItemAsync('supabase_refresh_token', refreshToken);
}
...
After successful authentication, access user data from the session:
...
if (user) {
  ...
  // Access Google-specific data from user_metadata
  console.log("Name:", user.user_metadata?.full_name);
  console.log("Avatar:", user.user_metadata?.avatar_url);
}
```

#### Parsing Google User Data

Extract name components with proper fallbacks:

```typescript
// After successful Google sign-in
const userInfo = await GoogleSignin.signIn();

// Parse first and last name with fallbacks
const firstName = userInfo.data.user.givenName || 
                 userInfo.data.user.name?.split(' ')[0] || '';
...
console.log("Parsed user data:", { firstName, lastName, email, photoUrl });
```
...
useEffect(() => {
  async function checkExistingSession() {
    try {
      // Check if there's an existing Google session
      if (GoogleSignin) {
      ...
      if (session) {
        ...
        // User is authenticated - update your app state
      } else {
...
// Configure Google Sign-In (do this once, typically in your app root)
GoogleSignin.configure({
...
function LoginScreen() {
  const handleGoogleLogin = async () => {
    try {
      ...
      // Update your app state or navigation here
    } catch (error) {
...
1. **Name Parsing**: Google provides `givenName` and `familyName` separately, but these can be null. Always provide a fallback to splitting the full `name` field.

2. **Error Handling**: The `@react-native-google-signin/google-signin` package provides specific error codes (`SIGN_IN_CANCELLED`, `IN_PROGRESS`, `PLAY_SERVICES_NOT_AVAILABLE`) that should be handled appropriately.

3. **Session Tokens**: Store Supabase access and refresh tokens securely using `expo-secure-store`. Never use AsyncStorage for sensitive authentication tokens.

4. **Android Play Services**: Always check for Play Services availability on Android before attempting sign-in to provide better error messages.

5. **Expo Go Limitation**: This implementation requires native modules that aren't available in Expo Go. You must use Expo Dev Builds or create a production build.
</TabPanel>
