...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Figma` as the provider:

```c#
var state = await supabase.Auth.SignIn(Provider.Figma);
