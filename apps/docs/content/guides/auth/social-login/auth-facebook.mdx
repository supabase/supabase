...
- Click on `Log In` at the top right to sign in.
...
- Click **Setup** under **Facebook Sign-in**
- Skip the Quickstart screen. Instead, in the left sidebar, click **Settings** under **Facebook Sign-in**
- Enter your callback URI under **Valid OAuth Redirect URIs** on the **Facebook Sign-in Settings** page
...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Facebook` as the provider:
...
- Other users will see an "App Not Setup" error when trying to sign in
...
3. **Submit for Review**: Click **Submit for Review** and provide:
   - Detailed instructions for how Facebook reviewers should test your sign-in flow
   - A screencast video demonstrating the Facebook Sign-in feature
...
If you only need basic authentication (name and profile picture), you may not need full App Review. Apps requesting only `public_profile` and `email` with the "Authenticate and request data from users with Facebook Sign-in" use case can often go live without a detailed review.
...
This error occurs when a user without a role on your app tries to sign in while the app is in Development mode.
