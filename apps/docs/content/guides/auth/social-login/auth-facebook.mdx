...
For Capacitor apps, you can use the [@capgo/capacitor-social-login](https://github.com/Cap-go/capacitor-social-login) plugin for native Facebook authentication across iOS and Android platforms.

<Admonition type="note">

For comprehensive setup instructions including Facebook App configuration, see our [Capacitor Social Login guide](/docs/blog/setup-supabase-with-capacitor-social-login).
...
2. Configure the plugin with your Facebook credentials:
...
async function signInWithFacebook() {
  try {
    ...
    if (!token) {
      throw new Error('No Facebook access token received from social login provider');
    }
...
**iOS Configuration:**
Add Facebook SDK configuration to your `ios/App/App/Info.plist`:
...
Add Facebook resources to `android/app/src/main/res/values/strings.xml`:
...
For detailed platform-specific setup including Facebook SDK integration, refer to our [comprehensive Capacitor guide](/docs/blog/setup-supabase-with-capacitor-social-login).
