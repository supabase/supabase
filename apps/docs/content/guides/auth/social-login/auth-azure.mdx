...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Azure` as the provider. Request the `email` scope:

```c#
var signInUrl = supabase.Auth.SignIn(Provider.Azure, "email");
```
...
```c#
var signInUrl = supabase.Auth.SignIn(Provider.Azure, "offline_access");
```
