...
Setting up Spotify sign-in for your application consists of 3 parts:
...
1. The user is redirected to the Spotify sign-in page.
...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Spotify` as the provider:
