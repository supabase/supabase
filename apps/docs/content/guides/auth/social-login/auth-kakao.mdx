...
- In the edit page, enter your callback URL in the **Kakao Sign-in Redirect URI** field.
...
- Go to **App Settings** > **App** > **Platform Key**.
- Click on the REST API key you want to use.
- Note the **Kakao Sign-in Client Secret code**. This serves as a `client_secret` for your Supabase project.
- Make sure you activate **Kakao Sign-in Client Secret**.

## Additional configurations on Kakao Developers portal

- Go to **Product Settings** > **Kakao Sign-in** > **General**.
- Set **State** to "ON" in the **Usage settings** section to enable Kakao Sign-in.
- Go to **Product Settings** > **Kakao Sign-in** > **Consent Items**.
...
When your user signs in, call [`SignIn()`](/docs/reference/csharp/sign-in-with-oauth) with `Provider.Kakao` as the provider:
...
## Using Kakao Sign-in JS SDK

[Kakao Sign-in JS SDK](https://developers.kakao.com/docs/latest/en/kakaologin/js) is an official Kakao SDK for authenticating Kakao users on websites.

Exchange the [authorization code returned by Kakao API](https://developers.kakao.com/docs/latest/en/kakaologin/rest-api#request-code) for an [ID Token](https://developers.kakao.com/docs/latest/en/kakaologin/common#sign-in-with-oidc).
...
1. Set **State** to "ON" under [OpenID Connect Activation](https://developers.kakao.com/docs/latest/en/kakaologin/prerequisite#kakao-sign-in-oidc) on the Kakao Developers portal.
