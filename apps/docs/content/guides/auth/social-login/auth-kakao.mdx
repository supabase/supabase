...
## Generate the REST API key and Client Secret
- Go to `App` > `App Settings` > `App` > `Platform Key`.
- Click `Add REST API key` to generate the key pair -- this will serve as the `REST API Key` and `Client Secret` for your Supabase project.

## Find your callback URL

<$Partial path="social_provider_setup.mdx" variables={{ "provider": "Kakao" }} />

- To add callback URL on Kakao, go to `App` > `App Settings` > `App` > `Platform Key` > `Edit Platform Key` > `Kakao Login Redirect URI`
