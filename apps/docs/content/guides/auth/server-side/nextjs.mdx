...
Go to the [Auth templates](https://supabase.com/dashboard/project/_/auth/templates) page in your dashboard. In each email template, change `{{ .ConfirmationURL }}` to `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=signup|invite|magiclink|recovery|email_change|email` and ensure you remove the extra `&type=` options to match the email template. Eg `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=signup`
