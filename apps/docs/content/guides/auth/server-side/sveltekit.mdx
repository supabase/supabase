...
<StepHikeCompact.Step step={12} title="Add OAuth login" id="oauth-login">

<StepHikeCompact.Details title="Extend your auth actions with OAuth">

Extend your `+page.server.ts` with an `oauth` action to handle third-party providers.  
Then, update your `+page.svelte` to include a login button.
...
export const actions: Actions = {
  // ...existing signup and login actions...
...
```svelte name=src/routes/auth/+page.svelte
<form method="POST" action="?/oauth&provider=github">
