...
### Handling OAuth in SSR

When using OAuth with SvelteKit SSR, you need to handle redirects for successful or failed authentication. 
Here’s an example:
...
export const actions = {
    default: async ({ url }) => {
        const provider = 'github'; // Replace with your desired OAuth provider (e.g., google, facebook)

        // Perform OAuth sign-in
        const { error } = await supabase.auth.signInWithOAuth({
            provider,
            options: {
                redirectTo: `${url.origin}/auth/callback`, // Specify the callback URL
            },
        });
        ...
        if (error) {
            ...
            throw redirect(303, '/auth/error'); // Redirect to error page
        }

        throw redirect(303, '/dashboard'); // Redirect after successful login
    },
};

export const load = async ({ locals }) => {
    // Example: Get user session data if needed
    const { user } = locals;
    return { user };
};
