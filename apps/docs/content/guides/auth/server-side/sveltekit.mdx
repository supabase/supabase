...
const supabase: Handle = async ({ event, resolve }) => {
  //  Create a Supabase server client for handling requests on the server
  event.locals.supabase = createServerClient(
    PUBLIC_SUPABASE_URL, // Your Supabase project URL
    PUBLIC_SUPABASE_PUBLISHABLE_KEY, // Your project's public (anon) key
    {
      cookies: {
        // Read all cookies from the incoming request
        // This lets Supabase identify the user's current session
        getAll: () => event.cookies.getAll(),

        // Write (or update) cookies in the outgoing response
        // Supabase uses this to manage session tokens automatically
        setAll: (cookiesToSet) => {
          cookiesToSet.forEach(({ name, value, options }) => {
            // Use the correct SvelteKit API to set cookies
            // Spreads existing cookie options and ensures they apply site-wide
