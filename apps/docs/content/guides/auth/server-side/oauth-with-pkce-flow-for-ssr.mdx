...
The `.signInWithOAuth` method uses the `window` object for the automatic redirect. For automatic redirect to work, it should be called from a Client component. If the method is used server-side, you need to handle the redirect yourself and set `skipBrowserRedirect` option to `true`.
...
<Tabs
  ...
  size="small"
  type="underlined"
  defaultActiveId="client-side"
  queryGroup="sign-in-with-oauth-ssr"
>
<TabPanel id="server-side" label="Server Action">
...
export function SignInWithProvider({ provider }: { provider: Provider }) {
  const signIn = async () => {
    'use server'
  ...
  return (
    <form action={signIn}>
      <button>Sign In with OAuth Provider</button>
    </form>
  )
}
```
...
<TabPanel id="client-side" label="Client Component">
...
export function SignInWithProvider({ provider }: { provider: Provider }) {
  ...
  return (<button onClick={signIn}>Sign In with OAuth Provider</button>)
}
```
