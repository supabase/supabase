...
Since NextJS Server Components can't write cookies, you need middleware to refresh expired Auth tokens and store them.
...
You should add a [matcher](https://nextjs.org/docs/app/building-your-application/routing/middleware#matching-paths) so the middleware doesn't run on routes that don't access Supabase.
...
<TabPanel id="root_server_layout" label="Root Server Load">
TBD
</TabPanel>
</Tabs>
</TabPanel>
...
<Tabs
  ...
  size="small"
  type="underlined"
  defaultActiveId="next-next-steps"
  queryGroup="nextsteps"
>
<TabPanel id="next-next-steps" label="NextJS">
### Create a login page
