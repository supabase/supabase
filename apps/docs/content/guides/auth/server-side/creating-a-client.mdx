...
To sign users in with an OAuth provider like Google or GitHub, create a form action that calls `signInWithOAuth`. The method returns a `url` to redirect the user to the provider's authentication page.

```typescript src/routes/auth/+page.server.ts
...
export const actions: Actions = {
  oauth: async ({ request, locals: { supabase } }) => {
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: `${request.url.origin}/auth/callback`,
      },
    })
...
Then, create a callback route to handle the OAuth redirect and exchange the code for a session:

```typescript src/routes/auth/callback/+server.ts
import { redirect } from '@sveltejs/kit'
...
Use the action in your sign-in form:
