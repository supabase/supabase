...
<Accordion>
    ...
    A refresh token can only be used once. Supabase allows a short window in which the same token can be presented again, which covers the normal SSR round trip. A second refresh outside that window revokes the whole session.
