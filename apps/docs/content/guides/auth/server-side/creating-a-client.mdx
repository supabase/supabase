...
<Tabs
  ...
  size="small"
  type="underlined"
  defaultActiveId="solidstart-middleware"
  queryGroup="solidstart-environment"
>

<TabPanel id="solidstart-middleware" label="Middleware">

SolidStart uses middleware to handle authentication and session management. Create a `src/middleware.ts` file to set up your Supabase server client.
...
<TabPanel id="solidstart-route" label="Route Loader">

Use the Supabase client in your route loaders for server-side authentication checks.

```ts src/routes/protected.tsx
import { createServerData$ } from 'solid-start/server'
import { redirect } from 'solid-start/server'

export const routeData = createServerData$(async (_, { request }) => {
  const { supabase, safeGetSession } = request.locals
...
<TabPanel id="solidstart-client" label="Client Component">
...
```tsx src/components/AuthButton.tsx
...
- Set up middleware to create a request-specific Supabase client and handle session management.
- Created a Supabase client for use in route loaders and server-side authentication checks.
- Set up a browser client for client components to handle authentication state.
