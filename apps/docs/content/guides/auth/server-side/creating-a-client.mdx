...
This client should be used within your loader and action functions.

```ts lib/supabase.server.ts
import { createServerClient, parse, serialize } from '@supabase/ssr'
import type { Database } from 'db_types' // Assuming you have generated types
...
Using the client in a `loader` function:

```ts app/routes/some-route.tsx
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return json({ session, data }, { headers })
}

export default function SomeRoute() {
  const { session, data } = useLoaderData<typeof loader>()
  // Render your component
  return <div>...</div>
}
```

Using the client in an `action` function (e.g., handling form submissions):

```ts app/routes/some-form-route.tsx
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  // Always return headers, even in redirects
...
When initiating an OAuth flow (e.g., `signInWithOAuth`), it's crucial to pass the headers generated by the Supabase client to the Remix `redirect` function. This ensures the necessary cookies (like the PKCE code verifier) are set in the browser before the user is redirected to the OAuth provider.

```ts app/routes/login.tsx {10, 14}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'github',
    options: {
      redirectTo: 'http://localhost:3000/auth/callback', // Your callback URL
    },
  })

  if (error) {
    console.error('OAuth Error:', error.message)
    // Handle error appropriately, perhaps return json with error
    return json({ error: 'OAuth sign-in failed.' }, { status: 500, headers })
  }

  if (data.url) {
    // Redirect to the provider's auth page, ensuring headers are passed
    return redirect(data.url, { headers })
  }

  // Handle unexpected case where data.url is null
  return json({ error: 'OAuth provider URL not found.' }, { status: 500, headers })
}

// Component code for login form...
```
