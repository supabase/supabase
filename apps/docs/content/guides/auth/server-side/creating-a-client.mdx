...
To implement OAuth authentication with SvelteKit SSR, you need to handle the OAuth flow on the server side. Here's how to set it up:

#### Create an OAuth callback route

Create a callback route at `src/routes/auth/callback/+server.ts` to handle the OAuth code exchange:
...
You can initiate OAuth sign-in from either a client component or a server action. Here's an example using a server action:

```ts name=src/routes/auth/login/+page.server.ts
import { redirect } from '@sveltejs/kit'
...
```svelte name=src/routes/auth/login/+page.svelte
<script lang="ts">
...
<button on:click={() => handleOAuthSignIn('github')}>
  Sign in with GitHub
</button>
```
...
2. `signInWithOAuth` is called, which redirects to the OAuth provider
3. After authentication, the provider redirects back to your callback route (`/auth/callback`)
4. The callback route exchanges the code for a session using `exchangeCodeForSession`
5. The user is redirected to the specified `next` URL or the home page
