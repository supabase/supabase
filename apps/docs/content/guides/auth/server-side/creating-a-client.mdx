...
Step 2 is what keeps users signed in. A refresh token can only be used once. Supabase allows a short window in which the same token can be presented again, which covers the normal SSR round trip, but a second refresh outside that window revokes the whole session. If the Proxy refreshes and a Server Component then refreshes again on the same request, users get signed out at random and the cause is hard to trace. Leave the refresh to the Proxy. See [refresh token reuse detection](/docs/guides/auth/sessions#what-is-refresh-token-reuse-detection-and-what-does-it-protect-from).
...
Return the same response object the cookies were written to. `setAll` builds a new `supabaseResponse` every time the library writes cookies, so any response you created earlier is already stale. Returning a stale one leaves the browser on the old token after the server has rotated it, and the user is signed out on the next request. When you need a different response, copy the cookies and the cache headers onto it first:
...
The cache headers matter as much as the cookies. They are what stops a CDN caching a response that carries someone's refreshed token.
