...
1. Write a server function that checks the user's session and redirects if the session isn't valid.
1. Call that server function from a layout route's `beforeLoad` hook, before any nested route renders.

<Admonition type="danger">

Skipping the check inside the server function exposes private data to unauthenticated users. `beforeLoad` only controls what the client renders — it's a UX guard, not a security boundary. Because there's no proxy re-checking every request, the server function is the only checkpoint before a redirect happens or private data returns, so it must call `supabase.auth.getClaims()` to authorize the request itself.
