...
- Social sign in with email confirmation
...
- Prefer social sign-in (OAuth) or SSO with SAML instead of email-based authentication flows in your apps.
...
- Disable email-based sign ups for the event and use social sign-in only. Alternatively you can deprioritize the email-based sign-up flows for the event by hiding them in the UI or making them harder to reach.
