...
>
  ...
  ### Deep link config
    ...
    Now we have to edit the Android manifest to make sure the app opens when the user clicks on the magic link.
    ...
    Next we have to specify the scheme and host in the Supabase Client:
    ```kotlin
    install(Auth) {
       host = "login-callback"
    ...
    At last we have to call `Auth#handleDeeplinks` when our app opens:
