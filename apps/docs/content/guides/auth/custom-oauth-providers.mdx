...
2. Scroll to **Custom OAuth Providers** and click **Add provider**.
3. Select **OAuth2** as the provider type.
4. Enter a unique identifier (must start with `custom:`, for example `custom:my-oauth-provider`).
5. Enter the provider's **Client ID** and **Client Secret**.
6. Enter the **Authorization URL**, **Token URL**, and **Userinfo URL**.
7. Optionally configure scopes and enable PKCE.
8. Click **Save**.
...
4. Enter a unique identifier (must start with `custom:`, for example `custom:my-regional-provider`).
5. Enter the provider's **Client ID** and **Client Secret**.
6. Enter the **Issuer URL**. The discovery document and endpoints are resolved automatically.
7. Optionally configure scopes and enable PKCE.
8. Click **Save**.
...
- The `openid` scope is always included — it is automatically prepended if missing from the `scopes` array.
...
Go to [Auth Providers](/dashboard/project/_/auth/providers) in the Dashboard and click on a provider under **Custom OAuth Providers** to view its details.

</TabPanel>
<TabPanel id="js" label="JavaScript">

```js
const { data, error } = await supabase.auth.admin.customProviders.getProvider(
  'custom:my-provider'
)
```
...
Update any provider fields except `provider_type` and `identifier`. Only provided fields are changed (partial update).
...
1. Go to [Auth Providers](/dashboard/project/_/auth/providers) in the Dashboard.
2. Click on the provider you want to update under **Custom OAuth Providers**.
...
1. Go to [Auth Providers](/dashboard/project/_/auth/providers) in the Dashboard.
2. Click on the provider you want to delete under **Custom OAuth Providers**.
...
```js
const { data, error } = await supabase.auth.admin.customProviders.deleteProvider(
  'custom:my-provider'
)
```
...
Update just the client secret without affecting other settings.
...
1. Go to [Auth Providers](/dashboard/project/_/auth/providers) in the Dashboard.
2. Click on the provider under **Custom OAuth Providers**.
3. Update the **Client Secret** field with the new secret.
4. Click **Save**.
