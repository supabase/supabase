...
```python
response = await supabase.auth.link_identity({'provider': 'google'})
```
...
You can use [`get_user_identities()`](/docs/reference/python/auth-getuseridentities) to fetch all the identities linked to a user. Then, call [`unlinkIdentity()`](/docs/reference/python/auth-unlinkidentity) to unlink the identity. The user needs to be logged in and have at least 2 linked identities in order to unlink an existing identity.

```python
# retrieve all identities linked to a user
response = await supabase.auth.get_user_identities()
...
# unlink the google identity from the user
if google_identity:
    response = await supabase.auth.unlink_identity(google_identity.identity_id)
```
