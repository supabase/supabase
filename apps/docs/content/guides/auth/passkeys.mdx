...
Open the [Passkeys settings](/dashboard/project/_/auth/passkeys) under **Authentication → Passkeys**, turn on **Enable Passkey authentication**, and fill in the WebAuthn [relying party](https://www.w3.org/TR/webauthn-3/#relying-party) details:
...
Passkeys are cryptographically bound to the Relying Party ID they were registered against. Changing the RP ID makes every existing passkey unusable for sign-in, and users will need to register a new one. Pick the RP ID carefully before users start enrolling, and keep it stable once they do.
...
`auth.registerPasskey()` runs the full WebAuthn ceremony: fetches a challenge, invokes the browser API, and verifies the response with Supabase Auth.
...
A friendly name is automatically derived from the authenticator's AAGUID (for example `iCloud Keychain`, `Google Password Manager`, `1Password`). Users can rename their passkey afterwards — see [Manage passkeys](#manage-passkeys).
