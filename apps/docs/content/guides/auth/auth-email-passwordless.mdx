...
By default, a user can only request a magic link once every <SharedData data="config">{({auth: {rate_limits: {magic_link: {tau}}}}) => `${tau.value} ${tau.unit}`}</SharedData> and they expire after <SharedData data="config">{({auth: {rate_limits: {magic_link: {validity}}}}) => `${validity.value} ${validity.unit}`}</SharedData>.
...
When using the [PKCE flow](/docs/guides/auth/concepts/session#pkce-flow), Magic Links can only be opened from the same browser and device they were requested from.
...
By default, a user can only request an OTP once every <SharedData data="config">{({auth: {rate_limits: {otp: {tau}}}}) => `${tau.value} ${tau.unit}`}</SharedData> and they expire after <SharedData data="config">{({auth: {rate_limits: {otp: {validity}}}}) => `${validity.value} ${validity.unit}`}</SharedData>.
