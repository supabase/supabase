...
Model Context Protocol (MCP) provides a standardized way for AI applications to:
- Access external data sources and APIs
- Authenticate users securely via OAuth 2.1
- Maintain user context across interactions
- Execute actions on behalf of users with proper authorization

Think of MCP as the standard that enables AI agents to interact with your application just like a third-party app would, but with the intelligence and capabilities of AI.

### Common use cases

- **AI assistants** that access user data from your platform
- **LLM tools** that integrate with your application's APIs
- **Chat bots** that perform actions on behalf of users
- **Automated agents** that monitor and respond to events
- **AI-powered workflows** that connect multiple services
...
Supabase Auth exposes OAuth 2.1 authorization server metadata at the standard MCP discovery endpoint:

```
https://<project-ref>.supabase.co/.well-known/oauth-authorization-server
```

This endpoint returns metadata about your OAuth server:
...
MCP clients use this metadata to automatically configure OAuth authentication without manual setup.

## Dynamic client registration

MCP clients can register themselves automatically using OAuth 2.1 dynamic client registration. This eliminates the need to manually register each AI tool.

### Enabling dynamic registration

Dynamic client registration is disabled by default for security. Enable it in your project settings:

1. In the Supabase dashboard, Go to **Authentication** > **OAuth Server** in the sidebar
2. Toggle **Dynamic Client Registration** to enable it
...
When building your own MCP server, you can integrate with Supabase Auth to leverage your existing user authentication. This allows AI agents to authenticate as your users and access data through your Supabase project.
