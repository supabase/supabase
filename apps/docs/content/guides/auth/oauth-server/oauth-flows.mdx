...
| `scope`   | Space-separated list of scopes (e.g., `openid email profile`). **Note:** Scope-based access control is not currently implemented. All tokens receive full access with a `client_id` claim. |
...
| `scope`         | Scopes from the authorization request (not enforced)        |
| `id_token`      | OpenID Connect ID token (always included, valid for 1 hour) |
...
**Scope-based access control is not currently implemented.** All OAuth access tokens have full access to user data (same as regular session tokens), with the addition of the `client_id` claim. Use Row Level Security policies with the `client_id` claim to control which data each OAuth client can access.
...
**ID tokens are always returned** with every authorization code flow, regardless of the `scope` parameter. ID tokens are valid for 1 hour.
...
Clients can also retrieve user information by calling the UserInfo endpoint with an access token:
...
Supabase Auth exposes an OIDC discovery endpoint that describes its capabilities:
