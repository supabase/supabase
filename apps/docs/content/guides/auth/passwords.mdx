...
In your terminal, run `supabase status` to get the Inbucket URL. Go to this URL in your browser to find your emails.

Configure local [email templates](/docs/guides/local-development/customizing-email-templates) in your supabase/config.toml file. 

To test [server-side PKCE auth flow](docs/guides/auth/passwords?queryGroups=flow&flow=pkce#signing-up-with-an-email-and-password) locally, make sure to enable email confirmations `email.confirmations = true` under `auth.email`.
