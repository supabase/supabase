...
const { data: factor, error } = await supabase.auth.mfa.verify({ factorId, credential, useMultiStep: true, allowAnyCredential: true })
```

In both modes, developers can set `allowAnyCredentials` to true to prioritize User Experience. Users often have multiple security keys for multi-factor authentication. These could be used for different services or different work contexts. These keys are physically identical, which can lead to confusion during the verification process where a user could use a key registered with service A to authenticate against service B

When a user has multiple registered security keys (e.g., known Key A and unknown Key B), they might accidentally use an unknown key during verification.

For example:

1. The app requests verification using known Key A.
2. The user inserts unknown Key B.
3. The app rejects the key, saying it's not recognized.

This creates a confusing experience for the user, who has the correct keys but in the wrong order.
