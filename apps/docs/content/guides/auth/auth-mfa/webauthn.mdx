...
Verification is often done on a page with a single button.
...
export default function MFAWebAuthn() {
  ...
  const startAuthentication = async () => {
    ...
    try {
      const { data, error: authError } = await supabase.auth.mfa.verify({
        factorType: 'webauthn'
      })
      ...
      // For example, navigate to protected route:
      // navigate('/dashboard')
  ...
  return (
    <div>
      ...
      <p>Please verify your identity using your security key.</p>
      ...
      <button onClick={() => navigate('/mfa-selection')}>
...
| Field Name                | Configurability  | Description                                                           |
| ------------------------- | ---------------- | --------------------------------------------------------------------- |
| `attestation`             | Developer choice | Type of attestation requested (e.g., 'direct', 'indirect', 'none')    |
| `authenticatorAttachment` | Developer choice | Preferred authenticator type ('platform' or 'cross-platform')         |
| `requireResidentKey`      | Developer choice | Whether a resident key is required (true or false)                    |
| `userVerification`        | Developer choice | User verification preference ('required', 'preferred', 'discouraged') |
| `rp.name`                 | Developer choice | Human-readable name for the Relying Party                             |
...
Build an intermediary page to allow users to select between factor types. Here's an example page which showcases how to switch between TOTP and WebAuthn factors.
...
Let the user pick their preferred authentication method and redirect the user to authenticate based on the factor they have selected.
