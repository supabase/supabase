...
TOTP multi-factor authentication involves a timed one-time password generated from an authenticator app in the control of users. The [TOTP MFA API](/docs/reference/javascript/auth-mfa-api) is enabled on all Supabase projects by default. Enrollment involves the display of a QR Code which a user can scan to capture a secret required to generate the one time password for subsequent authentication.
The TOTP QR code was [initially introduced by Google Authenticator](https://github.com/google/google-authenticator/wiki/Key-Uri-Format) but is now universally accepted by all authenticator apps. It uses a uri following the `otpauth` scheme such as:
...
TOTP MFA is available for free on all plans.

### Add enrollment flow for App Authenticator
