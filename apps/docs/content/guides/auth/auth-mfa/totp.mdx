...
Once a user has signed in via their first factor (email+password, magic link, one time password, social sign-in etc.) you need to perform a check if any additional factors need to be verified.
