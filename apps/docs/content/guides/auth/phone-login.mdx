...
Users can also sign in with their phones using native mobile sign-in with the built-in identity provider. For native mobile sign-in with Android and iOS, see the [social sign-in guides](/docs/guides/auth/social-sign-in).
...
```c#
var session = await supabase.Auth.VerifyOTP("+13334445555", "123456", MobileOtpType.SMS);
```
...
```c#
var response = await supabase.Auth.Update(new UserAttributes { Phone = "123456789" });
```
