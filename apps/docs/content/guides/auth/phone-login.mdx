...
Phone Login is a method of authentication that allows users to log in to a website or application without using a password. The user authenticates through a one-time password (OTP) sent via SMS or Whatsapp.

<Admonition type="info">

At this time, `whatsapp` is only supported as a channel for the Twilio Provider.
