...
[Enable Sign In with Web3](/dashboard/project/_/auth/providers) to allow users of your application to sign in to your application using only their Web3 wallet.
...
Many wallet applications will warn the user if the message sent for signing is not coming from the page they are currently visiting. To further prevent your Supabase project from receiving signed messaged destined for other applications, you will need to register your application's URL using the [Redirect URL settings](/docs/guides/auth/redirect-urls).
