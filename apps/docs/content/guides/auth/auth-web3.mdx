...
### How to associate an email address, phone number or social sign-in to a user signing in with Web3?
...
To associate an email address, phone number or other social sign-in with their account you can use the `supabase.auth.updateUser()` or `supabase.auth.linkIdentity()` APIs.
