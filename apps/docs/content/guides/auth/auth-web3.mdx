...
[RainbowKit](https://www.rainbowkit.com/) offers a polished wallet selector and session UX that slots neatly into Supabase’s Sign in with Web3 flow. The steps below assume a Next.js app, but the same approach works for any React environment.

### Prerequisites

- Supabase project with the Web3 wallet provider enabled and redirect URLs configured
- A Next.js (or React) project using Wagmi v2 and Viem v2+
- Packages installed:

  ```bash
  npm install @rainbow-me/rainbowkit wagmi viem@2.x @tanstack/react-query @supabase/supabase-js
  ```

### 1. Configure RainbowKit & Wagmi

Create or update `lib/wagmi/config.ts`:

```typescript
// filepath: lib/wagmi/config.ts
...
### 2. Build a SIWE adapter

RainbowKit needs an adapter that can fetch a nonce, craft the SIWE payload, and verify signatures. Supabase handles the verification:

```typescript
// filepath: lib/siwe/siweAdapter.ts
  ...
  return createAuthenticationAdapter({
    ...
    createMessage: ({ nonce, address, chainId }) =>
      createSiweMessage({
        ...
        address,
        statement: 'Sign in with your Ethereum wallet. Accept terms: https://yourapp.com/terms',
...
Centralize providers in `components/Providers.tsx` (or equivalent):
...
import { createSiweAdapter } from '@/lib/siwe/siweAdapter'
import { createClient } from '@/lib/supabase/client'
...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  const siweAdapter = useMemo(() => createSiweAdapter(), [])
