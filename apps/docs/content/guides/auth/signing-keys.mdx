...
You can import the generated private key to your project by creating a [new standby key](/dashboard/project/_/settings/jwt/signing-keys) from the dashboard. Remember to remove all unsupported fields, such as `use`, `key_ops`, `alg`, and `ext`. For e.g.
...
To mint a new JWT using the asymmetric signing key, you will need to set the following JWT headers to match your generated private key.
...
{
  "iss": "supabase",
  "ref": "vdpfzijcqftxdatebvcx",
  "role": "service_role",
  "exp": 1757749466
}
```

- `iss` is optional and can be set to any string that identifies the issuer of this JWT.
- `ref` must be set to the project ref which you have imported the signing key.
- `role` must be set to an existing Postgres role in your database, such as `anon`, `authenticated`, or `service_role`.
- `exp` must be set to a timestamp in the future (seconds since epoch) when this token expires.

Finally, you can use your newly minted JWT as the `Authorization: Bearer <JWT>` header to all Data API requests.

```sh
curl 'https://vdpfzijcqftxdatebvcx.supabase.co/rest/v1/cities?select=id' \
  -H "apikey: sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH" \
  -H "Authorization: Bearer eyJhbGciOiJFUzI1NiIsImtpZCI6IjNhMThjZmUyLTcyMjYtNDNiMC1iYmI0LTdjNTI0MmYyNDA2ZSIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZkcGZ6aWpjcWZ0eGRhdGVidmN4Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTc1Nzc1MTc3Mn0.mxPJhHlNnEou7Qqu9TJHFZ9EMH9DVLSGbeVg4RiuAEzgN0gQgyxP7BRARrkZyrjCqPVtJe5HSsOW-OVvFQCOYw"
```

Note that an `apikey` header is required to access your project's APIs. Use a [publishable, secret or the legacy `anon` or `service_role` keys](/docs/guides/api/api-keys) here. Using your minted JWT is not possible in this header.
