...
You can import the generated private key to your project by creating a [new standby key](/dashboard/project/_/settings/jwt/signing-keys) from the dashboard.
...
To mint a new JWT using the asymmetric signing key, you will need to set the following [JWT headers](/docs/guides/auth/jwts#introduction) to match your generated private key.
...
The `kid` header is used to identify your public key for verification. You must set the same value when importing on platform.
...
Finally, you can [use your newly minted JWT](/docs/guides/auth/jwts#using-custom-or-third-party-jwts) as the `Authorization: Bearer <JWT>` header to all Data API requests.
