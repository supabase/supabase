...
![Supabase Social Auth example](/docs/img/supabase-expo-social-auth-sign-in.png)
...
<$CodeSample
path="/auth/expo-social-auth/app/sign-in.tsx"
lines={[[1, -1]]}
meta="name=app/sign-in.tsx"
/>
...
For either option, you need to obtain a Service ID from the [Apple Developer Console](/docs/guides/auth/social-sign-in/auth-apple?queryGroups=framework&framework=nextjs&queryGroups=platform&platform=web#configuration-web).
...
For more information, follow the [Supabase Sign in with Apple](/docs/guides/auth/social-sign-in/auth-apple) guide.
...
For either option, you need to obtain a Web Client ID from the Google Cloud Engine, as explained in the [Google Sign In](/docs/guides/auth/social-sign-in/auth-google?queryGroups=platform&platform=react-native#react-native) guide.
