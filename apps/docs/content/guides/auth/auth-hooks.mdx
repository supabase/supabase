...
A hook is an endpoint that allows you to alter the default Supabase Auth flow at specific execution points, which can be configured as a HTTPS endpoint or a Postgres Function. Developers can use hooks to add custom behavior that's not supported natively.
...
| Hook                                                                                                                          | Description                                                 | Available on Plan    |
| ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | -------------------- |
| [Custom Access Token](/docs/guides/auth/auth-hooks?queryGroups=language&language=sql#custom-access-token)                     | Add additional claims to the user's access token returned   | Free, Pro            |
| [Send SMS](/docs/guides/auth/auth-hooks?queryGroups=language&language=sql#send-sms)                                           | Use a custom SMS provider to send authentication messages   | Free, Pro            |
| [Send Email](/docs/guides/auth/auth-hooks?queryGroups=language&language=sql#send-email)                                       | Use a custom email provider to send authentication messages | Free, Pro            |
| [MFA Verification Attempt](/docs/guides/auth/auth-hooks?queryGroups=language&language=sql#mfa-verification-attempt)           | Add additional checks to the MFA verification flow          | Teams and Enterprise |
| [Password Verification Attempt](/docs/guides/auth/auth-hooks?queryGroups=language&language=sql#password-verification-attempt) | Add additional checks to the password verification flow     | Teams and Enterprise |
