...
Each Hook has a dedicated `HOOK_SECRET` used to sign the payload. Ensure that you create an entry in your `.env.local` file for each Hook that you have. In this case, let’s assume we’re adding a  `SEND_SMS` Hook

```bash
SEND_SMS_HOOK_SECRET="v1,whsec_<base-64-secret>"
```

`v1` denotes the version of the hook while `whsec_` signifies that we’re using a symmetric secret. The secret is encoded via standard Base64 and can contain the characters `+`, `/` and `=`

Use the secret in conjunction with the Standard Webhooks package to verify the Payload at the start of every function:
...
HTTP Hooks support a limited set of status codes:
...
There are both retryable and non-retryable errors. On a retryable error, HTTP Hooks will attempt up to three retries with a backoff of two seconds while a Postgres Hook will fail on error. Here’s an overview of a HTTP retry schedule:
