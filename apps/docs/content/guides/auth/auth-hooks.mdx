...
`204` Please see the hook specific documentation for status code handling. Some hooks do not support `204` responses because they read the request body.
...
`Retry-After` We do not fully support the Retry-After header as described in RFC7231, we only check if it is a non-empty value such as `true` or `10`. Setting this to your preferred value is fine as a future update to the auth server may address this.
