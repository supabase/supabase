...
Disable JWT verification via the `--no-verify-jwt` to accomodate hooks which are run before a JWT is issued. Payload authenticity is instead protected via the appended security headers associated with the Standard Webhooks Standard.
...
Your company requires that a user can make an incorrect MFA verification attempt no more than three times in an hour. Redis is chosen as a backing store out of convenience since it is already present in the company setup.
...
- Use alternate messaging channels such as WhatsApp or Facebook Messenger
