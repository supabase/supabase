...
Each Hook description contains an example JSONSchema which you can use in conjunction with [JSONSchema Faker](https://json-schema-faker.js.org/) in order to generate a mock payload.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const timestampKey = `${user_id}:last_attempt`;
    ...
    if (!valid) {
      ...
      await redis.set(timestampKey, currentTimestamp);

      await redis.expire(userKey, secondsBeforeNextRetry);
      await redis.expire(timestampKey, secondsBeforeNextRetry);

      if (failedAttemptsCount >= 3) {
        return new Response(JSON.stringify({ decision: 'reject' }), { status: 403, headers: { "Content-Type": "application/json" } });
      }
    } else {
      // Reset the counter if the attempt is valid
      await redis.hdel('mfa-attempts', userKey);
...
Deno.serve(async (req) => {
    ...
    const headers = Object.fromEntries(req.headers);
    const wh = new Webhook(base64_secret);
    const secondsBeforeNextRetry = 3;

    try {
        const { user_id, valid } = wh.verify(payload, headers);
        ...
        const timestampKey = `${user_id}:last_password_attempt`;
        ...
        if (!valid) {
            ...
            // Reset the counter if the attempt is valid
            await redis.hdel('password-attempts', userKey);
            await redis.del(timestampKey);
        }
