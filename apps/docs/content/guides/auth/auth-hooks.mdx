...
For HTTP Hooks, errors are responses which contain status codes 400 and above. On a retryable error, such as an error with a `429` or `503` status code, HTTP Hooks will attempt up to three retries with a backoff of two seconds. Here’s a sample HTTP retry schedule:
