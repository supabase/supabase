...
When it comes to migrating your users to a different authentication service, these are some of the considerations that we've seen across other projects.

- How does the cost scale as your user base grows?
- Does the new auth provider selected cover the same set of features needed (e.g OAuth, password logins, SAML, MFA)?
...
1. Use [Auth0 export data functionality](https://auth0.com/docs/troubleshoot/customer-support/manage-subscriptions/export-data)
...
When migrating over users who use passwords to sign-in, we recommend adopting a hybrid approach to reduce downtime. This approach ensures that you won't have users that aren't migrated to Supabase Auth after the Auth0 service has been terminated.
...
- `auth.users.raw_user_meta_data` : For storing non-sensitive user metadata that the user can update (e.g full name, age, favourite colour).
...
These fields will be exposed in the user's access token JWT so we recommend not to store excessive metadata in these fields.
...
If you have a lot of user-specific metadata to store, it is recommended to create your own table in a private schema that uses the user id as a foreign key. For example:
...
All users stored in Supabase Auth use the UUID V4 format as the ID. If your UUID format is identical, you can simply specify it in the admin create user method like this:
