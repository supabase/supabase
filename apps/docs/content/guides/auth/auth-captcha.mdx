...
Create a empty state to store our `captchaToken`
...
We will pass it the sitekey we copied from the hCaptcha website as a property along with a `onVerify` property which takes a callback function. This callback function will have a token as one of its properties. set the token in the state using `setCaptchaToken`
...
Create an empty state to store our `captchaToken`
...
We will pass it the sitekey we copied from the Cloudflare website as a property along with a `onSuccess` property which takes a callback function. This callback function will have a token as one of its properties. set the token in the state using `setCaptchaToken`:
