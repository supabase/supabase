...
It is strongly recommended to enable captcha or turnstile to prevent abuse for anonymous sign-ins. There is also an IP-based rate limit for anonymous sign-ins at 30 requests per hour which can be configured in your [dashboard](/dashboard/project/_/auth/rate-limits).

## Automatic cleanup

To clean up orphaned anonymous users, you can enable automatic cleanup which will delete anonymous users that are older than 30 days.
