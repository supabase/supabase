...
1. **Your project's PostgreSQL database** - Stored in the `auth.audit_log_entries` table, queryable via SQL but uses database storage
2. **External log storage** - Cost-efficient storage accessible through the dashboard

You can disable PostgreSQL storage to reduce database storage costs while keeping the external log storage.
...
Disabling PostgreSQL storage reduces your database storage costs. Audit logs will still be available through the dashboard.
