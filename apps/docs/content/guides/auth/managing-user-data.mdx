...
Make sure to protect the table by enabling [Row Level Security](/docs/guides/database/rls). Reference the `auth.users` table to ensure data integrity. Specify `on delete cascade` in the reference. For example, a `public.profiles` table might look like this:
