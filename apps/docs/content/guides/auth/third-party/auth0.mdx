...
Auth0 can silently strip claims without a namespace when using `api.accessToken.setCustomClaim`. We recommend ID tokens or namespaced access token claims to avoid this issue.
