...
If you are using **ID tokens** (recommended for the TypeScript SPA example above):
...
If you are using **access tokens** instead, you must use a namespaced claim because [Auth0 strips non-namespaced custom claims from access tokens](https://auth0.com/docs/troubleshoot/product-lifecycle/past-migrations/custom-claims-migration):

```javascript
exports.onExecutePostLogin = async (event, api) => {
  api.accessToken.setCustomClaim('https://your-app.com/role', 'authenticated')
}
```

<Admonition type="caution">

Using `api.accessToken.setCustomClaim('role', 'authenticated')` without a namespace may appear to work but Auth0 can silently strip the claim. Use ID tokens or namespaced access token claims to avoid this issue.
