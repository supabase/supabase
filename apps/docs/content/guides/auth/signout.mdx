...
- `global` when all sessions active for the user are terminated.
- `local` (recommended default) which only terminates the current session for the user but keep sessions on other devices or browsers active.
...
// ---cut---
// For multi-device apps, use local scope (recommended)
await supabase.auth.signOut({ scope: 'local' })

// To sign out from all devices/sessions
await supabase.auth.signOut({ scope: 'global' })

// Note: Default behavior may vary by client library
// Always specify scope explicitly for predictable behavior
await supabase.auth.signOut({ scope: 'local' })
```
