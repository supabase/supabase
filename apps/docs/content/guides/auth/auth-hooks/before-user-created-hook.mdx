...
This example uses the [`ipaddr.js`](https://www.npmjs.com/package/ipaddr.js/v/2.2.0) npm package with the  for CIDR matching.
```ts
import { Webhook } from 'https://esm.sh/standardwebhooks@1.0.0'
...
Deno.serve(async (req) => {
  ...
  try {
    const { header } = wh.verify(payload, headers)
    const ip = header.ip_address
