...
Older projects don't have these keys yet. If you see a **Create new API keys** button, your project is still on legacy keys only. Creating the new keys is safe: it adds a publishable key and a secret key alongside your existing `anon` and `service_role` keys. Nothing breaks, and your legacy keys keep working.
...
The publishable keys work the same way through `SUPABASE_PUBLISHABLE_KEYS`. See [Managing Secrets](/docs/guides/functions/secrets) for more on environment variables in Edge Functions.
...
Once you've confirmed nothing uses the legacy keys, deactivate them in the [**Settings > API Keys**](/dashboard/project/_/settings/api-keys/) section. You can re-activate them if you find a client you missed, so this step is reversible.
