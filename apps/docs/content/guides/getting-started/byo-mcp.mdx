...
The `--no-verify-jwt` flag disables JWT verification at the Edge Function layer so your MCP server can accept unauthenticated requests. If your mcp server needs authentication, keep your function private until MCP auth support on Edge Functions is available.
