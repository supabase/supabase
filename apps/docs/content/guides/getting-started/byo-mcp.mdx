...
- [Docker](https://docs.docker.com/get-docker/) installed (required for local development)
...
- A sign-in method configured (e.g., [Google](/docs/guides/auth/social-login/auth-google)) in the [Supabase dashboard](https://supabase.com/dashboard/project/_/auth/providers) - see [Auth Providers](/docs/guides/auth#providers) for email, phone, and social login options
- ECC (P-256) signing keys enabled - [rotate keys here](https://supabase.com/dashboard/project/_/settings/jwt) if needed
...
```json name=supabase/functions/mcp/deno.json
{
  "imports": {
...
After this step, you should have a new file at `supabase/functions/mcp/index.ts` and `supabase/functions/mcp/deno.json`.
...
1. Client requests MCP endpoint -> Server returns 401 with `WWW-Authenticate` header containing metadata URL
2. Client discovers OAuth config -> Fetches `/mcp/oauth-protected-resource` to find authorization server
3. User authenticates -> Redirected to consent page, logs in, approves access
4. Client receives token -> Authorization server issues access token
5. Client retries request -> Includes token in `Authorization: Bearer` header
6. Server validates token -> Uses Supabase Auth to verify, grants access to MCP tools
...
Now you'll add OAuth authentication to your MCP server. This requires updating the dependencies and replacing the server implementation.

First, update `supabase/functions/mcp/deno.json` to include the Supabase client:
...
Then, replace the contents of `supabase/functions/mcp/index.ts` with the authenticated implementation:
...
1. Clone or deploy the [Next.js consent page example](https://github.com/supabase/supabase/tree/master/examples/auth/nextjs-oauth-consent) to your hosting platform
...
- [OAuth Consent Page block](https://ui.supabase.com/docs/nextjs/oauth-consent) - Supabase UI Kit block for consent pages
