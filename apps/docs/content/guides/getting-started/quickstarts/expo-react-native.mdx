...
Installing `expo-sqlite` adds a config plugin entry to `app.json`. To verify the instruments list without a simulator or device, install the web dependencies and run with `--web`:
