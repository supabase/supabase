...
Make sure you have Java 17 or later installed (`java -version`), then use [Spring Initializr](https://start.spring.io) to scaffold a new project with the Web, Spring Data JPA, and PostgreSQL Driver dependencies.
...
When your project is up and running, navigate to your project dashboard and click on [Connect](/dashboard/project/_?showConnect=true&method=session). Under the **Session pooler** (port `5432`), select the **JDBC** tab and copy the connection string. Replace the password placeholder with your saved database password. You can reset your database password in your [Database Settings](/dashboard/project/_/database/settings) if you do not have it.

<Admonition type="note">

Spring Data JPA uses Hibernate, which relies on server-side prepared statements, so the Transaction pooler (port `6543`) doesn't work as your app's main data source. Stick to the Session pooler, or use the direct connection string if you're in an [IPv6 environment](/docs/guides/troubleshooting/supabase--your-network-ipv4-and-ipv6-compatibility-cHe3BP) or have the IPv4 Add-On.

</Admonition>

Add the connection string and driver to `src/main/resources/application.properties`.

```text name=src/main/resources/application.properties
spring.datasource.url=jdbc:postgresql://xxxx.pooler.supabase.com:5432/postgres?user=postgres.xxxx&password=[YOUR-PASSWORD]
...
By default Hibernate creates tables in the `public` schema. We recommend changing this because Supabase exposes the `public` schema as a [data API](/docs/guides/api).

The schema has to exist on Supabase before your app starts, so create it from the [Table Editor](/dashboard/project/_/editor) first. Then point Hibernate at it in `application.properties`.
