...
Authenticate with a [personal access token](/dashboard/account/tokens). An OAuth application needs the `secrets:read` scope, and a fine-grained token needs both the `api_gateway_keys_read` and `api_gateway_keys_secret_read` permissions (the second one gates `?reveal=true`). Without either, the request returns 403 Forbidden.
