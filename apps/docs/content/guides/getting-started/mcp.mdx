...
You can add the Supabase MCP server to Gemini CLI.
Gemini CLI uses settings.json files for persistent configuration. There are two locations for these files:
`~/.gemini/settings.json` for all Gemini CLI sessions for the current user or 
`.gemini/settings.json` for project's root directory.


1. Create or edit the `settings.json` file and add the following configuration:
...
3. Start Gemini CLI to read the new configuration.
