...
You can use the Supabase MCP server either remotely (via Supabase's API servers) or as a local server running on your machine. Note that either way, the server must connect to Supabase Management API to manage your project.

To connect remotely, choose your project and MCP client and follow the installation instructions:

<McpConfigPanel />

## Local MCP

To run the MCP server on your local machine, create a personal access token and follow the instructions for configuring your MCP client:
