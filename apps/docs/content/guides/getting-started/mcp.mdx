...
2. **Configure Supabase MCP Server**
   ...
   - **Name**: `Supabase`
   - **Type**: `stdio`
   - **Command**: `npx -y @supabase/mcp-server-supabase@latest --read-only --project-ref=<project-ref>`
   - **Environment Variables**:
     - Add variable: `SUPABASE_ACCESS_TOKEN` with value `<personal-access-token>`

3. **Replace Placeholders**

   - Replace `<project-ref>` with your project reference from your Supabase dashboard
   - Replace `<personal-access-token>` with the personal access token you created in Step 1

4. **Verify Connection**
   - Save the configuration
   - The server should show as **active** in your MCP servers list
   - You may need to click the refresh button to populate the tool list

For additional configuration details, see the [Cursor MCP directory](https://cursor.directory/mcp/supabase).
