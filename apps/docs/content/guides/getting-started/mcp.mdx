...
1. Open [the Zed editor](https://zed.dev) and navigate to the Extensions page with the following keybinding `cmd-shift-x`/`ctrl-shift-x`.

1. Search for the Supabase MCP extension and install it.

1. Add your `supabase_access_token` in the configuration modal.

You can also install the MCP "directly" in Zed by adding the same JSON snippet in the `settings.json` that you would for any VS Code and forks.
