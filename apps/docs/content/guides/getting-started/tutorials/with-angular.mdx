...
```ts name=src/app/supabase.service.tsimport { Injectable } from '@angular/core'
import {
  AuthChangeEvent,
  createClient,
  Session,
...
Then add the widget on top of the `AccountComponent` HTML template:
...
Add an `updateAvatar` function along with an `avatarUrl` getter to the `AccountComponent` typescript file:
