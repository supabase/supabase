...
The **`web side`** is built with React. Redwood's router makes it to map URL paths to React "Page" components (and automatically code-split your app on each route).
...
Start building the RedwoodJS app from scratch.
...
And finally, you will also need to save **** the `web side` environment variables to the `redwood.toml`.
...
Test our setup at the moment by starting up the app:
...
You can stop the `dev` server if you want; to see your changes, be sure to run `yarn rw dev` again.
...
Create a new component for that called `Account.js`.
...
Create an avatar for the user so that they can upload a profile photo. We can start by creating a new component:
