...
```bash
npx nuxi@latest init nuxt-user-management

cd nuxt-user-management
```

Then let's install the only additional dependency: [NuxtSupabase](https://supabase.nuxtjs.org/). We can use `nuxi` to install the Supabase module.

```bash
npx nuxi@latest module add supabase
```

This will install the required dev dependencies and add it to the modules in the `nuxt.config.ts` file.
