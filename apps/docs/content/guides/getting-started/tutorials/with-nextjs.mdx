...
Save the environment variables in a `.env.local` file at the root of the project, and paste the API URL and the `publishable` key that you copied [earlier](#get-the-api-keys).
