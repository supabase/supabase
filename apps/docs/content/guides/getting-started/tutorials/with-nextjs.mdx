...
To add sign-in/signup page for your application, create a new folder named `login`, containing a `page.tsx` file with the following code for a sign-in/signup form:

<$CodeTabs>

<$CodeSample
path="/user-management/nextjs-user-management/app/sign-in/page.tsx"
lines={[[1, -1]]}
meta="name=app/sign-in/page.tsx"
/>

</$CodeTabs>

Create the sign-in/signup actions to hook up the form to the function which does the following:
...
Create the `action.ts` file in the `app/login` folder, which contains the sign-in and signup functions and the `error/page.tsx` file, which displays an error message if the sign-in or signup fails.

<$CodeTabs>

<$CodeSample
path="/user-management/nextjs-user-management/app/sign-in/actions.ts"
lines={[[1, -1]]}
meta="name=app/sign-in/actions.ts"
/>
...
And then open the browser to [localhost:3000/sign-in](http://localhost:3000/sign-in) and you should see the completed app.
