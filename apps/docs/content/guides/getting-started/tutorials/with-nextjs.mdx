...
All we need to do is create `.env.local` file at the root of the project, and paste the API URL and the `anon` key that you copied [earlier](#get-the-api-keys).
...
To better integrate with the framework, we've created the `@supabase/ssr` package for server-side auth. It has all the functionalities to quickly configure your Supabase project to use cookies for storing user sessions. For more information, checkout this [Next.js Server-Side Auth guide](/docs/guides/auth/server-side/nextjs).
...
<TabPanel id="js" label="JavaScript">
Create a `client.js` and a `server.js` with the following functionalities for client-side supbase and server-side supabase, respectively. 
...
Create a `middleware.js` file at project's root and another one within the `util/supbase` (since we need to create a server supabase client to refresh the tokens).
...
Create a `middleware.ts` file at project's root and another one within the `util/supbase` (since we need to create a server supabase client to refresh the tokens).
...
Standard to any application with authentication. We need to have a login/signup page. 
<Tabs
  scrollable
  size="small"
  type="underlined"
...
>
<TabPanel id="js" label="JavaScript">
First, we need to create a new folder `login`, and a new `page.jsx` with the following form to accept login/signup request.
...
<TabPanel id="ts" label="TypeScript">
First, we need to create a new folder `login`, and a new `page.tsx` with the following form to accept login/signup request.
...
Now you should have a login page at `http://localhost:3000/login` without much functionalities. Next is to create the login/singup actions. They need to fulfill the following requirement:
- Retrieve the user's given information.
...
<Admonition type="tip">
Supabase also offers some pre-built React component that handles all the authentication functionalities under the hood for you. Check this [guide](/docs/guides/auth/auth-helpers/auth-ui) out.
</Admonition>

### Proof Key for Code Exchange (PKCE)

Upon sign up, an email will be sent to the user with a unique hash code. This key can then be sent back to the application for verification, and thus authenticating the user. The method is referred to as Proof Key for Code Exchange (PKCE). As we are employing PKCE in our authentication flow, it is necessary to create a route handler responsible for exchanging the code for a session.
