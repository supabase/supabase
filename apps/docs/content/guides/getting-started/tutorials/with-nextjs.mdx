...
Create a `proxy.js` file at the project root and another one within the `utils/supabase` folder. The `utils/supabase` file contains the logic for updating the session. This is used by the `proxy.js` file, which is a Next.js convention.
...
```jsx name=utils/supabase/proxy.js
import { createServerClient } from '@supabase/ssr'
...
Create a `proxy.ts` file at the project root and another one within the `utils/supabase` folder. The `utils/supabase` file contains the logic for updating the session. This is used by the `proxy.ts` file, which is a Next.js convention.
...
<$CodeSample
path="/user-management/nextjs-user-management/utils/supabase/proxy.ts"
lines={[[1, -1]]}
meta="name=utils/supabase/proxy.ts"
/>
