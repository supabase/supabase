...
Start building the Flutter app from scratch.
...
Now that we have the dependencies installed setup deep links.
...
Create a new widget called `account_page.dart` for that.
...
Now that we have all the components in place, update `lib/main.dart`.
