...
When a user opens the magic link, Supabase may redirect back to your app with a `code` in the URL. The client exchanges this code for a session, which then triggers `onAuthStateChange`. If you need to handle the redirect flow manually, use `exchangeCodeForSession` with the returned code.
