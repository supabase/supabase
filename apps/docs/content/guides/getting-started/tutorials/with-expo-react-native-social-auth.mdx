...
Let's create a `.env` file containing these variables:
...
Next, we need to protect our navigation to prevent unauthenticated users from accessing protected routes. We'll use the [Expo SplashScreen](https://docs.expo.dev/versions/latest/sdk/splash-screen/) to display a loading screen while fetching the user profile and verifying authentication status.

#### Create the AuthContext

Let's create [a React context](https://react.dev/learn/passing-data-deeply-with-context) to manage the authentication session, making it accessible from any component:
...
Next, let's create a provider component to manage the authentication session throughout our app:
...
Let's create a `SplashScreenController` component to display the [Expo SplashScreen](https://docs.expo.dev/versions/latest/sdk/splash-screen/) while the authentication session is loading:
...
Let's create a logout button component to handle user sign-out:

<$CodeTabs>

```tsx name=components/social-auth-buttons/SignOutButton.tsx
import { supabase } from '@/lib/supabase';
...
and add it to the `app/(tabs)/index.tsx` file, that will be used to display the user profile data and the logout button:
...
Next, let's create a basic login screen component:
...
Now let's wrap our navigation with the `AuthProvider` and `SplashScreenController`.

Using [Expo Router's protected routes](https://docs.expo.dev/router/advanced/authentication/#using-protected-routes), we can secure our navigation:
...
Now let's integrate social authentication with Supabase Auth, starting from the Apple authentication.
...
Let's start by adding the button that we are going to create inside the login screen previously made:
...
For either option, you'll need to obtain a Service ID from the [Apple Developer Console](https://supabase.com/docs/guides/auth/social-login/auth-apple?queryGroups=framework&framework=nextjs&queryGroups=platform&platform=web#configuration-web).

<Admonition type="note">

To enable Apple sign-up on Android and Web, you'll also need to register the tunneled URL (e.g., `https://arnrer1-anonymous-8081.exp.direct`) obtained by running:

```bash
npx expo start --tunnel
```

and add it to the "Allowed Callback URLs" list in your Supabase dashboard Apple provider configuration.
...
>
  <TabPanel id="invertase-react-native-apple-authentication" label="Invertase">
    ...
    You'll also need to add two new environment variables to the `.env` file:
    ...
    Let's install the `@invertase/react-native-apple-authentication` library:
    ...
    ```tsx name=components/social-auth-buttons/apple/AppleSignInButton.ios.tsx
    import { supabase } from '@/lib/supabase';
    ...
    and add the capabilities to the `Info.plist` file by following the [Expo documentation](https://docs.expo.dev/build-reference/ios-capabilities/#xcode).
    ...
    Finally, let's update the iOS project by installing the Pod library and running the Expo prebuild command:
    ...
    If you get stuck while working through this guide, refer to the [full example guide on GitHub](https://github.com/invertase/react-native-apple-authentication?tab=readme-ov-file#react-native-apple-authentication).
    ...
    Next, let's create the Android-specific `AppleSignInButton` component:
    
    <$CodeTabs>

    ```tsx name=components/social-auth-buttons/apple/AppleSignInButton.android.tsx
    import { supabase } from '@/lib/supabase';
    ...
    We should now be able to test the authentication by running it on a phisical device or simulator:
  ...
  <TabPanel id="react-apple-signin-auth" label="Web">
    ...
    Before proceeding, as per the mobile options you'll need an Apple Service ID. To obtain it you can follow the [Invertase Initial Setup Guide](https://github.com/invertase/react-native-apple-authentication/blob/main/docs/INITIAL_SETUP.md) and the [Invertase Android Setup Guide](https://github.com/invertase/react-native-apple-authentication/blob/main/docs/ANDROID_EXTRA.md) mentioned in the Invertase tab.
    ...
    Next, let's create the Web-specific `AppleSignInButton` component:
    
    <$CodeTabs>

    ```tsx name=components/social-auth-buttons/apple/AppleSignInButton.web.tsx
    import { supabase } from '@/lib/supabase';
    import type { SignInWithIdTokenCredentials } from '@supabase/supabase-js';
  ...
  <TabPanel id="expo-apple-authentication" label="Expo">
    ...
    Let's install the `expo-apple-authentication` library:
    ...
    ```tsx name=components/social-auth-buttons/apple/AppleSignInButton.tsx
    import { supabase } from '@/lib/supabase';
    import { AppleButton, appleAuth } from '@invertase/react-native-apple-authentication';
    import type { SignInWithIdTokenCredentials } from '@supabase/supabase-js';
    import { router } from 'expo-router';
    ...
    The Expo Apple Sign In button does not support the Simulator, so you will need to test it on a physical device.
...
- [GN Google Sign In Premium](https://react-native-google-signin.github.io/docs/install#sponsor-only-version) - that supports iOS, Android, and Web by using the latest Google's OneTap sign-in (but [it requires a subscription](https://universal-sign-in.com/))
...
- Relying on the [signInWithOAuth](https://supabase.com/docs/reference/javascript/auth-signinwithoauth) function of the Supabase Auth - that also supports iOS, Android and Web (__useful also to manage any other OAuth provider__)

<Admonition type="note">

The [GN Google Sign In Free](https://react-native-google-signin.github.io/docs/install#public-version-free) doesn't support neither iOS nor Android, as [it doesn't allow to pass a custom nonce](https://github.com/react-native-google-signin/google-signin/issues/1176) to the sign-in request.

</Admonition>


For either option, you'll need to obtain a Web Client ID from the Google Cloud Engine, as explained in the [Google Sign In](https://supabase.com/docs/guides/auth/social-login/auth-google?queryGroups=platform&platform=react-native#react-native) guide.

In this guide we will only use the [@react-oauth/google@latest](https://github.com/MomenSherif/react-oauth#googlelogin) option for the Web, and the [signInWithOAuth](https://supabase.com/docs/reference/javascript/auth-signinwithoauth) for the mobile platforms.

Before proceeding, you'll need to add a new environment variable to the `.env` file:
...
>
  <TabPanel id="web" label="Web">
    ...
    Then create the iOS specific button component `GoogleSignInButton.web`:

    <$CodeTabs>
    ```tsx name=components/social-auth-buttons/google/GoogleSignInButton.web.tsx
    ...
    To allow the Google Sign In to work, as we did before for Apple, you'll need to register the tunneled URL (e.g., `https://arnrer1-anonymous-8081.exp.direct`) obtained to the Authorized JavaScript origins list of your [Google Cloud Console's OAuth 2.0 Client IDs](https://console.cloud.google.com/auth/clients/) configuration.
  ...
  <TabPanel id="mobile" label="Mobile">
    ...
    Then create the mobile generic button component `GoogleSignInButton.tsx`:

    <$CodeTabs>

    ```tsx name=components/social-auth-buttons/google/GoogleSignInButton.tsx
    import { supabase } from '@/lib/supabase';
