...
All we need are the API URL and the anon (public) key that you copied [earlier](#get-api-details).
You can find both values in your Supabase Dashboard under **Project Settings → API**.
The anon key is safe to use in a browser because it only grants access according to your database's [Row Level Security](/docs/guides/auth#row-level-security) policies.
