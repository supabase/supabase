...
After a user is signed, allow them to edit their profile details and manage their account.
...
Now that you have all the components in place, update `src/App.jsx`, which fetches the user session using the [`getUser`](/docs/reference/javascript/auth-getuser) method to get the current user details if there is an existing session. This method performs a network request to the Supabase Auth server. 
