...
- Learn how to [enable authorization](/docs/guides/auth/oauth-server/mcp-authentication) on your Supabase project to secure your MCP server
...
Secure your MCP server with [Supabase Auth OAuth 2.1 flow](/docs/guides/auth/oauth-server). Learn how to enable OAuth in your project [here](/docs/guides/auth/oauth-server/mcp-authentication).

Authentication occurs inside the MCP server, not at the Edge Function layer.
...
app.all('/mcp', async (c) => {
  ...
  // See MCP Authentication guide for details
...
### Expose OAuth metadata

Create a `.well-known/oauth-protected-resource` endpoint for MCP client discovery:

```ts
app.get('/.well-known/oauth-protected-resource', (c) => {
  const projectRef = Deno.env.get('SUPABASE_PROJECT_REF')
  return c.json({
    resource: `https://${projectRef}.supabase.co/functions/v1/simple-mcp-server`,
