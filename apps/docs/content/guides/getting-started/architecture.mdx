...
Supabase is open source. We choose open source tools which are scalable and make them to use.
...
Postgres is the core of Supabase. We do not abstract the Postgres database—you can access it and use it with full privileges. We provide tools which make Postgres as to use as Firebase.
...
To avoid lock-in, we make it to migrate in and out. Our cloud offering is compatible with our self-hosted product.
