...
Features in Beta are tested by an external penetration tester. The API is guaranteed to be stable and we follow a strict communication process if there is a breaking change.
