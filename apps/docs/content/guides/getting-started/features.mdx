...
Automatically replicate your database to destination systems like data warehouses and analytics platforms with external replication (ETL), with Supabase ETL. Read [the documentation](/docs/guides/database/replication/external-replication-setup) for more details.
...
Supabase Storage makes it to store and serve files. [Docs](/docs/guides/storage).
