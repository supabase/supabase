...
Whitelabel the Supabase APIs to create a branded experience for your users. [Docs](/docs/guides/platform/custom-domains).
...
Login to the Supabase dashboard via Single Sign-On. [Docs](/docs/guides/platform/sso).
...
Supabase Features are in 4 different states - Private Alpha, Public Alpha, Beta and GA.

### Private Alpha

Features are initially launched as a private alpha to gather feedback from the community. To join our early tester program, send an email to product-ops@supabase.io.

### Public Alpha

Even though our SLA does not cover products in Alpha, we do our best to have the service as stable as possible. The alpha stage indicates that the API might change in the future, not that the service isn’t stable.

### Beta

Features in Beta are tested by an external pentester. The API is guaranteed to be stable and we follow a strict communication process if there is a breaking change.

### GA

In addition to our commitment to Beta, features in GA are covered by our [uptime SLA](https://supabase.com/sla).
