...
The `event_message` and `timestamp` fields will be dropped from the events to avoid sending duplicate data.

Loki must be configured to accept **stuctured metadata**, and it is advised to increase the default maximum number of structured metadata fields to at least 500 to accomodate large log event payloads of different products.
