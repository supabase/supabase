...
| Swap usage                   | Free, Pro       | Swap space in use by the operating system    | Sustained swap usage indicates memory pressure                 |
...
Postgres instances on Supabase have a fixed 1 GB of swap allocated. The chart shows the amount of swap space currently in use by the operating system.

| Component | Description                                                                                     |
| --------- | ----------------------------------------------------------------------------------------------- |
| **Swap**  | Swap space in use by the operating system. A fixed 1 GB is allocated for all Postgres instances |

How it helps debug issues:

| Issue                     | Description                                                                      |
| ------------------------- | -------------------------------------------------------------------------------- |
| Memory pressure detection | Sustained swap usage signals that the database is under memory pressure          |
| Performance degradation   | Swap is much slower than RAM; high swap usage can significantly slow query times |

Actions you can take:

| Action                                                                      | Description                                                    |
| --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| [Upgrade compute size](/docs/guides/platform/compute-and-disk#compute-size) | Increase available RAM to reduce reliance on swap              |
| [Optimize queries](/docs/guides/database/query-optimization)                | Reduce memory consumption of expensive queries                 |
| [Tune Postgres configuration](https://pgtune.leopard.in.ua)                 | Adjust memory settings such as `work_mem` and `shared_buffers` |
