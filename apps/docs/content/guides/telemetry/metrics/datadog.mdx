...
Datadog publishes a <a href="https://docs.datadoghq.com/integrations/supabase/" target="_blank" rel="noreferrer">Supabase integration reference</a> that uses the same Metrics API outlined here. That integration is managed by Datadog and is not officially supported by Supabase.
...
If you already run <a href="https://prometheus.io/docs/prometheus/latest/installation/" target="_blank" rel="noreferrer">Prometheus</a> (self-hosted or managed), push scraped metrics into Datadog using the [Prometheus Remote Write integration](https://docs.datadoghq.com/integrations/guide/prometheus_remote_write/):
