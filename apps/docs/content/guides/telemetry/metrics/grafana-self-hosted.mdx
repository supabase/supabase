...
Grafana also documents a <a href="https://grafana.com/docs/grafana-cloud/monitor-infrastructure/integrations/integration-reference/integration-supabase/" target="_blank" rel="noreferrer">Supabase integration reference</a>. While it targets Grafana Cloud, the scrape and agent settings apply equally to self-hosted clusters and offer a community-maintained companion to this guide.
...
Install <a href="https://prometheus.io/docs/prometheus/latest/installation/" target="_blank" rel="noreferrer">Prometheus</a> using your preferred method (Docker, Helm, binaries). Then add a Supabase-specific job to `prometheus.yml`:
