...
- `@opentelemetry/api` resolvable at runtime (installed directly, or as a transitive
  dependency of your tracing SDK — e.g., Sentry v8+ and Datadog already include it)
- A tracing SDK that registers a W3C-compliant propagator with the OpenTelemetry API.

  <Admonition type="note">

  Vendor SDKs that wrap OTel (Sentry, Datadog, Honeycomb, etc.) work — but some require additional configuration to inject the standard `traceparent` header alongside their proprietary headers. Check your vendor's OTel integration docs.
