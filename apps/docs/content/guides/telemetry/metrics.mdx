...
The pre-configured [Supabase Grafana Dashboard](https://github.com/supabase/supabase-grafana/) is an advanced version of the [Dashboard's Database Reports](/dashboard/project/_/observability/database). It visualizes over 200 database performance and health metrics.

![Supabase Grafana](/docs/img/guides/platform/supabase-grafana-prometheus.png)

Instructions are included in the [README](https://github.com/supabase/supabase-grafana/blob/main/README.md) for deploying the repository using docker.
...
**For Grafana Cloud users** instead of manually configuring `prometheus.yaml`, use a `Metrics Endpoint` instead:

- In the left-hand menu, select **Connections** and click **Add new connection**.
- Search for **Metrics Endpoint** and select it.
- Click **Add new scrape job**.
- Enter the details specified in [Accessing the metrics endpoint](/docs/guides/telemetry/metrics#accessing-the-metrics-endpoint).
- Set the **scraping interval** to "60s".
- Test the connection. Once it has passed, click **Save Scrape Job**.
...
**Grafana Cloud users** refer to [Grafana Cloud Prometheus connection details](/docs/guides/telemetry/metrics#grafana-cloud-prometheus).

Name the datasource `prometheus` (if you use a different name, you will need to modify the `dashboard.json` file when adding the dashboard).

Under **Interval behavior**, set the **scraping interval** to 60s and test the data source. Once it has passed, save it.

#### Grafana cloud Prometheus connection details [#grafana-cloud-prometheus]

- Log into your [Grafana Cloud Portal](https://grafana.com/auth/sign-in/).
- Under `Grafana Cloud stack` click `Details`.
- Search for `Prometheus` and click `Details`.
- The `Grafana Data Source settings` section will contain the necessary connection details
  - "URL" (the format "https://UNIQUE_IDENTIFIER.grafana.net/api/prom")
  - "User" / "Password"
...
Copy the raw file from our [supabase-grafana](https://raw.githubusercontent.com/supabase/supabase-grafana/refs/heads/main/grafana/dashboard.json) repository.

- If your Prometheus data source is named "prometheus", paste the contents of the file (or upload the file).
- If your Prometheus data source is **not named "prometheus"**, find all occurrences of `"uid": "prometheus"` and replace `prometheus` with your custom data source name.

Click "Load" and the dashboard loads from the project specified in your Prometheus job.
