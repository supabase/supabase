...
**For Grafana Cloud users** instead of using the yaml file above, use a `Metrics Endpoint` instead:
...
Name the datasource `prometheus` (if you use a different name, you will need to modify the json file when adding the dashboard).
...
- If your Prometheus data source is **not named `prometheus`**, find all occurences of `"uid": "prometheus"` and replace `prometheus` with your custom data source name.
