...
Deno.serve(async (req) => {
  const { data: messages, error } = await supabase.schema('pgmq_public').rpc('read', {
    queue_name: queueName,
    sleep_seconds: 30, // Hide these messages from other readers for 30 seconds
...
4. If `processMessage` throws an error, the error is logged. In this case, the message is still in the queue. Once its 30-second visibility timeout expires, the next run of this Edge Function reads it again.

Set `sleep_seconds` longer than a run takes to process and delete its messages. If it's shorter, or `0`, another run can read the same messages before they're deleted and process them twice. See [`pgmq_public.read`](/docs/guides/queues/api#pgmqpublicreadqueuename-sleepseconds-n) for details.
