...
{/* <!-- vale off --> */}
This guide will help you read & process queue messages server-side with a Supabase Edge Function. Check out [Queues API Reference](/docs/guides/queues/api) for more details on our API.

## Concepts

Supabase Queues is a pull-based Message Queue consisting of three main components: Queues, Messages, and Queue Types. You should already be familiar with the [Quickstart](https://supabase.com/docs/guides/queues/quickstart).

### Consuming messages in an Edge Function

This is a Supabase Edge Function that will read 5 messages off the queue, process each of them, and delete each message when it is done.
...
Every time this Edge Function is run it will:

1. Read 5 messages off the queue
1. Call the `processMessage` function
1. At the end of `processMessage`, the message is deleted from the queue
1. If `processMessage` throws an error, the error is logged. In this case the message will still be on the queue, so the next time this Edge Function runs it will read the message again.
