...
The [`plugins`](https://www.npmjs.com/package/plugins) package auto-detects your installed AI coding agents and installs the [Supabase plugin](https://github.com/supabase-community/supabase-plugin) to all of them with one command. Use `--yes` to skip the confirmation prompt.

If you only need the skills without MCP, use `npx skills add supabase/agent-skills` instead. See [Agent Skills](/docs/guides/ai-tools/ai-skills).
