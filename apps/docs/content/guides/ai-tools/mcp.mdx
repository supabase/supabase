...
Parameters can be combined: <CustomContent path="mcp:servers">{(servers) => <code>{servers?.remote}?project_ref=abc123&read_only=true</code>}</CustomContent>

<Admonition type="tip">

When using [Supabase CLI](/docs/guides/cli) for local development, the MCP server is available at <CustomContent path="mcp:servers">{(servers) => <code>{servers?.local}</code>}</CustomContent>.
