...
The bundled version 0.13 CLI stdio server does not offer elicitation and uses the legacy cost confirmation workflow.
...
A supplied `confirm_cost_id` is ignored only for a tool call that uses form-based cost confirmation. A tool call that uses the legacy cost confirmation workflow still validates the cost confirmation against the current quote.
...
Skip form-based cost confirmation: <code><CustomContent data="mcp:servers">remote</CustomContent>?skip_elicitations=create_project,create_branch</code>
...
Skipping form-based cost confirmation retains the [legacy cost confirmation workflow](#cost-confirmation). Skipping SQL forms uses the existing SQL execution path without an additional MCP confirmation. Neither option bypasses authentication, permissions, read-only restrictions, or charges.
...
- **Cost confirmation**: When your connection uses form-based cost confirmation, the server asks for [confirmation through your client](#cost-confirmation) before it creates resources that incur charges. This is a guardrail on top of authentication and authorization, not a replacement for them.
