...
When using [Supabase CLI](/docs/guides/local-development) for local development, the MCP server is available at <code><CustomContent data="mcp:servers">local</CustomContent></code>. This address applies to a local project started with `supabase start`. Local projects started with the experimental `supabase stack start` command use assigned ports instead of the default. See [Running multiple local projects](/docs/guides/local-development/parallel-projects).
