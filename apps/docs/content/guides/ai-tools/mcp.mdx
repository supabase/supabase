...
Cost confirmations require the server to offer cost elicitation for the tool and your client to support form elicitations for the request. The MCP specification describes [form elicitations](https://modelcontextprotocol.io/specification/2026-07-28/client/elicitation), but the specification revision alone does not determine which flow your connection uses.

If cost elicitation is unavailable, unsupported by your client, or [skipped for the tool](#skip-form-confirmations), the existing flow applies: the agent quotes the cost in chat, uses `get_cost` and `confirm_cost`, and passes the returned `confirm_cost_id` to the creation tool. These helpers are account tools and are not exposed in project-scoped connections. Skipping the form does not approve the cost or avoid charges.

The bundled version 0.13 CLI stdio server does not offer elicitation and uses the existing cost confirmation flow.

{/* Verify this table against the launch verification results before publishing. */}

| Client                               | Cost confirmation                |
| ------------------------------------ | -------------------------------- |
| Claude Code version 2.1.232 or later | Confirmation dialog              |
| Other clients                        | `get_cost` / `confirm_cost` flow |
