...
To authenticate the MCP server in a CI environment, create a scoped personal access token (PAT) and pass it as a header to the MCP server. A scoped token can only call the MCP tools its permissions unlock. See [MCP tools](/docs/guides/platform/personal-access-tokens#mcp-tools) for the permission each tool requires.
...
1. Navigate to your Supabase [access tokens](/dashboard/account/tokens) and generate a new token. Name the token based on its purpose, e.g. "Example App MCP CI token". Scope it to the project the server connects to and grant only the permissions the tools you enable require.
