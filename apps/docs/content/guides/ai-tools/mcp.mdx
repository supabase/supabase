...
- `get_cost` / `confirm_cost` - Cost information for clients that don't support [cost confirmation](#cost-confirmation)
...
Before your agent takes an action that incurs additional charges, the Supabase MCP server sends an _elicitation_: your MCP client shows the expected cost, and the action proceeds only on explicit approval. [Elicitations](https://modelcontextprotocol.io/specification/2026-07-28/client/elicitation) are the MCP standard's way for a server to pause a tool call and request your input.
...
- Accepting resumes the same tool call and creates exactly one resource.
...
- The rate is checked again immediately before creation. If it has changed since you confirmed, nothing is created at the confirmed rate. Instead, you receive a fresh confirmation that shows the updated cost.

If a creation has no cost, for example a project that is covered by your plan, no confirmation is requested.

### Client support

Clients that connect using MCP specification revision [2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28) or later, and that support form elicitations, receive cost confirmations. Clients on older revisions of the specification use the `get_cost` and `confirm_cost` flow instead: the agent quotes the cost in chat before creating the resource.
...
To check which flow your connection uses, ask your client to list the available Supabase tools. If `get_cost` and `confirm_cost` are in the list, your connection uses that flow. On connections that support cost confirmation, these two tools are hidden and a supplied `confirm_cost_id` is ignored.
...
- **Cost confirmation**: The server asks for [confirmation through your client](#cost-confirmation) before it creates resources that incur charges. This is a guardrail on top of authentication and authorization, not a replacement for them.
