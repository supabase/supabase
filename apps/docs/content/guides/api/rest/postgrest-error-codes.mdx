...
PostgREST error codes are only captured in the logs for projects running V14+. You can check your PostgREST version and upgrade your project in the [Infrastructure Settings](https://supabase.com/dashboard/project/_/settings/infrastructure)
...
where
  status_code >= 300
   ...
   regexp_contains(path, '^/rest/v1/')
    and
  regexp_contains(proxy_status, '(?i)THE_RELEVANT_STATUS_CODE')
```
...
#### Find data API request from specific authetenticated user
...
where
  regexp_contains(path, '^/rest/v1/') 
    AND
  sb.auth_user = 'SOME_USER_ID' --<---ADD USER_ID from auth.users table
order by timestamp DESC;
```
