...
select
  ...
  event_message,
  if(
    log_attributes['response.headers.proxy_status'] = '',
    'not_recorded',
    log_attributes['response.headers.proxy_status']
  ) as error_codes,
...
where
  ...
  and match(log_attributes['response.headers.proxy_status'], '(?i)THE_RELEVANT_STATUS_CODE')
order by timestamp desc
limit 100;
```
...
where
  ...
  and toInt32OrZero(log_attributes['response.status_code']) >= 300
  and match(log_attributes['request.path'], '^/rest/v1/')
group by hour, error_codes, path
order by hour desc
limit 100;
```
...
select
  ...
  log_attributes['request.cf.continent'] as continent,
