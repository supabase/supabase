...
```bash
npm i supabase@">=2.66.0" --save-dev
```
...
```bash
npx supabase gen types --lang=python --local > database.types.ts
```
...
# Select
selected = [PublicMovies(m) for m in .select("*").execute().data]

# Insert
inserted = [PublicMovies(m) for m in users.insert(PublicMoviesInsert(name="foo", status="bar")) \
                                          .execute().data]

# Update
updated  = [PublicMovies(m) for m in users.update(PublicMoviesUpdate(name="bar")) \
                                        .eq("id", 5) \
                                        .execute().data]
```
