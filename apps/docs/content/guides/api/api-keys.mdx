...
`anon` and `service_role` keys are based on the project's JWT secret. They are generated when your project is created and can only be changed when you rotate the JWT secret. This can cause significant issues in production applications. You should now use the `sb_publishable_xxx` and `secret` keys instead.
...
- **For new keys**, open the **Publishable and secret API keys** tab, if you don't have a publishable key already, click **Create new API Keys**, and copy the value from the **Publishable key** section.
- **For legacy keys**, open the **Legacy anon, service_role API keys** tab, and copy the `anon` key for client-side operations and the `service_role` key for server-side operations.
