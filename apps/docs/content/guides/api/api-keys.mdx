...
| <span className="!whitespace-nowrap">`anon`</span>         | JWT (long lived)                                                 | Low        | Platform, CLI | Exactly like the publishable key.                                                                                                                                                                                                                                                                       |
| <span className="!whitespace-nowrap">`service_role`</span> | JWT (long lived)                                                 | Elevated   | Platform, CLI | Exactly like secret keys.                                                                                                                                                                                                                                                                               |
...
When using the publishable or `anon` key, access to your project's data is guarded by Postgres via the built-in `anon` and `authenticated` roles. To ensure protection, make sure:
...
- You do not modify the role's options without understanding the changes you are making.
...
- Admin and back-office tools, with prior authorization checks only.- Data processing pipelines.
...
- You cannot send a publishable or secret key in the `Authorization: Bearer ...` header, except if the value exactly matches `apikey` header. In this case, your request will be forwarded down to your project's database, but will be rejected on JWT parsing and validation logic.
