...
1.  Grant the `anon`, `authenticated`, and `service_role` roles usage on this schema.
    ...
    `service_role` needs `USAGE` on any custom schema to access it through the Data API (PostgREST). Without this grant, requests using the service role key will fail even though they bypass Row Level Security.
