...
The last statement intentionally leaves out `in schema public`. A schema-scoped default privilege is merged with Postgres's built-in default instead of replacing it, and the built-in default grants `EXECUTE` to `public` on every new function — so the revoke would silently have no effect. Only the schema-less form replaces that default.
