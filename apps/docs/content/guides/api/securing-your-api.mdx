...
Enable Row Level Security (RLS) on all tables, views, and functions you have exposed via the Data API. You can then write RLS policies to grant users access to specific database rows or functions based on their authentication token.

<Admonition type="danger">

Always enable Row Level Security on tables, views, and functions you expose via the Data API to protect your data.
