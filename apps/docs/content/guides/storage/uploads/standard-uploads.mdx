...
**Important**: When specifying the content type, always use lowercase `content-type` in the options. Using capitalized `Content-Type` may cause the file to be stored with multipart boundaries instead of the actual file content.

**Bug Details**: The storage API incorrectly handles capitalized `"Content-Type"` headers, causing files to be stored with multipart form data boundaries instead of just the file bytes. This results in malformed downloads.
...
```python
# ✅ Correct - Use lowercase 'content-type'
response = supabase.storage.from_('bucket_name').upload('file_path', file, {
  'content-type': 'image/jpeg',
})

# ❌ Incorrect - Using capitalized 'Content-Type' may cause issues
# response = supabase.storage.from_('bucket_name').upload('file_path', file, {
#   'Content-Type': 'image/jpeg',
# })
```
...
For more detailed information about this issue and troubleshooting steps, see the [Content-Type Header Case Sensitivity Bug](/docs/guides/storage/troubleshooting/content-type-header-bug) guide.
