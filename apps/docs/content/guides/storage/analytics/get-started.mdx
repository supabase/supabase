...
You can interact with them using tools such as [PyIceberg](https://py.iceberg.apache.org/) or [Apache Spark](https://spark.apache.org/) via a REST Catalog interface.
...
The iceberg client will need to interact with the underline S3 Bucket, at this point we only support static S3 credentials.

To create S3 Credentials go to [**Project Settings > Storage**](https://supabase.com/dashboard/project/_/settings/storage), for more information, see the [S3 Authentication Guide](https://supabase.com/docs/guides/storage/s3/authentication).
...
To connect to the Iceberg Catalog, you will need an Iceberg Client which support the REST Catalog interface.
