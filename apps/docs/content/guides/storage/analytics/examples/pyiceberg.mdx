...
Install PyIceberg and PyArrow directly:

```bash
pip install pyiceberg pyarrow
```

If you are using the [Supabase Python SDK](https://supabase.com/docs/reference/python/introduction), `pyiceberg` is an optional dependency. Install it with the `iceberg` extras:

```bash
pip install "supabase[iceberg]" pyarrow
```
