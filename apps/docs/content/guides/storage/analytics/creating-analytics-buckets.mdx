...
Analytics Buckets are still available, but managed replication into Analytics Buckets through Supabase Pipelines is no longer supported. If you need managed replication today, use [Supabase Pipelines](/docs/guides/database/replication/supabase-pipelines) with **BigQuery**. If you want to use Analytics Buckets, bring your own ingestion pipeline.
