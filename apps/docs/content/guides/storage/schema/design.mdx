...
As of April 2025, the internal supabase schemas has [restricted access](https://github.com/orgs/supabase/discussions/34270) to prevent unintentional damage by users. We strongly recommend refraining from making any alterations to the `storage` schema and treating it as read-only.
