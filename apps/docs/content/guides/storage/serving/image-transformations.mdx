...
`supabase/config.toml` is used by the Supabase CLI to configure your local stack (Auth, Storage, Studio, etc.). For the full config reference, see `/docs/guides/local-development/cli/config`.
...
Hit the `/render/image` route with a simple transformation, for example:

```text
https://<project-ref>.supabase.co/storage/v1/render/image/public/<bucket>/<path-to-image>?width=200
```
