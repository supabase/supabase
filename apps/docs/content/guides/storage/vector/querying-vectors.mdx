...
Vector similarity search finds vectors most similar to a query vector using distance metrics. You can query vectors using the JavaScript SDK or directly from PostgreSQL using SQL.
...
async function hybridSearch(queryVector, filters) {
  ...
  const { data: details } = await supabase
    .from('documents')
    .select('*')
    .in('id', vectorResults.vectors.map(v => v.metadata?.doc_id))
...
async function analyzeVectors() {
  ...
  allVectors.forEach(v => {
    const category = v.metadata?.category || 'uncategorized'
    if (!categoryStats[category]) {
      categoryStats[category] = { count: 0, prices: [] }
    }
    categoryStats[category].count++
    if (v.metadata?.price) {
      categoryStats[category].prices.push(v.metadata.price)
    }
  })
  ...
  Object.entries(categoryStats).forEach(([cat, stats]) => {
    const avgPrice = stats.prices.length > 0
      ? (stats.prices.reduce((a, b) => a + b) / stats.prices.length).toFixed(2)
      : 'N/A'

    console.log(`${cat}: ${stats.count} vectors, avg price: $${avgPrice}`)
  })
}
```
...
```sql
-- Analytics: vector distribution by category
SELECT
...
## Finding Vector Outliers

Identify vectors that stand out based on metadata patterns:

```sql
-- Find outlier vectors based on metadata patterns
SELECT
  key,
  metadata->>'title' as title,
  metadata->>'category' as category,
  ...
  metadata
FROM s3_vectors.documents_openai
WHERE (metadata->>'price')::numeric > (
  SELECT AVG((metadata->>'price')::numeric) * 2
  FROM s3_vectors.documents_openai
  WHERE metadata->>'category' = (
    SELECT metadata->>'category' FROM s3_vectors.documents_openai LIMIT 1
  )
)
