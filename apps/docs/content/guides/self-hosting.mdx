...
Note that the **Supabase CLI** is a separate tool from self-hosted Supabase and collects anonymous usage telemetry to help improve the developer experience. The CLI does not collect any personally identifiable information. You can opt out at any time:

- Run `supabase telemetry disable`
- Or set the `DO_NOT_TRACK=1` environment variable
