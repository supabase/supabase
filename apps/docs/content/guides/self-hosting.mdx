---
title: 'Self-Hosting'
description: 'Host Supabase on your own infrastructure.'
subtitle: 'Install and run your own Supabase.'
hideToc: true
---

Self-hosted Supabase lets you run the entire Supabase stack on your own computer, server, or cloud infrastructure. This is different from:

- **Supabase CLI / Local Development**: A lightweight local environment for development and testing only.
- **Managed Supabase** platform: If you want to try managed Supabase for free, visit [supabase.com/dashboard](/dashboard).

When you self-host, **you are responsible for**:

- Server provisioning and maintenance
- Security hardening and keeping services updated
- Backups and disaster recovery
- Monitoring and uptime

Self-hosting is a good fit if you need full control over your data, have compliance requirements that prevent using managed services, or want to run Supabase in an air-gapped environment.

## Officially supported

<div className="grid md:grid-cols-12 gap-4 not-prose">
  <div className="md:col-span-6 xl:col-span-3 relative" key="/guides/self-hosting/docker">
    <Link href="/guides/self-hosting/docker" passHref>
      <GlassPanel title="Docker">

      Deploy Supabase within your own infrastructure using Docker Compose.
      </GlassPanel>
    </Link>
  </div>
</div>

## Community supported

There are several community-driven projects to help you deploy Supabase. These projects may be outdated and are seeking active maintainers. If you're interested in maintaining one of these projects, please [reach out to join SupaSquad](https://supabase.com/open-source/contributing/supasquad).

<div className="grid md:grid-cols-12 gap-4 not-prose">
  {community.map((x) => (
     <div className="md:col-span-6 xl:col-span-3" key={x.href}>
      <Link href={x.href} passHref>
        <GlassPanel
          title={
            <span className="flex items-center gap-2">
              {x.name}
              <span className="uppercase text-xs whitespace-nowrap text-foreground-lighter font-mono border rounded-full px-2 py-0.5">
                Maintainer needed
              </span>
            </span>
          }
        >
          {x.description}
        </GlassPanel>
      </Link>
    </div>
  ))}
</div>

export const community = [
  {
    name: 'Kubernetes',
    description: 'Helm charts to deploy a Supabase on Kubernetes.',
    href: 'https://github.com/supabase-community/supabase-kubernetes',
  },
  {
    name: 'Traefik',
    description: 'A self-hosted Supabase setup with Traefik as a reverse proxy.',
    href: 'https://github.com/supabase-community/supabase-traefik',
  },
]
