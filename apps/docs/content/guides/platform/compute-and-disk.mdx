...
In Pro organizations, Nano instances are billed at the same price as Micro instances, so we recommend upgrading your Nano instances when it's convenient for you. We don't auto-upgrade because of the downtime incurred. See [Supabase Pricing](https://supabase.com/pricing) for more information.
