...
The compute size of your project affects the effective disk throughput and IOPS. The table below shows both the baseline (sustained) limits and the burstable maximum limits for each instance size. For instance, an 8XL compute instance has a throughput of 1,188 MB/s and IOPS of 40,000.
