...
if you are on a Paid plan and physical backups for your Supabase project, you should use the Restore to a new project feature instead.

[Learn more about the Restore to a new project feature](/docs/guides/platform/clone-project)
...
- What needs manual reconfiguration?
  ...
  - Storage objects & settings (See [storage object migration guide](/docs/guides/platform/migrating-within-supabase#migrate-storage) at the end of this page)
  - Edge Functions
...
>
    <div className="border-b mt-3 pb-3">
        <AccordionItem
        header={<span className="text-foreground">Database backup file (*.backup or *.sql only. <strong>Decompress the file if it is *.backup.gz</strong>)</span>}
        id="backup-project"
        >
        <StepHikeCompact>
        The database backup can be downloaded from the [Database backups](https://supabase.com/dashboard/project/_/database/backups/scheduled) page or from the [paused project](https://supabase.com/dashboard/project/_) page if paused for greater than 90 days.
        
        How to manually take a database backup if you do not have a database backup:
...
<StepHikeCompact.Step step={2}>
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Get the new project's database connection string" fullWidth>
  ...
  <StepHikeCompact.Step step={4}>
    <StepHikeCompact.Details title="Get the database password" fullWidth>
      Reset and copy the password in the [Database Settings](/dashboard/project/_/settings/database). Replace ```[YOUR-PASSWORD]``` in connection string with the database password.
    </StepHikeCompact.Details>
  ...
  <StepHikeCompact.Step step={5}>
      <StepHikeCompact.Details title="Backup database" fullWidth>
       Run these commands after replacing ```[CONNECTION_STRING]``` with your updated connection string from the previous steps:
