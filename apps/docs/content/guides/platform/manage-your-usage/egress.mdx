...
You are charged for the network data transmitted out of the system to a connected client. Egress is incurred by all services - Database, Auth, Storage, Edge Functions, and Realtime.
...
Egress is charged by byte. Charges apply only for usage exceeding your subscription plan's base quota. This quota is called the Unified Egress Quota because it can be used across all services (Database, Authentication, Storage etc.).
