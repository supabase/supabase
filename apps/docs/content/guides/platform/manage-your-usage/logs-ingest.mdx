...
You can view Logs Ingest usage on the [organization's usage page](/dashboard/org/_/usage). The page shows the usage of all projects by default. To view the usage for a specific project, select it from the dropdown. You can also select a different time period.

{/* TODO: Add screenshots once Studio surfaces are live */}

## Optimize usage

Logs are generated automatically by every service in your Supabase project — you don't write them directly. Log volume scales with your application's traffic and behavior. To reduce ingest volume:
...
- **Investigate spikes.** Use the [Logs Explorer](/dashboard/project/_/logs-explorer) to find services or endpoints producing unusually high volume.
