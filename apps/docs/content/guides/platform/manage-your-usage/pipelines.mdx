...
Pipeline data processed is the amount of Postgres row data successfully processed by a pipeline and accepted by its destination. It is measured from the source row representation emitted by Postgres, making usage consistent across destinations.

The measurement includes:

- **Initial sync and resynchronization**: Postgres COPY row payloads.
- **Inserts**: Inserted tuple values.
- **Updates**: New tuple values and any previous identity values Postgres emits.
- **Deletes**: Identity values Postgres emits.

Only Postgres row data is measured. Protocol- and destination-specific overhead is not included.

Failed destination writes are not counted. Data successfully processed again as part of a user-requested resynchronization, table restart, or pipeline reset is counted again. In rare cases, recovery may cause successfully delivered data to be processed more than once.
