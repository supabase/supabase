...
Two people asking similar-sounding questions can generate very different amounts of usage, depending on how they ask. These tips help you avoid scanning more data than a question actually requires — they're habits, not restrictions, and none of them ask you to give up visibility you need.
...
For example: if you exceed your allowance in January, February is degraded. If February also exceeds its allowance, March is a full cutoff, and April starts clean again. If you get back under your allowance in February, March starts clean too — only consecutive months of exceeding your allowance count toward a cutoff.

## Migrate from the logs.all endpoint

Supabase's `logs.all` Management API endpoint is being retired on September 23, 2026, in favor of the `logs` endpoint. If you have scripts or integrations that call `logs.all` directly, update them to use the new endpoint — see the [migration changelog](https://supabase.com/changelog/48235-migration-of-supabase-management-api-logs-all-analytics-endpoint-to-logs-endpoint) for the syntax changes.
