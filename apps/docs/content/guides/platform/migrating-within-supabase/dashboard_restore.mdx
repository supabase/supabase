...
<StepHikeCompact>
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Get your new database connection string" fullWidth>
      ...
      If you are unsure about which connection string to use, use the Session pooler connection string.
    </StepHikeCompact.Details>
...
<StepHikeCompact.Step step={2}>
  <StepHikeCompact.Details title="Get your database password" fullWidth>
    If you do not have your database password, you can reset your password in your [Database
...
<StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Restore your backup" fullWidth>
      Run this command after replacing ```[CONNECTION_STRING]``` and ```/file/path``` with the values from the previous steps:
...
The easiest option would be to use the Google colab script to migrate your downloaded storage objects to the new s3 buckets. However, if you want to run this process locally, scripts are also provided for you to use.
...
<Tabs scrollable size="small" type="underlined" defaultActiveId="colab">
  <TabPanel id="colab" label="Google Colab">
    [https://colab.research.google.com/drive/1uNh0fyWg27HuVmqMOj6piZOFZrTcG_KJ?usp=sharing](https://colab.research.google.com/drive/1uNh0fyWg27HuVmqMOj6piZOFZrTcG_KJ?usp=sharing)
  </TabPanel>
