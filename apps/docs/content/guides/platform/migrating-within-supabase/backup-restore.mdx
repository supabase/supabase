...
The diff engine decides what in these managed schemas belongs to you: your own triggers on managed tables (such as `auth.users`) and your RLS policies on `storage.objects` are included, while platform objects are excluded. Other objects you created inside these schemas, such as your own functions or indexes, are not diffed - recreate those on the new project from your own migration files or through the Dashboard.
