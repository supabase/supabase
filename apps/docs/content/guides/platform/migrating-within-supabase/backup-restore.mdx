...
async function listAllFiles(bucket, path = '') {
  ...
  if (error) throw new Error(`❌ Error listing files in bucket '${bucket}': ${error.message}`)

  if (!data || data.length === 0) {
    console.log(`ℹ️ No files found in '${bucket}${path ? '/' + path : ''}'`)
    return []
  }
...
async function ensureBucketExists(bucketName, options = {}) {
  ...
  // The error message might be different than expected - it could be "Bucket not found" instead of "The resource was not found"
...
async function migrateFile(sourceBucketName, targetBucketName, file) {
  try {
    const { data, error: downloadError } = await oldSupabase.storage.from(sourceBucketName).download(file.fullPath)
    ...
    const { error: uploadError } = await newSupabase.storage.from(targetBucketName).upload(file.fullPath, data, uploadOptions)
    if (uploadError) throw new Error(`Upload failed: ${uploadError.message}`)

    console.log(`✅ Migrated ${file.fullPath} in bucket '${sourceBucketName}' to '${targetBucketName}'`)
    return { success: true, path: file.fullPath }
  } catch (err) {
    console.error(`❌ Error migrating ${file.fullPath} in bucket '${targetBucketName}':`, err.message)
...
async function migrateBuckets() {
  ...
  // Check for existing buckets in the target project
  const { data: existingBuckets, error: existingBucketsError } = await newSupabase.storage.listBuckets()
  if (existingBucketsError) throw new Error(`❌ Error fetching existing buckets: ${existingBucketsError.message}`)

  const existingBucketNames = existingBuckets.map(b => b.name)
  const conflictingBuckets = oldBuckets.filter(b => existingBucketNames.includes(b.name))

  // Default conflict strategy (2 = merge files)
  ...
  if (conflictingBuckets.length > 0) {
    ...
    // Store the conflict resolution strategy
  ...
  for (const bucket of oldBuckets) {
    ...
    // Handle bucket conflicts based on user choice
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const results = await Promise.all(batch.map(file => {
        // Pass both source and target bucket names
        return migrateFile(bucketName, targetBucketName, file);
      }));
      ...
      console.log(`✅ Completed batch ${i + 1}/${batches.length}: ${batchSuccesses} succeeded, ${batchFailures.length} failed`)
    }
  ...
  // Print summary
  console.log('\n📊 Migration Summary:')
  console.log(`Buckets: ${migrationStats.processedBuckets}/${migrationStats.totalBuckets} processed, ${migrationStats.skippedBuckets} skipped`)
  console.log(`Files: ${migrationStats.successfulFiles} succeeded, ${migrationStats.failedFiles} failed (${migrationStats.totalFiles} total)`)
