...
The diff engine decides what in these managed schemas belongs to you: your own triggers on managed tables (such as `auth.users`) are included, but platform objects are excluded, and so are RLS policies on `storage.objects`. Recreate those on the new project from your own migration files or through the Dashboard.
