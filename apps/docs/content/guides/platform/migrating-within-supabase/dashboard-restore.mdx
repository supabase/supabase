...
Your psql is older than the `pg_dump` that created the backup. `pg_dump` writes `\restrict` at the start of the file and `\unrestrict` at the end as a security measure ([CVE-2025-8714](https://www.postgresql.org/support/security/CVE-2025-8714/)). Only psql 17.6, 16.10, 15.14, or later understands these commands.

psql skips the invalid commands and keeps going, so the rest of the backup still restores. If you run psql with `ON_ERROR_STOP` set, it stops at the first line instead. To avoid the errors, install the latest version of psql as per the instructions above. The psql version is what matters, not your database's Postgres version, so you can use a newer psql to restore into a Postgres 15 database.
