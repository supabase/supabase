...
1. Run the `update` subcommand with the CIDRs you want to allow:
   ...
   ```bash
   > supabase network-restrictions --project-ref {ref} update --db-allow-cidr 183.12.1.1/24 --db-allow-cidr 2001:db8:3333:4444:5555:6666:7777:8888/64
   DB Allowed IPv4 CIDRs: &[183.12.1.1/24]
   DB Allowed IPv6 CIDRs: &[2001:db8:3333:4444:5555:6666:7777:8888/64]
   Restrictions applied successfully: true
   ```
...
1. Run the `update` subcommand with the `--append` flag to add a CIDR without replacing existing restrictions:
   ...
   ```bash
   > supabase network-restrictions --project-ref {ref} update --db-allow-cidr 1.2.3.4/32 --append
...
- If network restrictions are enabled, direct database access from Edge Functions is blocked. Use [supabase-js](/docs/reference/javascript) to connect to the database from Edge Functions instead.
