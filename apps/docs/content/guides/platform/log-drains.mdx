...
Log drains will send all logs of the Supabase stack to one or more desired destinations. It is only available for customers on Teams and Enterprise plans.
...
The Filebeat server only handles HTTP1 requests. Basic Auth can be provided for users.
