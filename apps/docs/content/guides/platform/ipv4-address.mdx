...
The Supabase IPv4 add-on provides a dedicated IPv4 address for your Postgres database connection. It costs $0.0055 per hour enabled, or about $4 per month if used continuously. It can be configured in the [Add-Ons Settings](https://supabase.com/dashboard/project/_/settings/addons).
...
You can enable the IPv4 add-on in your project add-on settings page. Briefly disabling the database might occur during the process.

## Read replicas and IPv4 add-on

When using the add-on, each database (including read replicas) receives an IPv4 address with an additional $4 charge per replica.
...
1. **Supavisor Connection String**: Use this string (found in Supabase Dashboard) as a replacement for the direct connection string
2. **Supabase Client Libraries**: These libraries are compatible with IPv4 (details in Supabase Docs).
3. **Dedicated IPv4 Add-On (Pro Plans+)**: For a guaranteed IPv4 and static database address, enable this paid add-on.
