---
id: 'disk-metrics'
title: 'Understanding Database and Disk Metrics'
...
- _Disk Space Used_: Shows the overall disk space usage, which includes both the database size and additional system files like WAL files and log files. You can view this on the [Database Settings page](https://supabase.com/dashboard/project/_/settings/database).
...
<Admonition type="note">
  Note: Disk size refers to the total storage capacity of your database’s disk, and it does not
  include file storage. File storage is managed separately using S3, with no scaling limitations.
  For more details on storage quotas and limits across different plans, see our [pricing
  page](https://supabase.com/pricing).
</Admonition>
...
Disk Storage expands automatically when the database reaches 90% of the disk space used. The disk is expanded to be 50% larger (e.g., 8GB -> 12GB). Auto-scaling can only take place once every 6 hours. If within those 6 hours you reach 95% of the disk space, your project will enter read-only mode.
...
The maximum Disk Storage Size for the Pro/Team Plan is 60TB. If you need more than this, [contact us](https://forms.supabase.com/enterprise) to learn more about the Enterprise Plan.
