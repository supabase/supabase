...
We provide a dedicated server and database for every Supabase project. By default, your instance runs on the Micro Compute instance. You can upgrade your compute size in your [project settings](https://supabase.com/dashboard/project/_/settings/addons).
...
We count your total limit of 2 free projects across all organizations you're either an Owner or Administrator of. You could have two free plan organization with one project each, or one free plan organization with two projects. Paused projects do not count towards your free project limit.
...
In case of deploying additional [Read Replicas](/docs/guides/platform/read-replicas), each replica is a dedicated database that adds additional compute runtime hours.
