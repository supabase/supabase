...
We provide a dedicated server for every Supabase project. Each paid organization comes with <Price>$10</Price> in Compute Credits to cover one project on the default compute size. Additional projects start at ~<Price>$10</Price> a month (billed hourly).

Running 3 projects in a Pro Plan organization on the default Micro instance:

- <Price>$25</Price> Pro Plan
- <Price>$30</Price> for 3 projects on the default compute size
- <Price>$10</Price> Compute credits ⇒ <Price>$45</Price> / month
