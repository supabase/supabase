...
Supabase provides multi-factor authentication (MFA) enforcement on the organization level. Ensuring all organization members must use MFA and have a valid MFA backed session when interacting with the organization and projects belonging to the organization.
