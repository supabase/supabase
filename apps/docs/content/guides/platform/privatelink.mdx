...
5. Provide an association description (recommended)
6. Select the database target: the primary database or a specific read replica
7. Enter your AWS Account ID
8. Click **Add association** to submit

Each database, whether the primary or a read replica, needs its own association. Create a separate association for every database you want to reach over PrivateLink.

After submission, Supabase creates a VPC Lattice Resource Configuration for your project and sends an AWS Resource Share to the specified AWS Account ID. This process may take a few moments. Once complete, the association will show a "Ready" status, indicating that the resource share has been sent to your AWS account and is ready to be accepted. You must accept the resource share within 12 hours, or the request expires and can no longer be accepted in AWS. You'll need to create a new association to try again.

Once ready, select **View Association** to see the VPC Lattice resource configuration ID and ARNs for the association. This is useful for confirming which resource configuration corresponds to which database when a project has multiple associations, for example one for the primary database and one for each read replica.
...
4. Locate the resource share from Supabase.
   ...
   - If your project has multiple associations, for example one for the primary database and one for each read replica, match the share's ARN to the **Resource Share ARN** shown for that association in **View Association** to confirm you're accepting the correct one
...
6. In the **Resource configurations** section select the appropriate resource configuration
   ...
   - If you have multiple associations, match the **Resource Configuration ID** shown for that association in **View Association** to confirm you select the configuration for the correct database
