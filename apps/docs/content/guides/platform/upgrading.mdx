...
After upgrading, indexes on `ltree` columns that were built under the previous version can return incomplete results — for example, label searches silently miss rows that are present — until the index is rebuilt. This affects databases using a multibyte encoding (such as UTF-8) or a non-`libc` collation provider (ICU or builtin).
...
First, check whether your database needs reindexing:
...
If `reindex_required` is `false` (single-byte encoding such as LATIN1 with `libc` collation), no action is needed.
...
To remediate, reindex each affected index. `REINDEX INDEX CONCURRENTLY` runs online with no downtime:
