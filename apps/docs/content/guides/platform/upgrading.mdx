...
Given Postgres version `15.6.1.115`, start Postgres locally with `db_cluster.backup` pointing to the location of your backup file.
...
A Postgres database started with Supabase CLI is not production ready and should never be used outside a local development workflow.
