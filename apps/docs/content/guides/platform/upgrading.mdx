...
#### Check whether your project uses `pgjwt`

`pgjwt` installs `sign`, `verify`, `algorithm_sign`, `url_encode`, `url_decode`, and `try_cast_double` into the `extensions` schema by default. Run both queries below in the [SQL Editor](/dashboard/project/_/sql/new). If neither returns rows, nothing in your database calls the extension.

Postgres records a dependency when a view, materialized view, column default, constraint, or index calls one of those functions:

```sql
select distinct pg_describe_object(d.classid, d.objid, d.objsubid) as object
from pg_depend d
where
  d.refobjid in (
    select objid
    ...
    where
      ...
      and refobjid = (select oid from pg_extension where extname = 'pgjwt')
  )
  and d.deptype = 'n'
order by 1;
```

Calls from inside a function body don't create a dependency, so search the bodies by name:
...
where
  ...
  and p.prosrc ~* '\y(sign|verify|algorithm_sign|url_encode|url_decode)\s*\('
order by 1;
```

The second query matches on name alone, so read its results before acting on them. A function of your own named `sign()` matches too.
