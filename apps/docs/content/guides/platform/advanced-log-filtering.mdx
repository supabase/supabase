...
There are multiple filters that you should consider using:
...
Each product table operates independently without any relationships to other log tables. This means they cannot be joined together.

### The `with` keyword and subqueries are not supported

The query editor parses logs for optimization. The parser does not yet support `with` and subquery statements.
