...
The load balancer is geo-aware meaning that `GET` requests to this endpoint will automatically be routed to the Read Replica that is closest to your user ensuring the lowest latency. Non-`GET` requests can also be sent through this endpoint, and will be routed to the Primary database.

All Supabase services (Auth, Edge Functions, Realtime, and Storage) are supported meaning there is no need to worry about which endpoint to use in which situations.
