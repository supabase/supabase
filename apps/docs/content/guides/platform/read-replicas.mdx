...
Additionally, pre-signed URLs generated by the Storage service currently do not work with Experimental routing. You can either manually remove the `-all` suffix from the hostname of the generated URL, or use a non-LB client to generate the pre-signed URL. This limitation will be resolved in the coming weeks.
...
- New behavior: Geo-aware routing, that directs requests to the closest available database
