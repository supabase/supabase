...
Read Replicas are Postgres databases that are continuously synced to a Primary database. While you can read and write to a Primary database, you can _only_ read from a Read Replica, such as using `select` statements.
...
<Admonition type="note" label="Availaility: bRead Replicas are available Team and Enterprise organizations.">

Read replicas are available to projects created after the 20th of March, on the Teams and Enterprise Plans. We are progressively rolling it out to older projects, and will make it available to Pro Plan users in the future.

If your project doesn't meet these criteria you can use the [early access form](https://forms.supabase.com/read-replica) and we will try to prioritize your project.
...
When you launch a Supabase project we create a "primary" Postgres database. A Read Replica is a separate database that receives changes from the Primary database. This process is called "replication". Replication is asynchronous to ensure that transactions on the Primary aren't blocked. There is delay between an update on the Primary and the time that a Read Replica receives the change. This is called is "replication lag".

Read Replicas have many uses:

- **Load balancing:** They reduce read load on the Primary database. For example, you can use a Read Replica for complex analytical queries and reserve the Primary for user-facing create, update, and delete operations.
- **Improved latency:** For projects that need to cover a global audience, additional databases can be deployed closer to users in any of our twelve supported regions to reduce latency.
- **Redundancy:** Read Replicas provide data redundancy.

## Prerequisites

Projects must meet these requirements use Read Replicas:
...
Read Replicas are managed from the [infrastructure settings page](https://supabase.com/dashboard/project/_/settings/infrastructure). For now, each project can only deploy up to two Read Replicas and all Read Replicas inherit the compute size of their Primary database.
...
The load balancer uses a round-robin strategy to route `GET` (i.e. read-only) requests to any of the API endpoints available, including that of the Primary database. Non-`GET` requests can also be sent through this endpoint, and will be routed to the Primary database. Read-only Postgres functions can be called using a [`GET` request](https://github.com/supabase/supabase-js/blob/v2.42.3/src/SupabaseClient.ts#L176-L177) as well.

Similar to the Read Replica API endpoints, only requests to the [REST API](https://supabase.com/docs/guides/api) are available for now. Support for other Supabase products will become available once Read Replica support has been completed for them.

When a project no longer has any Read Replicas, the load balancer and its endpoint are brought down as well, and will no longer service any requests.

### Metrics

Observability and metrics for Read Replicas are being worked on, and not currently available in the Supabase Dashboard.

If you're ingesting your [project's metrics](https://supabase.com/docs/guides/platform/metrics#accessing-the-metrics-endpoint) into your own environment, you can update [your scraping setup](https://github.com/supabase/supabase-grafana?tab=readme-ov-file#read-replica-support) to additionally ingest metrics from your read replicas as well.
