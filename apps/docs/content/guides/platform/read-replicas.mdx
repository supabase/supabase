...
>
  <AccordionItem
    ...
    We recommend not scaling until CPU is sustained above 70%. Once it is, confirm your queries are already optimized and properly indexed (use `EXPLAIN ANALYZE`) before adding hardware. If your workload is less than roughly 80% reads, upgrade compute. Read Replicas only serve reads and won't help writes. If it's read-heavy, Read Replicas become the right choice once you've reached the largest compute size (16XL), or earlier if you need workload isolation or geographic distribution. Below 16XL with no isolation need, either option works: compute is simpler, Read Replicas scale further.
