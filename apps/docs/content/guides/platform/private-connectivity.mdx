...
description: 'Secure private network connectivity to your Supabase database using AWS PrivateLink via VPC Lattice.'
---

<Admonition type="note">

Private Connectivity is currently in beta and available only to Team and Enterprise customers.
Contact support if you would like to create a Private Connectivity connection for a read-only replica.

</Admonition>

Private Connectivity provides enterprise-grade private network connectivity between your cloud VPC and your Supabase database. On AWS, this is implemented using AWS PrivateLink via VPC Lattice. This eliminates exposure to the public internet by creating a secure, private connection that keeps your database traffic within the AWS network backbone.
...
Supabase Private Connectivity currently supports direct database and PgBouncer connections only. It does not support other Supabase services like API, Storage, Auth, or Realtime. These services will continue to operate over public internet connections.

## Requirements

To use Private Connectivity with your Supabase project:
...
Navigate to your project's Integrations section to set up Private Connectivity:

1. Go to your Supabase project dashboard
2. Navigate to [**Settings** > **Integrations**](/dashboard/project/_/settings/integrations)
3. Find the **Private Connectivity** section
4. Click **Add account**
...
##### Option A: Create an AWS PrivateLink endpoint
...
- **Read Replicas**: Private Connectivity does not currently support read replicas
- **Feature Evolution**: The setup process and capabilities may evolve as we refine the offering

## Compatibility

The Private Connectivity endpoint is a layer 3 solution so behaves like a standard Postgres endpoint, allowing you to connect using:
...
Ready to enhance your database security with Private Connectivity? [Contact our Enterprise team](/contact/enterprise) to discuss your requirements and begin the setup process.
...
Private Connectivity is not currently available in the following regions:
