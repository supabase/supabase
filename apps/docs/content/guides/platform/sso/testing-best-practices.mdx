...
**Recent improvement:** Auto-join now applies on EVERY login, not first signup. This resolves a common issue where org owners would test with auto-join disabled, enable it, then log in again expecting to auto-join.
...
- Existing users auto-join on their next login (not new signups)
...
- Restriction applies to all SSO accounts (not certain roles)
...
- Auto-join now applies on **every login**, not first signup
...
- Auto-join works on first login (not signup)
