...
- Sign-in flow type decided (IdP-initiated, SP-initiated, or both) - see [Choosing a Sign-in Flow](/docs/guides/platform/sso/choosing-sign-in-flow)
...
Before testing auto-join and other features, verify which sign-in flows work for your SSO configuration. See [Understanding SSO Sign-in Flows](/docs/guides/platform/sso/sign-in-flows) for technical details.
...
If you haven't configured domains or have "Enable SP-initiated sign-in" disabled, skip this test. SP-initiated will not work without domain configuration.

</Admonition>

**Prerequisites:**

- "Enable SP-initiated sign-in" toggle is **ON**
...
- "Enable SP-initiated sign-in" **OFF** (or no domains configured)
...
2. **Create non-SSO invitation**:
   ...
   - Send invitation
   - Recipient can use password or social sign-in to accept
...
3. **Test SSO mismatch scenario**:
   ...
   - Error should display: "Invite token SSO provider does not match the one you are signed in with"
...
- To test: Enable auto-join, sign out completely, log back in via SSO
...
1. **Add non-SSO owner:**
   - Create or invite a non-SSO user (password or social sign-in)
