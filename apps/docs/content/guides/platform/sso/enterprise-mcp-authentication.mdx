...
3. **The MCP client must be authorized for your organization.** An organization owner or admin does this once by connecting the MCP client to Supabase through the standard sign-in flow and approving it for your organization on the consent screen. The approved client then appears under [Authorized Apps](/dashboard/org/_/apps) in your organization settings, where you can review or revoke it.
...
5. **Supabase validates the ID-JAG**, checking it against your IdP's public keys, confirming the member belongs to your organization with a matching SSO identity, and confirming an organization owner or admin has authorized this MCP client. If everything checks out, Supabase issues a short-lived access token.
...
With enterprise-managed authentication, an organization owner or admin authorizes the MCP client once for the whole organization. From then on, access follows your existing SSO sign-in, with no separate approval and no manual reconnection when a token expires.
...
- **The MCP client must be explicitly authorized.** Adding an ID-JAG issuer alone doesn't grant it access; an organization owner or admin still has to approve it once through the standard sign-in flow. Approved clients are listed under [Authorized Apps](/dashboard/org/_/apps).
