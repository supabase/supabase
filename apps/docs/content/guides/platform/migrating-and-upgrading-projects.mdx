...
Downloaded backups are plain text SQL files that can be loaded into a Postgres instance using [psql](https://www.postgresql.org/docs/current/app-psql.html). You can use our open source build tooling if you would like to restore a backup into a local environment so you can interact with it. Here are the steps:
...
Be aware that this solution is only intended to give you access to the project to, for example, export data. `start-server` should never be used outside a local development workflow.
