...
Then restore the backup file into the Postgres instance from a separate shell session:

```sh
nix run github:supabase/postgres#start-client -- --file <BACKUP FILE>
```

Which loads the backup file into Postgres and drops you into a [psql](https://www.postgresql.org/docs/current/app-psql.html) `psql` prompt. For `start-client` you do not need to provide a reference to the backup's Postgres version.
