...
Restoring to a new project does not have any impact (e.g. performance, or load) on the source project. Switching to physical backups (if they weren't already enabled), or changing the disk attributes, can impact the source project.
