...
Supabase is a robust and open-source platform. Supabase provides all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, realtime subscriptions, and storage. Postgres is the core of Supabase—for example, you can use row-level security and there are more than 40 Postgres extensions available.
...
This guide provides three methods for migrating your PostgreSQL database to Supabase:
1. **Google Colab** - Guided notebook with copy-paste workflow
2. **Manual Dump/Restore** - CLI approach, works for all versions
3. **Logical Replication** - Minimal downtime, requires PostgreSQL 10+
...
| Port | Mode | Use Case |
|------|------|----------|
| 5432 | Session mode | Long-running connections, logical replication |
| 6543 | Transaction mode | Short connections, **recommended for pg_restore** |

**Note**: Direct connection (5432) is IPv6-only by default. Transaction pooler (6543) works over both IPv4 and IPv6. For IPv4 with direct connection, enable Supabase's IPv4 add-on.
...
This method works for all PostgreSQL versions using CLI tools.
...
- PostgreSQL client tools matching your source database version
...
**Critical**: For optimal performance, run the migration from a cloud VM, not your local machine. The VM should be in the same region as either your source or target database to minimize network latency. See the Resource Requirements table in Step 2 for VM sizing recommendations.
...
4. Get your connection string:
   ...
   - Select **Transaction pooler** (port 6543) and copy the connection string
...
- **Enable Row Level Security (RLS)** for tables in public schema after migration
- **Transaction pooler works over IPv4** (direct connection is IPv6-only by default)
...
Also temporarily increase disk size via Settings → Database → Disk for large imports.
...
For testing without maintenance window, skip this step but use lower -j values.
...
**Note**: For testing without maintenance window, use lower -j values to avoid impacting production performance.
...
```bash
# Set Supabase connection (Transaction Pooler on port 6543)
...
```bash
# For databases with regular tables (non-partitioned)
vacuumdb --dbname="$SUPABASE_DB_URL" --analyze --jobs=$RESTORE_JOBS --verbose

# For databases with partitioned tables
# vacuumdb --analyze doesn't handle parent tables properly
# Run this instead:
psql "$SUPABASE_DB_URL" -c "VACUUM VERBOSE ANALYZE;"
```

**Note**: For PostgreSQL 18+, pg_dump includes statistics with `--with-statistics`, but still run VACUUM for optimal performance.
...
1. **Region proximity matters**: VM should be in the same region as source or target for best performance
...
This method allows migration with minimal downtime using PostgreSQL's logical replication feature. Requires PostgreSQL 10+ on both source and target.

### When to Use Logical Replication

- You need minimal downtime (minutes instead of hours)
- Source database is PostgreSQL 10 or higher
- You can configure logical replication on source
...
# Restore schema to Supabase (use Transaction Pooler)
pg_restore \
  --dbname="postgresql://postgres.[ref]:[password]@aws-0-[region].pooler.supabase.com:6543/postgres" \
  --format=directory \
  --schema-only \
  --no-privileges \
  --single-transaction \
...
2. Stop writes to source database (if not already read-only)
...
-- Do NOT re-enable writes to avoid split-brain scenario!
```
...
| "out of replication slots" | Increase max_replication_slots in postgresql.conf |
...
For detailed restrictions, see [PostgreSQL Logical Replication Restrictions](https://www.postgresql.org/docs/current/logical-replication-restrictions.html)

### When to Use Which Method

**Use Dump/Restore when:**
- Downtime window is acceptable
- Source is PostgreSQL < 10
- Simpler process preferred
- Cannot configure logical replication on source

**Use Logical Replication when:**
- Minimal downtime required
- PostgreSQL 10+ on both sides
...
- [PostgreSQL Roles and Privileges Guide](https://supabase.com/blog/postgres-roles-and-privileges)
