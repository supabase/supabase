...
| Mode | Use Case |
|------|----------|
| Session mode | Long-running connections, logical replication, pg_restore |
| Transaction mode | Short connections |

Use session mode for the database migration tasks (pg_dump/restore and logical replication).
Use the direct connection string if your environment supports IPv6, and use Supavisor session pooler if you need IPv4 support.
...
- **Session pooler works over IPv4** (direct connection is IPv6-only by default)
...
```bash
# Set Supabase connection (Session Pooler on port 5432)
