...
Use these checks to identify evidence worth investigating. A finding does not establish a cause. The [monitoring routines](/docs/guides/observability/automate-with-agents) use these same checks.
...
**Input:** the last complete UTC hour and preceding complete hour, queried separately. Generalist compares each of the last 24 complete hours with its preceding hour. Evaluate each source separately; API Gateway and Auth events are different observations, not unique requests to add together.
...
**Input:** enabled [pg_stat_statements](/docs/guides/database/extensions/pg_stat_statements), query-identifier visibility, and three saved snapshots spaced one hour apart. They define the preceding and current hour. Daily routines use three snapshots spaced one day apart instead.
