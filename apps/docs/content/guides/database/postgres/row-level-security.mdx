...
To perform an `UPDATE` operation, a corresponding <a href="#select-policies">`SELECT` policy</a> is required. Without a `SELECT` policy, the `UPDATE` operation will not work as expected.
