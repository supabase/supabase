...
Further illustrate the difference with an example. We'll use the following data:
...
But, when we combine it with _other_ constraints, then any other constraints take precedence. For example, run the same but add a `mother` column that has a `CASCADE` delete:
