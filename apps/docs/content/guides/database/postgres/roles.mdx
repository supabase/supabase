...
Postgres manages database access permissions using the concept of roles. Generally you wouldn't use these roles for your own application - they are mostly for configuring _system access_ to your database. If you want to configure _application access_, then you should use [Role-based Access Control](/docs/guides/auth/custom-claims-and-role-based-access-control-rbac) and [Row Level Security](/docs/guides/database/postgres/row-level-security).
