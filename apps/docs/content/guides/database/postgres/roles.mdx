...
In Postgres, roles can function as users or groups of users. Users are roles with sign-in privileges, while groups (also known as role groups) are roles that don't have sign-in privileges but can be used to manage permissions for multiple users.
...
For unauthenticated, public access. This is the role which the API (PostgREST) will use when a user _is not_ signed in.
...
For "authenticated access." This is the role which the API (PostgREST) will use when a user _is_ signed in.
