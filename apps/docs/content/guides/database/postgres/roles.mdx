...
Used by [external replication (ETL)](/docs/guides/database/replication), powered by Supabase ETL, to replicate database changes to destination systems. Has read-all access and replication privileges for change data capture, bypasses Row Level Security, and can write to the `etl` schema.
