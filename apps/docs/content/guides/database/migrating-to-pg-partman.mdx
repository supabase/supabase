...
Starting from Postgres 17, Supabase projects will not have the `timescaledb` extension available. If your project relies on TimescaleDB hypertables, you will need to migrate to standard Postgres tables before upgrading.
...
`pg_partman` requires running maintenance to premake partitions and apply retention policies.
