...
A `security definer` function can return rows the caller isn't allowed to read. It runs as its creator, which on Supabase is `postgres`. That role bypasses Row Level Security. Any role can execute the function by default, including `anon`, the role that serves requests carrying no user session.

Execute privileges decide who can call the function. Pinning the `search_path` doesn't change them. When a `security definer` function reads user data, protect it in one or both of these ways:

- Check ownership in the function body.
- Revoke execute from `public` and `anon`, then grant it to the roles that need it. See the [Function privileges](#function-privileges) section of this page.
