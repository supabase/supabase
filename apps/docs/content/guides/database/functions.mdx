...
```python
data = await supabase.rpc('hello_world').execute()
```
...
```python
data = await supabase.rpc('get_planets').eq('id', 1).execute()
```
...
```python
data = await supabase.rpc('add_planet', params={'name': 'Jakku'}).execute()
```
