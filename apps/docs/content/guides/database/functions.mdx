...
1.  Restrict function execution by default. Specifically _grant_ access when you want a function to be executable by a specific role.
    ...
    The revoke from `public` has to leave out `in schema public`. A schema-scoped default privilege is merged with Postgres's built-in default instead of replacing it, and the built-in default grants `EXECUTE` to `public` on every new function — so a schema-scoped revoke would silently have no effect. Only the schema-less form replaces that default.
