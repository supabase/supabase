...
For Teams and Enterprise plans, Supabase provides advanced monitoring charts directly within the Dashboard. The `Database client connections` chart displays historical connection data broken down by connection type:
...
For more details on using these monitoring charts, see the [database debugging guide](/docs/guides/database/inspect#database-client-connections).
