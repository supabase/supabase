...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    ...
    `postgres.js` pipelines queries by default. Combined with transaction mode, this can hang queries or return mismatched rows. `postgres.js` has no working option to turn pipelining off directly: `max_pipeline: 0` breaks `sql.begin()` transactions instead, a known upstream bug ([porsager/postgres#1189](https://github.com/porsager/postgres/issues/1189)). Bind at least one parameter in every query instead, which forces a synchronous round trip and avoids pipelining, or use session mode. Supavisor v2.10 will add native pipelining support, removing the need for this workaround. If you still see issues after applying it, [contact support](/dashboard/support/new).
