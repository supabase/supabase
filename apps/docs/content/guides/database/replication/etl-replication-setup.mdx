...
title: 'Automatic Destinations Setup'
description: 'Set up replication to automatic destinations using Postgres logical replication.'
subtitle: 'Configure publications and destinations for automatic replication.'
sidebar_label: 'Automatic destinations'
---
...
Automatic destinations use **Postgres logical replication** to stream changes from your database. Supabase ETL, an [open source tool](https://github.com/supabase/etl) built for Postgres logical replication, provides a managed interface through the Dashboard to configure and monitor these replication pipelines.
...
Before adding destinations, you need to enable replication to built-in destinations for your project:
...
#### Available automatic destinations

For a complete list of available automatic destinations and how to choose the right one for your needs, see [Automatic Destinations](/docs/guides/database/replication/etl-destinations).
...
2. Configure the destination settings:
   ...
   **Destination-specific settings:**
   Each destination type requires different configuration. See the [Automatic Destinations guide](/docs/guides/database/replication/etl-destinations) for configuration details specific to your chosen destination.
...
Once configured, replication to automatic destinations:
...
Replication to automatic destinations currently performs data extraction and loading only, without transformation. Your data is replicated as-is to the destination.
...
Replication to automatic destinations has the following limitations:
...
Replication to automatic destinations is actively being developed. Planned improvements include:
