...
title: 'Managed Destinations Setup'
description: 'Set up replication to managed destinations using Postgres logical replication.'
subtitle: 'Configure publications and destinations for managed replication.'
sidebar_label: 'Managed Destinations'
---
...
Managed destinations use **Postgres logical replication** to stream changes from your database. Supabase ETL, an [open source tool](https://github.com/supabase/etl) built for Postgres logical replication, provides a managed interface through the Dashboard to configure and monitor these replication pipelines.
...
Before adding destinations, you need to enable replication to managed destinations for your project:
...
Follow these steps to configure your destination. The specific configuration depends on which destination type you choose.

<Admonition type="note">

Both Iceberg and BigQuery destinations are supported. However, availability varies based on our planned rollout strategy. The destinations you can access depend on your project and access level.
...
<Tabs
  ...
  size="small"
  type="underlined"
  defaultActiveId="iceberg"
  queryGroup="destination"
>
<TabPanel id="iceberg" label="Iceberg">

Apache Iceberg is an open table format for analytic datasets. Replication to Iceberg uses Supabase [Analytics Buckets](/docs/guides/storage/analytics) to store your replicated data in S3-compatible storage.
...
##### Step 2: Configure Iceberg as a destination
...
4. Configure Iceberg-specific settings:
...
Once configured, replication to Iceberg:

1. Captures changes from your Postgres database (INSERT, UPDATE, DELETE operations)
2. Batches changes for optimal performance
3. Writes data to Analytics Buckets in Iceberg format
4. Creates an append-only log with CDC metadata
...
- [Analytics Buckets documentation](/docs/guides/storage/analytics) - Learn more about Analytics Buckets and S3-compatible storage
- [Apache Iceberg](https://iceberg.apache.org/) - Official Iceberg documentation

</TabPanel>
<TabPanel id="bigquery" label="BigQuery">

BigQuery is Google's fully managed data warehouse. Replication to BigQuery allows you to sync your database tables for analytics and reporting.
...
For comprehensive monitoring instructions including pipeline states, metrics, and logs, see the [Replication Monitoring guide](/docs/guides/database/replication/etl-replication-monitoring).
...
Once configured, replication to managed destinations:

1. **Captures** changes from your Postgres database using Postgres publications and logical replication
2. **Streams** the changes through Supabase ETL's replication pipeline
...
Replication to managed destinations currently performs data extraction and loading only, without transformation. Your data is replicated as-is to the destination.
...
For more troubleshooting help, see the [Replication FAQ](/docs/guides/database/replication/etl-replication-faq).

### Limitations

Replication to managed destinations has the following limitations:
...
- **Schema changes**: Not automatically handled (DDL changes require manual intervention)
...
Destination-specific limitations (such as Iceberg's append-only log format or BigQuery's ingestion latency) are detailed in each destination tab in [Step 3](#step-3-add-a-destination) above.

### Future work

Replication to managed destinations is actively being developed. Planned improvements include:
...
- [View Replication FAQ](/docs/guides/database/replication/etl-replication-faq)
