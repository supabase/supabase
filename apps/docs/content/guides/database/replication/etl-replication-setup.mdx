...
Once replication is enabled and you have a publication, you can add a destination. ETL Replication currently supports two destination types:

### Available destinations

Choose a destination based on your needs:

- **[BigQuery](/docs/guides/database/replication/etl-bigquery)**: Google's fully managed data warehouse for SQL analytics
- **[Iceberg (Analytics Buckets)](/docs/guides/database/replication/etl-iceberg)**: Apache Iceberg tables in S3-compatible storage

For detailed configuration instructions for each destination type, see the destination-specific guides linked above.
...
2. Configure the destination settings in the form:
   ...
   - **Type**: Choose your destination type (BigQuery or Analytics Bucket)
   - Fill in destination-specific fields (see destination guides for details)
...
3. Review the **Advanced Settings** (optional):
   ...
   - Additional settings vary by destination type
...
- [BigQuery configuration →](/docs/guides/database/replication/etl-bigquery#configuration)
- [Iceberg configuration →](/docs/guides/database/replication/etl-iceberg#configuration)
