...
Both Analytics Buckets and BigQuery destinations are supported. However, availability varies based on our planned rollout strategy. The destinations you can access depend on your project and access level.
...
When you replicate to Analytics Buckets, your database changes are automatically written in Iceberg format, creating queryable tables in object storage optimized for analytics.
...
- Each table contains a `cdc_operation` column indicating the operation type: `INSERT`, `UPDATE`, or `DELETE`
