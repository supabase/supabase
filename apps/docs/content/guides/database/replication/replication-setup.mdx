...
Follow these steps to configure your destination. Right now, **BigQuery** is the only managed destination available in Replication. We are working on adding more destinations in the future.
