...
For troubleshooting guidance, see [Why is a table not being replicated?](/docs/guides/database/replication/pipelines-faq#why-is-a-table-not-being-replicated).
...
Postgres source partitioning and BigQuery destination partitioning are separate. The publication's `publish_via_partition_root` setting determines whether Pipelines creates one destination table for a partition root or separate destination tables for leaf partitions. BigQuery partitioning and clustering options then control the physical layout of each resulting destination table. Pipelines doesn't copy the source partition key, bounds, or partitioning method. See [Partitioned tables](/docs/guides/database/replication/pipelines#partitioned-tables) for the publication behavior.
...
Partitioning and clustering are independent: you can set either one or both for a table. Leave both unset to use BigQuery's default layout without partitions or clustering.
