...
- **Data type changes:** Require a [table restart](/docs/guides/database/replication/pipelines-monitoring#restarting-tables).
...
- **Publishing an existing array column:** Adding a previously excluded array column to the publication requires a resynchronization. BigQuery arrays can't represent `NULL`, so adding the destination column would expose an empty array for historical rows whose values Pipelines doesn't know. This restriction applies even if the source table is empty. Include the table in **Initial sync**, then [restart replication for the table](/docs/guides/database/replication/pipelines-monitoring#restarting-tables) to replace destination data and copy existing rows again. Adding a new array column in Postgres is allowed, but existing BigQuery rows expose an empty array until a later row change supplies the source value; Pipelines doesn't backfill them.
- **Interrupted schema changes:** BigQuery DDL is not transactional. If an interruption leaves completion uncertain, Pipelines blocks further writes until you [restart replication for the table](/docs/guides/database/replication/pipelines-monitoring#restarting-tables).
