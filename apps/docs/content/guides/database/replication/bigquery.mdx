...
BigQuery replication requires each source table to have a primary key. BigQuery uses the source primary key to apply CDC upserts and deletes.
...
- **Primary keys**: Source tables must have a primary key
