...
6. Click **Add and start** to begin replication
...
BigQuery replication requires each source table to have a primary key, and the publication must include the primary-key columns. Supabase Pipelines declares those columns as the BigQuery destination primary key so BigQuery change data capture (CDC) can apply `UPSERT` and `DELETE` rows.
...
For a general explanation of how replica identity affects update and delete events, see [How does replica identity affect updates and deletes?](/docs/guides/database/replication/supabase-pipelines-faq#how-does-replica-identity-affect-updates-and-deletes).
...
Schema change support for BigQuery is currently in beta. Supabase Pipelines supports a limited set of schema changes for BigQuery while the feature is developed further.
...
BigQuery requires added columns to be nullable. When a replicated `ADD COLUMN` includes a default, Supabase Pipelines can apply supported default metadata for future rows, but BigQuery does not backfill existing rows through that DDL. Existing destination rows remain `NULL` unless you run a separate backfill.
