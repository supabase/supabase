...
- **Data type changes:** Pipelines detects the change but doesn't update the BigQuery data type. Resynchronize the table after changing a replicated column's data type.
- **Adding `NOT NULL`:** BigQuery keeps the existing column nullable. Postgres still prevents new `NULL` values.
- **Primary-key changes:** Changing primary-key columns, names, or order isn't supported.
- **Publishing an existing array column:** BigQuery can't preserve the unknown historical values. Resynchronize the table to include the array.
- **Interrupted schema changes:** Pipelines stops instead of guessing which changes completed. Resynchronize the affected table.
