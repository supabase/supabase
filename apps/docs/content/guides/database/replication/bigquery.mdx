...
An initial sync copies a consistent snapshot of each table, then applies changes made during the copy. During ongoing replication, Pipelines applies supported schema changes before writing rows that use the new schema. Some unsupported changes stop the affected table; others leave the BigQuery schema unchanged, as described below.

If a table needs resynchronization, fix the cause and [restart that table's initial sync](/docs/guides/database/replication/pipelines-monitoring#table-errors). This replaces its destination data and processes billable data again. The reset temporarily stops the whole pipeline, but other tables retain their replication progress.
...
- **Data type changes:** Pipelines detects changes to a column's type or type modifiers, such as numeric precision, but leaves the BigQuery type unchanged and logs a warning. Later schema changes or writes can fail. Resynchronize the table after changing a replicated column's type.
