...
{/* SCREENSHOT SLOT: pipelines-bigquery-configuration.png — Add a sanitized 2x capture of the BigQuery destination drawer here. */}
...
6. Optionally expand **Advanced settings** for pipeline and BigQuery-specific tuning:
   ...
   | **Maximum staleness**    | Freshest results | Maximum acceptable staleness, in whole minutes, of table data returned by queries while BigQuery applies streamed `UPSERT` and `DELETE` changes in the background. Leave unset for the freshest table data. A larger number of minutes allows BigQuery to return older data, which can reduce query-time CDC merge cost and latency. For example, `15` allows data to be up to 15 minutes stale. This value is applied when Pipelines creates or recreates a table; changing it doesn't alter existing destination tables. |
...
You query a managed view for each replicated source table. Pipelines manages the physical BigQuery tables behind that view and replaces them when a truncate or full initial sync requires it. These internal versions don't provide a change history that you can query.
...
These semantics match the behavior implemented in [Supabase ETL PR #899](https://github.com/supabase/etl/pull/899).
