...
The settings of your Supabase project can be modified to support high volume replication, read the [customising postgres config](/docs/guides/platform/custom-postgres-config) guide for more information on the settings available.
...
Rising Wave has the following [documentation](https://docs.risingwave.com/ingestion/sources/postgresql/supabase#set-up-supabase-postgresql) on replicating to and from Supabase.
