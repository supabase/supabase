...
To set up replication to a manual destination, you'll need to:

1. Configure a **Postgres publication** (defines what to replicate)
2. Create a **replication slot** using the `postgres` user
3. Configure your external tool to connect to your Supabase database
4. Monitor replication lag and status

The specific steps depend on which external tool you're using. Below are instructions for popular replication tools.
