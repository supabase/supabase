...
title: 'External Replication Setup'
description: 'Set up external replication using Supabase ETL and Postgres logical replication.'
...
External replication uses **Postgres logical replication** to stream changes from your database to external data systems. Powered by [Supabase ETL](https://github.com/supabase/etl), an open source tool built for Postgres logical replication, it provides a managed interface through the Dashboard to configure and monitor replication pipelines.
...
3. Click **Add destination** to show the replication side panel
4. Select an external replication destination, such as **BigQuery**
5. Click **Enable external replication**
...
Follow these steps to configure your destination. Each destination has its own setup requirements and behavior. **BigQuery** is currently available, and new destinations are in the works.
...
Your replication pipeline will now start copying data from your database to your destination.
...
Supabase ETL automatically optimizes how changes are delivered to the destination. The replication pipeline currently performs data extraction and loading only, without transformation - your data is replicated as-is to the destination.
...
- **Data duplicates**: Duplicates can occur when stopping a pipeline if your database has transactions that take longer than a few minutes to complete. See [Can data duplicates occur?](/docs/guides/database/replication/external-replication-faq#can-data-duplicates-occur-during-pipeline-operations) for details
