...
Replication to managed destinations currently supports Analytics Buckets (Iceberg format) and BigQuery. See the destination tabs in the [Setup guide](/docs/guides/database/replication/replication-setup#step-3-add-a-destination) for configuration details.

Availability varies based on our planned roll-out strategy. The destinations you can access depend on your project and access level.
...
- **Primary keys**: Required on all tables (Postgres logical replication requirement)
- **Custom data types**: Not supported
- **Schema changes**: Not automatically handled (DDL changes require manual intervention)

Destination-specific limitations (such as Analytics Buckets' append-only log format or BigQuery's row size limits) are detailed in the [Setup guide](/docs/guides/database/replication/replication-setup#step-3-add-a-destination).
...
## What happens if I delete a table at the destination?

If you delete a table downstream at your destination (e.g., in your Analytics Bucket or BigQuery dataset), the replication pipeline will automatically recreate it.
