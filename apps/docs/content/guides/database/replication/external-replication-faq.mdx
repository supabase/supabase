...
`Lost` means Postgres has already removed WAL files that the pipeline's replication slot needed. The pipeline cannot continue from that slot. Recreate the pipeline, or set **Invalidated slot behavior** to **Recreate** and restart it.
