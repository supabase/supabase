...
## Which plans support external replication (ETL)?
...
## What does external replication (ETL) install in the database?
...
## What happens when you disable external replication (ETL)?
...
To remove external replication (ETL) from a project, delete all replication pipelines first. After all pipelines are deleted, the disable action becomes available. For the Dashboard steps, see [Disabling external replication (ETL)](/docs/guides/database/replication/external-replication-setup#disabling-external-replication-etl).
