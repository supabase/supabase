...
Table errors affect an individual table without stopping the whole pipeline. Open **View pipeline**, review the table's error, and fix its cause. Resetting the table deletes its destination data and starts a new, billable initial sync for that table.

### Pipeline errors

Non-retryable startup or replication errors put the whole pipeline in **Failed** state. Open **View pipeline** and [**Logs > Replication**](/dashboard/project/_/logs/replication-logs), fix the reported problem, then restart the pipeline.
