...
After setting up Supabase Pipelines, you can monitor the status and health of your pipelines directly from the Dashboard. A pipeline is the managed Postgres replication process that continuously sends changes from your database to your destination.
...
{/* SCREENSHOT SLOT: pipelines-metrics.png — Add a sanitized 2x capture of a healthy pipeline's metrics and table states here. */}
...
Pipeline errors occur during ongoing replication (the **Live** state) and affect the entire pipeline. If an error occurs, the entire pipeline stops and enters a **Failed** state instead of silently skipping the failed change.
