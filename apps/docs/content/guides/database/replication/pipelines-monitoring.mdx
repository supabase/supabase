...
| **Starting**   | Pipeline is starting or being replaced after a restart                                |
| **Running**    | Pipeline is running; check table states and lag to confirm replication progress       |
...
| **Failed**     | Pipeline currently reports a startup or runtime failure and may recover automatically |
| **Unknown**    | The Dashboard can't currently determine the pipeline status                           |

**Running** describes the pipeline process. It does not confirm that initial sync has finished, that the source is connected, or that destination writes are complete. During a restart, **Starting** can remain visible until the replacement process is ready.
...
The destinations list also shows a compact lag value. This value is byte-based: it shows how much WAL the pipeline has not confirmed as flushed yet. A value of **Caught up** means there is no WAL gap at the time of measurement. Check table states and destination behavior separately; this is not a measure of when data becomes visible to queries.

The detailed status page shows:

| Metric                      | What it means                                                                                                                                    | What to watch for                                                                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Waiting to sync**         | Bytes of WAL between the pipeline's confirmed flush position and the current Postgres WAL position. This is the main byte-based replication lag. | A value that keeps growing means the pipeline is confirming progress more slowly than Postgres produces WAL.                                                    |
...
| **Last check-in**           | How long it has been since the pipeline last sent replication feedback to Postgres.                                                              | An old value can indicate a connection or feedback problem. A recent value alone does not prove replication is progressing.                                     |
...
Pipelines keeps sending feedback during slow destination writes and when it temporarily stops reading WAL. These feedback messages repeat the last safe replication position; they do not acknowledge unfinished writes. A recent **Last check-in** and **Connected** status can therefore coexist with growing lag. Use **Waiting to sync**, **WAL retention remaining**, table states, and logs to assess progress.

Pipelines uses one main pipeline replication slot for ongoing replication. During the initial sync, it can also create temporary table-sync replication slots. These temporary slots let multiple tables sync in parallel and allow automatic retries of individual tables without restarting the whole pipeline. A manual [table reset](#table-errors) temporarily stops the pipeline.
...
| **Copied**        | Existing rows have been copied; changes made during the copy still need to be applied |
| **Live**          | Table is following WAL changes, including catch-up after copying                      |
...
- During pipeline creation, review any `max_slot_wal_keep_size` recommendation. Continue monitoring **WAL retention remaining** because later write bursts or pipeline downtime can need more retention.
...
- Verify destination permissions, connectivity, and source table requirements against the relevant destination guide.
...
Table errors can pause an individual table while other tables continue replicating. Open **View pipeline**, review the error and any scheduled retry, and fix the cause. Some errors retry automatically; a table reset is needed when replication cannot safely resume from its saved state.

Restarting a table's initial sync resets its replication state. Before applying the reset, Pipelines stops the whole pipeline and waits for shutdown to finish. When replication resumes, the selected table's destination data is replaced through a new, billable initial sync. Other tables retain their progress and resume from their saved positions.

A pipeline restart preserves table progress; it does not perform this reset. If a reset reports an error, refresh the pipeline and table states before retrying. If the pipeline is **Stopped** after the operation, start it explicitly.

### Pipeline errors

**Failed** reports a current startup or runtime failure. The pipeline can recover automatically, so inspect **View pipeline** and [**Logs > Replication**](/dashboard/project/_/logs/replication-logs) to determine whether it is recovering or repeatedly failing. Fix persistent configuration, connectivity, permission, or destination errors, then restart if needed. A pipeline that has moved to **Stopped** needs to be started manually.

## Viewing logs

Open [**Logs > Replication**](/dashboard/project/_/logs/replication-logs) for errors, retries, and performance warnings. Include the relevant error details when contacting support.
