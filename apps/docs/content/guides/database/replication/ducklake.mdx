...
- **Catalog URL**: A `postgres://` or `postgresql://` connection URL, including credentials and the required `sslmode`
...
See the [DuckLake documentation](https://ducklake.select/docs/stable/) for the current DuckDB extension, Postgres catalog, and S3 secret setup. If your query client can't read a Supabase-backed catalog during Early Access, [contact support](/dashboard/support/new).
...
Source schema and table names are preserved. ASCII uppercase letters in column names are converted to lowercase. Source columns whose names differ only by ASCII case therefore collide; use distinct lowercase column names.

Initial-copy data is written to Parquet. Small ongoing changes can be temporarily inlined in the Postgres catalog and remain visible to DuckLake queries. Supabase runs routine maintenance to flush inlined data, combine files, rewrite data files when needed, and clean up unreferenced files. Don't modify catalog tables or object-storage files manually.
...
| Catalog URL validation fails                           | Use a valid `postgres://` or `postgresql://` URL with credentials and the required `sslmode`. Confirm the catalog is reachable from managed Pipelines.                                              |
