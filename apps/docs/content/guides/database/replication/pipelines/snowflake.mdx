...
Create a Snowflake database, schema, role, and service user for Pipelines. Keep the schema empty so Pipelines can own and manage every table it creates. Use unquoted identifiers for the service user and role because Pipelines converts account and user names to uppercase during authentication.
...
Use a dedicated role such as `PIPELINES_ROLE`, grant it the permissions above, and set it as the service user's `DEFAULT_ROLE`. Leave **Role** empty in the Dashboard or enter the same role explicitly. This keeps SQL validation and streaming on the same role.

### Generate a key pair

Pipelines authenticates with an RSA key pair. Snowflake requires at least 2048 bits and recommends PKCS #8. From the directory where you want to store the keys, create `rsa_key.p8` with one of these commands.
...
Derive the matching public key. Snowflake uses it to verify connections signed with `rsa_key.p8`:
...
Keep `rsa_key.p8` for the Dashboard configuration. Paste or upload the complete file, including its begin and end lines, into **Private key**. If you generated a passphrase-protected key, enter its passphrase into **Private key passphrase**.
