...
Pipelines creates the destination tables using the pipeline role. That role must retain ownership so Pipelines can alter, truncate, or drop the tables when required. Do not create the tables in advance under another role.
...
Open `rsa_key.pub` and copy the text between the `BEGIN PUBLIC KEY` and `END PUBLIC KEY` lines. In the following statement, replace `<public-key-body>`, including the angle brackets, with the copied text:

```sql
alter user PIPELINES_USER set rsa_public_key = '<public-key-body>';
```

When you configure the destination in the Dashboard, paste or upload the complete `rsa_key.p8` file into **Private key**. Include the `BEGIN PRIVATE KEY` and `END PRIVATE KEY` lines. If the key is passphrase-protected, enter the passphrase into **Private key passphrase**.

Keep `rsa_key.p8` and its passphrase secret. Don't commit them, include them in logs, or send them to support. The Dashboard accepts unencrypted PKCS #1 and PKCS #8 keys. It also accepts encrypted PKCS #8 keys with a passphrase, but not encrypted PKCS #1 keys.
...
Follow [Set up Pipelines](/docs/guides/database/replication/pipelines#setup-overview), select **Snowflake**, and enter the following settings:
