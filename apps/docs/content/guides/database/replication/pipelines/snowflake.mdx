...
Pipelines needs a dedicated Snowflake database and schema for replicated tables, together with a role and service user that can manage them. Start with an empty schema so Pipelines can own every table it creates. Create the role and service user with unquoted identifiers, because Pipelines converts account and user names to uppercase during authentication.

Run this setup as a Snowflake administrator. Change the example names as needed:
...
The pipeline role must own the destination tables so it can alter, truncate, or drop them. Let Pipelines create these tables rather than creating them in advance under another role.

Snowflake automatically creates a managed default pipe, `<TABLE>-STREAMING`, for each table. You don't need to provide a virtual warehouse, stage, or manually created pipe. See [Snowpipe Streaming access privileges](https://docs.snowflake.com/en/user-guide/snowpipe-streaming/snowpipe-streaming-access-control) for the ingestion requirements.

Use a separate role and warehouse for downstream queries and transformations.

### Keep the SQL and streaming roles aligned

Snowflake handles SQL requests and streaming separately, so both interfaces need to use the same role:
...
To keep the interfaces aligned, grant the permissions above to a dedicated role such as `PIPELINES_ROLE` and set it as the service user's `DEFAULT_ROLE`. You can then leave **Role** empty in the Dashboard or enter the same role explicitly.

### Generate a key pair

Pipelines authenticates with an RSA key pair. Snowflake requires a key of at least 2048 bits and recommends the PKCS #8 format. Run one of the following commands from the directory where you want to store the keys. Each command creates a private key named `rsa_key.p8`.
...
Next, derive the matching public key. Snowflake uses this public key to verify connections signed with the private key in `rsa_key.p8`:

```bash
openssl rsa -in rsa_key.p8 -pubout -out rsa_key.pub
```

Open `rsa_key.pub` and copy the text between the `BEGIN PUBLIC KEY` and `END PUBLIC KEY` lines. Replace the entire `<public-key-body>` placeholder, including the angle brackets, with that text:

```sql
alter user PIPELINES_USER set rsa_public_key = '<public-key-body>';
```

Use `rsa_key.p8` when you configure the destination in the Dashboard. Paste or upload the complete file, including its begin and end lines, into **Private key**. If you generated a passphrase-protected key, enter its passphrase into **Private key passphrase**.
...
Click **Start pipeline** and complete the validation and cost confirmations.
...
Use the replicated change history to build current-state datasets for reports and analytics. Pipelines maintains the history table. You maintain the queries, views, or dynamic tables that read it.
