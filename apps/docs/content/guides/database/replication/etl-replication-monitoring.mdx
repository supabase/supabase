...
title: 'Monitoring Automatic Destinations'
description: 'Monitor the status and health of your replication pipelines to automatic destinations.'
...
After setting up replication, you can monitor the status and health of your replication pipelines directly from the Supabase Dashboard. A pipeline is the active process that continuously replicates changes from your Postgres database to your destination using Postgres logical replication.

<Admonition type="note">

Automatic destinations are powered by [Supabase ETL](https://github.com/supabase/etl), an open source tool built specifically for Postgres logical replication. The Dashboard provides a managed interface to configure and monitor your replication pipelines.
...
| **Failed**   | Pipeline has encountered an error (hover over the status to see a prompt to view the logs) |
...
1. Hover over the **Failed** status in the destinations list to see a prompt directing you to view logs
...
1. Navigate to [Database](/dashboard/project/_/database/etl) → **Replication**
...
1. Hover over the status to see a prompt directing you to view logs
...
4. If lag is consistently high, consider adjusting your Postgres publication or batch wait time settings
...
For more troubleshooting tips, see the [Replication FAQ](/docs/guides/database/replication/etl-replication-faq).

### Next steps

- [Set up Replication](/docs/guides/database/replication/etl-replication-setup)
