...
<Admonition type="note" label="Private Alpha">
...
This guide covers **ETL Replication**, Supabase's automated replication product. For manual replication using external tools, see [Manual Replication](/docs/guides/database/replication/manual-replication-setup). For deploying read-only databases across multiple regions, see [Read Replicas](/docs/guides/platform/read-replicas) instead.

</Admonition>

ETL Replication automatically syncs your data by configuring a source and a destination. The source is your Supabase database, defined in the [**Publications**](/dashboard/project/_/database/publications) section of the dashboard.

The destination is an external location, defined in the [**Replication**](/dashboard/project/_/database/replication) section of the dashboard. See the [supported destinations](#destinations).

## Getting started

To set up ETL Replication:

1. **[Create a publication](/docs/guides/database/replication/etl-replication-setup)** - Define which tables and changes to replicate
2. **[Add a destination](#destinations)** - Configure where your data should be synced
3. **[Monitor replication](/docs/guides/database/replication/etl-replication-monitoring)** - Track status and troubleshoot issues
...
For detailed instructions on creating publications with SQL, see the [ETL Replication Setup guide](/docs/guides/database/replication/etl-replication-setup).
...
After setting up replication, data begins flowing within a few minutes. You can monitor the replication status and review any errors directly from the [Supabase Dashboard](/dashboard/project/_/database/replication) by clicking the **View status** button for a replication. In the status view for the replication pipeline, you can also view logs by clicking the **View logs** button.

For detailed monitoring instructions, see the [ETL Replication Monitoring guide](/docs/guides/database/replication/etl-replication-monitoring).
