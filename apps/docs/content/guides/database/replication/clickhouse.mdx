...
A source `TRUNCATE` truncates the ClickHouse table for either engine, then ongoing replication continues. It does not start a new initial sync. Resetting a table drops and recreates its table and, for `ReplacingMergeTree`, its generated view. A reset erases the accumulated destination data and copies existing source rows only if the table is [selected for initial sync](/docs/guides/database/replication/pipelines#choosing-which-tables-to-copy).
...
| Update                                     | `REPLICA IDENTITY FULL` to reconstruct unchanged out-of-line values |
...
| A schema change fails                      | Check [supported changes](#schema-change-support). Don't repair managed objects manually; contact support if restarting does not resolve the failure. |
