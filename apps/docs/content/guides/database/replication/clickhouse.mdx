...
Replicate Postgres changes to [ClickHouse](https://clickhouse.com/) as current-state tables or an append-only history. [Prepare resources](#prepare-clickhouse-resources), [configure the destination](#configure-clickhouse-as-a-destination), and [choose a table engine](#choose-a-table-engine).
...
| -------------------- | ----------------------------- | ------------------------------- | ------------------------------------------- |
| `ReplacingMergeTree` | Current-state tables          | Required                        | Query the generated `<table>__current` view |
| `MergeTree`          | Append-only CDC event history | Optional for insert-only tables | Query the base table                        |
...
During Early Access, source primary-key values must remain immutable when using `ReplacingMergeTree`. Updating a primary-key value can leave the old key visible in the generated current-state view.
...
Inserts and updates append the complete new row. Deletes append the old row when the source uses `REPLICA IDENTITY FULL`. With primary-key identity, a delete contains the key values; other fields use `NULL` for nullable scalars or placeholders such as zero, empty strings, and empty arrays. Those placeholders are not the deleted row's original values.
...
Changing the publication's column list for an already tracked table is supported while the pipeline runs. A previously excluded scalar column is added as nullable without a default, so historical rows remain `NULL`. Removing a published column drops its destination values; adding it again does not restore them.

Publishing a previously excluded array column requires a new initial sync because ClickHouse cannot represent a nullable array. Include the table in **Initial sync**, then [restart replication for the table](/docs/guides/database/replication/pipelines-monitoring#restarting-tables) to replace destination data and copy existing rows again.
...
ClickHouse DDL is not transactional. An interrupted multi-column schema change can leave a partially applied destination schema. Don't repair managed tables or views manually. If the pipeline remains failed after a pipeline restart, [contact support](/dashboard/support/new).
...
| A schema change fails                      | Check [supported changes](#schema-change-support). Don't repair managed objects manually; contact support if restarting the pipeline does not resolve the failure. |
