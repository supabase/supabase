...
Changing the publication's column list for an already tracked table is supported while the pipeline runs. A previously excluded scalar column is added as nullable without a default, so historical rows remain `NULL`. Publishing a previously excluded array column requires a new initial sync because ClickHouse cannot represent a nullable array. Removing a published column drops its destination values; adding it again does not restore them.
