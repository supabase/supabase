...
6. Optionally expand **Advanced settings** to tune pipeline behavior:
   ...
   Leave these settings at their defaults unless you need to tune initial sync speed, latency, or recovery behavior. **Recreate** replaces every destination table through a new, billable initial sync when the main slot is invalidated.

7. Click **Create and start pipeline**. Before creating the pipeline, the Dashboard checks source access, logical replication settings and capacity, the publication and selected tables, destination connectivity, and destination-specific table requirements.

   Required issues block creation. Fix them, then try again. Warnings don't block creation, but you need to review them, click **Create and start pipeline** again, and confirm that you want to continue.

   One warning can recommend a larger `max_slot_wal_keep_size`. During the initial sync, the pipeline needs Postgres to retain changes while it copies each table so that replication can continue consistently. Increasing this setting can use more source database storage, so check the available disk before applying it.
