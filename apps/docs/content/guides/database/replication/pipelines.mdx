...
Follow these steps to configure your destination. Each destination has its own setup requirements and data model. **BigQuery** is currently available. **Snowflake** is in Early Access. [Request access](/go/supabase-pipelines-new-destinations) to Snowflake, ClickHouse, or DuckLake.
...
- **At-least-once processing**: In rare recovery cases, an acknowledged batch can be processed and counted again. BigQuery uses primary-key-based CDC to converge on current state. Snowflake uses committed channel offsets to suppress ordinary replay. Because its tables are append-only, consumers must still tolerate repeated events. See [Can data be processed more than once?](/docs/guides/database/replication/pipelines-faq#can-data-be-processed-more-than-once) for details.
