...
3. Select a Pipelines destination, such as **BigQuery** or **Snowflake** if your organization has Early Access
...
Follow these steps to configure your destination. Each destination has its own setup requirements and data model. **BigQuery** is currently available. **Snowflake** is available through Early Access; [request access](/go/supabase-pipelines-new-destinations) to enable it. You can use the same form to request early access to ClickHouse and DuckLake.
...
Don't delete or modify a table managed by Pipelines while it remains in the publication. This can stop replication and require a new initial sync. To remove it permanently, first remove it from the publication and restart the pipeline, or delete the whole destination. See the [Pipelines FAQ](/docs/guides/database/replication/pipelines-faq#what-happens-if-a-table-is-deleted-at-the-destination) for details.

</Admonition>

### Schema change support

Schema change support is destination-specific and limited. See [BigQuery schema change support](/docs/guides/database/replication/bigquery#schema-change-support) or [Snowflake schema change support](/docs/guides/database/replication/snowflake#schema-change-support) for the exact behavior.
...
- **Tables not showing in publication**: Verify your tables meet the requirements of the selected destination. BigQuery requires a source primary key. Snowflake requires `REPLICA IDENTITY FULL` when updates are published.
...
- **Row identity**: Requirements are destination-specific. BigQuery always requires a source primary key and its published columns. Snowflake insert-only tables don't require a key, deletes require a published primary-key or replica-identity index unless full identity is used, and updates require `REPLICA IDENTITY FULL`.
...
- **Replica identity**: Updates and deletes need the mode required by the destination. See the [BigQuery](/docs/guides/database/replication/bigquery#source-table-requirements) and [Snowflake](/docs/guides/database/replication/snowflake#source-table-requirements) requirements.
...
- **At-least-once processing**: In rare recovery cases, an acknowledged batch can be processed and counted again. BigQuery uses primary-key-based CDC to converge on current state. Snowflake uses committed channel offsets to suppress ordinary replay, but its append-only consumers must still tolerate repeated events. See [Can data be processed more than once?](/docs/guides/database/replication/pipelines-faq#can-data-be-processed-more-than-once) for details.
