...
New scalar columns are nullable, and Pipelines doesn't fill their values in existing BigQuery rows. Data type changes aren't applied to BigQuery and require a table resynchronization.

See [BigQuery schema change support](/docs/guides/database/replication/bigquery#schema-change-support) for supported changes and limitations.
