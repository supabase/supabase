...
Schema change support depends on the destination. BigQuery is the only managed destination with beta schema change support.

Pipelines can add, remove, or rename supported columns, drop `NOT NULL`, update supported defaults, and change the published column list for a table already in the pipeline.

New scalar columns are nullable, and Pipelines doesn't fill their values in existing BigQuery rows. Data type changes aren't applied to BigQuery and require a table resynchronization; Supabase is exploring how to support data type changes without a full resync, but this isn't implemented yet.

When Pipelines detects a schema change it can't apply safely, it stops replicating the affected table instead of risking an inconsistent copy, and requires you to resynchronize that table. Other tables in the pipeline keep replicating. See [BigQuery schema change support](/docs/guides/database/replication/bigquery#schema-change-support) for supported changes and limitations.
...
- **Schema changes**: In beta and limited to BigQuery. Data type changes and interrupted schema changes require a table resynchronization.
...
- **Automatic stop on repeated failure**: Transient errors retry automatically for a limited time. If a pipeline keeps failing and restarting despite those retries, Supabase automatically stops it to avoid consuming resources indefinitely, and you must fix the underlying issue and restart it manually. See [Why was the pipeline stopped?](/docs/guides/database/replication/pipelines-faq#why-was-the-pipeline-stopped).
