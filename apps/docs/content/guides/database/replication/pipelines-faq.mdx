...
Pipeline failures occur during ongoing replication when an error happens while replicating changes. The pipeline stops instead of silently skipping the failed change. To recover:
...
Table errors occur during the initial sync. To recover, click **View pipeline**, find the affected table, and reset the table state. This restarts that table's initial sync from the beginning.
