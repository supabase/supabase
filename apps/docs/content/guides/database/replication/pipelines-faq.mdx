...
Full replica identity increases WAL volume. It only affects new WAL records, so fix the setting and restart the pipeline before generating more changes. See [BigQuery source table requirements](/docs/guides/database/replication/bigquery#source-table-requirements) for the supported modes and partial-update troubleshooting.
...
1. Stop the pipeline, or delete the destination to end replication and pipeline-hour billing
