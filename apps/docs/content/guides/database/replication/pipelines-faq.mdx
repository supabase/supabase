...
Pipelines requires a Pro, Team, or Enterprise plan. During public alpha, access is being rolled out gradually, so an eligible plan does not guarantee that Pipelines is enabled for every organization yet. If it is not available, request access from the **Database > Pipelines** page or contact your account manager.
...
4. Start or restart the pipeline from [**Database > Pipelines**](/dashboard/project/_/database/pipelines)
...
Open [**Database > Pipelines**](/dashboard/project/_/database/pipelines), click **View pipeline**, and check **Waiting to sync**, **WAL retention remaining**, **Last check-in**, **Connected**, and **Slot status**. See [Dealing with replication lag](/docs/guides/database/replication/pipelines-monitoring#dealing-with-replication-lag) for the full investigation and response flow.
...
Check the [**Database > Pipelines**](/dashboard/project/_/database/pipelines) section of the Dashboard:
...
After restarting the project, restart each replication pipeline manually from the [**Database > Pipelines**](/dashboard/project/_/database/pipelines) section of the Dashboard.
