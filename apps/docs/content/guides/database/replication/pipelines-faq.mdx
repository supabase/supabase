...
Schema change support is in beta and limited to the BigQuery destination.

Pipelines supports adding, removing, and renaming non-primary-key columns; dropping `NOT NULL`; updating supported defaults; and changing the published column list for a table already in the pipeline.

New scalar columns are nullable. Existing BigQuery rows remain `NULL`, and future changes contain the value from Postgres. Removing a published column also removes its stored BigQuery values.

Data type changes aren't applied to BigQuery and require a table resynchronization. Supabase is exploring how to support data type changes without a full resync, but this isn't implemented yet. See [BigQuery schema change support](/docs/guides/database/replication/bigquery#schema-change-support) for the full behavior and limitations.
...
A pipeline is stopped when you select **Stop pipeline**, when its project becomes inactive, when an operation requires it to restart, or when Supabase stops it automatically to protect system resources. A non-retryable configuration, schema, or data error instead puts the pipeline in **Failed** state (see [Why is a pipeline in failed state?](#why-is-a-pipeline-in-failed-state)).

Transient connection and destination errors retry automatically when possible. If a pipeline keeps failing and restarting despite those retries, Supabase automatically stops it rather than letting it keep restarting in a loop and consuming resources indefinitely. You'll receive an email explaining that the pipeline was stopped after repeated failures. This is the same **Stopped** state as a manual stop; the pipeline isn't deleted or canceled, and it doesn't restart on its own.

To recover:

1. Check whether the pipeline is **Stopped**, **Stopping**, or **Failed**
2. If it's **Failed**, or it was automatically stopped after repeated failures, review the error details and [replication logs](/dashboard/project/_/logs/replication-logs) to find the underlying cause, such as an unsupported schema change or a persistent destination error
...
A table can enter an error state during the initial sync, or afterward if Pipelines detects a schema change it can't apply safely, such as an unsupported data type change, primary-key change, or an interrupted schema change (see [BigQuery schema change support](/docs/guides/database/replication/bigquery#schema-change-support)). To recover, click **View pipeline**, find the affected table, fix the underlying issue if needed, and click its restart action. This restarts that table's initial sync from the beginning, deletes its existing destination data, and bills the successfully processed row data again.
