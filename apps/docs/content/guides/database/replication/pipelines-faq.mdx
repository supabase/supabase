...
**BigQuery** is currently available as a managed destination. **Snowflake** is available through Early Access; see the [Snowflake destination guide](/docs/guides/database/replication/snowflake) for its setup and append-only data model.

{/* supa-mdx-lint-disable-next-line Rule003Spelling */}
[Request access](/go/supabase-pipelines-new-destinations) to enable Snowflake for your organization or to request early access to ClickHouse and DuckLake. Supported destinations can be Supabase-managed or third-party systems as support expands.
...
Managed Pipelines run in **AWS `eu-central-1` (Frankfurt)**. For the best performance, place both your source project and destination as close to Frankfurt as possible. Choose the closest practical BigQuery dataset location or Snowflake account region.
...
As a result, replication into Analytics Buckets via Pipelines is no longer supported. **BigQuery** is currently available as a managed destination, and **Snowflake** is available through Early Access.
...
Snowflake Early Access supports adding, renaming, and dropping columns. It doesn't support column type changes or table and schema renames. Changes to whether a column accepts `NULL` and changes to defaults on existing columns are ignored. These operations alter the append-only history's schema; a drop removes the field from older events. See [Snowflake schema change support](/docs/guides/database/replication/snowflake#schema-change-support) for details.
...
- **Insufficient replica identity**: Snowflake updates require `REPLICA IDENTITY FULL`. Deletes require a primary key, `USING INDEX`, or full identity, and the publication must include identity columns.
...
For Snowflake, updates require `REPLICA IDENTITY FULL`. Deletes require a primary key, `REPLICA IDENTITY USING INDEX`, or full identity, and all identity columns must be included in the publication. Insert-only Snowflake tables don't require row identity.

```sql
alter table public.your_table replica identity full;
```

Full replica identity increases WAL volume and only affects new WAL records. Fix the setting before generating more changes. If the failing update is already retained in WAL, restarting may fail again; recreate the pipeline or restart the affected table's initial sync. See the [BigQuery](/docs/guides/database/replication/bigquery#source-table-requirements) or [Snowflake](/docs/guides/database/replication/snowflake#source-table-requirements) requirements for details.
...
BigQuery uses the replicated source primary key and CDC ordering metadata to converge on the current table state. Snowflake uses committed channel offsets to suppress ordinary replay, but its tables are append-only event histories and consumers must still tolerate repeated events. `_cdc_sequence_number` is an ordering and checkpoint token, not a globally unique event ID. Pipelines doesn't guarantee exactly-once event processing.
