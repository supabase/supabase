...
**BigQuery** is currently available as a managed destination. **Snowflake** is in Early Access. See the [Snowflake destination guide](/docs/guides/database/replication/snowflake) for setup instructions and details of the append-only data model.

{/* supa-mdx-lint-disable-next-line Rule003Spelling */}
[Request access](/go/supabase-pipelines-new-destinations) to Snowflake, ClickHouse, or DuckLake. Supported destinations can be Supabase-managed or third-party systems as support expands.
...
As a result, replication into Analytics Buckets via Pipelines is no longer supported. **BigQuery** is currently available as a managed destination, and **Snowflake** is in Early Access.
...
BigQuery uses the replicated source primary key and CDC ordering metadata to converge on the current table state. Snowflake uses committed channel offsets to suppress ordinary replay. Its tables are append-only event histories, so consumers must still tolerate repeated events. `_cdc_sequence_number` is an ordering and checkpoint token, not a globally unique event ID. Pipelines doesn't guarantee exactly-once event processing.
