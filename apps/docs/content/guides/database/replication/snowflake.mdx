...
To set up replication to Snowflake, follow these steps:

1. [Prepare the necessary resources in Snowflake](#prepare-snowflake-resources) such as databases, schemas, roles, and users.
2. [Configure Snowflake as a destination](#configure-snowflake-as-a-destination) in Supabase Pipelines using your prepared resources.
3. Finally, [query your replicated Postgres data](#query-and-materialize-current-state) in Snowflake, and learn how to materialize the current state from the replication stream.
   Each step is described in detail below to help you successfully replicate and work with your data.
