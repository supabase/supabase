...
After changing a replicated column's type or type modifiers, restart the table's initial sync. Pipelines logs a warning and leaves the destination type unchanged, which can cause later schema changes or writes to fail. This replaces the destination history with a new initial sync and future changes; it cannot recover past events. See [Table errors](/docs/guides/database/replication/pipelines-monitoring#table-errors) for the reset procedure and billing impact.
