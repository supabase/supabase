...
Applications that store large values in a single column — LLM prompts and responses, chat memory, big JSON documents, base64 blobs — can grow a table to tens or hundreds of GB even with relatively few rows. This guide explains how Postgres stores large values, how to decide between keeping them in Postgres and offloading them to [Storage](/docs/guides/storage), and how to keep tables lean.

## How Postgres stores large values (TOAST)

When a row is wider than ~2 kB, Postgres automatically moves large column values out of the main table into a companion **TOAST** table, compressing them first. This is transparent — your queries don't change — but it means a table's size is often dominated by its hidden TOAST relation, not by the columns you see.
...
| `EXTERNAL` | No | Yes | Faster substring reads, larger storage |
| `MAIN` | Yes | Last resort | Keeps data in-line when it fits |
| `PLAIN` | No | No | Only for non-TOASTable types |

<Admonition type="note">

LLM payloads are usually high-entropy and unique, so Postgres's default compression recovers little. The value still gets TOASTed — it just doesn't shrink much. That's why _how much_ you store matters more than compression settings.
...
## Decide: keep in Postgres, or offload to Storage
...
For AI apps, a common split: keep small, queryable fields (status, token counts, a short summary, embeddings) in Postgres; offload the raw prompt/response/log to [Storage](/docs/guides/storage) and keep a path pointer.
...
- **Set a retention policy.** If old rows aren't needed, delete them on a schedule with [pg_cron](/docs/guides/database/extensions/pg_cron). (Deleting alone won't shrink the file — see the troubleshooting guide for reclaiming.)
...
Store the artifact in a bucket and keep a pointer plus queryable metadata in Postgres:
...
This keeps the table small and queryable while the heavy bytes live in object storage.
