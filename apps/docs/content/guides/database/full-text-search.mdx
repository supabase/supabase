...
You can give different weights to different parts of your documents. Create a function that prioritizes title matches over description matches:
...
begin
  ...
  select 
    ...
    books.description,
    ts_rank(
      '{0.1, 0.2, 0.4, 1.0}'::real[], -- weights array {D, C, B, A}
      setweight(to_tsvector('english', books.title), 'A') || 
      setweight(to_tsvector('english', books.description), 'B'),
      to_tsquery(search_query)
    ) as rank
  from books
  where (setweight(to_tsvector('english', books.title), 'A') || 
         setweight(to_tsvector('english', books.description), 'B')) 
        @@ to_tsquery(search_query)
  order by rank desc;
end;
...
```js
const { data, error } = await supabase
  .rpc('search_books_weighted', { search_query: 'big' })
```
...
```sql
select * from search_books_weighted('big');
```

</TabPanel>
</Tabs>

This gives title matches higher priority (weight 'A' = 1.0) than description matches (weight 'B' = 0.4).
