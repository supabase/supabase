...
PostgreSQL provides several functions to create tsquery objects:
...
### Comparison with other tsquery functions

| Function | Input | Output | Best for |
|----------|-------|---------|----------|
| `to_tsquery()` | `'big & dreams'` | `'big' & 'dream'` | Manual control over operators |
| `plainto_tsquery()` | `'big dreams'` | `'big' & 'dream'` | Simple AND searches |
| `phraseto_tsquery()` | `'big dreams'` | `'big' <-> 'dream'` | Phrase searching |
| `websearch_to_tsquery()` | `'"big dreams" or small'` | `'big' <-> 'dream' \| 'small'` | User-friendly search interfaces |
...
PostgreSQL provides ranking functions to sort search results by relevance, helping you present the most relevant matches first. Since ranking functions need to be computed server-side, we'll use RPC functions and generated columns.

### Creating a search function with ranking [#search-function-ranking]

First, create a PostgreSQL function that handles search and ranking:
...
```js
const { data, error } = await supabase
  .rpc('search_books', { search_query: 'big' })
```
...
PostgreSQL allows you to assign different importance levels to different parts of your documents using weight labels. This is especially useful when you want matches in certain fields (like titles) to rank higher than matches in other fields (like descriptions).

#### Understanding weight labels

PostgreSQL uses four weight labels: **A**, **B**, **C**, and **D**, where:
...
First, let's create a weighted tsvector column that gives titles higher priority than descriptions:
...
const { data, error } = await supabase
  .rpc('search_books_weighted', { search_query: 'Harry' })

// Search with custom weights  
const { data: customData, error: customError } = await supabase
  .rpc('search_books_custom_weights', { search_query: 'Harry' })
```
...
Let's say you search for "Harry". With weighted columns:
...
This ensures that books with "Harry" in the title will rank significantly higher than books that only mention "Harry" in the description, providing more relevant search results for users.

### Using ranking with indexes [#ranking-with-indexes]

When using the `fts` column we created earlier, ranking becomes more efficient. Create a function that uses the indexed column:
...
```js
const { data, error } = await supabase
  .rpc('search_books_fts', { search_query: 'little & big' })
```
...
### Using websearch syntax with ranking [#websearch-ranking]
...
const { data, error } = await supabase
  .rpc('websearch_books', { search_text: '"little puppy" or train -vegetables' })
```
