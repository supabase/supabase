...
<Tabs size="small" type="underlined" defaultActiveId="hosted" queryGroup="platform">

<TabPanel id="hosted" label="Hosted Supabase">
...
<TabPanel id="self-hosted" label="Self-hosted">

1.  Connect to your database and drop the `pg_graphql` extension:

    ```sql
    drop extension if exists pg_graphql;
    ```

1.  In your `docker/.env` file, set `PGRST_DB_SCHEMAS` to some dummy value that won't be used as a valid schema, for example `dummy_schema_never_used`. It can't be left empty because PostgREST will fall back to the default value of `public`.

1.  Restart the `rest` container to pick up the changes.
...
1.  In your `docker/.env` file, remove `public` from `PGRST_DB_SCHEMAS`.
...
1.  <Tabs size="small" type="underlined" defaultActiveId="hosted" queryGroup="platform">

    <TabPanel id="hosted" label="Hosted Supabase">

    Go to [API Settings](/dashboard/project/_/settings/api) in the Supabase Dashboard.

    Under **Data API Settings**, add `api` to **Exposed schemas**. Make sure it is the first schema in the list, so it will be searched first by default.
    ...
    In your `docker/.env` file, add `api` to `PGRST_DB_SCHEMAS`. Make sure it is the first schema in the list, so it will be searched first by default.

    Restart the `rest` container to pick up your changes.
