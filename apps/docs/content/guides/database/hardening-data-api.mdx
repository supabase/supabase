...
1.  Grant the `anon` and `authenticated` roles usage on this schema. If you also need server-side access using the `service_role` key, include `service_role` in the grant as well.
