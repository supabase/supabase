...
When using logical replication, Postgres keeps WAL files around for longer than it otherwise needs them. If the files are removed too, then your `replication slot` can become inactive or lost if the database receives a large number of changes in a short time.
