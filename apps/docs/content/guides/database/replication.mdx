...
| [ClickHouse](/docs/guides/database/replication/clickhouse) | ✅ Supported | `REPLICA IDENTITY FULL` | Primary-key or full identity | ✅ Supported | Limited Early Access | Current-state tables by default, or append-only CDC history.                     |
...
| [Snowflake](/docs/guides/database/replication/snowflake)   | ✅ Supported | `REPLICA IDENTITY FULL` | Row identity required        | ✅ Supported | Limited Early Access | Append-only CDC history. Source `TRUNCATE` operations and table resets erase it. |
