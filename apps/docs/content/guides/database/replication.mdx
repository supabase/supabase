...
BigQuery is currently available as a managed destination. ClickHouse and Snowflake are in Early Access. [Request access](/go/supabase-pipelines-new-destinations) to ClickHouse, Snowflake, or DuckLake.
...
| [ClickHouse](/docs/guides/database/replication/clickhouse) | ✅ Supported | Primary-key or full identity | Primary-key or full identity | ✅ Supported | Early Access (limited) | Current-state tables (default) or append-only CDC history.                       |
