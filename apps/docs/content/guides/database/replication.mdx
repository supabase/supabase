...
BigQuery is currently available as a managed destination. Snowflake is available through Early Access; [request access](/go/supabase-pipelines-new-destinations) to enable it for your organization. You can use the same form to request early access to ClickHouse and DuckLake.
...
| [Snowflake](/docs/guides/database/replication/snowflake) | ✅ Supported | `REPLICA IDENTITY FULL` | Row identity required | ✅ Supported | Early Access (limited) | Append-only CDC history; truncate and reset erase the history. |
