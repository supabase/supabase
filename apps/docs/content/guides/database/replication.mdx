...
description: 'Replicate your Postgres database to external destinations using logical replication.'
subtitle: 'An introduction to Postgres replication and change data capture.'
sidebar_label: 'Overview'
---

Database replication is the process of copying changes from your database to another location. It's also referred to as change data capture (CDC): capturing all the changes that occur to your data.

Supabase uses **Postgres logical replication**, a native Postgres feature that streams changes from your database's Write-Ahead Log (WAL) to external destinations in near real-time.
...
Supabase supports replicating your database to two types of destinations:
...
Supabase provides managed replication to popular data platforms powered by [Supabase ETL](https://github.com/supabase/etl), an open source tool for Postgres logical replication. Configure your destination through the Dashboard, and Supabase ETL handles the replication pipeline for you.

**Supported managed destinations:**
- **[Analytics Buckets](/docs/guides/storage/analytics/introduction)**: Specialized storage buckets in Supabase Storage designed for analytical workloads. They provide S3-compatible storage and use the Apache Iceberg open table format.
- **BigQuery**: Google's fully managed data warehouse

<Admonition type="note">

Destination availability varies based on our planned roll-out strategy. The destinations you can access depend on your project and access level.
...
Configure your own replication destination using external tools and Postgres's native logical replication. This gives you full control over the replication process and allows you to replicate to any system that supports Postgres logical replication.

- [Set up manual destinations](/docs/guides/database/replication/manual-replication-setup)
