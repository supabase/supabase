...
`explain()` is a method that provides the Postgres `EXPLAIN` execution plan of a query. It is a tool for debugging slow queries and understanding how Postgres will execute a given query. This feature is applicable to any query, including those made through `rpc()` or write operations.
