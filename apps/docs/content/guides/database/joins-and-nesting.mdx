...
```python
data = await supabase.from_('countries').select('id, name, cities(id, name)').execute()
```
...
```python
data = await supabase.from_('teams').select('id, team_name, users(id, name)').execute()
```
...
```python
data = await supabase.from_('shifts').select("""
  *,
  scans:scan_id_start (
    id,
    user_id,
