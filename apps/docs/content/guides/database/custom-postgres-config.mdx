...
In the case of Read Replicas, conflicting configuration values will prevent replicas from starting cleanly and accepting traffic.
...
1. If you use the --no-restart flag, particularly where Read Replicas exist, and later one of the databases is restarted, this might lead to a conflict of mismatching parameters and prevent one of the databases coming up correctly.
