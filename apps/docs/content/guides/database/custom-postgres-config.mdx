...
Supabase projects come with a pre-configured Postgres cluster optimized for various workloads based on [compute add-on size](/docs/guides/platform/compute-add-ons). While Postgres offers hundreds of settings, you can override _some_ to better suit your specific needs.
...
Postgres settings have contexts that define how they can be modified and what privileges are needed. By default, any setting with a `user` context can be changed for a specific `role` or `database` with [SQL](https://supabase.com/dashboard/project/_/sql/).
...
### Supautils enabled settings

Some settings require superuser privileges to modify. Supabase pre-enables the [supautils extension](https://supabase.com/blog/roles-postgres-hooks#setting-up-the-supautils-extension), which allows the postgres role to retain certain superuser privileges. It enables modification of the below reserved configurations at the `role` level:
