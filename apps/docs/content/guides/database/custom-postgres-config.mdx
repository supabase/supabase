...
- If the setting is reloadable (`pg_settings.context = 'sighup'`), then the Management API will detect this and apply the change with a configuration reload.
...
You can also pass the `--no-restart` flag to attempt a reload-only apply. If the parameter isn’t reloadable, the change stays pending until the next restart.
