...
If you want to expose any of the foreign table columns to your public API, you can create a [Database Function with security definer](https://supabase.com/docs/guides/database/functions#security-definer-vs-invoker) in the `public` schema, and then you can interact with your foreign tables through API.

As an example, let's go to [SQL Editor](https://supabase.com/dashboard/project/_/sql/new) first and then follow below steps,
...
2. Create a security definer function that queries the foreign table:

   ```sql
   create function public.get_stripe_products()
   returns table (
     id text,
     name text,
     active boolean,
...
3. Restrict the function execution to a specific role, for example, the authenticated users only:
   ...
   -- grant execute permission to specific role only
   grant execute on function public.get_stripe_products to authenticated;
   ```

Once the above steps are finished, we can invoke the function to query foreign table from Supabase client:

```js
let { data, error } = await supabase.rpc('get_stripe_products').select('*')
if (error)
  console.error(error)
else
  console.log(data)
