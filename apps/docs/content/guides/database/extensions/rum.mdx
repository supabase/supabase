...
RUM works best in scenarios when the possible keys are highly repeatable. That is, all texts are composed of a
...
The does the same with `anyarray` index as `rum_tsvector_addon_ops` that is, allows to order select results using distance
