...
pgaudit can be configured to focus on multiple different levels of precision
...
Inside the session, you can change the variable to log different events:
...
To ensure reliability, you may want to log all actions initiated by `zapier`. That can be done with the following command:
...
With object mode, you assign database users to the `pgaudit.role` variable. Whatever the users could hypothetically do, pgaudit will log.
...
The `ROLE` cannot be impersonated by any other user. It cannot be logged into or used. In this case, it solely exists to securely define what pgaudit will record.
...
> Copied from the [pgaudit official docs](https://github.com/pgaudit/pgaudit/blob/master/README.md#format)
...
Some users may feel compelled to monitor all write initiated by the API roles:
...
Cron job executions are recorded by default in Supabase and do not require pgaudit to observe. pg_cron only logs start and completion events.
...
-- disable
ALTER ROLE postgres SET pgaudit.log_parameter TO 'off';
```
