...
- For Postgres clients, use a connection string
  ...
  For persistent clients (VMs, Containers) requiring IPv4 support, use the Direct Connection (which operates in Session Mode)
...
The connection string uses port `5432`: ``` postgresql://postgres:[YOUR-PASSWORD]@db.abcdefghijklmnopqrst.supabase.co:5432/postgres 
