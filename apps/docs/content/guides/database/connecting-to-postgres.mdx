...
**Query pipelining** isn't supported yet either: transaction mode returns a connection to the pool as soon as it sees one reply finish, even if the client already queued more queries on it. An upcoming Supavisor release will add pipelining support.
