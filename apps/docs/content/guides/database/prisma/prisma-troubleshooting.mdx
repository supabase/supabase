...
- **Increase pool size**: If you are connecting with Supavisor, try increasing the pool size in the [project connect page](https://supabase.com/dashboard/project/_?showConnect=true).
...
- **When working in session mode (port 5432):** The max amount of clients is restricted to the "Pool Size" value in the [project connect page](https://supabase.com/dashboard/project/_?showConnect=true). If the "Pool Size" is set to 15, even if the pooler can handle 200 client connections, it will still be effectively capped at 15 for each unique ["database-role+database" combination](https://github.com/orgs/supabase/discussions/21566).
