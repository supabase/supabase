...
- [Ensure you have enabled row level security (RLS) on all tables](/dashboard/project/_/database/tables)
...
- Enable replication on tables containing sensitive data by enabling RLS and setting row security policies:
  - Go to the [**Authentication > Policies**](/dashboard/project/_/auth/policies) page in the Supabase Dashboard to enable RLS and create security policies.
  - Go to the [**Database > Publications**](/dashboard/project/_/database/publications) page in the Supabase Dashboard to manage replication tables.
...
- Ensure you enable email confirmations in the [**Settings > Auth**](/dashboard/project/_/auth/providers) page of the dashboard.
- Ensure that you've set the expiry in the [**Settings > Auth**](/dashboard/project/_/auth/providers) page of the dashboard for one-time passwords (OTPs) to a reasonable value that you are comfortable with. We recommend setting this to 3600 seconds (1 hour) or lower.
...
- Use your own SMTP credentials so that you have full control over the deliverability of your transactional auth emails in the [**Authentication > Emails > SMTP Settings**](/dashboard/project/_/auth/smtp) section of the dashboard.
  ...
  - The default rate limit for auth emails when using a custom SMTP provider is _30 new users per hour_, if doing a major public announcement you will likely require more than this.
...
- Database backups are not available for download for Free Plan projects.
  ...
  -  If you need a lower recovery point objective (RPO), enable Point-in-Time Recovery (PITR). PITR allows you to backup a project at shorter intervals. This provides users an option to restore to any chosen point of up to seconds in granularity.
