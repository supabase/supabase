...
- Review table privileges in addition to RLS policies.
  - On many projects, tables in `public` already receive broad default grants. Bare `GRANT` statements are often a no-op — revoke first, then grant only what each role needs.
  - Postgres RLS policies cover `SELECT`, `INSERT`, `UPDATE`, and `DELETE` only. **`TRUNCATE` is not protected by RLS**, so keep it revoked unless you intentionally need it.
  - See the [RLS guide](/docs/guides/database/postgres/row-level-security) for the recommended revoke-then-grant pattern.
