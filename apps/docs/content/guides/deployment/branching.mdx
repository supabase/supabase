...
2. **Pull** - Retrieves database migrations from your base project (also initialises the migration history table when Branching via Dashboard)
...
4. **Configure** - Updates service configurations based on your config.toml file (not available for Branching via Dashboard at the moment)
...
If a parent node fails, all dependent children nodes will be skipped. For e.g., if your database migrations failed at step 5, we will not seed your branch because step 6 is skipped. If you are using [GitHub integration](/docs/guides/deployment/branching/github-integration), the same deployment workflow will be run on every push to your git branch.
