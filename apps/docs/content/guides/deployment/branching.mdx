...
- **Built from your base project**: Each new branch starts from your base project, the main project that your production branch runs on. Supabase creates a fresh database for the branch and replays that project's migration history against it, so the branch ends up with the same schema. Schema changes that your migration files don't capture are not reproduced.
