...
Branching uses the `config.toml` file in your `supabase` directory to prepare and deploy the different services to each branch. These are:

1. Database - Using the `migrations` directory for schema changes and `[db.seed]` block to declare the `sql_paths` for seeding data.
2. Auth - The `auth` section of the `config.toml` file is used to sync your project config with the Branch
3. Storage - Buckets defined in the `storage` section of the `config.toml` are created upon deployment of a new Branch and are seeded with files within the directory defined in the `objects_path` property
4. Edge Functions - Functions are declared in `[functions.my-slug]` block with `entrypoint_path` specifying the location of `index.ts` or `index.js` file to your Edge Function.

<Tabs scrollable size="small" type="underlined" defaultActiveId="auth" queryGroup="platform">
  <TabPanel id="api" label="API Config Example">
    ```toml
    [api]
    enabled = true
  ...
  <TabPanel id="auth" label="Auth Config Example">
    ```toml
  ...
  <TabPanel id="storage" label="Storage Config Example">
    ```toml
    [storage]
  ...
  <TabPanel id="function" label="Edge Function Config Example">
    ```toml
    [functions.hello-world]
    entrypoint_path="functions/hello-world/index.js"
    ```
  </TabPanel>
</Tabs>
...
<Tabs scrollable size="small" type="underlined" defaultActiveId="existing" queryGroup="platform">
...
<Admonition type="caution" label="Prepare your GitHub repository before enabling Branching">
  If your repository doesn't have all the migration files, your production branch could run an
  incomplete set of migrations. Make sure your [GitHub repository is
  prepared](#preparing-your-git-repository).
</Admonition>
