...
If a parent node fails, all dependent children nodes will be skipped. For e.g., if your database migrations failed at step 5, we will not seed your branch because step 6 is skipped. If you are using GitHub integration, the same deployment workflow will be run on every push to your git branch.
