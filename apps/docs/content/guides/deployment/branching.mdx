...
Supabase Branching allows you to create separate, temporary instances of your project, similar to Git branching. This enables you to experiment with new configurations, database schemas, or features in isolation. When ready, you can merge your changes into your production instance.
...
You can create a corresponding Preview Branch for any Git branch in your repository. Each time a new Preview Branch is created and configured based on the [`config.toml`](/guides/local-development/cli/config) configuration on this branch, the migrations in the Git branch of that Preview Branch are run on the Preview Branch.

The Preview Branch is also [seeded](/guides/local-development/seeding-your-database) with sample data based on `./supabase/seed.sql` by default, if that file exists.
...
When you merge your Git branch into the production branch, all new migrations will be applied to your Production environment based on the changes made to your [`config.toml`](/guides/local-development/cli/config).
...
Your Preview Branches are seeded with sample data using the same as [local seeding behavior](/guides/local-development/seeding-your-database).
...
Under the hood, you can see Supabase branching as a way to programmatically "duplicate" your Supabase project via git flow. This allows you to spawn a new configured (via [`config.toml`](/guides/local-development/cli/config)) and seeded instance of your database and the adjacent Supabase services (buckets, edge-functions, etc.).
When you create a new branch via a git flow, here is what happens on the Supabase side:
1. We deploy a new project on your behalf on the Supabase side as the new "branch" if it doesn't already exist. This includes the database, storage, edge-function, and all Supabase-related services.
2. We clone your branch and configure the new project based on the [`config.toml`](/guides/local-development/cli/config) committed into this project branch.
3. We apply migrations and run seeding scripts (the first time) for this branch.
...
Supabase branching is a new and exciting part of the Supabase development ecosystem. We're monitoring its success and open to any feedback.
