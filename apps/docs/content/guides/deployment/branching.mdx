...
To update configuration for a Supabase branch, simply modify `config.toml` and push to your VCS. The Supabase integration will detect the changes and apply them to the corresponding branch.
...
<Admonition type="note" label="Create persistent branches before configuring">
...
Then add the returned project ID to your `config.toml`.
