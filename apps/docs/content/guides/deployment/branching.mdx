...
If you want to use your own ORM for managing migrations and seed scripts, you can run them in GitHub Action after the preview branch is ready. The branch credentials can be fetched using the following example GHA workflow.
...
jobs:
  ...
  migrate:
    ...
    steps:
      ...
      - name: ORM migration or seed
        run: npm run migrate $POSTGRES_URL_NON_POOLING
```
