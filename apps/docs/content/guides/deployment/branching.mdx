...
Supabase Branching allows the creation of separate, temporary instances of a project, similar to Git branching. This enables experimentation with new configurations, database schemas, or features in isolation. When ready, changes can be merged into the production instance.
...
To update configuration for a Supabase branch, simply modify `config.toml` and push to git. The Supabase integration will detect the changes and apply them to the corresponding branch.
