...
To understand how database migrations work, it is useful to think of database schemas as interfaces in TypeScript. You database schemas define the structure of your tables, functions, and other database entities. When you change an interface in TypeScript, you need to update any code that accesses the changed fields. Similarly when you modify a database schema, you have to update all other entities that reference it using a migration.

## Schema migrations

For this guide, we'll create a table called `employees` and see how we can make changes to it. You will need to install the Supabase CLI and start the local development stack.

View the [quickstart](/docs/guides/local-development#quickstart) guide to setup your local environment.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    <StepHikeCompact.Details title="Modify your employees table">
      Next, let's modify your `employees` table by adding a column for department.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={6}>
    <StepHikeCompact.Details title="Apply your migration">
      ...
      Now you should see the department column added to your `employees` table in the local Dashboard.
    </StepHikeCompact.Details>
