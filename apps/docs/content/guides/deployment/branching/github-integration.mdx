...
Every Supabase branch, preview or persistent, is created as a clone of your base project. The new branch starts with the schema, migration history, and configuration of that project rather than with an empty database.

Your production data and storage objects are not cloned by default. See [Seeding](#seeding) to populate them with sample data.
...
Cloning your base project copies its schema and configuration, but not its data or storage objects. This is meant to protect your sensitive production data. Your branch starts with the tables of your base project, and the only rows in them are the ones your seed files add.
