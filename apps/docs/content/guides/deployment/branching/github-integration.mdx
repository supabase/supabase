...
Every Supabase branch, preview or persistent, starts from your base project rather than from a blank slate. Supabase creates a fresh database for the branch and replays the migration history of your base project against it, so the branch ends up with the same schema and configuration. Schema changes that were never captured as migration files, such as edits made directly in the SQL Editor, are not reproduced. See [Troubleshooting MIGRATIONS_FAILED](/docs/guides/troubleshooting/branch-in-migrations-failed-status) if the replay fails.

Your production data and storage objects are not copied by default. See [Seeding](#seeding) to populate the branch with sample data.
...
Your branch starts with the configuration of your base project. The settings in your `config.toml` file are applied on top of it.

A comment is added to your PR with the deployment status of your preview branch.

### Migrations

The migrations in the `migrations` subdirectory of your Supabase directory are automatically run. Your base project's migration history is replayed when the branch is created, so each later commit runs only the migrations that haven't been applied yet.
...
Replaying your migration history reproduces the schema and configuration of your base project, but not its data or storage objects. This is meant to protect your sensitive production data. Your branch starts with the tables of your base project, and the only rows in them are the ones your seed files add.
