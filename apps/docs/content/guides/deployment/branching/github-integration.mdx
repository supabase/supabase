...
Every Supabase branch, preview or persistent, is created as a clone of your base project, the project that your production branch runs on. The branch starts with the schema, migration history, and configuration of that project rather than with an empty database.

Your production data is the exception. It isn't cloned, so your branch starts with empty tables. See [Seeding](#seeding) to populate them with sample data.
...
The migrations in the `migrations` subdirectory of your Supabase directory are automatically run. Because your branch is a clone of your base project, it inherits that project's migration history, so only the migrations that haven't been applied yet are run.

### Seeding

Cloning your base project copies its schema and configuration, but not its data. This is meant to protect your sensitive production data. Your branch starts with the tables of your base project and no rows in them.
