...
The new branch is created as a clone of your base project, so it starts with that project's schema, migration history, and configuration rather than with an empty database. Your production data and storage objects are not cloned.
