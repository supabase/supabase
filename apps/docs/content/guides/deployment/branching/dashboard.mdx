...
Supabase creates a fresh database for the branch and replays your base project's migration history against it, so the branch ends up with the same schema. Schema changes that were never captured as migration files, such as edits made directly in the SQL Editor, are not reproduced. If the replay fails partway through, the branch is left empty or incomplete. See [Troubleshooting MIGRATIONS_FAILED](/docs/guides/troubleshooting/branch-in-migrations-failed-status) to repair your migration history.
...
A branch created with **Include data** holds a copy of your production data, so treat it with the same care as production. The branch also uses a larger disk and matches the compute size of your project, which increases its cost.
...
- If you have run migrations on main, the schema of a new branch is rebuilt from those existing migrations instead of from a full schema dump, so objects that your migrations don't capture might be missing
