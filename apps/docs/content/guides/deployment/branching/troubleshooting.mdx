...
If using [Branching with Github](/docs/guides/deployment/branching/github-integration) ensure that the folder in the connected repo that you are pointing to is not the path that ends in `/supabase`, but the root level where this folder lives. Otherwise you may see errors inside of your branch logs such as `INFO Config file not found... path=supabase/supabase/config.toml` which indicates it is looking at the wrong level for configuration/migration files.

Similarly, if using [Branching via Dashboard](/docs/guides/deployment/branching/dashboard), ensure you are not also using the github integration as you will be prompted to point to a branch inside of the connected repo.

### Missing Extensions

If using an extension such as pgmq, you should enable the pgmq extension from your migration files to ensure the schema in the preview branch matches your main databse.
