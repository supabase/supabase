...
The [Typescript client library](https://www.npmjs.com/package/@supabase/realtime-js/v/2.10.0-next.6) is still under heavy development and as such is under the `next` version to ensure we're able to improve the developer experience.
...
If you do not create a channel, we will have the assumption that you do not want RLS checks on the channel.
...
The rules you set for realtime.channels will impact your Postgres Changes channels. At the moment we do not advice to use this as a feature as this was built with the intent to be used in Broadcast and Presence features.
