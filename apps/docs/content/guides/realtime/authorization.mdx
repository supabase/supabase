...
You can control user access to Realtime Channels using RLS policies. By applying RLS, you can:

- Control which users can broadcast to a Channel
- Control which users can receive broadcasts from a Channel
- Control which users can publish their presence to a Channel
- Control which users can receive messages about the presence of other users

<Admonition type="caution">

Realtime Authorization is in public alpha. To use Authorization for your Realtime Channels, use `realtime.js` version `2.10.0` or later.

</Admonition>

## How it works

Realtime uses tables in your database's `realtime` schema to track users and Channel topics. By writing RLS policies against these tables, you can control the access users have to a Channel topic and features within a Channel topic.

The validation is done when the user connects. When their WebSocket connection is established, their permissions are calculated based on:
...
using (
  exists (
    ...
    where
      ru.user_id = (select auth.uid())
      and ru.topic = (select realtime.channel_name())
...
with check (
  exists (
    ...
    where
      ru.user_id = (select auth.uid())
      and ru.name = (select realtime.channel_name())
      and realtime.messages.extension in ['presence']
  )
);
```
...
Postgres Changes and Realtime Channel authorization are backwards compatible with your production application.
