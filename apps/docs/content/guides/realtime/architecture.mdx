...
{/* Double headed arrows have some issues, this fixes that, but the rendering isn't great */}

```mermaid
flowchart LR
  UserB[User in Region B] --> RBNode["Realtime Node"]
  UserA[User in Region A] --> RANode["Realtime Node"]
  subgraph GC[Realtime Global Cluster]
    subgraph RB[Region B]
      RBNode[Region B]
    end
    subgraph RA["Region A"]
      RANode["Realtime Node"]
      TDB[(Tenant Database)]
    end
  end
  RBNode --> RANode
