...
- supabase projects is `wss://<PROJECT_REF>.supabase.co/realtime/v1/websocket?apikey=<API_KEY>`
- self-hosted projects is `wss://<HOST>:<PORT>/socket/websocket?apikey=<API_KEY>`

As an example, using the websocat project, you would run the following command in your terminal:
...
After this you would need to send the [phx_join](#phx_join) event to the server to join the Channel.
...
- `ref`: A unique reference ID for the message. This is used to track the message and its response on the client side when a reply it's needed to proceed.

### Event types

The following are the main event types used in the Realtime protocol:
...
##### phx_reply response to phx_join
...
##### phx_reply response to presence

When replying to presence events, it returns an empty object.

##### phx_reply response on heartbeat
...
Structure of the broadcast event to be sent to all clients in a channel. The `payload` field contains the event name and the data to be broadcasted.
...
- `event`: The name of the event being broadcasted.
