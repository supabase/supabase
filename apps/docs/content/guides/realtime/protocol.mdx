...
- `config`:
  ...
  - `postgres_changes`: Array of configurations for Postgres changes
    ...
    - `select`: Comma-separated list of columns to include in the change payload (e.g. `"id,body"`). When omitted, all columns are returned.
...
- `postgres_changes`: Array of Postgres changes that the client is subscribed to, each object contains:
  - `id`: Unique identifier for the Postgres changes subscription, computed as `phash2` of the subscription params (including `select` when provided)
  ...
  - `filter`: The filter applied to the subscription, if any
  - `select`: The column selection applied to the subscription, if any
