...
### Realtime broadcast from websocket performance
...
### Realtime broadcast from database performance

This workload evaluates the system's capacity to handle use broadcast from database using the `realtime.broadcast_changes` function. Each virtual user (VU) in the test:

- Establishes and maintains a WebSocket connection
- Joins a distinct channels:
...
![Broadcast from Database Performance](/docs/img/guides/realtime/broadcast-from-db-performance.png)
