...
When a `sync` event triggers, you may receive `join` and `leave` events simultaneously. This is expected behavior and occurs as the Presence system reconciles its local state with the server. It does not necessarily indicate that a user has physically joined or left.

**Tip:** You should process `leave` events before `join` events to ensure your local presence state remains accurate during these updates.
