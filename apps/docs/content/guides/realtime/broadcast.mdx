...
The private flag (the last argument in `realtime.send(payload, event, topic, is_private)`) only affects who can subscribe to the topic not who can read messages from the database.
...
select
  realtime.send(
    ...
    'event', -- Event name
    'topic', -- Topic
    false -- private: true = private channel (default), false = public channel
  );
```
...
select
  realtime.send_binary(
    ...
    'event',           -- Event name
    'topic',           -- Topic
    true               -- private: true = private channel (default), false = public channel
  );
```
