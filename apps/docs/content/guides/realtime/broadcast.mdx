...
You can use the `realtime.broadcast_changes()` helper function to broadcast messages when a record is created, updated, or deleted. To do this, you can attach a [Postgres trigger](/docs/guides/database/postgres/triggers) to any table. This function uses a private channel and requires Realtime Authorization RLS policies to be met.
