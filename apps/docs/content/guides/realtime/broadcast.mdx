...
When a message is sent whether from your client, a REST request, or your database using realtime.send(), the message gets inserted into the table realtime.messages.

The Realtime service continuously listens for new rows in this table (using logical replication). When a new message is inserted and passes any applicable RLS policies, Realtime reads it and broadcasts it to all connected clients that are subscribed to the topic.
...
However, regardless of whether it's public or private, the Realtime service connects to your database as the authenticated Postgres role, and must be allowed to SELECT from realtime.messages under your RLS policies.
