...
You can provide a callback for the `broadcast` channel to receive message. This example will receive any `broadcast` messages that are sent to the `room-1` channel:
...
>
  ...
  <TabPanel id="python" label="Python">
    ...
    # Subscribe to the Channel
    channel_a
      .on_broadcast('test', message_received)
      .subscribe()
    ```
...
You can send a Broadcast message by making an HTTP request to Realtime servers. This is useful when you want to send messages from your server or client without the overhead of establishing a WebSocket connection.
...
>
  ...
  <TabPanel id="js" label="JavaScript">
    ...
    {/* prettier-ignore */}
    ```js
    // Available in version 2.37.0 and later.
...
This feature is currently in Public Alpha. [Submit a support ticket](https://supabase.help) if you have any issues.
...
It's a common use case to broadcast messages when a record is created, updated, or deleted. We provide a helper function, `realtime.broadcast_changes()`, to make this simple. For more details, check out the [Subscribing to Database Changes](/docs/guides/realtime/subscribing-to-database-changes) guide.
