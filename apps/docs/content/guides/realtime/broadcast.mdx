...
Broadcasting from database allows you to trigger messages from your database. To achieve it Realtime is directly reading your WAL (Write Append Log) file using a publication against the `realtime.messages` table so whenever a new insert happens a message is sent to connected users.
...
- `realtime.broadcast_changes` uses `realtime.send` but sets the payload to emit a database change to clients. This helps you set up triggers on your tables to emit changes.

### Broadcasting a JSON payload from your database
...
Realtime Authorization is required when using `realtime.broadcast_changes`. To allow your users to listen to messages from topics, create a RLS (Row Level Security) policy:
...
First, set up a trigger function that uses `realtime.broadcast_changes`. The event is set up to include data on the schema, table, operation, and field changes that triggered it.

For this example, we want to have a topic with the name `topic:<record id>` to which we're going to broadcast events.
