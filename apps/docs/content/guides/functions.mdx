...
{/\* <div className="video-container mb-8">
  ...
  <iframe
    ...
    title="Edge Functions behind the scenes"
...
*/}
<div className="video-container mb-8 w-1/2 mx-auto">
  <iframe
    className="w-full"
    src="https://www.youtube-nocookie.com/embed/za_loEtS4gs"
    title="Edge Functions behind the scenes"
...
- **Deno edge runtime executes your function** — the function runs on a regionally-distributed Deno node closest to the user for minimal latency.
...
- **Runtime:** Deno (TypeScript-first). Functions are simple `.ts` files that export a handler.
- **Local dev parity:** `supabase functions serve` provides a local runtime similar to production for faster iteration.
- **Global deployment:** `supabase functions deploy` publishes functions to the edge network.
...
- **Database connections:** treat Postgres like a remote, pooled service — use connection pools or serverless-friendly drivers. See `/guides/functions/connect-to-postgres`.
...
- Small AI inference tasks or orchestrating calls to external ML APIs.
- Transformations, proxying, and facade endpoints for third-party services.

## Best practices

- Avoid long CPU-bound work; keep invocations short.
- Reuse clients where possible but design for ephemeral execution contexts.
- Prefer streaming or incremental responses for longer operations.
- Use proper retry and idempotency for webhook handlers.
- Monitor logs and set up error reporting (Sentry, etc.) for production observability.
