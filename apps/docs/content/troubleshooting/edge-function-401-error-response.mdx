...
summary = "A 401 occurs when the platform rejects a missing or invalid authorization token, or when the function code returns 401."
...
Run this query in [Logs Explorer](/dashboard/project/_/logs/explorer) to distinguish 401 responses returned by your code from requests rejected before execution:
...
select
  ...
  case
    ...
    else 'platform_auth_check_failed'
  end as cause
from logs
...
| `platform_auth_check_failed` | [Built-in JWT check failures](#built-in-jwt-check-failures)   |
