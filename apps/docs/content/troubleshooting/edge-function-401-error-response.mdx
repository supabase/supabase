...
By default, edge functions are configured to check requests for a valid [legacy key](https://supabase.com/docs/guides/api/api-keys#overview).
...
The JWT check provides minimal security benefits, so we now recommend handling authentication through app logic instead. See the [Edge Function Auth Doc](https://supabase.com/docs/guides/functions/auth) for details.

If you've migrated to [asymmetric keys (publishable/secret)](https://supabase.com/dashboard/project/_/settings/api-keys) or no longer need the JWT check, you can disable it using one of the three options below:
...
>
  <div className="border-b mt-3 pb-3">
    <AccordionItem
      ...
      id="item-1"
    >
        In the [Function Dashboard](https://supabase.com/dashboard/project/_/functions/), under the offending function's detail tab, you can toggle off the check:
  ...
  <div className="border-b mt-3 pb-3">
    ...
    1. Generate a secret token in the [Account Preferences Dashboard](https://supabase.com/dashboard/account/tokens)
    2. Get your project ID from the [General Settings](https://supabase.com/dashboard/project/_/settings/general)
    3. Substitute in the `PROJECT_ID`, `FUNCTION_NAME`, and `SECRET_TOKEN` in below [management endpoint](https://supabase.com/docs/reference/api/v1-update-a-function). Then execute the request from a terminal environment
...
—data '""'
    ...
    <AccordionItem
      ...
      id="item-3"
    >
With the [Supabase CLI](https://supabase.com/docs/guides/local-development/cli/getting-started), you can redeploy your edge function without the JWT check:
...
If you would like to continue using the JWT check, make sure that your [Supabase Client](https://supabase.com/docs/guides/api/rest/client-libs) only uses your [legacy keys](https://supabase.com/dashboard/project/_/settings/api-keys/legacy).
