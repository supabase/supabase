...
summary = "Postgres returns `42P01` when the relation is absent, outside the search path, in an unexposed schema, or referenced with incorrect letter casing."
diagnostic_sources = [ "postgres-logs", "api-logs", "database-inspection" ]
