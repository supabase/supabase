...
A custom `raise exception` using SQLSTATE `40001` (`serialization_failure`) in your PL/pgSQL function tells PostgREST the failure is transient, so PostgREST retries the transaction. That floods your logs with duplicates and splits one API request into several independent PostgreSQL transactions.
...
To resolve this, update your PostgreSQL function to use standard exception handling or SQLSTATEs that map correctly to HTTP status codes.
