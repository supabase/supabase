...
summary = "Supabase HTTP status codes distinguish successful requests, client mistakes, service restrictions, paused projects, gateway failures, and product-specific server errors."
diagnostic_sources = [ "api-logs", "logs-explorer" ]
topics = [ "platform" ]
keywords = [ "HTTP" ]
...
The Supabase platform offers several HTTP APIs for each project. These APIs use status codes to indicate the state of the project and the request being processed. Inspect the returned status codes in [Logs](/docs/guides/monitoring-and-debugging/logs#product-logs).
...
5XX status codes can be the result of the project not having enough [compute](/docs/guides/platform/compute-and-disk) to process a complex request being made by a client or not being able to keep up with the volume of requests made against the project.
