...
summary = "A client-library query cannot express the required database operation, so the logic needs a Postgres function called through RPC."
diagnostic_sources = [ "client-tracing", "api-logs", "postgres-logs" ]
...
**Solution**
When a client library cannot express an operation directly, move the operation into a stored function in your database.
