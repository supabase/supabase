...
If you do need Presence for state that changes often, throttle the `track()` calls so the client sends at most a few updates per window. Only call `track()` when the value you're sharing changes, and coalesce bursts into a single update rather than tracking on every event.
