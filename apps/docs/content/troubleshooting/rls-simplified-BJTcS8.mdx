...
summary = "Row Level Security hides or rejects rows when a policy's `USING` or `WITH CHECK` condition does not pass for the active role."
diagnostic_sources = [ "security-advisor", "database-inspection", "postgres-logs" ]
