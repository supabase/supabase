...
summary = "SQLAlchemy connection errors and excess connections often come from using a persistent client pool in auto-scaling environments or the wrong Supavisor mode."
diagnostic_sources = [ "client-tracing", "supavisor-logs", "postgres-logs", "metrics", "database-inspection" ]
