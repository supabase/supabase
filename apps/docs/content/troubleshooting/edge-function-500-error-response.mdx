...
summary = "A 500 occurs when function code throws an unhandled JavaScript error or deliberately returns an internal-server-error response."
...
If the body contained a custom message, or nothing at all, set the relevant time range and run this query in [Logs Explorer](/dashboard/project/_/logs/explorer):
...
where runtime.source = 'function_logs'
  ...
  and toInt32OrZero(invocation.log_attributes['response.status_code']) = 500
  and runtime.log_attributes['level'] = 'error'
  and runtime.log_attributes['event_type'] in ('Log', 'UncaughtException')
  and runtime.event_message like '%Error:%file:///%'
order by function_name, runtime.timestamp desc
limit 50;
```
