...
**- Index Scan:** Uses an index to find rows. This indicates that the query is able to use an index to efficiently locate data.

**- Bitmap Heap Scan:** Uses a bitmap index to find rows and then retrieves the actual rows from the table. This type of scan is efficient when retrieving a moderate number of rows.
...
| **Index Scan** | Uses an index to find rows, efficient for small fractions of rows. |
...
| **Limit** | Returns a specified number of rows, used with `LIMIT` clause. |
...
If the total execution time of a query significantly exceeds 100 milliseconds, then this particular step may not represent the main bottleneck. For instance, in queries that take several seconds to execute, a component that consumes 100 milliseconds may not be the critical target for optimization efforts.
