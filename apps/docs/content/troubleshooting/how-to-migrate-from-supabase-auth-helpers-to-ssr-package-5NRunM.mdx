...
summary = "Deprecated Auth Helpers should be replaced with @supabase/ssr so server and browser clients use the supported cookie-based session flow."
...
Depending on your implementation, you may ignore some parts of this documentation and use your own implementation (that is, using API routes instead of Server Actions). What's important is you replace the clients provided by `auth-helpers` with the utility functions created using clients provided by `@supabase/ssr`.
