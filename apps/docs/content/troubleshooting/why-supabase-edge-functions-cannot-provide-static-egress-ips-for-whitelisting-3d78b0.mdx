...
summary = "Hosted Edge Functions do not have fixed outbound IP addresses because executions can run across a distributed, dynamically addressed edge network."
...
**Supabase Edge Functions** are serverless functions that run globally, close to your users, using Deno Deploy. They are designed for low-latency execution and adhere to "serverless" and "edge-first" principles:
