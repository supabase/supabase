...
If you reset the password of a custom role the first pooled connection can still fail with `FATAL: password authentication failed` (SQLSTATE `28P01`) even when the new password is correct. This is the pooler's credentials cache catching up, not a rejection of your credentials. Retry with the correct new password and leave a couple of seconds between attempts. The connection should go through once the cache refreshes, which is usually a single retry.
