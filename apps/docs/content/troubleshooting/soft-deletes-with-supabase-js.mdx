...
summary = "Soft deletes retain rows by marking them with a deletion timestamp and expose active rows through a query or security-invoker view."
diagnostic_sources = [ "database-inspection", "security-advisor" ]
topics = [ "database" ]
...
`security_invoker = true` makes the view use the querying user's permissions and row-level security policies. Without it, a view created by a privileged role can bypass RLS on its underlying table. Keep RLS enabled on `public.items`, and grant access to the view only to roles that should query it.
...
Implement soft deletes with Supabase and Postgres by combining a `deleted_at` column with views. This separates active and deleted records while keeping the application logic maintainable.
