...
- **Cleaner Queries:** No need to add `WHERE deleted_at IS NULL` to every query. query the view (`active_items`).
...
Soft deletes are to implement with Supabase and Postgres. By combining a `deleted_at` column with views, you can cleanly separate active and deleted records, keeping your application logic and maintainable.
