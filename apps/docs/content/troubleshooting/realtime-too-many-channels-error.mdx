...
**Common culprits: React useEffect and component re-renders**

Here's the most common mistake that causes TooManyChannels errors:
...
function ChannelDebugger() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      console.log('Channel topics:', channels.map(c => c.topic))
    }, 2000)
