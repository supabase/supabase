...
summary = "Prepared statements can fail through transaction-mode pooling because successive transactions can use different database connections."
diagnostic_sources = [ "client-tracing", "supavisor-logs", "postgres-logs", "database-inspection" ]
...
> Disable automatic use of prepared statements by passing `statement_cache_size=0` to [asyncpg.connect()](https://magicstack.github.io/asyncpg/current/api/index.html#asyncpg.connection.connect) and [asyncpg.create_pool()](https://magicstack.github.io/asyncpg/current/api/index.html#asyncpg.pool.create_pool). Do not use [Connection.prepare()](https://magicstack.github.io/asyncpg/current/api/index.html#asyncpg.connection.Connection.prepare).
