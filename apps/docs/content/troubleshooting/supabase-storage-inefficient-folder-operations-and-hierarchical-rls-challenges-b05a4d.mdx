...
summary = "Folder-scale Storage operations become expensive because object paths are flat keys and each object is authorized individually through RLS."
diagnostic_sources = [ "storage-logs", "database-inspection" ]
---
...
- **Optimize RLS policies for performance.** `JOIN`s in RLS policies can slow authorization checks on large datasets. Add indexes for columns used by policies and inspect the query plan. Do not add `SECURITY DEFINER` only to bypass a policy error; privileged functions require separate authorization checks and restricted execute permissions.
