...
summary = "RAM pressure becomes unhealthy when database or platform workloads leave too little memory for stable performance rather than using memory for normal caching."
diagnostic_sources = [ "metrics", "reports", "performance-advisor", "database-inspection" ]
topics = [ "platform" ]
