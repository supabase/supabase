...
By default, that role has `log_statement` set to `ddl`, which means PostgreSQL logs only schema-level changes such as `CREATE`, `ALTER`, and `DROP`. It does not log data-modifying statements such as `INSERT`, `UPDATE`, `DELETE` or `TRUNCATE`.
...
Setting it to `mod` tells PostgreSQL to log all data-modifying statements. Once that's in place, try running something like a `TRUNCATE` from the Dashboard. In the logs, you'll see an entry similar to:
...
That UUID corresponds to the team member who logged in via the Supabase Dashboard and executed queries in the [SQL Editor](https://supabase.com/dashboard/project/_/sql/new). But at this point, it's just an ID - not yet a name or email.
...
Generate a token from your [account settings](https://supabase.com/dashboard/account/tokens).
...
PostgreSQL supports these `log_statement` values:
