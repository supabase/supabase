...
summary = "Dashboard SQL runs as the `postgres` role, so identifying the team member requires the user metadata comment appended to logged statements."
...
A Postgres log identifies the `postgres` role and a Dashboard SQL statement, but not the team member's name or email address.
...
## Collect data modification logs
...
select
  ...
  extract(event_message, '-- user: ([0-9a-fA-F-]{36})') as dashboard_user_id,
  log_attributes['parsed.error_severity'] as error_severity,
  event_message
from logs
where source = 'postgres_logs'
  and log_attributes['parsed.user_name'] = 'postgres'
  and match(event_message, '(?i)(DELETE|TRUNCATE|UPDATE|ALTER|DROP)')
...
- Searches the time range selected in the Logs Explorer
- Filters for common data-modifying operations (`DELETE`, `TRUNCATE`, `UPDATE`, `ALTER`, `DROP`)
- Extracts the Dashboard user ID from the statement metadata comment
...
1. Run a test statement from the Dashboard SQL Editor.
2. Confirm that a new Postgres log contains `-- source: dashboard` and a user ID.
3. Match that ID to the expected organization member.
4. Reset any temporary increase in `log_statement` after collecting the required evidence.

## Escalate

Open a [support ticket](/dashboard/support/new) if the Dashboard does not append user metadata to newly logged SQL Editor statements. Include the project reference, timestamp with time zone, statement type, active `log_statement` value, and a redacted log example.
