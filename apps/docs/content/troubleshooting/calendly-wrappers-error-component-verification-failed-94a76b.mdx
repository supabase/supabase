---
title = "Calendly Wrappers error: 'component verification failed'"
...
If you are observing an `ERROR: HV000: guest fdw error: component verification failed` when querying Calendly foreign tables, it indicates that the configured Wasm package metadata does not match the downloaded component. As a security measure, the Wrappers extension blocks the module when this mismatch occurs.

To resolve this, update your foreign server configuration to use package version `0.2.0` and the correct checksum.

**How to fix the configuration:**
Run the following SQL via the [SQL Editor](/dashboard/project/_/sql/new):

```sql
ALTER SERVER example_calendly_server OPTIONS (
  SET fdw_package_url 'https://github.com/supabase/wrappers/releases/download/v0.2.0/calendly_wrapper.wasm',
...
where srvname = 'example_calendly_server';
```

Test the connection by querying your foreign table:

```sql
select * from example_calendly_table limit 1;
```
