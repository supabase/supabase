...
summary = "CPU-intensive code can retire an isolate or terminate a request after it exhausts the Edge Functions CPU budget."
...
An isolate is a worker that can handle multiple requests for a function. Its maximum lifetime is 150 seconds on the Free plan and 400 seconds on paid plans. Edge Functions use isolates with soft and hard CPU limits.
...
- It keeps going until it hits the hard CPU limit or its plan-specific wall-clock limit, whichever comes first
