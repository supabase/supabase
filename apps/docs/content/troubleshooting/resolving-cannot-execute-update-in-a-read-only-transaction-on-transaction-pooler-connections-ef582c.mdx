...
summary = "Session-level `default_transaction_read_only` state can leak through transaction pooling and make a later write use a read-only backend connection."
diagnostic_sources = [ "supavisor-logs", "postgres-logs", "database-inspection", "client-tracing" ]
