...
summary = "A privileged client loses its RLS bypass when a user session replaces the secret-key Authorization header or the request uses a different client than expected."
diagnostic_sources = [ "api-logs", "auth-logs", "database-inspection" ]
