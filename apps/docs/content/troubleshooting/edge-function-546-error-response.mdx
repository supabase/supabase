...
countIf(toInt32OrZero(log_attributes['response.status_code']) = 546) / count() \* 100
as pct_546
...
and log_attributes['request.pathname'] = '/functions/v1/FUNCTION_NAME' --<-- add your function name to inspect specific endpoints
) as fel on fl.execution_id = fel.execution_id
group by isolate_id
limit 100;
````
