...
summary = "Shutdown events distinguish normal isolate retirement from wall-clock, CPU, memory, and platform-requested termination."
...
Make your functions safe to run multiple times with the same input. Pass an application-level idempotency key with the request and store completed keys in durable storage before repeating side effects.
