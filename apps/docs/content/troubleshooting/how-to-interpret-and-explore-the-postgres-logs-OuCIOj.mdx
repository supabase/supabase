...
where
  ...
  and -- only search for connections from outside the database (excludes cron jobs)
  regexp_contains(backend_type, 'client backend')
  and -- only view successful authentication events
  regexp_contains(event_message, '^connection authenticated')
...
- [pg_audit](https://supabase.com/docs/guides/database/extensions/pgaudit)
