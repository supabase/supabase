...
| parsed.internal_query | An internal query that is used to help a primary query. Often used by realtime for certain tasks | `select to_jsonb()`             |
...
You can use the [pg_audit](/docs/guides/database/extensions/pgaudit) extension to selectively log relevant queries (not errors) by certain roles, against specific database objects.
