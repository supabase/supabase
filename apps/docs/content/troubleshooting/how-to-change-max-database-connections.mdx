...
> WARNING: Updating the max_connections configuration will restart your database, which may result in downtime while the changes are applied. For more details, refer to [managing Postgres configuration with the CLI.](/docs/guides/database/custom-postgres-config#managing-postgres-configuration-with-the-cli)
