...
Note that queries that use limit and offset will usually have to query all rows to determine order, not the limit amount so they are impacted too.
...
Never use RLS involving `auth.uid()` or `auth.jwt()` as your way to rule out 'anon' role.
