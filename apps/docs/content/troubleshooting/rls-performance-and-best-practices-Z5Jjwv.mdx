...
summary = "RLS slows queries when policies repeatedly scan unindexed columns, evaluate row-dependent expressions, or invoke joins and functions for every candidate row."
diagnostic_sources = [ "performance-advisor", "database-inspection", "api-logs" ]
