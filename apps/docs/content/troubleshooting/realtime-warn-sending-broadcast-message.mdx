...
- They all end on a past date. Your window lapsed, and the newest name is the last day a client connected.
- No rows. No client has ever connected, so no partition was ever created.

Zero rows does not mean the Realtime migrations are missing. Those would fail this query with `42P01 relation "realtime.messages" does not exist`. If you see `42P01`, please open a support ticket.
