...
summary = "Vector searches remain slow when the embedding column lacks an HNSW index that matches the query's distance operator."
diagnostic_sources = [ "database-inspection", "performance-advisor", "metrics" ]
...
> Building an index without the `CONCURRENTLY` modifier blocks writes but usually finishes sooner. `CREATE INDEX CONCURRENTLY` reduces write disruption but takes longer and uses more resources. For general advice, see [Postgres indexes](/docs/guides/database/postgres/indexes).
...
**1. Use an available pgvector version that supports HNSW.**

Check the installed version on the [Extensions page](/dashboard/project/_/database/extensions). Install available database software updates from [Infrastructure Settings](/dashboard/project/_/settings/infrastructure) before a large build when the installed release lacks a required fix.
...
HNSW indexes can produce temporary files during construction that consume additional disk space.
