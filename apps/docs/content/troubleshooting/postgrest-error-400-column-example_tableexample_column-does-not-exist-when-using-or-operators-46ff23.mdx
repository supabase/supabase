...
If you receive a `400` error with the message `column example_table.example_column does not exist` only on mutation requests (`PATCH`, `POST`, `DELETE`) — while `SELECT` queries on the same column work fine — this is a known bug in PostgREST versions prior to 14.4 ([issue #3707](https://github.com/PostgREST/postgrest/issues/3707)).
...
Navigate to [Project Settings > Infrastructure](/dashboard/project/_/settings/infrastructure) and upgrade to the latest PostgreSQL version. This automatically upgrades PostgREST to 14.5+, where the bug is resolved.
