...
summary = "PostgREST resolves unqualified objects against the first exposed schema, so objects in another schema need explicit schema selection."
diagnostic_sources = [ "api-logs", "postgres-logs", "database-inspection" ]
topics = [
"database",
"platform"
]
