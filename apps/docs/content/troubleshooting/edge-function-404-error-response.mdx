...
One cannot inspect the function dashboard to find platform 404 errors, instead, you must run the below query in the [log explorer](/dashboard/project/_/logs/explorer?its=&ite=&s=select+distinct%0A++req.pathname+as+function_name,%0A++res.status_code,%0A++CASE+%0A++++WHEN+metadata.execution_id+is+null+THEN+%27FUNCTION_NOT_FOUND%27%0A++++ELSE+%27FUNCTION+RECOGNIZED:+CUSTOM+404+MESSAGE+IN+APP+LOGIC%27%0A++END+AS+type_of_404%0Afrom%0A++function_edge_logs%0A++cross+join+UNNEST(metadata)+as+metadata%0A++cross+join+UNNEST(metadata.request)+as+req%0A++cross+join+UNNEST(metadata.response)+as+res%0Awhere+status_code+=+404%0Alimit+10;). Results will show all requests that reached Supabase but were rejected as unrecognizable.
...
select distinct
  ...
  res.status_code,
  CASE 
    WHEN metadata.execution_id is null THEN 'FUNCTION_NOT_FOUND: the function was not recognized by the platform'
    ELSE 'FUNCTION_RECOGNIZED: custom 404 message in app logic'
  END AS type_of_404
from
  function_edge_logs
