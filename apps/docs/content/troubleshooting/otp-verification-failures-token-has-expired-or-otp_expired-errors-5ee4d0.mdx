...
summary = "OTP links can appear expired immediately when an email security scanner consumes the single-use token before the user opens it."
diagnostic_sources = [ "auth-logs", "api-logs", "client-tracing" ]
topics = [ "auth", "cli" ]
keywords = []
