...
- Review the [troubleshooting doc on "Error: Failed to run sql query: Connection terminated due to connection timeout"](/docs/guides/troubleshooting/failed-to-run-sql-query-connection-terminated-due-to-connection-timeout) to determine if it could be a resource issue.
