...
summary = "Rotating a legacy JWT secret immediately invalidates legacy API keys and signed sessions, so applications must migrate or deploy replacement credentials in a coordinated sequence."
