...
B-tree with multiple attributes will perform better than several only in case the likely SELECT queries use several attributes that include the first attributes that are in the index. That is, select by 1-st, 2-nd, 3-d but not by 2-nd, 3-d and 5-th index attributes.
...
The solution is: use some king of hashing to transfer the values to much narrower space. That is, md5. The solution is, you build a functional index (=index by expression):
...
Then you must modify you SELECTs to be using the same function (otherwise the functional index will not be used in SELECT queries). That is.
...
For some datatypes other than text that allows queries by partial inclusion (that is, that the pair key-value is includes in a JSON or for implementing tsvector phrase search) you'd use GIST/GIN indexes that inherently have values space much narrower that the whole to be indexed.
