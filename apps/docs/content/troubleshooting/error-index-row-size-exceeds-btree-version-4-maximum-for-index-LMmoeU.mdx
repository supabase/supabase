...
For some datatypes other than text that allows queries by partial inclusion (i.e. that the pair key-value is includes in a JSON or for implementing tsvector phrase search) you'd use GIST/GIN indexes that inherently have values space much narrower that the whole to be indexed.
