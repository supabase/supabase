...
Supabase doesn't raise this limit on a per-project basis today. The workaround below gets around it — though the per-secret size limit below s
till applies, so it isn't a way around every constraint.
