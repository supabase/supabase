...
If some of the values you're storing are tied to individual users, organizations, or other row-level data — rather than being static, project-wide credentials — consider [Supabase Vault](/docs/guides/database/vault) instead. Vault stores encrypted secrets in Postgres and isn't subject to this 100-secret limit, but reading a Vault secret from an Edge Function requires a database round-trip (via `supabase.rpc()` to a `SECURITY DEFINER` function, or a service-role query), so it adds latency the JSON-bundling approach above doesn't have. For a flat list of global API keys, bundling is simpler.
