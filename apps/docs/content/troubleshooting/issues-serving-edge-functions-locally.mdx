...
If the conflict comes from another Supabase project, you can run both projects at the same time with the experimental `supabase stack` commands, which assign each local project its own ports. See [Running multiple local projects](/docs/guides/local-development/running-multiple-local-projects).
