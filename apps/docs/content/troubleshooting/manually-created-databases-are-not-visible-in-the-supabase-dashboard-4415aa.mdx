...
- **What is a PostgreSQL Cluster?**
  In PostgreSQL terminology, a "cluster" refers to a collection of databases managed by a single PostgreSQL server instance. This single instance can host multiple independent databases, each with its own set of tables, users, and permissions. Every Supabase project runs on top of a full PostgreSQL cluster.
- **What is a Supabase Project?**
  A Supabase project is an integrated platform that includes a dedicated PostgreSQL database, authentication services, storage, real-time capabilities, and more. Each project is configured to interact seamlessly with its primary database.
...
- **Using Manually Created Databases (with caveats):** Creating additional databases within a single Supabase project's PostgreSQL cluster (e.g., `example_database`) is technically possible and accessible via external tools. However, this approach is generally suitable only if:
  ...
  - You are comfortable managing these databases entirely through direct PostgreSQL client connections, essentially treating them as standard PostgreSQL databases within the Supabase-provided cluster, but operating outside the Supabase platform's feature set.
