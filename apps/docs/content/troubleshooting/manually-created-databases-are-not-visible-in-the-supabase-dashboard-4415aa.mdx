...
summary = "The Supabase Dashboard manages the project's default `postgres` database and does not surface additional databases created in the same cluster."
...
- **What is a Supabase Project?**
  A Supabase project includes a dedicated Postgres database, authentication services, Storage, and Realtime. Each project is configured to use its primary database.
...
- **What is PostgREST?**
  PostgREST is a standalone web server that turns your Postgres database into a REST API. It generates API endpoints from your database schema, allowing you to access data with HTTP requests without writing custom backend code. Supabase uses PostgREST for its Database API.
...
- **The Disconnect Explained:**
  ...
  - **Service Integration:** Supabase's integrated services are tightly coupled to the `postgres` database. For example, the Database API provided through PostgREST exposes schemas _from the `postgres` database_; it does not manage or expose separate databases. This design keeps service behavior consistent across a project.
