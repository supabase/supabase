...
- **What is a Postgres Cluster?**
  In Postgres terminology, a "cluster" refers to a collection of databases managed by a single PostgreSQL server instance. This single instance can host multiple independent databases, each with its own set of tables, users, and permissions. Every Supabase project runs on top of a full PostgreSQL cluster.
