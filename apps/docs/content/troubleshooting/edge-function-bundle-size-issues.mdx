...
summary = "Deployment fails when an Edge Function bundle exceeds the limit for local or server-side bundling."
...
If your function is under 20 MB but exceeds the 5 MB server-side limit, bundle it locally by running `supabase functions deploy` without the `--use-api` flag. The `--use-api` flag selects server-side bundling.
