...
summary = "Queries remain slow when their filters and joins lack useful indexes or when existing indexes do not match the access pattern."
diagnostic_sources = [ "performance-advisor", "database-inspection", "metrics" ]
...
A lot of the [queries for inspecting performance](/docs/reference/cli/supabase-inspect-db) are pre-bundled as part of the [Supabase CLI](/docs/guides/local-development/cli/getting-started). For instance, there is a command for testing which indexes of yours are unnecessary and are needlessly taking up space:
...
Index_advisor won't test indexes added through extensions nor will it test GIN/GIST indexes. For JSON or ARRAY columns, consider exploring GIN/GIST indexes separately from index_advisor. If you're using pgvector, evaluate an [HNSW index](/docs/guides/ai/vector-indexes/hnsw-indexes).
...
Some indexes may take a long time to build. See the [HNSW index troubleshooting guide](/docs/guides/troubleshooting/increase-vector-lookup-speeds-by-applying-an-hsnw-index-ohLHUM) for preparation steps that also apply to other large index builds.
