...
summary = "Overly verbose Postgres logging can generate excessive IO and disk usage, which can degrade database performance."
diagnostic_sources = [ "postgres-logs", "logs-explorer", "metrics" ]
...
Postgres logs provide detailed evidence about database activity and performance. Keep the configured scope narrow enough to avoid excessive IO and disk usage.
