...
However, because the base requirements (relevant column, filter, and operators) are present, doesn't mean that an index will be used.
...
Although a GIN/GIST index can be used to index entire JSONB bodies, you can also target specific Key-values with standard BTREE indexes:
