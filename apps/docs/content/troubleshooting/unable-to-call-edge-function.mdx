...
summary = "An Edge Function invocation can fail because of CORS handling, authentication, an incorrect URL, or network connectivity."
diagnostic_sources = [ "edge-function-logs", "client-tracing" ]
---

## Symptoms

An Edge Function invocation fails in the client, returns an authentication error, or works outside the browser but fails there with a CORS error.
...
Access these tools by opening the function in the Supabase Dashboard. Invoke the function again and confirm that **Invocations** records the expected request and response and **Logs** contains no unexpected errors.
...
Invoke the function from the affected client and confirm that it returns the expected status and response body without a browser CORS error.

## Escalate

If the request still fails after the invocation reaches the function, [open a support ticket](/dashboard/support/new) with the client error, invocation timestamp, and relevant function logs.
