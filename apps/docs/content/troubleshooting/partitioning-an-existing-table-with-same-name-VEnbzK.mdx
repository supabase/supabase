...
summary = "Converting an existing table to partitioning requires a new partitioned parent and a controlled data and name switchover."
...
- You can keep the old table while you verify the partitioned replacement. Do not delete it until the new table and dependent objects have been verified. Queries use the renamed partitioned table after the switchover.
