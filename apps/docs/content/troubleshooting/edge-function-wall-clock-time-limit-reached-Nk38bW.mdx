...
summary = "A wall-clock shutdown occurs when an Edge Function isolate reaches its plan-specific lifetime, including time spent waiting for I/O."
...
- Wall-clock limit: 150 seconds on the Free plan and 400 seconds on paid plans.
- CPU time: 2 seconds of active computing per request.
...
- Monitor execution time: Open your function in the [Supabase Dashboard](/dashboard/project/_/functions), select **Logs**, and compare slow requests with shutdown events.

- Check Our Guides: For more tips, refer to our debugging guide here: [Debugging Edge Functions](/docs/guides/functions/logging)

Hosted limits cannot be configured per project. If you need a different limit, you can [self-host Edge Functions](https://github.com/supabase/edge-runtime/) and configure the runtime.
