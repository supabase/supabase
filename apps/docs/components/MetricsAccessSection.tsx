...
export function MetricsAccessSection({ defaultOpen = true }: MetricsAccessSectionProps) {
  return (
    <section className="my-6 not-prose">
      <details
        open={defaultOpen}
        className="rounded-lg border border-foreground/10 bg-surface-100 px-5 py-4 [&_summary]:cursor-pointer [&_summary]:text-base [&_summary]:font-medium [&_summary]:text-foreground"
      >
        <summary>Access the Metrics API</summary>
        ...
        <div className="space-y-4 pt-4 text-sm text-foreground-light">
          ...
          <p>
            Testing locally is as simple as running <code>curl</code> with your service role secret:
          </p>
