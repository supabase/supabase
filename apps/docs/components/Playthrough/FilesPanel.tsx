...
export function FilesPanel() {
  const { entries, currentPath, isCurrentDirty } =
    useSubscription(entriesWatcher);
  return (
    <div className="bg-gray-900 w-56" style={{ background: "#232323" }}>
      <div className="text-gray-100 px-2 py-1 border-b text-sm border-gray-500 flex justify-between">
        <span className="">Files</span>
        <button
          className={`bg-gray-700 rounded px-2 transition-opacity ${
...
function EntryTree({ entry }: { entry: Entry }) {
  if (entry.type === "file") {
    return (
      <div className="">
        <div
          className={`pl-2 text-gray-400 hover:text-white cursor-pointer ${
    ...
    return (
      <div className="">
        <div className="text-gray-400 pl-2">{entry.name}</div>
...
const entriesWatcher = mergeWatchers(
  fileTreeWatcher,
  currentFileWatcher,
  (fileTree, file) => {
    if (!fileTree)
