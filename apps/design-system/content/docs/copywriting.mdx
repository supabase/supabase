...
Use **Sign in**, **Sign out**, and **Sign up** for button and menu labels. Use
sentence case.

| Use case                     | Standard                       | Example                    |
| ---------------------------- | ------------------------------ | -------------------------- |
| Button or menu label (verb)  | Sign in, Sign out, Sign up     | Dropdown item: "Sign out"  |
| Noun, adjective, or URL slug | sign-in, sign-out, sign-up     | `/sign-in`, "sign-in page" |
| Loading state                | Signing in..., Signing out...  | Button while submitting    |
| Avoid as UI labels           | Login, Logout, Log in, Log out |                            |
| Avoid for account creation   | Register                       | Use "Sign up" instead      |

Keep `login`, `logout`, and `logOut` in code, routes, and CLI commands when they
match implementation names.
