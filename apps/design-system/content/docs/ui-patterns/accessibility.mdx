...
Chromium-based browsers and Firefox handle this automatically via the Tab key. Safari on macOS, by default, requires the Option key to also be held down.

Enabling _Keyboard navigation_ on macOS Settings [removes this requirement](https://mayank.co/blog/safari-focus/#keyboard-navigation) but makes links non-tabbable as a result. To make things more difficult, Safari on iOS does not focus on interactive elements whatsoever.
