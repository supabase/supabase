...
The TableRow itself must be made interactive (rather than an absolutely0positioned button inside) for it to work reliably in all browsers. Consider also affordances like `ctrl` and `meta` key support for opening in a new tab. Anything that you can do with a mouse input should be replicable by keyboard.
