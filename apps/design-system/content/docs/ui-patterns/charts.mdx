...
1. **Use provided chart types first**: Always try to use the default provided charts first, otherwise passing Recharts compoennts to `<ChartContent>` is possible but not recommended. 
...
| `isLoading` | `boolean` | `false` | Shows loading state in child components |
| `isDisabled` | `boolean` | `false` | Disables chart interactions |
