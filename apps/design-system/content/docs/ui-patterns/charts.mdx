...
pink, violet, tomato, indigo, green, purple) so adjacent series never share a family. Every
adjacent pair clears colorblind and normal-vision separation in both themes; only slot 1 sits
below 3:1 against the surface, which is why charts using it always carry a legend.
