...
Series colors come from a fixed palette of eight categorical slots (`--chart-1` …
`--chart-8`) defined in `packages/config/css/charts.css`. Slots are assigned in order and never cycled — a ninth
...
`var(--chart-n)` and never branches on light/dark. Hue families alternate (brand, blue,
