...
Series colors come from a fixed palette of eight categorical slots (`--chart-series-1` …
`--chart-series-8`) defined in `packages/config/css/charts.css`. Slots are assigned in order and never cycled — a ninth
series folds into "Other", becomes small multiples, or gets a second encoding. Each slot has a matching
`-fill` token for bar and area fills.

Slots point at the Radix scales in `colors.css` and resolve per theme, so chart code passes
`var(--chart-series-n)` and never branches on light/dark. Hue families alternate (brand, blue,
pink, violet, tomato, indigo, green, purple) so adjacent series never share a family. Light mode
uses the 900 and 1000 steps; dark mode overrides every slot with the brighter 1100 tier so the
palette sits at the same lightness as brand green instead of sinking into the surface. Every
adjacent pair clears colorblind and normal-vision separation in both themes. Only slot 1 in
light mode sits below 3:1 against the surface, which is why charts using it always carry a
legend.

Reference lines and "absence of data" (headroom, idle, unused capacity) use
`--chart-reference` and `--chart-muted` / `--chart-muted-fill`. Directional series use the pinned
`--chart-in` / `--chart-out` pair (slots 2 and 5) so that read and write keep the same hue
across every chart that plots them.

Status colors — `--chart-status-success`, `--chart-status-warning`,
`--chart-status-destructive`, each with a `-muted` tier — are reserved and resolve to the
semantic `--warning` and `--destructive` tokens the rest of the UI uses. `--chart-warning` is
the alias `LogsBarChart` and the report blocks read. Never assign a status color to a series
just to get a color: amber reads as "warning", so spending it on "series 3" makes a neutral
metric look like a problem. They always ship with an icon or a label, because state must
never be carried by color alone.

No amber, orange or yellow step in the Radix scales clears the dark-mode lightness band, so
warm hues are limited to tomato (slot 5) and the reserved warning token.

<ComponentPreview name="chart-palette" wide />

### Eight series at once

The case that breaks palettes: every slot stacked on screen together. Check it in both
themes, and check that adjacent segments stay separable.
