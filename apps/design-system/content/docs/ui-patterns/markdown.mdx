...
A composable `react-markdown` component with defaults for all standard markdown elements (headings, paragraphs, lists, blockquotes, links, images, tables, code) to standardize rendering markdown content across Supabase applications. Customize any element via the `components` prop and import optional components such as Quote or Avatar.
