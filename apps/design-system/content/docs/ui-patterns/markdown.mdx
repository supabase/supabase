...
A composable `react-markdown` component with common defaults for all standard markdown elements (headings, paragraphs, lists, blockquotes, links, images, tables, code). Customize any element via the `components` prop, and import optional components such as Quote or Avatar to opt-in to enhanced functionality.
...
Override any element via the `components` prop, enable syntax highlighting, or combine multiple customizations:
