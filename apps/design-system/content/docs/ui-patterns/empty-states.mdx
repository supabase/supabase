...
**Note:** In the Studio app, use the `EmptyState` component from `components/ui/EmptyState` for consistent presentational empty states. It supports icons (both Lucide icons and custom icon components from the `icons` package), titles, descriptions, and optional action buttons or other children.
...
For presentational empty states (initial states with value propositions and actions), use the `EmptyState` component from `components/ui/EmptyState`. This component provides a consistent structure with support for icons, titles, descriptions, and action buttons.
