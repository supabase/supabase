...
Use `MultiSelectorInput`to add a seach input.
...
`badgeLimit` prop can also be "auto" to dynamically calculate the available space.

<ComponentPreview name="multi-select-badge-limit-auto" />
