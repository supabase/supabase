...
Use Admonition instead of the low-level [Alert](../components/alert) primitive unless you specifically need custom composition. If in doubt, stick with Admonition.

Use [Collapsible Alert](./collapsible-alert) when the callout needs to remain visible while optional detail can be expanded. Use [Collapsible](../components/collapsible) for generic disclosure behaviour that is not itself a callout.
...
When the primary action has related secondary actions, pair a main button with a chevron `DropdownMenu` trigger. Use this instead of crowding the callout with multiple buttons.
...
See also [Button — Split with dropdown](../components/button#split-with-dropdown) for the standalone pattern outside Admonition.
