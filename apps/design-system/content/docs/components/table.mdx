...
`TableHead` supports sorting by passing `sortable`, `column`, `currentSort`, and `onSortChange` optional props.
...
```tsx
<TableHead sortable column="name" currentSort={sort} onSortChange={handleSortChange}>
  Name
</TableHead>
```
