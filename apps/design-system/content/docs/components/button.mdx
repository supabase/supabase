...
Hover uses a translucent accent wash in both themes.

### Floating over content

If a default button is absolutely or sticky-positioned over code, tables, maps, or other busy UI, wrap it in a solid plate so nothing bleeds through:
...
Use `bg-background` or `bg-popover` to match the surrounding surface. Keep positioning and visibility on the wrapper when the control needs to stay aligned.

A dedicated `floating` prop (for example backdrop blur) may replace these wrappers later. Until then, wrap floaters at the callsite.
