...
When an action is unavailable for a non-obvious reason and needs a tooltip, use `unavailable` instead of native `disabled` so keyboard users can still focus the control. See [Disabled controls](../accessibility#disabled-controls).

### Unavailable

Use `unavailable` when the action is blocked for a non-obvious reason and you need a tooltip or other explanation. The control stays in the tab order and uses `aria-disabled` instead of native `disabled`.

```tsx
<Button unavailable>Pause project</Button>
```

In Studio, `ButtonTooltip` maps `disabled` + tooltip text to `unavailable` automatically.
