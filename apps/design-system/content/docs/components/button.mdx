...
Compact icon-only buttons may also benefit from the `hit-area-2` utility. This increases their tap target by `8px` on each side without changing the visual layout. See the [Table](./table#actions) component for more information.
