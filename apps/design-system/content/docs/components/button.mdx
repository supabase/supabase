...
### Split with pDropdown

Pair a primary button with a chevron `DropdownMenu` trigger when one action is default and the rest belong in a menu.

<ComponentPreview name="button-split-dropdown" peekCode />

Shared middle border rules:

- Primary: `rounded-r-none` and `hover:z-10`.
- Chevron trigger: `rounded-l-none`, `shrink-0`, `px-[4px] py-[5px]`, and `-ml-px` (overlap — do not use `border-l-0`).
- Chevron trigger only: `aria-label` for the menu.
