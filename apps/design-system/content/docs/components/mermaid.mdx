...
For more control (custom className, disabling zoom), import and use the component directly:
...
Use the component when you need to pass props like `className` or `zoomable`, conditionally render, or integrate with React logic.
...
| `zoomable`  | `boolean` | `true`  | Enable click-to-zoom on the diagram  |
