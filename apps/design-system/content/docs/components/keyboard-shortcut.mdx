...
- Prefer logical keys such as `Meta` instead of branching on platform yourself.
- Use `variant="inline"` for button accessories and inline copy.
- Use the default pill variant for menus, tooltips, and standalone chips.
- Keep the hint aligned with the actual registered shortcut.
