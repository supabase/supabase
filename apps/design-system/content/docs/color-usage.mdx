...
Use `text-brand` for readable branded text such as links, labels, and statuses. Reserve
`text-brand-display` for large, expressive type where the brighter green is not the only way
information is communicated. These text tokens are mode-aware and intentionally separate from
the canonical brand colour used by non-text utilities.
