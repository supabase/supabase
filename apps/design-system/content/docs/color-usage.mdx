...
Use `text-brand` for readable branded text such as links, labels, and statuses. It meets the 4.5:1
WCAG AA requirement for normal text on light-theme surfaces. Reserve `text-brand-display` for
large, expressive type. These text tokens are mode-aware and intentionally separate from the
canonical brand colour used by non-text utilities.
