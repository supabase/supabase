...
Use `text-brand` for large or non-essential branded text where colour is not the only way
information is communicated. It does not meet the 4.5:1 WCAG AA requirement for normal text in
light mode. Reserve `text-brand-display` for large, expressive type. These text tokens are
mode-aware and intentionally separate from the canonical brand colour used by non-text utilities.
