...
Supabase green has two shades: a darker green for readable UI and primary buttons, and a brighter green for brand chrome such as fills, borders, and focus rings.

### Primary green

`--primary` is the functional green. In light mode it meets WCAG AA for normal text on surfaces. In dark mode it is a deeper fill so primary buttons do not read as lime. It is also used for small selected indicators. Utilities include `text-primary`, `bg-primary`, and `border-primary`.

### Brand green

`brand-default` is the bright brand green. Use it for focus rings (`--ring` / `ring-ring`), highlight rings, and other bright fills or borders such as `bg-brand-default` and `border-brand-default`.

Toggle the theme to compare the swatches below. Both themes keep `--primary` darker than `brand-default`; light mode shows the larger gap because of text contrast requirements:

<Colors definition={'colors'} classes={['bg-primary', 'bg-brand-default']} />
...
chrome, see [Supabase green](#supabase-green).
