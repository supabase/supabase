...
Focus rings (`ring-ring` / `focus-ring`) also stay on that bright brand; they do not track the
darkened accessible `--primary` used for text.
...
`border-brand-default`. Shared focus chrome (`--ring`) and larger selected fills (sliders,
checked field cards) stay on `brand-default`. Small selected indicators that need to read
clearly on light surfaces (radio borders/dots, step dots) use accessible `--primary` /
`border-primary` instead. Keep `text-primary` / `text-brand` for readable branded text.
