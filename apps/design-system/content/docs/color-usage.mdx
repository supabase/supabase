...
In light mode, Supabase green has two shades.

`--primary` is the darker green. It meets WCAG AA for normal text on light surfaces, and it is also used for primary buttons and small selected indicators. Utilities include `text-primary`, `bg-primary`, and `border-primary`.
...
Toggle to light mode to compare the two. Dark mode barely differs between them.
