...
`border-brand-default`. Shared focus chrome (`--ring`) and selected control chrome (radio
borders, slider fills, checked field cards) are derived from `brand-default` too. Keep
`text-primary` / `text-brand` for readable branded text.
