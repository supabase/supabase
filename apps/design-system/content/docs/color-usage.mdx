...
| ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Readable ink  | `--primary`, `text-primary`, `border-primary`                                      | The colour must be _read_ or recognised at a small size: links, labels, statuses, radio borders/dots, step dots                                               |
| Bright chrome | `brand-default`, `--ring`, `bg-brand-default`, `border-brand-default`, `ring-ring` | The colour should feel on-brand and obvious as chrome: focus rings, highlight rings, larger selected fills (choice cards, slider ranges), solid brand buttons |

**Rule of thumb:** if thin 12px type in that green would fail WCAG AA on the surface, use accessible primary. If the green is identification, energy, or a ring around a control, use bright `brand-default`.

Do not point focus rings or other bright chrome at `--primary` just because text uses it. That was how light-mode focus rings went muddy after brand text was darkened for AA. Dark mode is largely the same green for both jobs; the split matters most on light surfaces.
