...
Interactive page elements should also provide visual feedback upon selection via a `focus-visible` state. We use one shared focus ring so users recognize this state instantly. The ring color comes from bright `brand-default`, not the accessible text primary. Darkening the ring with body-text green is not required for WCAG focus visibility and makes the indicator harder to spot on light surfaces. See [Brand green: two jobs](../docs/color-usage#brand-green-two-jobs).
...
- Always use the shared color (`ring-ring` / `outline-ring`). Variants (primary, danger, warning) do not change focus color. The shared color is bright brand green (`brand-default`), independent of accessible `text-primary` / `--primary`
