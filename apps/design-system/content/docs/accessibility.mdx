...
Interactive page elements should also provide visual feedback upon selection via a `focus-visible` state. We use one shared focus ring so users recognize this state instantly. Its color comes from bright `brand-default` in both themes. See [Supabase green](../docs/color-usage#supabase-green).
...
- Always use the shared color (`ring-ring` / `outline-ring`). Variants (primary, danger, warning) do not change focus color. The shared color comes from `brand-default`, independent of accessible `text-primary` / `--primary`
