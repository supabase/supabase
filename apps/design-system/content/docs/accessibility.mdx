...
Not every unavailable control should use the same disabled pattern. Choose based on whether the user needs to discover why an action is blocked.

### Native `disabled`

Use native `disabled` when the control should be fully inert and the reason is obvious from context:

- Submit buttons until a form is valid
- Loading states
- Pagination at the first or last page

Native `disabled` removes the control from the tab order, suppresses interaction, and applies browser styling. That is appropriate when no extra explanation is needed. Screen readers can still discover native `disabled` controls via browse or virtual cursor navigation and will announce their name and disabled state. Only Tab navigation skips these `disabled` controls.

<ComponentPreview name="disabled-native-form" />

### `unavailable`

Use the `unavailable` prop when an action is unavailable for a reason that is not obvious, especially when you show a tooltip explaining why:

- Permission gates
- Plan or infrastructure restrictions
- Business rules that block an otherwise visible action

`unavailable` sets `aria-disabled="true"`, keeps the control in the tab order, and applies disabled styling without `pointer-events-none`. Guard handlers are built into [Button](components/button). See also [MDN: aria-disabled](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-disabled).

Tab through the comparison below with the keyboard. Only the `unavailable` button stays focusable and exposes its tooltip.

<ComponentPreview name="disabled-comparison" />

Implementation checklist for `unavailable`:

- Prefer `unavailable` over native `disabled` when the reason is not obvious
...
<Tooltip>
  <TooltipTrigger asChild>
    <Button unavailable={unavailable}>Pause project</Button>
  </TooltipTrigger>
...
Tooltips alone are not enough for significant restrictions. Pair `unavailable` controls with visible page context (for example: an [Admonition](fragments/admonition), empty state, or inline copy) so the reason is available even without hover or focus.
