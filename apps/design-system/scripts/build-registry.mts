...
const ICONS_RAW_DIR = path.join(process.cwd(), '../../packages/icons/src/raw-icons')
const ICONS_COMPONENTS_DIR = path.join(process.cwd(), '../../packages/icons/src/icons')
...
async function buildIconsRegistry() {
  const iconNames = (await fs.readdir(ICONS_RAW_DIR))
  ...
  const entries = await Promise.all(
    iconNames.map(async (iconName) => {
      ...
      const raw = await fs.readFile(path.join(ICONS_COMPONENTS_DIR, `${iconName}.ts`), 'utf8')
      const svg = await fs.readFile(path.join(ICONS_RAW_DIR, `${iconName}.svg`), 'utf8')
  ...
  component: React.lazy(() => import(${JSON.stringify(`icons/src/icons/${iconName}`)})),
  import: ${JSON.stringify(`import { ${componentName} } from 'icons'`)},
  ...
  const index = `// @ts-nocheck
// This file is autogenerated by scripts/build-registry.mts
// Do not edit this file directly.
import * as React from "react"

export const IconsIndex: Record<string, any> = [
${entries.join(',\n')}
]
`

  rimraf.sync(path.join(process.cwd(), '__registry__/icons.tsx'))
  await fs.writeFile(path.join(process.cwd(), '__registry__/icons.tsx'), index)
}
