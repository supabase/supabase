...
export default function Stage4TimeAllocation({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {(Object.keys(FIELD_CONFIG) as TimeAllocationKey[]).map((key) => {
        const config = FIELD_CONFIG[key]
        return (
          <Panel key={key} outerClassName="w-full" innerClassName="p-4 md:p-5 flex flex-col gap-2">
            <label className="text-foreground text-sm">{config.label}</label>
            <Input
              type="number"
              size="small"
              layout="vertical"
              min={0}
              value={merged[key] == null ? '' : merged[key]}
              placeholder="Use default"
              onChange={(e: any) => {
                const raw = e.target.value
                if (raw === '') return clearOverride(key)
                return setOverride(key, Number(raw) || 0)
              }}
              onBlur={() => {
                if (merged[key] != null) {
                  setOverride(key, Math.max(0, merged[key]))
                }
              }}
            />
            <p className="text-foreground-lighter text-xs">{config.hint}</p>
            <p className="text-foreground-lighter text-xs italic">{config.examples}</p>
          </Panel>
        )
      })}
    </div>
