...
const LW13Game = forwardRef<ElementRef<'form'>, ComponentPropsWithoutRef<'form'> & Props>(
  ({ setIsGameMode, className, ...props }, ref) => {
    const gameRef = useRef<HTMLFormElement>(null)
    useImperativeHandle(ref, () => gameRef.current as HTMLFormElement)
    ...
    if (gameState === 'loading')
      return (
        <div className="relative w-full mt-[100px] md:mt-44 lg:mt-32 xl:mt-32 2xl:mt-[120px] flex flex-col items-center gap-6 text-foreground pointer-events-none">
          <svg
    ...
    return (
      <form
        ...
        className={cn(
          'flex flex-col items-center text-center gap-2 md:gap-4 w-full max-w-[200px]',
          className
        )}
        {...props}
      >
        <div className="flex flex-col items-center h-fit text-foreground-light">
          <div
            className={cn(
              'flex flex-col gap-2 opacity-0 translate-y-2 transition-all',
          ...
          <div
            className={cn(
              'flex justify-center opacity-0 translate-y-2 transition-all',
              !hasWon && 'opacity-100 translate-y-0'
            )}
          >
            Guess the word
        ...
        <div className="flex items-center justify-center gap-2 flex-wrap font-mono min-h-[100px]">
          <InputOTP
            ...
            autoFocus
            containerClassName="flex-wrap justify-center gap-y-4 max-w-screen pointer-events-none [&*]:!pointer-events-none"
            inputMode="text"
            value={value}
            spellCheck={false}
            onChange={(e) => {
              ...
              // console.log('onChange', valueAtPhraseIndex, currentVal, isMatch)
              // if (isMatch) {
              //   setValue(e)
              // }
            }}
        ...
        <div className="flex gap-4 md:gap-10 items-center text-xs text-foreground-lighter">
