...
const GitHubButton = () => {
  ...
  const kFormatter = (num: any) => {
    ...
    const showDecimal = kFormat.toString().split('.')[1].split('')[0] !== '0'
    return kFormat.toFixed(showDecimal ? 1 : 0)
  }
  ...
  return (
    <a href="https://github.com/supabase/supabase" target="_blank">
      <Button className="hidden group lg:flex" type="text">
        <span className="flex items-center gap-1">
          ...
          {repo ? kFormatter(repo?.stargazers_count) : '56'}K
        </span>
      </Button>
    </a>
  )
