...
import { ChevronLeft, ClipboardSignature } from 'lucide-react'
import { getNotionDatabase, addSlugToEvents, getNotionPageContent, fetchEventsWithBlocks, addSlugToEvent, getDatabaseWithBlocks, getDatabaseWithParsedBlocks } from '../../lib/notion'
...
export async function getStaticPaths() {
  ...
  const eventsWithSlug = addSlugToEvents(notionEvents)
  const paths = eventsWithSlug.map((event: any) => ({params: {slug: event.slug}}))
...
export const getStaticProps: GetStaticProps<EventPageProps, Params> = async ({ params }) => {
  if (params?.slug === undefined) {
    throw new Error('Missing slug for pages/event/notion-[slug].tsx')
  }

  const page_id = params.slug
  console.log("page_id", page_id)
  const notionEvents = await getNotionPageContent(page_id)

  console.log('notionEvents', notionEvents)

  const eventWithSlug = addSlugToEvent(notionEvents)
  const eventWithBlocks = await getDatabaseWithBlocks(eventWithSlug)
  console.log("eventWithBlocks", eventWithBlocks)
  const eventWithParsedBlocks = [eventWithBlocks].map(getDatabaseWithParsedBlocks)
  console.log("eventWithParsedBlocks", eventWithParsedBlocks)
  const eventContent = eventWithParsedBlocks[0].parsedBlocks
  console.log("eventContent", eventContent)
  const { data, content } = matter(eventContent) as unknown as MatterReturn
...
const EventPage = ({ event }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  // const IS_REGISTRATION_OPEN = event.onDemand || Date.parse(_event.date) > Date.now()

  // const ogImageUrl = encodeURI(
  //   `${process.env.NODE_ENV === 'development' ? 'http://127.0.0.1:54321' : 'https://obuldanrptloktxcffvn.supabase.co'}/functions/v1/og-images?site=events&eventType=${event.type}&title=${event.meta_title ?? event.title}&description=${event.meta_description ?? event.description}&date=${dayjs(_event.date).tz(event.timezone).format(`DD MMM YYYY`)}&duration=${_event.duration}`
  // )

  const meta = {
    title: `${_event.title} | ${dayjs(_event.date).format(`DD MMM YYYY`)} | ${capitalize(event.type)}`,
              ...
              <div className="h-full flex flex-col justify-between">
                <div className="flex flex-col gap-2 md:gap-3 items-start mb-8">
                  <div className="flex flex-row text-sm items-center flex-wrap">
                    ...
                    <span className="uppercase text-brand font-mono">{_event.categories ? _event.categories[0] : "event"}</span>
                    <span className="mx-3 px-3 border-x">
                      {dayjs(_event.date).tz(event.timezone).format(`DD MMM YYYY [at] hA z`)}
                    </span>
                    <span className="">Duration: {_event.duration}</span>
                  </div>

                  <h1 className="text-foreground text-3xl lg:text-4xl">{_event.title}</h1>
                  <p>{event.subtitle}</p>
                  <Button
                    type="primary"
                    size="medium"
