...
import { getTicketNumberByUserId, updateUserWithGitHubUser } from '~/lib/launch-week-ticket/db-api'
...
export default async function saveGithubToken(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { username, name } = await updateUserWithGitHubUser(body.id, body.token)
