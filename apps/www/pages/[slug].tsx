...
export async function getStaticPaths() {
  const paths = getAllPostSlugs(directory)
  return {
    paths,
    fallback: false,
  }
...
export async function getStaticProps({ params }: any) {
  ...
  const postContent = await getPostdata(filePath, directory)
  ...
  const relatedPosts = getSortedPosts(directory, 5, mdxSource.scope.tags)

  const allPosts = getSortedPosts(directory)
