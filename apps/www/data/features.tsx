...
## Requirements
Requirements depend on the destination. BigQuery and ClickHouse ReplacingMergeTree require source tables to have primary keys and require the publication to include those columns. ClickHouse updates and deletes require primary-key or full replica identity. DuckLake updates and deletes require a published primary-key identity, replica-identity index, or full identity. Snowflake updates require REPLICA IDENTITY FULL. Snowflake deletes require a published row identity.
...
BigQuery and DuckLake keep current-state tables synchronized. ClickHouse supports current-state ReplacingMergeTree tables or append-only MergeTree CDC history. Snowflake stores append-only CDC history. Source \`TRUNCATE\` operations and table resets erase accumulated destination data. Schema changes can alter historical data.`,
    icon: CloudCog,
    products: [PRODUCT_SHORTNAMES.DATABASE],
