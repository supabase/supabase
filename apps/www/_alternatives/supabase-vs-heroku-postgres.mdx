...
Migrating is surprisingly simple. You just need to use the standard Postgres `pg_dump` and `psql` tools to dump and restore from a backup. We created a handy guide for [migrating from heroku to supabase](https://supabase.com/docs/guides/migrations/heroku).
