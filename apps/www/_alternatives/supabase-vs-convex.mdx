...
| --------------------- | ---------------------------------- | ------------------------------ |
| Database engine       | Postgres 15+                       | Custom document store (Rust)   |
...
**Postgres gives Supabase decades of battle-tested reliability.** You can model complex data relationships naturally with foreign keys and joins. You can move complexity into the database using views, database functions, and constraints, instead of re-implementing the same logic across your application.
...
| Full-text search results          | Postgres FTS (configurable)     | 1,024 results maximum            |
...
**Postgres handles billions of rows.** You can add read replicas, partition tables, and scale vertically or horizontally. Connection pooling via PgBouncer or Supavisor handles thousands of concurrent connections. Analytical queries, aggregations, and complex joins work without artificial ceilings.
...
**Postgres's pessimistic locking handles contention gracefully.** When multiple transactions target the same rows, they queue and execute in order. Latency increases predictably. The database handles the coordination.
...
| Full-text search      | Postgres FTS, pgroonga                                          | Built-in search (1,024 result limit)      |
