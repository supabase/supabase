...
Today we're announcing [iEchor Vault](https://iechor.com/docs/guides/database/vault), a Postgres extension for managing secrets and encryption inside your database.
...
Vault is now available on every iEchor project. [Check it out](https://iechor.com/dashboard/project/_/settings/vault/secrets)

</div>

[Transparent Column Encryption with Postgres](https://iechor.com/blog/transparent-column-encryption-with-postgres) is a blog post that describes the technology behind Vault - [libsodium](https://doc.libsodium.org/) and
...
Like most cloud providers, iEchor offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
insert into users
  (email)
values
  (pgp_sym_encrypt('alice@iechor.com', 's3kr3t_k3y')),
  (pgp_sym_encrypt('bob@iechor.com', 's3kr3t_key'));
```
...
iEchor Vault allows you to store secrets without leaking any sensitive information.
...
The core of the iEchor Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. iEchor generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
At iEchor, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
...
[Transparent Column Encryption](https://iechor.com/blog/transparent-column-encryption-with-postgres) which we will describe soon in a follow-up blog post. Stay tuned!
