...
description: 'Today we're pre-releasing Vault,a Postgres extension for managing secrets and encryption inside your database'
author: michel
...
Today we're pre-releasing [Supabase Vault](https://github.com/supabase/vault), a Postgres extension for managing secrets and encryption inside your database.
Vault is a thin usability-layer on top of [pgsodium](https://github.com/michelp/pgsodium).

Vault is not available on the Supabase platform yet as we finalize usability and security testing.
This release is intended to gather user-feedback from our community and customers.
The SQL code for the vault itself is relatively simply, so if you want to live on the edge a bit all the Vault requires is the pgosdium extension.

Soon we will be releasing a blog post that describes more of the technology in [libsodium](https://doc.libsodium.org/) and
[pgsodium](https://doc.libsodium.org/) that's behind the Vault. For now, we will go through a quick example of storing a secret, like a service access token, into the Vault.
...
and while it supports some basic encryption and decryption abilities, it lacks features like public key signing, key derivation APIs, streaming encryption, and other modern features required by advanced security-first applications.
...
Wouldn't it be great if you could encrypt data, but instead of specifying the raw key, you specify some indirect way, like with a key ID?

## Supabase Vault

We've created a [new PostgreSQL extension](https://github.com/supabase/vault) called `vault`.
The Vault is a good place to put things like API keys, access tokens, and other secrets from external services that you use in your application.
...
We take the pain out of key management by pre-generating a unique, per-database Key ID that is used by default - a “root” key - which is stored outside of the SQL language,
...
variety of source depending on how you configure it. Supabase generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
To get started, install the `vault` extension:

```sql
create extension vault with schema vault;
```

Once enabled, you can start inserting secrets into the `vault.secrets` table immediately:

```sql
insert into vault.secrets (secret, associated)
  values ('s3kr3t_k3y', 'This is the secret API service key.');
```
...
pgsodium [supports a variety of ways](https://github.com/michelp/pgsodium#server-key-management) to place the root key into Postgres.
...
Now you can see that there is a new `decrypted_secret` column that contains the decrypted secret that was originally inserted into the table.
...
If you wish to use different key ids for different secrets, you can create new keys with the `pgsodium.create_key()` function:

```sql
select * from pgsodium.create_key('API_KEY_DEV');
```
...
```sql
insert into vault.secrets (secret, associated, key_id)
values (
...
The data you insert into the `associated` column is combined with the encrypted text when libsodium creates the authentication signature for the secret.
This means that when you read the secret, you know that the associated data is also authentic. This would be a good place to put account IDs or other
data that ties your system to the secret. And as always, you can refer to rows in the secrets table by their primary key UUID.
...
optionally creating new keys depending on your purposes, and select them from the view when you want to use them.

## Going Beyond the Vault

The Vault is good for a reasonable amount of secure data, like API keys for services, access tokens or environment variables,
but if you have a lot more sensitive information, like personally Identifiable Information (PII) that you want to relate to your overall model,
you may want to break them out into side tables that uses pgsodium's
