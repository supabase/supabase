---
title: 'Supabase Auth: Identity Linking, Hooks, and Have I Been Pwned integration'
description: Supacharging Supabase Auth with Identity Linking, Session Control, Passwords, and Hooks
launchweek: X
categories:
  - product
tags:
...
Welcome to Launch Week X Day 4! We're excited to announce four new features for Supabase Auth:
...
> For more information about how identity linking works, see the [Identity Linking docs](https://supabase.com/docs/guides/auth/auth-identity-linking).
...
> For more information, see the [Session Management docs](https://supabase.com/docs/guides/auth/sessions).

## Leaked Password Protection

Passwords often lack security due to user habits like weak choices or reuse across several sites. 
...
> Check out the [password docs](https://supabase.com/docs/guides/auth/passwords) for more information.
...
Then once you’ve created the function in the database, you only need to register it with Supabase Auth:

IMAGE GOES HERE

Initially, you can register an Auth Hook for the following points in the flow:

- **Custom access token:** called each time a new JWT is generated.
- **MFA verification attempt**: called each time an MFA factor is verified, allowing you finer control over detecting and blocking attempts.
- **Password verification attempt**: called each time a password is used to sign-in a user, allowing you finer control over the security of the user’s accounts.

Auth Hooks is available today for self-hosting and will be rolling out to the platform next month. Reach out to us via [support](https://supabase.help) if you need access sooner!
