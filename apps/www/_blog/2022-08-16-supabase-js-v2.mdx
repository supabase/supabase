...
description: Today we're releasing supabase-js v2, which focuses on quality-of-life improvements for developers.
author: inian
...
Today we're releasing [supabase-js v2](https://github.com/supabase/supabase-js), which focuses on “quality-of-life” improvements for developers.

Update your library by running `npm i @supabase/supabase-js@latest`.
...
```tsx
import Definitions from './DatabaseDefinitions'

const supabase = createClient<Definitions>(SUPABASE_URL, ANON_KEY)
...
## Updated Realtime API

There are a few changes to our Realtime interface:
...
There is a new `channel()` function which requires a string to denote the name of a channel.
While this may seem more verbose, it's an important change for our upcoming [multiplayer](https://supabase.com/blog/supabase-realtime-with-multiplayer-features) features.
...
Update today by running `npm i @supabase/supabase-js@latest`.
