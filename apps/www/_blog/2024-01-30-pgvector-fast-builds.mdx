...
pgvector 0.6.0 was released today, with a significant improvement: parallel index builds for HNSW. Building an HNSW index is now up to 30x faster for unlogged tables.
...
With parallel index builds you can build an index on 1 million vectors of 1536 dimensions in 9.5 minutes (with `'m'=16, 'ef_construction'=200`) - 9 times faster.
...
pgvector 0.6.0 was [just released](https://github.com/pgvector/pgvector/releases/tag/v0.6.0) and will soon be available for Supabase projects! Again, a special shout out to Andrew Kane and everyone else who [worked on parallel index builds](https://github.com/pgvector/pgvector/issues/409).
