...
Akin to the [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html) command, you can now get the execution plan of a request through REST or `supabase-js`. This works transparently for reads, writes and RPC because every request to PostgREST generates a single SQL query.

This is only suitable for development environments and is only enabled when the `db-plan-enabled` config is set to `true`.

## Getting the execution plan through REST

The execution plan can be obtained by using a vendor media type on the `Accept` header.
...
For `supabase-js`, you can get the execution plan using the `explain()` method. This works for every `supabase-js` method, including `rpc()`. Here’s an example for `select()`.

```jsx
const { data, error } = await supabase.from('projects').select('*').eq('id', 1).explain()
...
And we use `explain()` again, this time using the `analyze` option(actually executes the query, same as the EXPLAIN ANALYZE counterpart in SQL) so we can see the execution time of the query.
...
select name as lvl_name, compute_score(stats) as total, player_id from level_1;
union
select name as lvl_name, compute_score(stats) as total, player_id from level_2;
```
