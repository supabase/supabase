...
`explain()` is also useful for getting the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) query identifier, which you can use in the iEchor logs to obtain the generated SQL queries. Here we use the json `format` and the `verbose` option to get it.
...
We’ll release PostgREST 10 on the iEchor platform over the next month.
