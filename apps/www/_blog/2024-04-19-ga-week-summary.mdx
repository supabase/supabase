...
Supabase Bootstrap is the fastest to spin up a new hosted Supabase project from existing starter templates. Just run `supabase bootstrap` with our CLI and we’ll help you launch a new application and attach a remote database to get you started.
...
There’s been a tonne of other stuff worth highlighting this week:

### Supabase + Fly updates

Last Launch Week we announce [Fly Postgres, a managed offering from Supabase](https://supabase.com/blog/postgres-on-fly-by-supabase). We’ve had a tonne of feedback on this, and we’re working hard to make the service available and as resilient for production workloads.

Today we’re opening up access to everyone **for testing**. We have also enabled Branching, which creates an ephemeral test environment for every git branch.

<aside>
ℹ️ The service is still in alpha. We’re still in the feedback stage and we don’t recommend using it for production.
