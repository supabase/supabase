---
title: JSON Schema support for JSON/B Columns
description: '...'
author: oli_rice
...
# Supabase: JSON Schema support for JSON/B Columns

## TL;DR

Released on the [Supabase](https://supabase.com/) platform today, [supabase/pg_jsonschema](https://github.com/supabase/pg_jsonschema) is a Postgres extension adding [JSON Schema](https://json-schema.org/) validation support for `json` and `jsonb` data types.
...
![Table 1](/images/blog/lw5-pg_jsonschema/table-1.png)

| SQL input                       | Inserted | metadata column value (json) | note                                |
| ------------------------------- | -------- | ---------------------------- | ----------------------------------- |
| '{"foo": "bar"}'                | Yes      | {"foo": "bar"}               | expected payload                    |
| null                            | No       |                              | sql null                            |
| ‘null’                          | Yes      | null                         | json null                           |
| to_json('null'::text)           | Yes      | “null”                       | json string containing value “null” |
| to_json('{"foo": "bar"}'::text) | Yes      | "{\"foo\": \"bar\"}”         | object serialized as a string       |
| '{"fov": "bar"}'                | Yes      | {"fov": "bar"}               | object with typo “fov” vs “foo”     |
| '{"foo": 5}'                    | Yes      | {"foo": 5}                   | attribute “foo” has wrong type      |
| '{"foo": "bar", “x”: 1}'        | Yes      | {"foo": "bar", “x”: 1}       | extra attribute “x”                 |
|                                 |          |                              |                                     |

Only 2 of our 8 test cases match our target data model.
...
```json
# objects with a string attribute named foo and no additional attributes
{
    "type": "object",
    "properties": {
        "foo": {
...
[supabase/pg_jsonschema](https://github.com/supabase/pg_jsonschema) is a Postgres extension that can validate `json` and `jsonb` data types against a [JSON Schema](https://json-schema.org/). The extension offers two functions:
...
![Table 2](/images/blog/lw5-pg_jsonschema/table-2.png)

| SQL input                       | Inserted | metadata column value  | note                                |
| ------------------------------- | -------- | ---------------------- | ----------------------------------- |
| '{"foo": "bar"}'                | Yes      | {"foo": "bar"}         | expected payload                    |
| null                            | No       |                        | sql null                            |
| ‘null’                          | No       | null                   | json null                           |
| to_json('null'::text)           | No       | “null”                 | json string containing value “null” |
| to_json('{"foo": "bar"}'::text) | No       | "{\"foo\": \"bar\"}”   | object serialized as a string       |
| '{"fov": "bar"}'                | No       | {"fov": "bar"}         | object with typo “fov” vs “foo”     |
| '{"foo": 5}'                    | No       | {"foo": 5}             | attribute “foo” has wrong type      |
| '{"foo": "bar", “x”: 1}'        | No       | {"foo": "bar", “x”: 1} | extra attribute “x”                 |
