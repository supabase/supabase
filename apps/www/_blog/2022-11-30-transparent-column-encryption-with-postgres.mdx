...
> We're using “credit cards” in our examples below, because they are very easy to understand. Please don't store credit cards in your Supabase database - we are not a certified PCI Service Provider.
...
- Add Stripe webhook_secret_ XXXYYYY
- Receive a payload from Stripe
- Get signing key out of vault
- Authenticate the signature - HMAC
- Store the payload into your database

```sql
create function receive_stripe_event(payload jsonb)
language pgpsql
returns jsonb
begin
  # 1. Get the 
	webhook_secret := select decrypted_secret from vault.decrypted_secrets where associated = 'STRIPE_WEBHOOK'

  # 2. Use webhook secret to authenticate the signature
  authenticated :=  authenticate_hmac_signature(payload, webhook_secret)

	# 3. Throw if not auth
  if (!authenticated) raise error

  # 3. Store the payload
  insert into stripe.events (payload)
  values (validated);
end;
```
