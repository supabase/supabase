...
<iframe width="560" height="315" src="https://www.youtube.com/embed/lNgsIITU3pc?si=X6KE7X819Hk_yMT0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
...
The Management API generates a new access token for your account. To send it back to your CLI it needs to be stored in a database.

## Token encryption with Elliptic-Curve Diffie-Hellman
...
Meanwhile, the CLI continues its role in the process. It periodically polls our public API for the specific session ID it generated. Upon a successful poll, it extracts the encrypted token from the response, uses the server's public key, and this time, employing its private key pair, decrypts the previously encrypted token, securely storing it in your operating system's vault.
