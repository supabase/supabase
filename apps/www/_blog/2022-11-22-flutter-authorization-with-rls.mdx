...
We will take a look at more specific examples of authorization using RLS through out this article.
...
We will procede with defining the states for the rooms page. Create `lib/cubit/rooms/rooms_state.dart` file and paste the following code.
...
At this point, we seemingly have a complete app, except if we open the app right now, we will see every user's room with all the messages that have ever been sent within the app by any user.
This is because we have not set up Row Level Security to prevent unauthorized users to access the rooms yet.
There are two ways we can define Row Level Security policies in Supabase, with the GUI or through SQL, but we will set them using SQL today.
