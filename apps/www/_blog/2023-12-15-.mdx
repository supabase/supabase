---
title: 'Supabase Libraries: Python, Swift, Kotlin, Flutter, and Typescript'
...
Heres what you you need to know:
...
All of the libraries mentioned in this post are now on v2. This is because we want the API for each library to “move in lock step”. You should be able to jump around each of the client libraries and they should operate the same, barring any language idiosyncrasies. 
...
<aside>
ℹ️ Don’t forget, we also have a [Python library for working with embeddings](https://supabase.com/docs/guides/ai/python/api)!
...
- [Getting started with Supabase on Android](https://www.youtube.com/watch?v=QaLuM6Uwy4U) (Video)
- Demos:
...
The core theme of Flutter v2 has been stability and better DX. We want to give a shout-out to [Vinzent](https://github.com/Vinzent03), a community maintainer who has done the majority of the work. Some notable improvements:
...
We just reached 1000 contributors to our [main repo](https://github.com/supabase/supabase). It’s not easy to build a community of contributors, it’s something that need to be fostered.

The client libs are one of the best ways to foster the community because they lower complexity than some of the tools we maintain (do you know [Haskell](https://github.com/PostgREST/postgrest)?).

<Img
  src={{
    dark: '/images/blog/lwx-supavisor/community.png',
    light: '/images/blog/lwx-supavisor/community.png',
  }}
  alt="Diagram explaining Supabase community"
/>

We want the Supabase community to outlive us. With more community maintainers, you should feel *even safer* knowing that there is already a continuity plan in place. We’re [giving back to the community](https://supabase.com/blog/supabase-series-b#giving-back) as those contributions grow, and we hope to expand this as we become even more commercially successful.

### Modularity

As a [Principle](https://supabase.com/docs/guides/getting-started/architecture#product-principles), we develop a single library for each tool we support. While Supabase may “feel” like a single tool when you’re using it, it is actually a set of individual tools which you can use independently (especially useful for self hosting):

<Img
  src={{
    dark: '/images/blog/lwx-supavisor/modularity.png',
    light: '/images/blog/lwx-supavisor/modularity.png',
  }}
  alt="Diagram explaining how Supabase is built behind the scenes"
/>

We have libraries for each of the middleware components. For example, `supabase-js` is simply a wrapper around `postgres-js`, `storage-js`, etc. If you want to self-host PostgREST with your own database, it should feel very familiar:
...
- Each language has it’s own idiosyncrasies. Developers using generated libraries often find themselves writing code that feels unnatural in their chosen language.
...
If you want to become a maintainer, please just get started with PRs. If, after a few PRs, you enjoy the process, ping one of the team on Discord and let us know - we’ll work with you to become a community maintainer.
