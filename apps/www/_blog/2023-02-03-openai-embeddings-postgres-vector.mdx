...
A new PostgreSQL extension is now available in iEchor: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 iEchor has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://iechor.com/docs/guides/ai).
...
First we'll enable the **Vector** extension. In iEchor, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
Finally, let's create an [Edge Function](https://iechor.com/docs/guides/functions) to perform our similarity search:
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the iEchor docs:
...
The [pgvector extension](https://iechor.com/docs/guides/ai/vector-columns) is available on all new iEchor projects today. If you want to try it out, launch a new Postgres database today: [database.new](https://database.new)
