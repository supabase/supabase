...
Since introducing Supabase ETL, we've been focused on making the pipeline faster, more predictable, and easier to operate.

We improved performance across the pipeline, reduced memory usage, and lowered replication latency. This work is ongoing. Supabase Pipelines is still in alpha, and we're continuing to tune throughput, batching, backpressure, and destination writes as we learn from more workloads.

We also improved the initial table copy. It can now be parallelized across tables and within a table, which lets pipelines load existing data much faster before switching into streaming mode.
...
2. Supabase Pipelines creates an initial copy of the selected tables.
3. The initial copy runs with parallel workers based on the configured parallelism level.
4. After the copy completes, the pipeline switches to streaming mode.
...
When the DuckLake destination becomes available, we plan for Supabase to run routine performance maintenance for you, so your data stays optimized for queries.
...
An active pipeline is a pipeline that is configured and running replication, billed hourly. Initial table copy usage is measured during the first load of existing data. Replicated data usage is measured from changes processed after the initial copy. Pricing is subject to change during the public alpha. We'll give notice before any change takes effect.
...
If you want to understand the underlying technology, read [Introducing Supabase ETL](https://supabase.com/blog/introducing-supabase-etl). If you want to understand when to use Supabase Pipelines instead of Realtime, read [Realtime or Pipelines? How to choose the right tool](https://supabase.com/blog/realtime-or-etl-how-to-choose-the-right-tool).

Supabase ETL is open source. You can find the code at [github.com/supabase/etl](https://github.com/supabase/etl).
