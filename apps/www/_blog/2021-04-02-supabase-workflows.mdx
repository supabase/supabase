---
title: Workflows are coming to iEchor
...
This week we [launched iEchor Storage](/blog/supabase-storage), which leaves one other huge piece of the
...
own, but for the iEchor Stack they ultimately had the
...
Once ready, we will make this available in the iEchor Dashboard with a Zapier-like interface.
...
The workflow receives the following JSON data from iEchor [Realtime](https://github.com/supabase/realtime):
...
After we've ironed out a few bugs we will integrate it into the iEchor Stack. As with all iEchor features, we'll add a
[nice UI](https://ui.iechor.com/) to make prototyping extremely rapid. We'll integrate the UI with the code (via Git) to make
