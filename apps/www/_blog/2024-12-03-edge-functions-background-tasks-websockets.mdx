...
In the past couple of months, we have made many [performance, stability,](https://supabase.com/blog/edge-functions-faster-smaller) and [DX improvements](https://github.com/orgs/supabase/discussions/30307) to Edge Functions.

Built on those bases, we are excited to announce three long-awaited features: Background Tasks, Ephemeral File Storage, and WebSockets.

Starting today, you can use these features in any project. Let’s explore what exciting things you can build with them.

## Background Tasks

Sometimes, you want to use Edge Functions to do more than respond to a request. For example, you may want it to process a bunch of files and upload the results to Storage or read multiple entries from a database table and generate embeddings for them. With the introduction of background tasks, executing these long-running workloads with Edge Functions is super easy.
...
You can get notified when the function is about to be shutdown by listening to `beforeunload` event. Read the guide for more details on [how to use background tasks](https://supabase.com/docs/guides/functions/background-tasks).
...
### Example: Extracting a zip file and uploading its content to Storage
...
We can modify it instead to write the zip file to a temporary file. Then, use a background task to extract and upload it to Storage. This way we only read parts of the zip file to the memory.
...
OpenAI recently introduced a [Realtime API](https://openai.com/index/introducing-the-realtime-api/), which uses WebSockets. This is tricky to implement purely client-side because you’d have to expose your OpenAI key publicly and OpenAI [recommends](https://platform.openai.com/docs/guides/realtime/overview?text-generation-quickstart-example=audio) building a server to authenticate requests.

With Supabase Edge functions support for WebSockets, you can easily do this without standing up any infrastructure. Additionally, you can use [Supabase Auth](https://supabase.com/docs/guides/auth) to authenticate users making the requests.
