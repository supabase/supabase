...
![database.build v2 BYO-LLM](/images/blog/launch-week-13/day-5-database-build-v2/byo-llm.png)

From here, pass in your LLM provider’s base URL, your associated API key, and the model you wish to use.

![database.build v2 BYO-LLM API key](/images/blog/launch-week-13/day-5-database-build-v2/byo-llm-api-key.png)
...
This possible thanks to two important pieces:
...
![database.build v2 BYO-LLM API key](/images/blog/launch-week-13/day-5-database-build-v2/llm-service-worker-dark.png)
...
Getting deployments to work took quite a bit more engineering than it might seem at first glance. To copy your in-browser database to the cloud accurately, we needed a way to dump and restore. ElectricSQL has been hard at work on a WASM version of `pg_dump` that can run directly in the browser—a mind-blowing feat in its own right—but unfortunately, it wasn’t quite ready in time for this launch.

In the meantime, we decided to piggyback on top of our existing Live Share feature, which creates a reverse connection between your in-browser database and an external PostgreSQL client. With Live Share enabled, we connect a server-side `pg_dump` program directly to your in-browser database. The resulting dump is then piped into `pg_restore`, which transfers it to your new cloud database.

![database.build v2 Deploy to Supabase](/images/blog/launch-week-13/day-5-database-build-v2/deploy-to-supabase-dark.png)

Yes, it’s a lot of moving parts! But this approach let us deliver cloud deployments today while we wait for WASM `pg_dump` to be production-ready. Kudos to the ElectricSQL team for continually breaking new ground with embeddable Postgres. We can’t wait to simplify this process even further in the future.
...
We’re excited to share that database.build now works on mobile! Whether you’re on the train, stuck in a meeting, or lounging on the couch, you can spin up databases and tweak your schema with just a few taps.

![database.build v2 mobile support](/images/blog/launch-week-13/day-5-database-build-v2/mobile-support.png)
