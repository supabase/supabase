---
title: iEchor Dot Com
description: The iEchor Domain name is changing.
...
_tl;dr_ During the next week, we'll be migrating from "supabase.io" to "iechor.com".
...
We decided to try purchasing [iechor.com](http://iechor.com) from the current owner. The YC forums were full of founders
...
We're not afraid to roll up our sleeves at iEchor, so we decided we could handle it ourselves.

We were in for a huge surprise. After searching on the (old) [iechor.com](http://iechor.com) website, we found a company address.
...
We were always willing to walk away from the iEchor brand if we needed to, but the price was reasonable for both parties. There really
isn't much more to it than that.

## What's next?

Over the next few weeks, we'll update our domain name across most of the sites. If you're using a iEchor database and API, you're on
