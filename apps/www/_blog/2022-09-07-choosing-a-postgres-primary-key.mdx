...
SELECT * FROM users;
 ...
 id |      email       |   name
----+------------------+----------
  1 | new@example.com  | new_user
  2 | new2@example.com | new_user
(2 rows)
```
...
```sql
INSERT INTO users_serial (email, name) VALUES ('user@example.com', 'new user');
...
[Postgres 10 added support for the `IDENTITY` column](https://www.postgresql.org/docs/10/release-10.html#id-1.11.6.26.5.7) syntax in `CREATE TABLE` ([EDB has a great writeup on the addition](https://www.enterprisedb.com/blog/postgresql-10-identity-columns-explained)).
...
Along comes UUIDs -- you're probably used to seeing them now, values like `468e8075-5815-4fe2-80d3-45a31827954b` (this is a UUIDv4).
...
A cambrian explosion of UUIDs resulted, as noticed by the IETF -- this resulted in [the new UUID formats (v6,v7,v8) being published in 2021](https://www.ietf.org/archive/id/draft-peabody-dispatch-new-uuid-format-01.html).
...
```
| ID type                           | Runtime (ms) | Notes (if present)         |
|-----------------------------------|--------------|----------------------------|
| `serial` (baseline)               | `511.925`    |                            |
| `xid`                             | `1017.814`   |                            |
| `ulid`                            | `2816.970`   |                            |
| `timeflake`                       | `2835.145`   |                            |
| `uuid_v6`                         | `3117.796`   |                            |
| `uuid_v7`                         | `3374.221`   |                            |
| `gen_random_uuid` (UUID baseline) | `3886.347`   |                            |
| `ksuid`                           | `4386.010`   | CPU fans go whirr          |
| `cuid`                            | `8446.882`   | CPU fans go whirr          |
| `nanoid`                          | `19718.094`  | CPU fans go whirr          |
| `pushid`                          | `45757.820`  | suboptimal implementation? |
```
...
```
| Table name          | Table size (mb) | Index size (mb) | Total size (mb) |
|---------------------|-----------------|-----------------|-----------------|
| baseline            | `89`            | `39`            | `110`           |
| benchmark_xid       | `104`           | `39`            | `143`           |
| benchmark_nanoid    | `104`           | `39`            | `143`           |
| benchmark_pushid    | `104`           | `39`            | `143`           |
| benchmark_ulid      | `112`           | `47`            | `159`           |
| benchmark_cuid      | `112`           | `47`            | `159`           |
| benchmark_uuidv7    | `120`           | `56`            | `177`           |
| benchmark_uuidv6    | `120`           | `56`            | `177`           |
| benchmark_timeflake | `120`           | `56`            | `177`           |
| benchmark_ksuid     | `128`           | `65`            | `177`           |
```
