...
This makes sense - AI companies typically generate a lot of logs and and analytical data, which is better suited for an OLAP database like ClickHouse.

<Quote img="lee-robinson.png" caption="Justin Torre, CTO @ Helicone.ai">
...
<Quote img="lee-robinson.png" caption="Aaron Katz, CEO @ ClickHouse Inc.">
...
### Better Together: Supabase + ClickHouse
...
```sql
 create foreign table user_analytics (
  ...
  _event text,
   ...
   server clickhouse_server
   options (
     table '(select * from UserAnalytics(event=${_event}))'
   );
...
<div>
  <Img
    ...
    zoomable={false}
    src={{
      light: '/images/blog/clickhouse-supabase/diagram--light.png',
      dark: '/images/blog/clickhouse-supabase/diagram--dark.png',
    }}
  />
</div>

This is driven directly by customers using tools like PeerDB. With adjustable WAL configuration, it's important that developers can manage the disk as well.

For example, on the Pro Plan's 8GB disk, you can configure your project with options like:

- Default: 7GB database space, 1GB Write Ahead Log
- Custom example: 6GB database space, 2 GB Write Ahead Log

Additionally, we're introducing High-performance Disks. High-performance Disks. We'll release more details about this later.

## What's next?

Improving the experience between Postgres and ClickHouse is the first phase of this partnership. Over the coming months we'll release platform integrations. If you're already using Supabase and ClickHouse together please [reach out](https://supabase.com/enterprise) so that you can be a design partner for this integration.
...
If you simply want to try out the tools and updates we've described above, you can get started with all of then, free of charge:
