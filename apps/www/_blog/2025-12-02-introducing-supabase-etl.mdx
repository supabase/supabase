...
Click `Add Destination` and choose your destination type. _For Analytics Buckets, you will need to create an analytics bucket first in the Storage section._
...
For example, a source table `public.users` maps to a BigQuery view called `public_users`. Behind the scenes, this view points to a versioned table (`public_users_[version]`), but you don't need to think about that, just query the view, and ETL handles version management for you.
...
After changing your publication, restart the pipeline from the Dashboard actions menu for the changes to take effect. Note that ETL does not remove data from your destination when you remove a table from a publication, this is by design.

## When to use ETL vs read replicas

Read replicas and ETL solve different problems.

Read replicas distribute query load across multiple Postgres instances. They help when concurrent connections compete for resources, but they're still Postgres, not optimized for analytical workloads like aggregations over large datasets.

ETL moves data to systems designed for analytics. Columnar storage enables fast scans over millions of rows, compresses well for lower storage costs, and outputs open formats that any tool can query. It also separates your analytical workload from your transactional database entirely.
