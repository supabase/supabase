...
Before choosing a scaling strategy, figure out what is actually causing the slowdown. Throwing hardware at the wrong problem wastes money. The [database performance guide](/docs/guides/database/debugging-performance) covers diagnostics in depth. Here are the essentials.
...
Supabase offers [compute tiers](/docs/guides/platform/compute-and-disk) from Micro ($10/month) to 16XL ($3,730/month). If you are on Medium and experiencing slowdowns, upgrading to Large or XL is straightforward and immediate.
...
Run `EXPLAIN ANALYZE` on slow queries to see if Postgres is using indexes or doing sequential scans. If you see "Seq Scan" on a large table, you probably need an index. The [Database Advisor](/docs/guides/database/database-advisors) in your Supabase Dashboard can also identify missing indexes and other performance issues automatically.
...
[Read Replicas](/blog/introducing-read-replicas) add complexity but unlock capabilities that vertical scaling cannot provide. Choose Read Replicas when:
...
Read Replicas provide a warm standby. If your primary has issues, your replica has a recent copy of your data. This is not automatic failover (that is coming with [Multigres](/blog/multigres-vitess-for-postgres)), but it reduces your blast radius.
...
**Recommendation:** Upgrade to 8XL ($1,870/month). Write-heavy workloads need a bigger primary. Read Replicas would only help with the 40% that is reads. Consider [Supabase Queues](/docs/guides/queues) to batch and process writes asynchronously.
...
This is where [Supabase Pipelines](/blog/introducing-supabase-pipelines) comes in. It solves a different problem: not just isolating analytics, but moving data into infrastructure designed for analytical workloads.

**How it works:**

1. Pipelines captures changes from your Postgres tables using change-data-capture (CDC)
2. Changes stream in near real time to a supported analytical destination
3. The destination stores the data for large scans and aggregations
4. You query the data with BI tools or downstream analytics workflows

Analytical systems are optimized for large scans and aggregations. They keep heavy reporting workloads away from your production database and give data teams a better place to run long-running queries.

**When to use Pipelines instead of Read Replicas:**
...
- You need a complete downstream copy for reporting, audit trails, or data warehousing
- Your data team wants to use dedicated analytics and BI tools
...
- You also need geo-distribution, which Pipelines does not provide
...
Read the full setup guide in [the documentation](/docs/guides/platform/read-replicas).
