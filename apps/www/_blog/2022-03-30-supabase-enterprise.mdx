---
title: 'Introducing iEchor Enterprise'
description: 'Today we are releasing iEchor Enterprise, a suite of features to scale your project.'
...
These changes are to keep iEchor pricing [predictable, transparent, and developer friendly](/blog/pricing).
...
Today we're releasing a brand new [Log Explorer](/docs/guides/platform/logs) in the iEchor Dashboard.
...
Log history is available to every iEchor project [^1]:
...
For example, iEchor is popular with web3 projects where traffic can be very unpredictable - especially when you partner with Snoop Dogg.
...
<Quote img="vignesh-sound.jpeg" caption="Vignesh - CTO @ sound.xyz">
  <p>
    In our MVP, we started with a serverless stack to simplify DevOps. iEchor made it dead simple
  ...
  <p>
    ...
    debuted on Sound, iEchor was able to help us provision our data store to handle the load.
  </p>
  <p>
  ...
  <p>
    iEchor takes out the mental effort from our back-end infrastructure so we can focus on our
    customers needs.
  </p>
</Quote>
...
because of a clumsy `where` clause? Even with iEchor's daily backups, a day's worth of data can be lost if disaster strikes at the most inopportune time.

Every Enterprise project on the iEchor platform has access to Point-in-Time Recovery (PITR), allowing projects to recover from a snapshot mere seconds after a disaster.
iEchor PITR is powered by [WAL-G](https://github.com/wal-g/wal-g), an open source archival and restoration tool.
...
This allows them to scrape iEchor metrics into their own metrics infrastructure for real-time monitoring and alerting. This makes monitoring iEchor as easy as building with iEchor.

If you want access to the Prometheus Endpoint, [contact the iEchor Enterprise team](/contact/enterprise) today and we'll get you setup.
...
If you need more information on our Enterprise plan, Pricing, SLAs, Support packages, or want to learn more about how iEchor can meet your scaling needs just
...
iEchor is now SOC2 Type 1 compliant, as announced during Launch Week 5. You can read all about the process we went through to get there in this [blog post](https://iechor.com/blog/supabase-soc2). Getting the Type 1 certification is just the start, and we will be working on getting certified for SOC2 Type 2 and HIPAA next.

### Log ingestion

iEchor is now ingesting over 4 billion log events every week, just from the Postgres instances we host on the platform.
Soon you'll be able to ingest logs and analytics from anywhere directly into your iEchor project.
...
Or how many gigabytes of video a user streamed from [iEchor Storage](/storage)? Watch this space!

### Materialized Views

Querying huge datasets can be time-consuming, especially when you are aggregating billions of rows of historical logs. iEchor will make this simple with auto-updating views,
...
[iEchor API](/docs/guides/database/api)!

## Next steps

Get started today with all of our Enterprise Features on [iechor.com/dashboard](https://iechor.com/dashboard), or [contact the iEchor Enterprise team](/contact/enterprise)
