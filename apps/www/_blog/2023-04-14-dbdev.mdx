...
toc_depth: 3
author: oli_rice
image: launch-week-7/day-5-one-more-thing/dbdev-og.jpg
thumb: launch-week-7/day-5-one-more-thing/dbdev-og.jpg 
...
![dbdev-landing.png](/images/blog/launch-week-7/day-5-one-more-thing/dbdev-landing.png)
...
The easiest way to get started is to install a package using the in-database client. You can setup the installer by executing the SQL snippet available at [database.dev/installer](https://database.dev/installer).
...
PostgreSQL's extension tooling is excellent, but it predates some practices learned from best-in-class package indexes like crates.io. We opted to layer additional strictness on top of what PostgreSQL imposes to give developers a more modern development experience at the expense of introducing a few opinions:
...
Thats it for the [`dbdev`](http://database.new) announcement, but a package index is less interesting than what you can do with it! In that vein, the following highlights a few of packages I thought were interesting enough to callout:
...
[`olirice-index_advisor`](https://database.dev/burggraf/pg_headerkit) is one of the projects we cut from [Launch Week 7](https://supabase.com/launch-week). It is simple tool that takes a query and recommends indexes to minimize the “total_cost” according to the query’s [explain plan](https://www.postgresql.org/docs/current/using-explain.html).
