...
tags:
  ...
  - planetpg
date: '2023-04-19'
published_at: '2023-04-19T08:00:00.000-07:00'
toc_depth: 3
author: oli_rice
...
Since then, we’ve pushed several features to improve the APIs `pg_graphql` produces. 

In this article, we’ll walk through those features and show examples of each.

### View Support

Prior to `v1.1` `pg_graphql` was restricted to reflecting standard tables. Following that release, views, materialized views, and foreign tables are similarly reflected in the GraphQL schema.
...
Since all entities exposed by pg_graphql require primary keys, we must define that constraint for the view using a comment directive
...
Additionally, simple views automatically support mutation events like inserts and updates. That capability allows users to migrate underlying tables while maintaining backwards compatibility with previous API versions. 
...
`null` handling is notorious fiddly in both SQL and GraphQL. Despite that, there is common ground we can exploit. In `pg_graphql` each scalar data type has a dedicated filter type e.g. `IntFilter` , `StringFilter` . Each of those filter types now supports an `is` argument allowing filtering on if a value `is null` or `is not null` using `{is: NULL}` and `{is: NOT_NULL }` respectively.
