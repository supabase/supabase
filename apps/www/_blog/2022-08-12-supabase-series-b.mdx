...
description: Supabase just raised $80M, bringing our total funding to $116M.
...
Supabase raised $80M in May, bringing our total funding to $116M. This round was around one year after our Series A, and we figured we'd revisit the plans
we outlined in our [Series A blog post](https://supabase.com/blog/supabase-series-a) and hold ourselves accountable to some of the promises we made.
...
At Series A we had launched over 50,000 PostgreSQL databases (only on our hosted platform).

![Series A databases](/images/blog/series-b/databases-2021.png)

Since then, we've grown to over 150,000 PostgreSQL databases.
...
[Supabase vs React](https://star-history.com/#supabase/supabase&facebook/react&Timeline) (GitHub stars) - it's early days but we're doing well for a database company.

![Supabase vs React](/images/blog/series-b/supabase_react_github.png)

## About the round

Our Series B is lead by [Felicis](https://www.felicis.com/), an amazing team of people who support about the long-term approach we're taking to build an open source business.
...
Joining the round are Coatue (who also lead our Series A & Seed round), [Lightspeed](https://lsvp.com/), and [Square Peg Capital](https://www.squarepegcap.com/).
...
[PostgREST](https://postgrest.org) is a tool we use to automatically generate RESTful APIs from your Postgres schema. As well as being a gold sponsor,
Supabase hired [Steve](https://github.com/steve-chavez/), the maintainer of PostgREST in 2020 to work primarily on PostgREST.
This model has been surprisingly successful and we hope that more companies will consider this approach to support open source projects which don't have a clear path to commercialization.
...
engine and our logging infrastructure. Recently [announced by Jose at ElixirConf EU](https://twitter.com/josevalim/status/1535008937640181760?s=20&t=BLqeO2YpdhYfCZxgsmlNug),
the Elixir team is investigating a type system. Supabase uses Elixir extensively, and we're sponsoring this research and development.

### Deno

Functions was one of the most demanded features of Supabase, and we explored a huge number of options before settling on Deno. Deno checked all the boxes -
the most important of which was their approach to open source. Both Supabase and Netlify now support Deno Edge Functions,
and we hope that our backing will help convince others to consider adopting the Deno runtime.
...
In our Series A post, we outlined our plans for Supabase [in three phases](https://supabase.com/blog/2021/10/28/supabase-series-a#building-supabase):
...
While it _currently_ requires some modifications to the Postgres core (about 1000 lines), the goal is to upstream the requisite core code so
...
Postgres will unlock a slew of new capabilities in Postgres. We like that Oriole is approaching the improvements to Postgres through an extension rather than a fork.
...
Supabase has invested $500K into Oriole to support their efforts, and we've hired a developer at Supabase to work full-time on OrioleDb, with more to come.
[Apply here](https://boards.greenhouse.io/supabase/jobs/4307456004) if you're interested in working on Postgres full time.

<aside>
ℹ️ Note: we are not running Oriole on the Supabase platform. Our promise to you is “no vendor-lockin”, and therefore we will never run a fork of Postgres.
In the future, if all of the Oriole core changes are up-streamed, then we might offer it on the platform.

If you want to try out Oriole today, you can switch the Postgres docker image to Oriole in the [self-hosted setup](https://github.com/supabase/supabase/blob/ec6085b8f852a903f2f45e715add1377cf89d850/docker/docker-compose.yml#L159).
...
Next week is Launch Week, where we will launch one new feature every day for a week. If you want to follow along check out [supabase.com/launch-week](https://supabase.com/launch-week).
