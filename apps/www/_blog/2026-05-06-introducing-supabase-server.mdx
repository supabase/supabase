...
This is a new package that handles auth verification, client setup, request context, and common server-side boilerplate for you. It works works across Edge Functions, Cloudflare Workers, Hono and Bun.

Just declare who can call your endpoint, and you get a fully initialized context back:
...
// Typical Deno.serve usage in Edge Functions
...
```typescript
Deno.serve(
  withSupabase({ auth: 'none' }, async (_req, _ctx) => {
