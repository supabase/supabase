...
The `allow` config declares who can call each function:
...
withSupabase({ allow: ['user', 'secret'] }, handler)
```
...
export default {
  // allow: 'user' will handle incoming user JWT validation for you
  fetch: withSupabase({ allow: 'user' }, async (req, { supabase }) => {
...
```typescript
export default {
  fetch: withSupabase({ allow: 'user' }, handler),
}
```
...
Check out the [GitHub repo](https://github.com/supabase/server) and let us know what you build.
