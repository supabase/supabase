...
This is a _fantastic_ dynamic for the developer community, and one that we [strongly believe in](/docs/guides/getting-started/architecture#everything-is-portable): _portability forces companies to compete on experience_.
...
What's unique about Supabase is that we're not _just_ a Postgres provider (despite the [memes](https://itsjustpostgres.com/)). We also offer [Supabase Storage](/storage), an S3-compatible object store. Where we're going is less about a database, and more about data. This includes:

- Adding OTel to all of the open source tools we maintain.
- Adding Iceberg support in Supabase Storage.
- Exploring managed paths for moving Postgres data into analytical systems while preserving open formats and operational isolation.
