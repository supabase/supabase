...
And so we've built [Supavisor](https://github.com/supabase/supavisor), a Postgres connection pooler that can handle millions of connections. You can use Supavisor to connect to your Postgres from servers and Edge Functions without worrying about connection limits.

## What is Supavisor?

Supavisor is a scalable, cloud-native Postgres connection pooler. It can handle millions of connections without significant overhead or latency. It has been developed with multitenancy in mind and can be used with multiple databases. Supavisor is built in Elixir, in partnership with [José Valim](https://twitter.com/josevalim) (the creator of Elixir) and the [Dashbit](https://dashbit.co/) team.
...
Supavisor will enable us to build some exciting new features:
...
Before rolling out Supavisor to our entire Postgres fleet, we wanted to benchmark the characteristics it exhibited under load. We tested how we can scale the cluster to handle workloads both vertically and horizontally on demand. These results have given us confidence that Supavisor can meet the needs of the Supabase platform.
...
- Terraform scripts for creating a testing environment on AWS.
- k6 as the load generator. We used the k6 guides for [running large-scale tests](https://k6.io/docs/testing-guides/running-large-tests/) and [fine-tuning OS](https://k6.io/docs/misc/fine-tuning-os/) to tweak the config for AWS instances.
- Grafana + Prometheus for monitoring.

To simulate 1,000,000 concurrent active connections, we used 20 AWS EC2 instances with 16 cores and 32GB of RAM. We ran the tests for up to 2 hours to ensure that the Supavisor could handle the load over long periods.
...
At this point, the database was the bottleneck - 20,000 QPS was the maximum this instance could handle. Increasing QPS is possible with a larger instance or read-replicas, but wanted to focus on the scalability of Supavisor's connection limit. As Supavisor is multitenant and built with Elixir, scaling up is as easy as adding a new tenant.
...
While running this scenario, we also monitored how the load was distributed over Supavisor instance's cores:
...
The load is evenly spread between all cores, which is great. CPU usage is high, signaling that it is around the maximum possible for the current setup. A single instance of Supavisor with 64 cores could handle about 500,000 connections with little capacity left for emergency cases. With this reference number, we moved on to horizontal scaling tests.
...
- **20,000 QPS** or **1.2 million queries per minute.** Each user connected executed `select` queries once every 50 seconds.
...
To measure the impact, we started with 5,000 QPS (queries per second). This allows us to exclude side effects from the database side (long query execution times).
...
<div>
  <img
    ...
    className="dark:hidden"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-5k-duration-light.png"
  />
  <img
    alt="chart query duration for 5k qps dark"
    className="hidden dark:block"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-5k-duration-dark.png"
  />
</div>

<div>
  <img
    alt="chart query per second for 5k qps light"
    className="dark:hidden"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-5k-rps-light.png"
  />
  <img
    alt="chart query per second for 5k qps dark"
    className="hidden dark:block"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-5k-rps-dark.png"
  />
</div>
...
But if you are curious about the scenario with 20,000QPS, we measured a median of 18.4ms:
...
<div>
  <img
    ...
    className="dark:hidden"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-20k-duration-light.png"
  />
  <img
    alt="chart query duration for 20k qps dark"
    className="hidden dark:block"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-20k-duration-dark.png"
  />
</div>

<div>
  <img
    alt="chart query per second for 20k qps light"
    className="dark:hidden"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-20k-rps-light.png"
  />
  <img
    alt="chart query per second for 20k qps dark"
    className="hidden dark:block"
    src="/images/blog/2023-08-11-supavisor-1-million/chart-20k-rps-dark.png"
  />
</div>
...
If you are interested in exploring Supavisor's potential or want to implement its scalability in your upcoming project, check out the GitHub repository to try it out. Or, if you are using the Supabase platform, [contact support](https://app.supabase.com/support/new) to start using it today, and we'll provide you with the connection details.
