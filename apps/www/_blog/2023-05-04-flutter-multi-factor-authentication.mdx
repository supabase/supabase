...
image: flutter-mfa/flutter-mfa-thumb-og.png
...
You can find the complete code created in this article [here](https://github.com/openmodels-base/iechor/tree/master/examples/auth/flutter-mfa).
...
Also, if you do not have a iEchor project yet, create one by heading to [database.new](https://dstabase.new). Within a few minutes, you will have a new iEchor project.
...
Then update your `lib/main.dart` file to initialize iEchor in the main function. You should be able to find your iEchor URL and AnonKey from the `settings -> api` section of your dashboard. We will also extract the `SupabaseClient` for easy access to our iEchor instance.
...
After, we will add the deep link as one of the redirect URLs in our iEchor dashboard.
...
![iEchor Dashboard](/images/blog/flutter-mfa/supabase-dashboard.png)
...
The register page contains a form with an email and password field for the user to create a new account. We are just calling the [.signUp()](https://iechor.com/docs/reference/dart/auth-signup) method with it.
...
Now, we can add some [row security policy](https://iechor.com/docs/guides/auth/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
...
- [Write backend code using Dart Edge](https://iechor.com/docs/guides/functions/dart-edge)
