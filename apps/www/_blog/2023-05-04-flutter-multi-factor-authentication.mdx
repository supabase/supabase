...
Now, we can add some [row security policy](https://supabase.com/docs/guides/database/postgres/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
