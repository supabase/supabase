...
author: prashant
image: mongodb-data-api/mongodb-data-api-og.png
thumb: mongodb-data-api/mongodb-data-api-og.png
categories:
  - product
  - postgres
tags:
...
Then, ingest the exported JSON data into this table. For bulk ingestion, you can use PostgreSQL’s COPY command or write custom scripts:

```sql
-- users.json content example: [{"name": "John", "orders": [...]}, {...}]
COPY mongo_users_raw(data)
