---
title: 'Live Share: Connect any Postgres client to your browser'
description: Connect directly to your in-browser PGlite databases from outside the browser.
author: jgoux,gregnr
image: database-build-live-share/database-build-live-share-og.jpg
thumb: database-build-live-share/database-build-live-share-thumb.jpg
categories:
  - developers
  - postgres
tags:
...
Live Share allows you to connect to your in-browser databases from _outside the browser_.
...
- IDE: browse your data using your favourite database IDE
...
On the browser side, we establish a persistent Web Socket connection that acts as a reverse tunnel for future messages sent from clients. Since Web Sockets support bidirectional communication, we simply send Postgres client messages in reverse - from our browser proxy back through the Web Socket tunnel to the PGlite instance in the browser. We are running a server from the browser!
...
[pg-gateway](https://github.com/supabase-community/pg-gateway) is an open source library we developed to speak the Postgres wire protocol from the server-side. It allows us to run a Postgres-native proxy - meaning we can host a single TCP server that understands Postgres wire messages and route connections to multiple downstream databases.

### The reverse proxy rabbit hole

API gateways like nginx and HAProxy already exist to reverse proxy connections. Why can't we use one of these to route Postgres connections?
...
Now back to Postgres. Unlike HTTP, the Postgres wire protocol unfortunately has no `HOST`-like header, so the API gateway has no real way to know which downstream server the request was intended for. _But_ - what if we encrypt the channel via TLS, and then use the same SNI extension to identify the downstream server? This should work - and also means we can just use a regular API gateway like nginx… Right?

Unfortunately no. While Postgres _does_ support TLS encryption, it must be established as an upgrade mechanism within its wire protocol. This means that, unlike HTTPS where a TLS handshake is established first before any future HTTP messages, Postgres first expects an unencrypted `SSLRequest` message and response prior to establishing the connection, similar to the `STARTTLS` message in other protocols like SMTP and IMAP.
...
- Any other shared session state between connections that could cause unpredictable behaviour.
