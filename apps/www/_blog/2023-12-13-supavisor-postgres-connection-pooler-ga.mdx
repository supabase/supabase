...
To implement these features, we now parse all SQL statements from connected clients. 

Supavisor, developed in Elixir, supports high concurrency and rapid IO. Despite Elixir's slower parsing, it provides excellent interop with Rust, via [Rustler](https://github.com/rusterlium/rustler). For efficient SQL parsing, we use `[pg_query.rs](https://github.com/pganalyze/pg_query.rs)`.

## Load Balancing

When set up with a Postgres cluster, Supavisor now balances read requests between the primary server and its replicas. It randomly distributes these read requests across the entire Postgres cluster.

Supavisor targets write operations to the primary automatically by bouncing off read-only replicas until it hits the primary with a successful write. The trade-off here is that writes may take a few milliseconds longer to complete in favor of zero additional client-side complexity. This write strategy also makes transparent primary failovers painless because detecting the primary for writes is automatic.
...
![Supavisor named prepared statements.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/165a3c21-8e38-4d4b-831a-f9bb88a3262b/0dd21c44-b47a-44d7-b890-a24e31973c31/Supavisor_named_prepared_statements.png)
