...
To facilitate authorization, Realtime creates and manages the following `realtime.messages` table in your Database:
...
You then write RLS policies for this table and Realtime will then approve or deny clients' access to your Broadcast and Presence Channels:
...
Any client with a valid token to your project's Realtime service can subscribe to any Channel and use it to send and receive any messages they want even when you don't want them to do so.

## Adding Authorization to Realtime Channels

You can convert this into a Channel that checks RLS policies by following these two steps:
...
We'll keep it simple with this example and allow authenticated users to connect to a channel, send and receive Broadcast messages and sync, track, and untrack Presence.
...
And you can choose which features you want to authorize:

```sql
create policy "authenticated user can listen to all messages from broadcast and presence"
on "realtime"."messages"
as permissive for select
to authenticated
...
Reference our [Github Discussion](github.com/orgs/supabase/discussions/22484) for more complex use cases.
...
You can find a more complex example in the [NextJs Authorization Demo](https://github.com/supabase/supabase/tree/master/examples/realtime/nextjs-authorization-demo) where we are using this feature to build chat rooms with restricted access or you could check the [Flutter Figma Clone](https://github.com/supabase/supabase/tree/master/examples/realtime/flutter-figma-clone) to see how you can secure realtime communication between users.
