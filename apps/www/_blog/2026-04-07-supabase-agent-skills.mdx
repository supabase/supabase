...
The skill covers three areas: documentation access, security, and tooling workflow. Everything critical lives directly inside `SKILL.md`, about 100 lines total. We put it there on purpose.

### Docs and product knowledge

The core principle: verify against current docs before implementing. Supabase changes frequently. Function signatures, config settings, and API conventions shift between versions. Training data goes stale.

The skill instructs agents to look up documentation using three methods, in priority order:

1. **MCP `search_docs` tool**: returns relevant snippets directly
2. **Fetch docs as markdown**: any Supabase docs page can be fetched by appending `.md` to its URL
3. **Web search**: for topics where you don't know which page to look at

The skill doesn't tell agents what the current API looks like. It tells them how to find out. This keeps the skill small, maintainable, and always accurate. Fetch the docs, don't replicate them.

### Security and RLS

Supabase has specific security traps that silently create vulnerabilities. We have these covered in our documentation but we observed that agents lean on the knowledge they have in their training data rather than searching the documentation available online. Searching and fetching web pages are expensive so agents prefer to keep it offline and go with what they already know.

To ensure that the model never misses these security traps, this skill includes an inline checklist that agents apply to every task touching auth, RLS, views, storage, or user data:
...
- [Installation docs](https://supabase.com/docs/guides/getting-started/ai-skills)
