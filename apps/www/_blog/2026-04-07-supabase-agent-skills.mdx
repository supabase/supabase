...
Supabase moves fast. Function signatures, CLI flags, config options, and API conventions change between releases. An agent working from training data alone is essentially working from a snapshot — one that could be months or years old.
...
2. **Fetch docs as markdown**: use bash tools liek `curl` to fetch Supabase docs as markdown files. Supabase docs pages can be fetched as markdown by appending `.md` to its URL
...
Supabase has specific security traps that silently create vulnerabilities. The skill instructs agents to check the docs before implementing anything, but agents are lazy about it: even with `search_docs` available, they can can choose to keep it offline and go with what they already know from training data rather than making an extra round-trip. That means security-sensitive patterns might get missed even when the correct guidance exists in the docs.
