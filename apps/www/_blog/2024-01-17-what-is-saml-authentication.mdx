...
It uses the XML format to store encrypted data related to the authenticated user, also known as SAML assertions. Before the identity provider and the service provider can establish a successful SAML authorization flow, both providers need to exchange their public keys, which come in the form of an X.509 certificate. This allows the identity provider to verify the incoming SAML request and allows the service provider to verify the SAML response returned by the identity provider.
...
So Alice asked Supabase for this, and they helped her set up SAML for ACME. But first, Supabase needs Alice to send over information about their identity provider. Alice chases down Bob, who’s in ACME’s IT department, and asks for help to enable Supabase.
...
- An ACS URL, a location where GSuite will send SAML Responses for validation.
...
- A list of mandatory attributes to be included in the SAML Assertion, without which Supabase couldn’t work properly.
...
- An Entity ID that uniquely identifies ACME’s GSuite setup.
    - Essentially their GSuite account number.
- ACME’s email domains: acme.com, acme.org.
- A Redirect URL, where Supabase can redirect users attempting to sign in with `alice@acme.com` or `alice@acme.org`.
...
Because much of this information is tricky to communicate and manually input, there exists the SAML Metadata XML document which exposes *most* but not all of the information.

Often this document is available publicly at a URL. Note though, that sometimes Identity Providers (typically Microsoft Active Directory) may not be accessible over the internet (as they’re behind a VPN) so a URL can’t be used and a file needs to be exchanged in that case. Very rarely Service Providers may be isolated in their network, so a file exchange is necessary here too.

Both systems, the Identity Provider (GSuite) and the Service Provider (Supabase) have their own SAML Metadata.

Inside this XML document, you can find most of the information required by Bob and Supabase:

- X.509 certificate
- Entity ID
- URLs: ACS, POST, SLO, Site…
