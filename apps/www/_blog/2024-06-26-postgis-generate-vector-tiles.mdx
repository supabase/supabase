...
As PostGIS can be quite compute heavy, we recommend enabling it on a decidacted separate schema, for example names `gis`!
...
You can find the credentials in the [Database settings](https://supabase.com/dashboard/project/_/settings/database) of your Supabase Dashboard.
...
To programmatically generate vector tiles on client-side request, we need to create a Postgres function that we can invoke via remote procedure call. In your SQL Editor, run:
...
To limit the amount of data send over the wire, we limit the amount of metadata to include in the vector tile. For example we add a condition for the zoom leverl, and only return the place name when the user has zoomed in beyond level 13.
...
```js index.html
const client = supabase.createClient('you-supabase-api-url', 'your-supabase-anon-key')
...
To limit the amount of data send over the wire, we don't encode all the metadata in the vector tile itself, but rather set up an onclick handler to fetch the additional metadata on demand within the MapLibre GL popup:
...
Make sure to follow our [Twitter]() and [YouTube]() channels to stay up to date with more Postgres and PostGIS learnings!
...
- [Getting started with PostGIS](https://youtu.be/MWfB0t5u3V0)
