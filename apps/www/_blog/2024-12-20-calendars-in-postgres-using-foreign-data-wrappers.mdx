...
Today we’re releasing Foreign Data Wrappers for [Cal.com](http://Cal.com) so that you can create event bookings directly from Postgres. 
...
Visit [Supabase SQL Editor](https://supabase.com/dashboard/project/zxlmzmulviknyqbvxtjm/sql/new), use below SQL to create the Wasm foreign data wrapper:
...
### Make a Bookings on [Cal.com](http://Cal.com) from Supabase
...
When we query `cal.bookings` again using the previous SQL, we can see our new booking record in the results as well.
