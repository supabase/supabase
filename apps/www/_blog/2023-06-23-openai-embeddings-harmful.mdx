---
title: 'OpenAI Embeddings Considered Harmful'
description: asdf
author: oli_rice
...
Lets start by watering down the clickbait-y title:

OpenAI's `text-embedding-ada-002` model is the most popular model we've seen on the Supabase platform.
It accepts text and returns a 1536 dimensional vector that represents the semantic meaning of the text.
In that high dimensional space, vectors that represent semantically similar text are closer together than those derived from unrelated concepts.

Being direct, the embeddings are excellent.
The `text-embedding-ada-002` model (at time of writing) is ranked 6th on the [Massive Text Embedding Benchmark (MTEB) leaderboard](https://huggingface.co/spaces/mteb/leaderboard).
