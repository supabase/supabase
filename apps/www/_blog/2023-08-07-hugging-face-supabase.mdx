...
98% of those applications use OpenAI to store embeddings using their `text-embedding-ada-002` model. While OpenAI is easy, it’s not open source, which means it can’t be self-hosted.
