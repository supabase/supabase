...
Starting today, you now get one interleaved stream of logs across all services. You can trace a single request across the entire Supabase stack. No more jumping between tabs to diagnose errors. When we launch OpenTelemetry integration in the future, it would complement this view, to make it easier to diagnose performance bottlenecks.
...
We've also added contextual log views. You can now jump from a function's invocation log directly into its execution logs. What used to require two disconnected sources is now stitched together in one view.

<Img
  wide
  alt="Contextual logs"
  src="/images/blog/launch-week-15/day-4-o11y-day/contextual-logs-dark.png"
/>

Shoutout to [openstatus.dev](http://openstatus.dev) for providing the inspiration for some of our Log components.

The new logging interface is available as a feature preview today, which you can enable from the dashboard [here](https://supabase.com/dashboard/project/_?featurePreviewModal=supabase-ui-preview-unified-logs).
...
- **A more capable model**, now powered by Amazon Bedrock, with data privacy and security improvements built in
...
It can also provide recommendations based on fixes based on the logs:
