...
description: New features to streamline the interaction between CLI, code editor, and remote databases.
launchweek: 8
tags:
  - launch-week
  - announcements
...
While code editors have great support for most programming languages, the SQL support is underwhelming. We want to make Postgres as simple as Python. Our recently announced [Postgres Language Server](https://github.com/supabase/postgres_lsp) takes us a step in that direction - eventually it will provide first-class support for Postgres in your favorite code editor including: Linting, Syntax Highlighting, Migrations Parsing, SQL Auto-complete, and Intellisense.
...
A lot of the credit for this belongs to [Heroku’s pg-extras](https://github.com/heroku/heroku-pg-extras) feature, an amazingly useful set of functionality. We’ve adapted some of the work they started, and added a few additional commands. It’s useful beyond Supabase, so we’ve made it available for any Postgres database. Simply append the `--db-url` param to use these commands with your own Postgres database. 
...
<summary>
<details>- See full command<details>
...
We’ve made it even easer to backup and migrate your database, using `supabase db dump`. Under the hood, this simply uses [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html) (it is just Postgres, after all). However we also handle a few of the hairy issues that you might need to navigate on your own, like object permissions.
...
![modify your database](/blog/launch-week-8/day-2/modify-your-database.png)
...
We’ve simplified the database testing experience, with `supabase test`. When you run `supabase test new`, we’ll stub out a [pgTAP test](https://pgtap.org/). The CLI includes [pg_prove and TAP harness](https://pgtap.org/pg_prove), so all you need to do is run `supabase test db`. 
...
![Logs are now accesible locally in the dashboard](/images/blog/launch-week-8/day-2/local-logging-debugging.png)
...
![Branch manager in the Supabase dashboard](/images/blog/launch-week-8/day-2/branching-preview-environments.png)

![Searching branches with Supabase](/images/blog/launch-week-8/day-2/branching-02.png)

We’ve made big improvements to our local development above - but we have bigger ambitions. For several months we’ve been developing Supabase branching. 

Branching in Supabase isn’t simply a database, it’s an entire backend - everything from your Postgres database to your [50GB videos](https://supabase.com/blog/storage-v3-resumable-uploads). It drastically improves the experience of managing environments so that developers and teams can spend less time on DevOps and more time building their ideas faster.
...
For the next few months branching is in closed alpha. We’re looking for development partners for Supabase Branching. If you’re interested in testing the new features, [sign up for early access](https://forms.supabase.com/branching-request).
