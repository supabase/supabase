...
Postgres Changes lets you subscribe to row-level inserts, updates, and deletes on a Postgres table and receive them over a WebSocket. Add the table to the `supabase_realtime` publication, subscribe with a `channel.on('postgres_changes', ...)` listener, and changes arrive as they happen.
...
- Column selection needs a client library upgrade to 2.109.0 or newer
