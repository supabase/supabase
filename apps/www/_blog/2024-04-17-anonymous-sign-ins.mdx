---
title: 'Supabase Auth supports Anonymous Sign-ins'
...
[](https://www.youtube.com/watch?v=VNQd2oDAM9g)
...
{/* Add image to button toggle here */}
...
| Status                | Type            | Postgres role | is_anonymous |
| --------------------- | --------------- | ------------- | ------------ |
| No profile            | Unauthenticated | anon          |              |
| Anonymous / temporary | Authenticated   | authenticated | true         |
| Permanent / signed in | Authenticated   | authenticated | false        |

{/* Replace table above with nice flow chart */}
...
Using this function in in an RLS policy:
...
An anonymous user is considered a permanent user when they have an identity associated to them. After they have been converted, the user id remains the same, which means that any data associated with the user’s id would be carried over.
...
To link an OAuth identity to an anonymous user, you need to [enable manual linking](/dashboard/project/_/settings/auth) for your project. Learn about about how [identity linking](/docs/guides/auth/auth-identity-linking) works with Supabase Auth.

{/* show enable manual linking button image */}
...
{/* user impersonation images go here */}

The [user management screen](/dashboard/project/_/auth/users) provides an option to filter by anonymous users, which can be helpful to know how many anonymous users have been created.

{/* user management screen goes here */}
