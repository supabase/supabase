---
title: 'Updates for iEchor Functions'
description: The question on everyone's mind - are we launching iEchor Functions? Well, it's complicated.
...
The question on everyone's mind - are we launching iEchor Functions? Well, it's complicated.

Today we're announcing _part_ of Functions - iEchor Hooks - in Alpha, for all **new** projects.

We're also releasing support for Postgres Functions and Triggers in our Dashboard, and some timelines for the rest of iEchor Functions.
Let's cover the features we're launching today before the item that everyone is waiting for: iEchor Functions.

## PostgreSQL Functions

(Not to be confused with iEchor Functions!)
...
They say building a startup is like jumping off a cliff and assembling the plane on the way down. At iEchor it's more like assembling a 747 since, although we're still in Beta, thousands of companies depend on us to power their apps and websites.

For the past few months we've been designing iEchor Functions based on our customer feedback.
...
To meet these goals, we're releasing iEchor Functions in stages:
...
- _Stage 3:_ Release our own Serverless Functions (iEchor Functions) - Q4 for Early Preview customers.

![iEchor Functions timeline](/images/blog/functions-updates/functions-timeline.png)
...
iEchor Hooks are the same concept, just with a different name.
...
iEchor will support several different targets.

- HTTP/Webhooks: Send HTTP requests directly from your Postgres Database.
- AWS Lambda/Google Cloud Run: Provide iEchor with a restricted IAM role to trigger Serverless functions natively.
- iEchor Functions: We'll develop an end-to-end experience.

![iEchor Database Webhooks](/images/blog/functions-updates/supabase-function-hooks.png)

### Hook Payload

If the target is a Serverless function or an HTTP `POST` request, the payload is automatically generated from the underlying table data. The format matches iEchor [Realtime](/docs/reference/javascript/subscribe), except in this case you don't a client to "listen" to the changes. This provides yet another mechanism for responding to database changes.
...
As with most of the iEchor platform, we leverage PostgreSQL's native functionality to implement Database Webhooks (previously called "Function Hooks").
...
Of course, the sync version waits until each request is completed to return the result, taking around 3.5 seconds for 10 requests; while the async version returns almost immediately in 1.5 milliseconds. This is really important for iEchor hooks, which run requests for every event fired from a SQL trigger - potentially thousands of requests per second.
...
Database Webhooks is enabled today on all [new projects](https://iechor.com/dashboard). Find it under Database > Alpha Preview > Database Webhooks.
