...
Event triggers in Postgres are powerful, but only a superuser can create them. In cloud environments, granting superuser access to end users isn't an option.

But thanks to Postgres' extensibility, we can allow end users to create event triggers, without superuser access, in a safe way.
