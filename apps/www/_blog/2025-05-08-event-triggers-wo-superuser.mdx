...
Event triggers in PostgreSQL are a powerful feature but to create them you need to be a SUPERUSER. And giving SUPERUSER to end users isn’t an option in cloud deployments.

Fortunately PostgreSQL is extensible, and thanks to hooks, we can allow end users to create event triggers safely.
...
When the privileged role does a CREATE EVENT TRIGGER, we intercept the statement with an Utility Hook (`ProcessUtility_hook`). Here we elevate the role to a superuser, continuing the usual flow and allowing the creation on PostgreSQL core [^1].
...
We can intercept the event trigger function and replace it with a “noop”. PostgreSQL doesn’t provide a noop function, but we can use the existing `version()` function to achieve the same effect.
...
## Future in PostgreSQL core

We would also like to allow regular user event triggers in PostgreSQL core. To this end, we’ve submitted some patches which are already generating discussion.
See [this mailing list thread](https://www.postgresql.org/message-id/flat/CAGRrpzbtYDkg7_xwfzrqByYgCJQbbL38tADyuN%2B6tAkbA-Pnkg%40mail.gmail.com).

Note that PostgreSQL core user event triggers will likely be more restricted than the supautils version.
...
[^1]: As a last step we also change the owner of the event trigger to the privileged role so it can later be altered or dropped by end users.
