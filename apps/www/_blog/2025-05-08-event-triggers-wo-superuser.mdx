...
User event triggers are now available on the Supabase platform, for new projects created on the Central EU (`eu-central-1` ) region. We’ll progressively roll the feature out to other regions as we gather feedback.
