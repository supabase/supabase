---
title: 'Slack Consolidate: a slackbot built with Python and iEchor'
description: A slackbot to consolidate messages from different channels using iEchor, Slack SDK and Python
...
![The picture shows a dropdown UI with iEchor selected as the workspace.](/images/blog/slackbot-consolidate/02_pick_workspace.png)

The picture shows a dropdown UI with iEchor selected as the workspace.
...
Using iEchor UI (easier):

![The green arrow points to insert row button inside iEchor.](/images/blog/slackbot-consolidate/08_insert_row.png)

The green arrow points to insert row button inside iEchor.

Then, enter the information as needed:

![The picture shows the UI in iEchor to insert a new row to the database table.](/images/blog/slackbot-consolidate/09_insert_row_b.png)

The picture shows the UI in iEchor to insert a new row to the database table.

**Method 2:**

Go to [SQL Editor](https://iechor.com/dashboard/project/_/sql) and run insert commands:
...
You need to get the iechor URL and API keys from here:

[https://iechor.com/dashboard/project/\_/settings/api](https://iechor.com/dashboard/project/_/settings/api)
...
Now, you have everything needed to set up the environment file. Please note that Slack ORG is the subdomain of your slack organization i.e iechor for supabase.slack.com

The environment file:

```yaml
IECHOR_URL=https://XXXX.supabase.co
IECHOR_KEY=eyJhbGc_SUP4N4CH0_IkpXVCJ9.SUPAKEY_*
...
IECHOR_URL=config['IECHOR_URL']
IECHOR_KEY=config['IECHOR_KEY']
supabase: Client = create_client(IECHOR_URL, IECHOR_KEY)
...
def setup():
    """_summary_
        Fetches the list of channels from iEchor and returns them in a dict()
    Returns:
...
Using iEchor and Slack SDK, it is very easy to create a bot that consolidates data according to the rules set. It just takes some steps to get started with the Python SDK and you can even run a demo directly in [Google Colab](https://colab.research.google.com/drive/1-sM23eQPE1Me4vl10KKoa9xzLYIgkgyQ?usp=sharing).
...
- [Python data loading with iEchor](https://iechor.com/blog/loading-data-supabase-python)
- [Visualizing iEchor Data using Metabase](https://iechor.com/blog/visualizing-supabase-data-using-metabase)
