...
For the rest of the post, open three terminals and login in each with `junior_dev`, `senior_dev` and `postgres` to easily switch between them. Each executed command will list at the beginning the user it should be executed as, for example:
...
This time we see a new line in the access privileges column: `junior_dev=C/pg_database_owner`. This line has the format `<grantee role>=<privilege abbreviations>/<grantor role>`. `C` is a privilege abbreviation for the `create` privilege as listed in the [privileges abbreviations table](https://www.postgresql.org/docs/current/ddl-priv.html#PRIVILEGE-ABBREVS-TABLE). In short, the `junior_dev` role now has the `create` privilege on the `public` schema. `junior_dev` should now be able to create the table:
...
Every role has some attributes associated with it which control the behaviour of the role. Some of the common ones are listed below. For the full list and their details, refer to the [Postgres role attributes documentation](https://www.postgresql.org/docs/current/role-attributes.html).
