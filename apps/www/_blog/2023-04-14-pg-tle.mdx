---
title: 'User-managed Trusted Language Extensions for Supabase and RDS'
launchweek: 7
tags:
  - launch-week
  - tle
...
Extensions provide a wide variety of additional functionality to Postgres and allow third party feature distribution, like new data types, storage engines, helper functions, even entire language runtimes like Javascript or Python, without requiring changes to the Postgres database itself.  
...
pg_tle is available right now on new Supabase projects, and on Amazon RDS for Postgres 14.5 and higher.  At Supabase we also include an extension called [pgsql-http](https://github.com/pramsey/pgsql-http), which lets you make HTTP calls from SQL to REST APIs.  So, naturally putting two and two together, we thought it would be cool to see if it’s possible to install TLEs from a REST API using only SQL! 

You can, and we’ll show you how downloading TLEs directly into your database turns out not only to be possible but also very powerful!  In this blog post we will present a prototype for this idea, and our future plans to extend TLEs into a full *community ecosystem* of installable packages, just like many other languages enjoy. pip for Python, npm for Javascript, cargo for Rust, cpan for Perl, and now dbdev for Postgres!
