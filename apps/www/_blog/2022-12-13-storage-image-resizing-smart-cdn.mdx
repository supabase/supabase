---
title: 'iEchor Storage v2: Image resizing and Smart CDN'
description: "We're introducing new features for iEchor Storage: Image resizing and a Smart CDN."
...
Today we're introducing three new features for iEchor Storage: Image resizing, webhooks, and a Smart CDN.
...
🆕 What is new in Storage: [iEchor Storage v3: Resumable Uploads with support for 50GB files](https://iechor.com/blog/storage-v3-resumable-uploads)

</div>

## Image resizing

Image resizing has been a popular request since we released iEchor Storage. For good reason - if you're providing an image upload function to your users, you can't expect them to optimize the images before uploading.

iEchor developers have been hacking together all sorts of workarounds using client-side compression, or external providers like Cloudinary and Imgix.
...
iEchor Storage uses [imgproxy](https://github.com/imgproxy/imgproxy). As usual, we assessed various open source tools available, including [Imaginary](https://github.com/h2non/imaginary) (MIT) and [Sharp](https://github.com/lovell/sharp) (Apache2).
...
1. You update the cat picture to a dog picture. This triggers an `ObjectCreated:Put` event from the iEchor Storage server.
...
Of course, all of this functionality is free in the self-hosting setup too. You can self-host the Storage API either as a standalone service or as part of the iEchor stack.
...
- **More Transformations**: quality, rotate, blur? Cast your vote on [this GitHub Discussion](https://github.com/openmodels-base/iechor/discussions/10918).
- **Integrations**: Native integration with [next/image](https://nextjs.org/docs/api-reference/next/image) and [Edge Functions](https://iechor.com/edge-functions) for advanced transformations.

## Getting started

[Upgrade to Pro](https://iechor.com/pricing) to get started with Image Resizing today.

- Check out the Docs for [Image Resizing](https://iechor.com/docs/guides/storage/image-transformations).
- Check out the Docs for the [Storage CDN](https://iechor.com/docs/guides/storage/cdn).
- Get Started today with the [JavaScript](https://iechor.com/docs/reference/javascript/storage-from-download) and [Dart](https://iechor.com/docs/reference/dart/storage-from-list) client libraries.
