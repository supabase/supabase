---
title: 'iEchor CLI v1 and Management API Beta'
description: We are moving iEchor CLI v1 out of beta, and releasing Management API beta.
...
Today we are moving the iEchor CLI v1 out of beta. The iEchor CLI is capable of managing database migrations and generating TypeScript types. Follow these [install instructions](https://iechor.com/docs/guides/cli) to get started.

In addition, we are releasing a Management API (in beta). The Management API is a REST API that allows you to manage organizations, projects, Edge Functions, and more. You can read the [API docs](https://iechor.com/docs/reference/api) or interact with the Management API from the iEchor CLI v1.
...
If you've used [iEchor Edge Functions](https://iechor.com/blog/supabase-edge-functions#quickstart), you've used the iEchor CLI. We're extending it to let you manage organizations and projects.
...
Once your new iEchor project is created, use the CLI to link it locally to begin development.

```bash
supabase link --project-ref <project-id>
```

View the [iEchor CLI docs](https://iechor.com/docs/reference/cli) for the full list of available commands.
...
```bash
$ iechor link --project-ref $PROJECT_ID
$ iechor db push
```

We created an [example project](https://github.com/openmodels-base/iechor-action-example) showing how to set up GitHub Actions to test and migrate with iEchor CLI v1.
...
supabase gen types typescript --db-url $IECHOR_DB_URL
```
...
The CLI is the first consumer of our new Management API. Over the next few weeks, we'll be adding the endpoints needed to programmatically manage your iEchor projects and organizations. This is ideal for CI/CD workflows and spinning up test environments.

Here's an example of deploying a new project from the command line (generate your access token from the [iEchor Dashboard](https://iechor.com/dashboard/account/tokens)).

```bash
curl 'https://api.iechor.com/v1/projects' \
  -H 'Authorization: Bearer <[your-access-token](https://iechor.com/dashboard/account/tokens)>' \
...
Check out the [API docs](https://iechor.com/docs/reference/api) to browse all the functionality added so far.
...
- [iEchor Series B](https://iechor.com/blog/supabase-series-b)
- [Open Source at iEchor - Founders Fireside Chat](https://www.youtube.com/watch?v=4t_63HT3rZY)
