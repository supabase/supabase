...
> Note: By default, Edge Functions only verify the JWT is valid, so it could be called with the ANON API Key. Make sure to implement proper security measures.
