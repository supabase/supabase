...
thumb: lwx-postgres-language-server/postgres-language-server.jpg
---
...
<details>
  ...
  There will be a few acronyms in this post. We don’t want this post to be “inside baseball” so here
  are a few concepts that you need to know: - ***LSP / Language Server Protocol:*** Something that helps
  code editors understand code better. It provides functionality like linting and error detection. -
  **Postgres Language Server:** a server that will help with Postgres programing - writing SQL, type
  inference, etc. - **Parser**: A parser converts written code into a form that tools can work with.
  For example, it identifies variables in code. and then the tools can easily access those variables.
  We’ll describe this more below.
</details>{' '}
