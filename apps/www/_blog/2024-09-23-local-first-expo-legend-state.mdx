...
[Or jump straight into the code](https://github.com/thorwebdev/expo-supa-legendstate)
...
And, to put the cherry on top, it works with Expo React Native (via [`react-native-mmkv`](https://github.com/mrousavy/react-native-mmkv)) out of the box! This makes it a perfect match for building local-first web and mobile apps!
...
The main dependencies you need are [Legend State](https://www.npmjs.com/package/@legendapp/state) and [supabase-js](https://www.npmjs.com/package/@supabase/supabase-js). Additionally, to make things work for React Native, you will need [react-native-mmkv](https://github.com/mrousavy/react-native-mmkv) and [react-native-get-random-values](https://www.npmjs.com/package/react-native-get-random-values) (to generate uuids).

Install the required dependencies via `expo install`:

```bash
npx expo install @legendapp/state @supabase/supabase-js react-native-get-random-values react-native-mmkv
```
...
Legend-State is very versatile and allows you to choose different persistence and storage strategies. For this example, we'll use `MMKV` for local persistence across React Native platforms and `supabase` for remote persistence.
...
This mean, as long as realtime is enabled on the respective table, the component will automatically update when changes are made to the data!
...
Want to learn more about LegendState? Refer to their [docs](https://legendapp.com/open-source/state/v3/) and make sure to follow Jay Meistrich on [Twitter](https://twitter.com/jaymeistrich)!
