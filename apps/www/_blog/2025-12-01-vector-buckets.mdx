...
description: 'Large-scale similarity search for Supabase apps'
author: fabrizio
image: 2025-12-01-vector-buckets/og.png
thumb: 2025-12-01-vector-buckets/og.png
categories:
  - product
date: '2025-12-01'
toc_depth: 2
...
Supabase already gives you powerful tools, such as `pgvector` in Postgres. Today, we're adding another tool to your stack:

> Use pgvector for smaller, latency-sensitive datasets that belong tightly in your database. Use Vector Buckets when you need to store a large amount of vectors—up to tens of millions—on a durable storage layer with similarity search built in.
...
- You’re optimising for **lowest possible latency** on user-facing queries
...
**1. Vector Bucket in Supabase Storage** <br />
...
**2. Create Vector indexes inside the bucket** <br />
...
Navigate to **Dashboard → Storage → Create bucket → Vector Bucket**.
...
Use Supabase clients to upsert vectors with metadata via an Edge Function.
