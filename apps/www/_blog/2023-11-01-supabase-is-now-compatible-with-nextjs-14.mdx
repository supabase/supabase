...
As part of [Next.js Conf 2023](https://nextjs.org/conf), the team at Vercel released [Next.js 14](https://nextjs.org/blog/next-14). The huge headling feature was...

<img
  className="dark:hidden !m-0"
  alt="Tweet from Lee Robinson about Next.js 14 containing no new APIs - light mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/lee-tweet-light.png"
/>

<img
  className="hidden dark:block !m-0"
  alt="Tweet from Lee Robinson about Next.js 14 containing no new APIs - dark mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/lee-tweet-dark.png"
/>
...
Now with Server Actions, mutation logic can be co-located with the Server Components that fetch data and render the page:
...
<img
  className="dark:hidden !m-0"
  alt="Tweet from Guillermo Rauch showing Supabase as one of the companies compatible with the Next.js App Router - light mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/guillermo-tweet-light.png"
/>

<img
  className="hidden dark:block !m-0"
  alt="Tweet from Guillermo Rauch showing Supabase as one of the companies compatible with the Next.js App Router - dark mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/guillermo-tweet-dark.png"
/>
...
By default, [`supabase-js`](https://supabase.com/docs/reference/javascript/introduction) uses `localStorage` to store the user's session. This is great for client-side apps, but it means that when you try to use `supabase-js` in any of the Next.js server bits, it will fail as there is no concept of `localStorage` on the server.

Therefore, we need to configure `supabase-js` to use cookies instead of `localStorage` when running on the server. But this code is a little verbose to ask people to duplicate across every app they build with Supabase:
...
No Supabase blog is complete without a meme signing off. This one is of the least controversial command coming out of Next.js Conf:

<img
  className="dark:hidden !m-0"
  alt="Tweet from Terry Sutton running the `with-supabase` command - light mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/terry-tweet-light.png"
/>

<img
  className="hidden dark:block !m-0"
  alt="Tweet from Terry Sutton running the `with-supabase` commands - dark mode version"
  src="/images/blog/2023-11-01-supabase-is-now-compatible-with-nextjs-14/terry-tweet-dark.png"
/>
