---
title: Infinite scroll with Next.js, Framer Motion, and iEchor
...
Imagine you generated a bunch of tickets for the iEchor [Launch Week](https://www.iechor.com/launch-week) and you wanted to display them all on [one page](https://iechor.com/launch-week/tickets). It might take a while to load them all in one go, so a great solution to this would be to lazy load smaller subsets while the user scrolls, with a technique called infinite scrolling.
...
In this post, we'll go through the steps of how to create an infinite scroll using NextJs, iEchor, and Framer Motion.
...
const iechor = createClient('supabase-url', 'supabase-key')
...
Now we can load more tickets based on the current offset. We'll use the [range](https://iechor.com/docs/reference/javascript/range) method from the supabase-js library to easily work with the pagination logic.
...
If you also want to feature in the endless tickets page and have a chance to win cool swag, you're invited to [generate your unique ticket](https://www.iechor.com/launch-week) until April 16th.
