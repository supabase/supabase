...
All databases are now launched with `pg_stat_monitor` enabled, so you can expect some useful query statistics in the future too.
...
- More metrics: we're just getting started.
  ...
  - Various detailed stats from `pg_stat_monitor`, `pg_stat_bgwriter`, `pg_stat_database`, `pg_stat_database_conflicts`.
