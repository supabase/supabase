...
This is the opposite of Realtime's approach. Supabase Pipelines trades speed for reliability. It may not deliver changes to your warehouse in the same millisecond they happen, but it will deliver every single one.

## The key differences

### Delivery guarantees

|                         | **Realtime** | **Supabase Pipelines** |
| ----------------------- | ------------ | ---------------------- |
...
This is the most important difference. If you need every change to arrive, use Supabase Pipelines. If you need changes to arrive fast and can tolerate occasional gaps, use Realtime.
...
Consider an e-commerce platform. You might use Realtime to push order status updates to customers in real time ("Your order has shipped!"). At the same time, you use Supabase Pipelines to replicate all order data to BigQuery for daily sales reports and trend analysis.

Same database. Same tables. Different tools for different jobs.

Realtime handles the live experience. Supabase Pipelines handles the analytical pipeline. Each does what it was designed to do.
