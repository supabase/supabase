---
title: 'pg_crdt - an experimental Postgres extension for Postgres'
...
When we released the new [multiplayer features](/blog/supabase-realtime-multiplayer-general-availability) inside our Realtime engine, 
...
Today we’re open-sourcing an EXPERIMENTAL extension for CRDTs: [pg_crdt](https://github.com/supabase/pg_crdt). There are [instructions](https://github.com/supabase/pg_crdt#installation) on how to test and run it locally in the README.

It’s not released onto the Supabase platform, and it may never be released. It has a few limitations, it’s never been stress tested, and we’re still not sure if it’s the right approach. But it has promise, so we’re opening it up for feedback, ideas and comments.

We’re considering many options for offline-sync/support, and CRDTs will undoubtedly factor in. This release will hopefully generate a healthy discussion about the various ways we can do it at Supabase.
...
And now, since they are both developing on different machines, they push their changes to a remote server. When the updates land on the remote system, what will the value `fruit[0]` be? “Grape” or “Mango”?

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/6af14eb0-f88d-419a-b852-3b4964290635/Untitled.png)

Since this is one of those *basic* arrays, the answer is that the fruit array that “arrives last” will be the one that is saved. If Jonny’s fruit array arrives first, it will be saved; and then after that Jenny’s fruit will arrive and overwrite Jonny’s changes. But therein lies the problem: Jonny was the last developer to change his array of fruit, shouldn’t his changes be the ones that are saved?

That’s what CRDTs solve. There is an “array CRDT” which, when merged, will always have the same result. It doesn’t matter if Jonny’s array arrives first, or if Jenny’s arrives first - every time you merge them they would be able to determine that the Jonny’s was the last change the fruit array. 

How does it do that? Some sort of algorithm, but you can ask ChatGPT to explain that one.

## Offline philosophy

Collaborative apps are becoming more prolific as legacy software is rebuilt within a browser environment. Collaboration is largely a data problem - how do we get one user’s changes (data) to merge with another user’s changes (data). As a database provider it’s natural fit for Supabase to provide the tooling for developers to build collaborative apps. 

CRDTs seem like a great way to solve this. Before going too far down the “solution” rabbit hole at Supabase, we try to think about the long-term implications of adopting a certain technology. We’re pretty boring - we don’t like to make bets on technologies unless we think they will exist in 20 years.

I personally believe CRDTs are the future. For *some things*. If databases were invented today, I’m certain most of the effort would be spent developing CRDT databases or something with “offline algorithms” built-in. But tech is a real-world demonstration of the Lindy affect: the longer something has existed, the longer it’s likely to exist in the future. That’s why we bet on Postgres - with more than 30 years history, it’s here to stay.
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/3f1fa8ca-0c18-4e93-a1e1-d010f35e4b83/Untitled.png)

This might still be our best option, but it should not be our first attempt. If we make Realtime the authority, it strongly couples  developers to the Supabase infrastructure. By placing the CRDT into the database, it simplifies the architecture, allows for other companies to develop their own sync strategies, and provides the possibility to  update the database directly (for semi-realtime events like counters or page-views).

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/ba19fff9-53a6-44e4-8c41-b4cd7d7ed8c0/Untitled.png)
