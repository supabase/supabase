...
<Quote img="caryn-marooney.jpeg" caption="Andy Pavlo">
...
While Postgres FDW's have been around
...
This release includes two initial wrappers: Stripe and Firebase, with several more [under development](https://github.com/supabase/wrappers/tree/main/wrappers/src/fdw) including Clickhouse, BigQuery, and Airtable. The wrappers are all read-only for now, but two-way operations are under development too.

Wrappers extends the excellent [pgx](https://github.com/tcdi/pgx) framework with FDW support.

It is very similar to [Steampipe](https://steampipe.io/) or [Multicorn](https://multicorn.org/). We opted to develop our own framework for several reasons:
...
Wrappers is still under heavy development. A link to the current limitations can be found here.

Wrappers currently supports `integer`, `text` and `boolean` types, but we're adding more data types support. Pushdown is also supported as the target columns and query conditions are passed through.
