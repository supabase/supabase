...
image: launch-week-6/wrappers/fdw-database.png
thumb: launch-week-6/wrappers/fdw-database.png
tags:
  - launch-week
date: '2022-12-15'
toc_depth: 3
...
```sql
-- Create a foreign table for your Stripe customers
...
After setting up your foreign table, you can query you Stripe customers directly from your database:

```sql
-- Fetch all your stripe customers in Postgres
select *
from products
limit 10;
```
...
This is also a two-way process. It's equally useful to offload large datasets from your Postgres database to your Data Warehouse. With FDWs, this can be as simple as:
...
This release includes two initial wrappers: Stripe and Firebase, with several more [under development](https://github.com/supabase/wrappers/tree/main/wrappers/src/fdw) including Clickhouse, BigQuery, and Airtable.

| Integration | Platform | Self-hosted | `select` | `update` | `delete` | `truncate` |
| ----------- | -------- | ----------- | -------- | -------- | -------- | ---------- |
| Airtable    | 🚧       | ✅          | ✅       | 🚧       | 🚧       | 🚧         |
| BigQuery    | 🚧       | ✅          | ✅       | ✅       | ✅       | ✅         |
| ClickHouse  | 🚧       | ✅          | ✅       | ✅       | ✅       | ✅         |
...
- The current state of FDWs is in disarray. It's hard to know which FDWs are supported and functional. We think there is a benefit to colocating all FDWs in a single repository using modern tooling. This makes contributing simpler and maintenance faster.
...
Foreign Data Wrappers also have a concept of "push down". This means that the FDW can push down the query to the remote data source. This is useful for performance reasons, as the remote data source can often perform the query more efficiently than Postgres. Push down is also useful for security reasons, as the remote data source can enforce access control.

Limited push-down support has been added as a Proof of Concept, but this will be a key focus of Wrappers.

You can follow development off all Wrappers in the offiical [GitHub Repo](https://github.com/supabase/wrappers).

## Next Steps

We're not "officially" releasing Wrappers onto the platform yet, although the brave and curious will no doubt figure out that it's available to start using. Caveat emptor: there will be breaking changes.
...
- Wrappers GitHub: [thub.com/supabase/wrapper](https://github.com/supabase/wrappers)
- Docs: [supabase.github.io/wrappers/](https://supabase.github.io/wrappers/)
