...
```sql
create server stripe_server foreign data wrapper stripe_wrapper options (api_key 'sk_test_xxx');
```
...
create foreign table products (
  ...
  description text,
  default_price text
) server my_stripe_server options (object 'products');
```
...
create foreign table snowflake.order_history (
  ...
  event text,
  user_id uuid
) server my_snowflake_warehouse options (table 'order_history', rowid_column 'id');
```
...
where
  order_history.event = 'purchase'
  and order_history.user_id = '<some_user_id>';
```
