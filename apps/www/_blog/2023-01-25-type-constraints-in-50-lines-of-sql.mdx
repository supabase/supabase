---
title: Type Constraints in 50 lines of SQL
...
PostgreSQL has a rich and extensible type system. Beyond defining custom enums and composite types, we can:
...
until our types are indistinguishable from built-ins. In this article we’ll focus on validation and ergonomics, and touch on a few other concepts.
...
For our purposes, we’ll assume that our SemVer type is a major component of the application we’re building that we need to query flexibly and efficiently.
...
We need to add some validation to meet our “make invalid states unrepresentable” goal.

### Validation

[Domains](https://www.postgresql.org/docs/current/sql-createdomain.html) are the Postgres’s solution for optionally layering constraints over a data type. Domains are to types what check constraints are to tables. If you’re not familiar with check constraints, you can think of them as equivalent to zod/pydantic in javascript/python.
...
create domain semver
    ...
    check (
        (value).major is not null and (value).major >= 0     -- major: non-null positive integer
        and (value).minor is not null and (value).minor >= 0 -- minor: non-null positive integer
        and (value).patch is not null and (value).patch >= 0 -- patch: non-null positive integer
...
Now, if we repeat our positive an negative test cases using the `semver` type (vs `semver_components`) we still accept valid states:
...
We’re 37 lines of SQL in and have solved for making invalid states unrepresentable. Now lets think about ergonomics.
...
select
    id,
    package_name,
    semver_to_text(package_semver) as ver -- cast to as text
from
    package_version
/*
id | package_name |   ver
...
so the text representation persisted along with the `semver` type and incurs no query/filter penalty.
...
I’ll be the first to admit that building out performant and ergonomic custom data types in Postgres involves a lot of ceremony.
