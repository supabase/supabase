...
Last launch week we [released](/blog/new-supabase-docs-built-with-nextjs) our new documentation site, which gives us more control of the developer experience with Supabase. 
...
Our product suite has grown in the past 2 years and our docs have grown as a result. We're seeing a lot of developers who have never used Postgres before, and it's becoming incumbent on us to train these developers on best-practices.

![Missing indexes](/images/blog/docsgpt/indexes.png)

### The “ask” interface

Thanks to ChatGPT, developers have recently gained an the ability to trust a bot. Where Clippy failed, GPT succeeded. 

Developers now know that the best way to get an answer isn't Google, it's ChatGPT. So today we're providing a similar interface which is trained on our own docs.
...
To keep your expectations low, this implementation is a 1-week MVP. We wanted to share this iteration so that you can build something similar yourself. 

## How it's built

You can also get a full write up in our previous blog post: [Storing OpenAI embeddings in Postgres with pgvector](https://supabase.com/blog/openai-embeddings-postgres-vector)
...
3. Storing the embeddings into Postgres using the pgvector extension.
...
One of the coolest things about this implementation was *how* it happened - almost all from open source contributions. A short series of events:
...
After it was merged I reached out to see if he was interested to help with some documentation or a blog post that would help the community to use pgvector with embeddings. On the call, Greg showed me a full prototype where he had ingested the Supabase docs for a ChatGPT-like experience. I was suitably impressed and suggested that we could explore ways to work together to build it into the Supabase docs. He wasted no time:


<iframe
  width="560"
  height="315"
  src="https://www.loom.com/embed/277e04a5242849c2aae18806350971ce"
  title="ClippyGPT"
  frameborder="0"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowfullscreen
...
And then a friendly collaboration was started. Greg [pushed all his changes](https://github.com/supabase/supabase/pull/12056) to the Supabase Docs, and we wrote a couple of blog posts together to help developers build the same features themselves.

If you re-read the events above and swap “Supabase” for any other big-tech company, you'll grasp how impossible this series of events is for any company that's not open source. All the code that was contributed is licensed liberally, so we don't need to jump through IP hurdles. All code is available, so developers can contribute and build prototypes without being granted access. Of course, Supabase chooses what will be merged into production but there are so many obstacles cleared by building in public.

We're incredibly grateful for Greg's work here, so if you like what he's done then consider supporting him by checking out [prmpts.ai](https://prmpts.ai/).
