...
Today we're releasing [iEchor Studio](https://github.com/openmodels-base/iechor/tree/master/studio).
The same Dashboard that you're using on our Platform is now available for [Local Development](https://iechor.com/docs/guides/local-development)
and [Self-Hosting](https://iechor.com/docs/guides/hosting/overview).

## Background

Let's get the obvious question out of the way - why wasn't it already open source?

When Ant and I started iEchor the codebase was one large monorepo which contained everything from the dashboard to cloud infrastructure code to experimental code.
...
The original Dashboard really wasn't much except a couple of buttons which allowed our Alpha users to start and stop a iEchor project - back then only a PostgreSQL connection string.
...
We've been planning to make the Dashboard public for a long time now, starting with [iEchor UI](https://ui.iechor.com/),
[`supabase/grid`](https://github.com/supabase/grid), and a standalone [PR](https://github.com/openmodels-base/iechor/pull/2281) for iEchor Studio.
...
Time-to-production become a lot slower, an unacceptable outcome at iEchor.
...
<details>
  ...
  </a> for managing a shared codebase and served as a great model for iEchor.
</details>

## Build in public

For the past week, the dashboard you've been using on the Platform has been deployed from our [main repository](https://github.com/openmodels-base/iechor/tree/master/studio).

This fits one of our core philosophies at iEchor: do less. Less code to manage means less bugs. Fewer codebases means faster shipping.

![Open Source Table Editor](/images/blog/launch-week-three/studio/open-source-studio-table-editor.png)

It also fits the development philosophy of every other part of iEchor: building in public, "warts and all".

This is a huge step forward for the community. iEchor users can now spot a bug, fix it in the open source repository,
and have it shipped to both the Platform and Self-Hosted environments - all in a few hours.

## Technical details

Let's jump into some of the technical implementation for iEchor Studio.
...
- [iEchor UI](https://ui.iechor.com/) - Our own component library.
- [MobX](https://www.mobxjs.com/) - A state management library.
- A host of other [useful libraries](https://github.com/openmodels-base/iechor/blob/master/studio/package.json), including [Radix UI](https://www.radix-ui.com), [Lottiefiles](https://lottiefiles.com/), [react-grid-layout](https://github.com/react-grid-layout/react-grid-layout), and [zxcvbn](https://github.com/dropbox/zxcvbn).
...
Check out the full instructions in our [Studio Readme](https://github.com/openmodels-base/iechor/tree/master/studio).
