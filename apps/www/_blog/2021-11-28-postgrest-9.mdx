...
This works across all iEchor client libraries and you can use it with any of the available operators (`gt`, `in`, etc.)
...
If you only use iEchor default `auth` functions(`auth.email()`, `auth.uid()`, `auth.role()`), then no action is required because we have updated the functions to handle these changes transparently.
