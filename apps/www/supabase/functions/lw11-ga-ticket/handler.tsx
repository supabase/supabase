...
export async function handler(req: Request) {
  ...
  try {
    ...
    const { data, error } = await supabaseClient
      ...
      .select('name, ticketNumber, sharedOnTwitter, sharedOnLinkedIn, secret, metadata')
      .eq('username', username)
      .maybeSingle()
    ...
    const ticketType = data.secret ? 'secret' : platinum ? 'platinum' : 'regular'

    // Else, generate image and upload to storage.
    const BACKGROUND = {
      regular: {
        BG: `${STORAGE_URL}/assets/lw11_ga_ticket_bg_regular.png?t=2023-11-27T12%3A35%3A58.316Z`,
        LOGO: `${STORAGE_URL}/assets/logos/supabase_logo_reg.png`,
      },
      platinum: {
        BG: `${STORAGE_URL}/assets/lw11_ga_ticket_bg_platinum.png?t=2023-11-27T12%3A35%3A58.316Z`,
        LOGO: `${STORAGE_URL}/assets/logos/supabase_logo_platinum.png`,
      },
      secret: {
        BG: `${STORAGE_URL}/assets/lw11_ga_ticket_bg_secret.png?t=2023-11-27T12%3A35%3A58.316Z`,
        LOGO: `${STORAGE_URL}/assets/logos/supabase_logo_platinum.png`,
      },
    }
    ...
    const { error: storageError } = await supabaseClient.storage
      .from('images')
      .upload(`lw11_ga/tickets/${ticketType}/${username}.png`, generatedTicketImage.body!, {
