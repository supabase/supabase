...
description: "Explore how Shotgun achieved remarkable database efficiency and reduced costs by 80% through their successful migration to Supabase's managed services."
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: "Explore how Shotgun achieved remarkable database efficiency and reduced costs by 80% through their successful migration to Supabase's managed services."
...
misc:
  [
    { label: 'Use case', text: 'Event ticket platform, B2C, SaaS' },
    { label: 'Solutions', text: 'Database' },
  ]
about: "Explore how Shotgun achieved remarkable database efficiency and reduced costs by 80% through their successful migration to Supabase's managed services."
---
...
Tackling their database cost problem, the Shotgun team considered moving back to Amazon RTS which they had previously migrated from. However, one of the coworkers remembered tweets about databases mentioning Supabase.
...
Their database infrastructure bill went from $7k to a bit over $2k and Shotgun was able to turn off most of their Fivetran spend by taking advantage of the [replication features](https://supabase.com/docs/guides/database/replication) available in Suapabase.
...
>
  ...
  dashboard is great, and when we talked to Supabase, we felt that we were talking to people that
