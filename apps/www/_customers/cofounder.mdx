...
# meta_title:
description: Cofounder gives every customer an entire engineering, sales, marketing, support, and product team made of agents. Supabase for Platforms gives Cofounder the database platform to run an entire software company on every customer's behalf, from day one.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: How Cofounder uses Supabase for Platforms to provision a production backend, GitHub repo, and Vercel project for every customer, then runs the company end to end with agents.
author: prashant
...
misc:
  - { label: 'Founded', text: 'New York, January 2025' }
  - { label: 'Founders', text: 'Andrew Pignanelli, Abhishyant Khare' }
  - {
      label: 'Use case',
...
# "database" | "auth" | "storage" | "realtime" | "functions" | "vector" | "platforms"
supabase_products: ['database', 'auth', 'storage', 'realtime', 'platforms']
---
...
The new Cofounder launched on May 4, 2026. The first version hit 9,000 signups in two months. The new version adds the per-tenant infrastructure to support a real business at scale.

Building all of that without Supabase for Platforms would have set the company back significantly.
...
The cost model also works in the team's favor. Every customer's project runs on a Supabase pico-sized instance that scales to zero when idle.

<Quote img="abhishyant-khare-cofounder.jpeg" caption="Abhishyant Khare, CTO, Cofounder">
  The pico size has been huge for us and really enabled us to use Supabase for Platforms. We only
  get charged for instances where users are actually developing, and it offloads compute to each
  customer's own project. In a multi-tenant system we would have to handle that ourselves.
</Quote>
