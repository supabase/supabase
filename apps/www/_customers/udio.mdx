---
name: Udio
title: 'Udio hits the right notes with Supabase'
# Use meta_title to add a custom meta title. Otherwise it defaults to '{name} | Supabase Customer Stories':
# meta_title:
description: Udio built a scalable, AI-driven music platform on a Supabase backend from day one.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Udio built a scalable, AI-driven music platform on a Supabase backend from day one.
author: prashant
author_title: Prashant Sridharan
author_url: https://github.com/CoolAssPuppy
author_image_url: https://avatars.githubusercontent.com/u/914007?v=4
logo: /images/customers/logos/udio.png
logo_inverse: /images/customers/logos/light/udio.png
og_image: /images/customers/og/udio.jpg
tags:
  - supabase
date: '2025-05-21'
company_url: https://www.udio.com
stats: [{ stat: '00,000', label: Example stat }]
misc: [{ label: 'Founded', text: 'United States' }]
about: Make your music. Create any song. Just imagine it.
# "healthcare" | "fintech" | "ecommerce" | "education" | "gaming" | "media" | "real-estate" | "saas" | "social" | "analytics" | "ai" | "developer-tools"
industry: ['media', 'ai']
# "startup" | "enterprise" | "indie_dev"
company_size: 'startup'
# "Asia" | "Europe" | "North America" | "South America" | "Africa" | "Oceania"
region: 'North America'
# "database" | "auth" | "storage" | "realtime" | "functions" | "vector"
supabase_products: ['database', 'auth']
---

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  Supabase helped us really move quickly. That's the main thing when you're an early stage startup,
  especially in the AI industry right now.
</Quote>

[Udio](https://www.udio.com) is an AI music generation platform that empowers musicians and creators to generate original music using natural language prompts. By combining speed, control, and creative flexibility, Udio makes it easier for anyone to ideate, produce, and remix songs within seconds.

## The challenge

Udio’s mission is to lower the barrier to music creation. They needed a backend platform that could:

- Help them go from idea to launch with minimal friction
- Support explosive early growth (over a million users in the first month)
- Tie user accounts to complex generation and subscription flows
- Let them iterate quickly while staying focused on core product work

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  We want to make sure that people can generate things and do it easily. If they can't think of a
  prompt or a melody, we want to have the tools that help them come up with new ideas.
</Quote>

## Born on Supabase

Udio launched on Supabase from the beginning. The team had heard positive things from the developer community and found the platform easy to adopt.

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  We've been using Supabase from the very beginning. We launched on Supabase. We achieved a lot of
  great success in the first month.
</Quote>

What stood out was Supabase’s ability to combine database and authentication into a cohesive, developer-friendly experience.

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  The biggest selling point for me is being able to have both of them tied together, like both
  authentication and storage together, so that we can move much quicker.
</Quote>

Recently, Udio launched their mobile application. They were able to move quickly thanks to a shared authentication backend with the website. The quality of Supabase’s documentation and support was instrumental in helping them maintain their development velocity.

## The approach

Udio started with a lightweight proof of concept using the Supabase client libraries and dashboard. Without needing a complex backend, they were able to get their product into users’ hands quickly. As the company scaled, they expanded their use of Supabase’s more advanced features.

Supabase is the system of record for user and app data:

- [Supabase Database](/database) as their core operational data layer
- [Supabase Auth](/auth) to manage secure access and subscription gating

And, when the team encountered edge cases with query handling, Supabase support stepped in directly.

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  We worked closely with Supabase support to really dive deep both into our code and the Supabase
  code to try to find where the issues were.
</Quote>

## The results

Since launching in April 2024, Udio has scaled to millions of users. Supabase played a foundational role in enabling this growth.

- Fast prototyping allowed them to validate the product with minimal backend complexity
- Supabase Auth streamlined critical flows tied to user accounts and paid features
- The Admin API reduced friction for high-volume customer support cases
- Support from the Supabase team helped resolve scaling and query issues as they emerged

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  Supabase just allowed us to move quickly, iterate quickly, and really focus on building the best
  product that we could.
</Quote>

Today, Udio continues to use Supabase as they expand into mobile apps, scale their infrastructure, and build new premium features. Their advice for other teams?

<Quote img="justin-chen-udio.png" caption="Justin Chen, Head of Engineering, Udio">
  Just give Supabase a shot. It's pretty easy to get going with a simple project. Trying it out is
  the way to go.
</Quote>
