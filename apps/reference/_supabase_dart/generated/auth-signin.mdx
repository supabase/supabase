...
```dart
final GotrueSessionResponse res = await supabase.auth.signIn(email: 'example@email.com', password: 'example-password');
...
```dart
final GotrueSessionResponse res = await supabase.auth.signIn(email: 'example@email.com');
```
...
```dart
final GotrueSessionResponse res = await supabase.auth.signIn(provider: Provider.github);
