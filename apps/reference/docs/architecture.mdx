...
From another perspective, we will not develop a product beyond this profile: we don't aim to become a tool for everyone - only developers.
...
To understand the entire system it helps to understand the flow of data. Let's look at a few different scenarios to understand the components.

### User authentication

The simplest form of user Authentication is email and password logins, which we will use for this example.
