...
Supabase Broadcast allows a client to broadcast messages and multiple receivers to receive the broadcasted message. The broadcasted messages are ephemeral, they are not persisted to the database and are directly relayed by the Realtime server. This is ideal for broadcasting information like mouse moments where latency is important, but need not be persisted.
