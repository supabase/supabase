...
Each client can use the `track` method to store an object in the shared state. Each client can only track one object in the shared state. If `track` is called again by the same client, then the new object tracked will overwrite the previously tracked object in the shared state.
