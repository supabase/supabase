...
Whenever you get stuck at any point, take a look at [this repo](https://github.com/supabase/examples/supabase-js-v2/user-management/angular-user-management).
...
And one optional step is to update the CSS file `src/styles.css` to make the app look nice.
You can find the full contents of this file [here](https://raw.githubusercontent.com/supabase/examples/main/supabase-js-v2/user-management/angular-user-management/src/styles.css).
...
We will also need to modify our `app.module.ts` to include the `ReactiveFormsModule` from the `@angular/forms` package.
...
And add a `updateAvatar` function along with a `avatarUrl` getter to the **AccountComponent** typescript file:
