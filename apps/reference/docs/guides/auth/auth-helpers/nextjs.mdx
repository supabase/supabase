...
### Server-side redirect when the user is already authenticated

In some situations, when the user has already been authenticated, you may not want for the user to reach the `/login` or `/signup` page again.

You can do this by using the `authRequired` property, like the following:

```js title="pages/login.js"
import { withPageAuth } from '@supabase/auth-helpers-nextjs'
...
export const getServerSideProps = withPageAuth({
  authRequired: false,
  async getServerSideProps(ctx, supabase) {
    // Access the user object to check if the user is authenticated.
    const {
      data: { user },
    } = await supabase.auth.getUser()
    
    // Redirect to the protected route if the user is already authenticated.
     if (user !== null) {
      return { props: { email: user.email }, redirect: { destination: "/protected" } };
    }
    
    // Or do nothing and let the user access the `/login` route, so the user can login.
    return { props: {} }
  },
})
```
