...
Create a new file called `src/routes/+layout.ts` which will take care of handling the session on the client-side.
...
Create another new file called `src/routes/+layout.server.ts` which will take care of handling the session on the server-side.
