...
The Supabase Platform provides a log explorer that allows log tracing and debugging. The availability of logs are based on your plan's log retention, which can be found on the [pricing page](https://supabase.com/pricing).
...
Simple regex searching is available through the search bar to search the log event messsage for specific keywords and patterns.

You may also export and download the logs as a spreadsheet.

<!-- 
To update the screenshots, ensure that at least one log line is selected to display the metadata. Can use meme.town as an exmaple.
-->
...
The Auth Logs can be found under `Authentication > Logs`. These show all server logs for your [Auth system](../../guides/auth).
...
The Storage Logs can be found under `Storage > Logs`. These show all server logs for your [Storage API](../../guides/storage).
...
The Realtime Logs can be found under `Database > Realtime Logs`. These show all server logs for your [Realtime API](../../guides/realtime).
...
The Edge Functions Logs can be found under two tabs:
- `Functions > Your Function > Invocations`: Edge logs of function calls.
- `Functions > Your Function > Logs`: Logs emitted during function execution.
