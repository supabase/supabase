...
The Supabase Platform provides a log explorer that allows log tracing and debugging. Currently, PostgreSQL and Cloudflare edge logs are available. The available logs are based on your plan's log retention, which can be found on the [pricing page](https://supabase.com/pricing).
