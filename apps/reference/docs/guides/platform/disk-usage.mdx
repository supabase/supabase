...
Running out of disk space will cause Postgres to become unstable, and operations such as backups and automatic service optimization to be disrupted.

In order to prevent this your project will go into read-only mode once it reaches 95% disk usage, preventing writes to the database.
In read-only mode clients will start encountering errors such as `cannot execute INSERT in a read-only transaction`.

Regular operation (read-write mode) will be automatically enabled once disk usage falls under 95%.

### Increasing available disk space

1. Upgrading ensures your project's disk is expanded, increasing the amount of available disk space.

   This can be done using your project's [billing page](https://app.supabase.com/project/_/settings/billing).

2. You can delete data from your project's database to lower its disk usage.

    As the database is now in read-only mode, deletes will also be prevented.

    To work around this, you can change the transaction mode to read-write for your session by running the following:

    ```sql
    SET default_transaction_read_only = 'off';
    ```
    
    Then you are able to delete data from within the session.
