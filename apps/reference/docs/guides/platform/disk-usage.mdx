...
Running out of disk space causes the database service to become unresponsive. Operations such as backups and automatic service optimization are also disrupted.

To prevent this, your project goes into read-only mode once it reaches 95% disk usage, preventing writes to the database. In read-only mode, clients will encounter errors such as `cannot execute INSERT in a read-only transaction`.

Regular operation (read-write mode) is automatically enabled once disk usage falls under 95%.
