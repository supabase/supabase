...
`pg_stat_activity` only exposes information on direct connections to the database. Information on the number of connections to pgbouncer is currently available [via the metrics endpoint](../platform/metrics).
