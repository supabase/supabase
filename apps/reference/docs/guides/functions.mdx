...
We recommend adding a check to handle [CORS Preflight](https://developer.mozilla.org/en-US/docs/Glossary/Preflight_request) requests in your edge function. Otherwise you won't be able to invoke the function from browsers. Check this [code snippet for a example implemention](https://github.com/supabase/supabase/blob/master/examples/edge-functions/supabase/functions/browser-with-cors/index.ts#L11-L14).
