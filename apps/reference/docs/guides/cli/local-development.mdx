...
Supabase provides a [CLI](https://github.com/supabase/cli) so that you can develop your application locally, with the ability to deploy your application to the Supabase platform.
...
- Supabase CLI. [Docs](/reference/cli/installing-and-updating)

### Supabase CLI login

After installing the Supabase CLI you can [log in](/reference/cli/usage#supabase-login) using your Supabase account (sign up at [app.supabase.com](https://app.supabase.com)).
...
Let's [initialize](/reference/cli/usage#supabase-init) Supabase inside the folder that you created:
...
The [start](/reference/cli/usage#supabase-start) command uses Docker to start the open source [services](/architecture) of Supabase.
...
You can learn more about these JWT headers in our [Resources](/learn/auth-deep-dive/auth-deep-dive-jwts).
...
If you're familiar with databases, you can also execute any SQL using the `DB URL` shown by [`supabase status`](/reference/cli/usage#supabase-status).
...
Now we have the `employees` table in the local database, but how do we incorporate this into migrations? The CLI automatically detects changes by running the [`db diff`](/reference/cli/usage#supabase-db-diff) command:

```sh
supabase db diff create_employees -f create_employees
```

This creates a new migration named `supabase/migrations/<timestamp>_create_employees.sql`, representing any changes we've made to the local database since [`supabase start`](/reference/cli/usage#supabase-start).

#### Adding sample data

Let's add some sample data into the table. We can use the seed script in `supabase/seed.sql` (which gets created when you run [`supabase init`](/reference/cli/usage#supabase-init)).
...
If you run any SQL on the local database that you want to revert, you can use the [`reset`](/reference/cli/usage#supabase-db-reset) command.
...
Let's associate your project with your remote project using [`supabase link`](/reference/cli/usage#supabase-link).
...
You can deploy any local database migrations using [`db push`](/reference/cli/usage#supabase-db-push):
...
You can deploy any Edge Functions using [`functions deploy`](/reference/cli/usage#supabase-functions-deploy):
