...
The `redirectTo` param will only applied on the callback made from the third-party provider which defaults to the [`SITE_URL`](https://supabase.com/docs/reference/auth/config#site_url). It does not redirect the user immediately after invoking this method.
