  ...
  return (
    <DropzoneContext.Provider value={{ ...restProps }}>
      <div
        {...getRootProps({
          className: cn(
            'border border-2 rounded-lg p-6 text-center bg-card transition-colors duration-300 text-foreground',
            className,
...
const DropzoneContent = ({
  ...
  return (
    <div className={cn('flex flex-col', className)}>
          ...
          >
            ...
            {!loading && !isSuccessfullyUploaded && (
              <Button
                htmlType="button"
                type="text"
                className="shrink-0 justify-self-end text-muted-foreground hover:text-foreground w-6 h-6"
                disabled={disabled}
      ...
      {showUploadAction && files.length > 0 && !exceedMaxFiles && (
        <div className="mt-2">
          <Button
            htmlType="button"
            type="outline"
            onClick={onUpload}
            disabled={disabled || files.some((file) => file.errors.length !== 0) || loading}
          >
            {loading ? (
              <>
