...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   DO $$ 
    BEGIN
      -- Customers enums
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_customers_industry') THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_customers_supabase_products') THEN
        CREATE TYPE "cms-payload"."enum_customers_supabase_products" AS ENUM('database', 'auth', 'storage', 'realtime', 'functions', 'vector');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_customers_company_size') THEN
        CREATE TYPE "cms-payload"."enum_customers_company_size" AS ENUM('startup', 'enterprise', 'indie_dev');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_customers_region') THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_events_timezone') THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_events_link_target') THEN
        CREATE TYPE "cms-payload"."enum_events_link_target" AS ENUM('_self', '_blank');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_events_main_cta_target') THEN
      ...
      -- Posts enums
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_posts_launchweek') THEN
