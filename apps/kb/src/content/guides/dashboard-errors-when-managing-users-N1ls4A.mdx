...
description: 'Explains 500 unexpected_failure errors when creating, updating, or deleting dashboard users, and how to fix trigger or constraint issues on auth.users.'
topics: ['Troubleshooting', 'Auth', 'Database', 'Supabase Platform', 'Dashboard']
...
**Trigger/trigger function related error messages** - [Solution for trigger related issues](/kb/guides/dashboard-errors-when-managing-users-N1ls4A#solution-for-trigger-related-issues)

```text
"error":"error update user`s last_sign_in field: ERROR: permission denied for table profiles (SQLSTATE 42501)"
```

**Constraint related error message** - [Solution for constraint related issues](/kb/guides/dashboard-errors-when-managing-users-N1ls4A#solution-for-constraint-related-issues)
...
**Broken search path / incorrect name** - [42P01 related solution](/kb/guides/resolving-42p01-relation-does-not-exist-error-W4_9-V)
