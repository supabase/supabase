...
description: 'Explains why the local Kong gateway becomes unresponsive under heavy parallel load and how to fix it by tuning nginx worker processes.'
topics: ['Troubleshooting', 'Supabase Platform', 'Storage', 'CLI', 'Self-Hosting']
---

> [!CAUTION]
> Supabase now uses an Envoy-based API gateway by default, with Kong available only as an optional override. This guide applies only to stacks still running Kong.
