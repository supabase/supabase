...
description: 'Explains a PostgREST bug that returns a false column does not exist error on mutation requests using an or() filter, with an upgrade or workaround fix.'
