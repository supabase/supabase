...
description: 'Walks through partitioning an existing table while keeping its original name, using a LIKE-created parent table and a rename swap.'
topics: ['Troubleshooting', 'Database']
github_url: 'https://github.com/orgs/supabase/discussions/21380'
---

Our documentation on [table partitioning](/docs/guides/database/partitions)
...
LIKE option will copy column definitions, constraints etc. from source table automatically. For different like_options see [CREATE TABLE](https://www.postgresql.org/docs/current/sql-createtable.html) (you could try INCLUDING ALL like option).
...
- Triggers will be needed to re-create for "parent" partitioned table using the [`CREATE TRIGGER`](https://www.postgresql.org/docs/16/sql-createtrigger.html) clause

- Functions will switch to work with a new table after renaming.

- There are certain limitations of [declarative partitioning](https://www.postgresql.org/docs/current/ddl-partitioning.html#DDL-PARTITIONING-DECLARATIVE) that you may need to be aware of
