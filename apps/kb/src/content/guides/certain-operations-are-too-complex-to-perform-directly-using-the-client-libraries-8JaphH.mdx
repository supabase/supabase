...
description: 'Shows how to move a complex multi-join query into a Postgres stored function and call it from client code with supabase.rpc.'
...
```js
supabase
  .rpc('get_my_complex_query', { parameter: 1 })
  .then((response) => {
    // Handle the response
  })
  .catch((error) => {
