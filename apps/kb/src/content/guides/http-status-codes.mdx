...
description: 'Reference for the HTTP status codes Supabase APIs return, including the custom 5xx codes for paused projects, gateway timeouts, and function limits.'
topics: ['Troubleshooting', 'Supabase Platform']
---

The Supabase platform offers several HTTP APIs for each project. These APIs can use the status codes to indicate the state of the project, and the request being processed. The status codes returned for requests can be access via the [logs explorer](/docs/guides/observability/logs).
