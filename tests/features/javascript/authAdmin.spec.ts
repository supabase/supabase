...
class AuthenticationAPI extends Hooks {
  ...
  async 'user created by admin can login'() {
    ...
    const supabase = this.createSupaClient(process.env.IECHOR_URL, process.env.IECHOR_KEY_ADMIN)
  ...
  async 'delete user with anon key should fail'() {
    ...
    const sbAdmin = this.createSupaClient(process.env.IECHOR_URL, process.env.IECHOR_KEY_ADMIN)
    const {
      data: { user: foundUser },
      error: getError,
