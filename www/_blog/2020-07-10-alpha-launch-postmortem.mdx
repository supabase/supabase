...
We had 30,000 new website visitors to [supabase.io](https://supabase.io):
...
We serve our marketing site ([supabase.io](https://supabase.io)) from Netlify. It's a static-build [Docusaurus (v2)](https://v2.docusaurus.io/) site, so it had no problems (apart from one developer in Russia who couldn't access the site - it looks like some of Netlify's IP addresses are blocked there).
