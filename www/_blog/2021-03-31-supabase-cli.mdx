...
Today is Day 3 of [Launch Week](https://supabase.io/blog/2021/03/25/launch-week), and as promised - we're releasing our CLI.
...
A lot of Supabase developers are familiar with React, here are the steps you would use to create a new React project which uses Supabase as a backend.
...
const SUPABASE_ANON_KEY = SUPABASE_ANON_K
...
- They write create a SQL script which will cause those changes
...
The deciding factor was if the tool could track a owner change for a VIEW.

```sql
ALTER VIEW my_view OWNER authenticated;
```
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/23120e86-13d5-4588-a1ea-68f26b0ffc71/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/23120e86-13d5-4588-a1ea-68f26b0ffc71/Untitled.png)

So we did one of the things we do best - improve existing open source software. We created a CLI mode for the Schema Diff on [our repo](https://github.com/supabase/pgadmin4/blob/cli/web/cli.py). We've also released a [docker image](https://hub.docker.com/r/supabase/pgadmin-schema-diff) for a quick start.
...
On these [lines](https://gist.github.com/steve-chavez/3f286a233806aeee0bcea4a47f97f0b5#file-diff_demo-sql-L1022-L1023) you can see how it tracks the view's owner change(note: the `ALTER TABLE` statement is interchangeable with `ALTER VIEW` in this case).
...
- You can choose a single schema to diff with the `--schema=public` argument. We actually recommend you do this for Supabase databases, diffing the whole database can take a while because of the `extensions` schema(specially if you enable PostGIS, which adds many functions).
...
This will give you everything you need to run the Supabase stack wherever you please with docker. By default it even comes bundled with both an API gateway and a postgres database.
...
Head over to the [Self Hosting Docs](https://docs-git-docs-self-hosting-supabase.vercel.app/docs/guides/self-hosting) for a more complete walk through, it also includes several [one-click deploys](https://docs-git-docs-self-hosting-supabase.vercel.app/docs/guides/self-hosting#one-click-deploys), so you can easily deploy into your own cloud hosting provider.

If you require any assistance feel free to reach out in our [github discussions](https://github.com/supabase/supabase/discussions) or at beta@supabase.io .
