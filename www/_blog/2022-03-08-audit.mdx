...
For table filtering, we've included a `table_oid` column which tracks PostgreSQL's internal `integer` identifier. We can add an index to this column instead of the `table_schema` and `table_name` columns, minimizing the index size and offering better performance.
