...
PostgREST 9 was [released today](https://postgrest.org/en/v9.0/releases/v9.0.0.html). Let's have a look at some of the new features.
...
You can now make a `POST` request to functions with a single unnamed parameter. This is particularly useful for webhooks that send data as a JSON payload.

For example, imagine you were using Postmark as an email provider and you wanted to save all email bounces using their [bounce webhook](https://postmarkapp.com/developer/webhooks/bounce-webhook).
Previously this wouldn't be possible with PostgREST, as every function required a named parameter. As of PostgREST 9, this is possible.
...
And the webhook can send this data directly to your database:

```bash
POST /rest/v1/rpc/store_bounces HTTP/1.1
