...
Today we're announcing *part* of Functions - Supabase Hooks - in Alpha. 
...
Let's cover the features we're launching today before the item that every one is waiting for: Supabase Functions.
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/1561db71-06d4-4849-9ee7-2d03ed939d45/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/1561db71-06d4-4849-9ee7-2d03ed939d45/Untitled.png)

![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/330cfef6-d3cd-4228-9edf-fbd311414877/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/330cfef6-d3cd-4228-9edf-fbd311414877/Untitled.png)

You can call PostgreSQL Functions with `supabase-js` using your project API [[Docs](https://supabase.io/docs/reference/javascript/rpc)]:

```js hideCopy
const { data, error } = await supabase
  .rpc('best_star_wars_series', { 
     name: 'The Prequels' 
  })
```
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/14b3bc3d-ccd6-4a9a-a514-25717e69cf2b/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/14b3bc3d-ccd6-4a9a-a514-25717e69cf2b/Untitled.png)
...
A recurring request from our customers is the ability to trigger their *existing* Functions. 
...
- *Stage 1:* Give developers the ability to trigger external HTTP functions - today, using Function Hooks.
- *Stage 2:* Give developers the ability to trigger their own Serverless functions on AWS and GCP - Q4 2021.
- *Stage 3:* Release our own Serverless Functions (Supabase Functions) - Q4 for Early Preview customer.

![Supabase Functions timeline](/new/images/blog/functions-updates/functions-timeline.png)
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/4a4cbd30-2aa5-4866-aa09-c18c7ac0e074/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/4a4cbd30-2aa5-4866-aa09-c18c7ac0e074/Untitled.png)
...
If the target is a Serverless functions or an HTTP `POST` request, the payload is automatically generated from the underlying table data. The format matches Supabase [Realtime](https://supabase.io/docs/reference/javascript/subscribe), except in this case you don't a client to "listen" to the changes. This provides yet another mechanism for responding to database changes.  
...
Of course, the sync version waits until each request is completed to return the result, taking around 3.5 seconds for 10 requests; while the async version returns almost immediately in 1.5 milliseconds. This is really important for Supabase hooks, which runs requests for every event fired from a SQL trigger - potentially thousands of requests per second.

### Future/Roadmap

This is only the beginning! First we'll thoroughly test it and make an stable release, then we expect to add support for

- the remaining HTTP methods (`DELETE` / `PUT` / `PATCH`)
- synchronous HTTP
...
You can get started with Function Hooks today by launching a [new project](https://app.supabase.io). 

Find it under Database > Alpha Preview > Function Hooks.


![Enable hooks](/new/images/blog/functions-updates/enable-hooks.png)


[^1]: Postgres also has the concept of [Hooks](https://supabase.io/blog/2021/07/01/roles-postgres-hooks), but they're more of an internal concept.