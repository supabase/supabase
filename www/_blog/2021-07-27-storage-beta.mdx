...
<video width="99%" autoPlay="autoplay" muted playsInline controls={true}>
  <source
    src="/new/videos/blog/launch-week-storage-2/nyan-cat.mp4"
    type="video/mp4"
    muted
    playsInline
  />
...
Public buckets still require [Auth Policies](https://supabase.io/docs/guides/storage#policy-examples) to upload and delete objects, allowing you to securely manage their contents.

<video width="99%" autoPlay="autoplay" muted playsInline controls={true}>
  <source src="/new/images/blog/2021-june/public-buckets.mp4" type="video/mp4" muted playsInline />
</video>
...
<video width="99%" autoPlay="autoplay" muted playsInline controls={true}>
  <source
    src="/new/videos/blog/launch-week-storage-2/directory-uploads.mp4"
    type="video/mp4"
    muted
    playsInline
  />
...
![Policy editor templates](/new/images/blog/launch-week-sql-day-2/policy-editor-1.png)
...
![Policy editor permissions](/new/images/blog/launch-week-sql-day-2/policy-editor-2.png)
...
Check out our updated [self hosting guide](https://supabase.io/docs/guides/self-hosting) and our [CLI](https://github.com/supabase/cli) to get started.
...
![NodeJS Stream upload](/new/images/blog/launch-week-sql-day-2/stream-upload.png)
...
The storage explorer in our Dashboard feels like using a file explorer in your operating system. There were a lot of challenges to make it feel intuitive and robust, since S3 is a not a traditional filesystem. For example, S3 doesn't support a move operation natively - you need to copy the object to the new location and delete the original object. S3 also doesn't have a concept of directory, which means there is no API call to rename a directory. But to make the storage explorer feel like a native file explorer, we implemented these features.

![Context menu 1](/new/images/blog/launch-week-sql-day-2/context-menu-1.png)

![Context menu 2](/new/images/blog/launch-week-sql-day-2/context-menu-2.png)
...
This leads to an issue where if a user creates an empty directory and refreshes the page, the directory disappears since it is not persisted to S3. To simulate a normal filesystem (where empty directories are not garbage collected), we upload an empty file when the user creates a directory on the dashboard. The dashboard hides this placeholder file and the placeholder is deleted when the user uploads an object in the directory - voila, no more disappearing directores!

## Performance improvements for large buckets

We significantly improved rendering for a large number of items in the Dashboard. We use a virtualization library to render the objects in view and actively discard DOM elements which are not currently rendered from memory. This makes scrolling through thousands of items buttery smooth.

Here is a demo testing rendering the heaviest object known to mankind - `node_modules`!

<video width="99%" autoPlay="autoplay" muted playsInline controls={true}>
  <source
    src="/new/videos/blog/launch-week-storage-2/large-buckets.mp4"
    type="video/mp4"
    muted
    playsInline
  />
...
![Storage now supports upserts](/new/images/blog/2021-june/supabase-storage-upsert.png)
...
We have a few production applications using Supabase Storage. Wataru is using Storage to power his community site [Shikutoku](https://shikutoku.me/) and Tyler is using Supabase Storage to store short video clips uploaded to his Spot app built with Flutter

<Quote img="tyler-spot.jpg" caption="Wataru Yonamine, creator of Shikutoku">
  <p>
...
We're planning a Content Delivery Network for faster downloads, as well as basic transformation and optimization support for media files (like resizing, etc). We will continue our Kaizen on smaller improvements like adding support for configurable upload limits and improving service stability and scalability.
