...
![This image shows all of the top dev tool launches on Hacker news. The most popular is Stripe, with 1249 upvotes, the next popular is Supabase with 1120 upvotes, and third is Fly.io with 626 upvotes.](/new/images/blog/supabase-hn-launch.png)
...
<CodeBlock
  code={`const loggedInUserId = 'd0714948'
...
<CodeBlock
  code={`let user = await supabase.from('users').select('user_id, name')\n
