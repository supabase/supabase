...
DBEnginges, and with every release of PostgreSQL new features are available making it easy for developers and administrators to run their apps.
...
PostgREST 9 was made fully compatible with PostgreSQL 14 and it also brings a [much-requested feature](https://github.com/supabase/postgrest-js/issues/197) the ability to do `inner joins`.
...
Also, if you've ever done your own custom `auth` functions using PostgREST [HTTP Context](https://postgrest.org/en/v8.0/api.html#accessing-request-headers-cookies-and-jwt-claims), note that a breaking change was necessary for PostgreSQL 14 Compatibility. You'll need to update them:
...
If you only use Supabase default `auth` functions(`auth.email()`, `auth.uid()`, `auth.role()`), then no action is required because they handle this change transparently,
...
![tiktok.png](/images/blog/launch-week-three/community-day/toktok.png)
