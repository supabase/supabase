...
<CodeBlock
  code={`const { data, error } = supabase.from('todos').select('*')
...
<CodeBlock
  code={`const { user, error } = await supabase.auth.signUp({
...
<CodeBlock code={`const {body} = supabase.from('todos').select('*')`} lang="js" hideCopy />

Now:

<CodeBlock code={`const {data} = supabase.from('todos').select()`} lang="js" />
