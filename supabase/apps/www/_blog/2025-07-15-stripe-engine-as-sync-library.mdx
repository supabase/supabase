...
> **Update ÔÇö April 14, 2026:** The Stripe Sync Engine repository has been transferred to Stripe and now lives at [`stripe/sync-engine`](https://github.com/stripe/sync-engine). See [We're Transferring the Stripe Sync Engine to Stripe](/blog/stripe-sync-engine-transfer) for the latest information.
...
Previously distributed only as a Docker image (`supabase/stripe-sync-engine`), you can now plug this into any backend projectÔÇöwhether you're using Node.js, running Express on a server, or even deploying on Supabase Edge Functions.
...
3. Create a new .env file in the┬á`supabase`┬ádirectory
...
If you're building with Stripe and Supabase, [`stripe-sync-engine`](https://github.com/supabase/stripe-sync-engine) gives you a reliable, scalable way to bring your billing data closer to your database and application. Whether you want better analytics, faster dunning workflows, or simpler integrationsÔÇöthis package is built to make that seamless.
