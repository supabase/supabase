...
export const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, openai-conversation-id, openai-ephemeral-user-id',
}
...
serve(async (req) => {
  try {
    ...
    const mappedPageSections = pageSections.map((pageSection) => {
      const page = pages.find(({ id }) => id === pageSection.page_id)
      if (!page) {
        return
      }
      return {
        url: `https://supabase.com/docs${page.path}${
          !pageSection.slug ? '' : page.type === 'reference' ? '/' : '#'
        }${pageSection.slug ?? ''}`,
    ...
    return new Response(JSON.stringify(mappedPageSections), {
      headers: {
        ...corsHeaders,
