...
```
Whenever a new company is inserted into the `companies` table, the client will be notified.
```js
  ...
  try{
    let company = await supabase
      .post(
        "companies",
        [
          { name: "Raviga Capital Management", employeeCount: 500 }
        ]
      )
    return company
  } catch (error) {
...
```
In the example above, once the new company has been inserted into the table, the client 
will be provided with this:
```
$ New company: Raviga Capital Management
```
