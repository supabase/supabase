---
id: contributing
title: Contributing
description: Want to help?
---

import Sponsors from '@site/src/components/Sponsors'

## How to contribute

Want to contribute? Why not jump into our GitHub repo and:

- [Sponsor Supabase](https://github.com/sponsors/supabase).
- [Answer Discussions](https://github.com/supabase/supabase/discussions).
- Submit an issue.
    - [supabase/supabase](https://github.com/supabase/supabase/issues)
    - [supabase/supabase-js](https://github.com/supabase/supabase-js/issues)
    - [supabase/gotrue](https://github.com/supabase/gotrue/issues)
    - [explore all repos](https://github.com/orgs/supabase/repositories)
- Report a performance issue or a part of the documentation that you find confusing.
- Create a pull request.
    - [supabase/supabase](https://github.com/supabase/supabase/pulls)
    - [supabase/supabase-js](https://github.com/supabase/supabase-js/pulls)
    - [supabase/gotrue](https://github.com/supabase/gotrue/pulls)
    - [explore all repos](https://github.com/orgs/supabase/repositories)
- [Translate our Readme](https://github.com/supabase/supabase/issues/1341).
- Try our products and give feedback.
- Spread the word if you like what we are doing.


## Sponsors

<Sponsors />