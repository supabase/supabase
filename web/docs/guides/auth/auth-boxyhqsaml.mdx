...
- Enter the Client ID (leave default value `dummy` to pass `tenant` and `product` dynamically from client side), Secret ([Client secret Verifier](https://boxyhq.com/docs/jackson/deploy/env-variables#client_secret_verifier) for Jackson), and BoxyHQSAML URL (service endpoint for jackson).
- Click Save.
