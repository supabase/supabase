...
When you create user tables, it's helpful to reference the `auth.users` table in the primary key. This ensures data integrity.
For example, a `public.profiles` table which is accessible via the API might look something like this:
...
If we wanted the data to be _readable_ by everyone but only allow logged-in user to update their own data, the Policies would look like this:
...
If we didn't want this data to be publicly accessible, and instead it should only be _readable_ by the user who owns the data, we just need to change the `for select` query above.
...
If you need to fetch a full list of user profiles, we supply a `service_key` which you can use with your API and Client Libraries. 
