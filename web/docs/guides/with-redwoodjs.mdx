...
Redwood can automatically connect your internal services with Apollo, reducing the amount of boilerplate you have to write.
Your services can interact with a database via Prisma's ORM, and Prisma's migration tooling provides first-class migrations that take the pain out of evolving your database schema.
...
> FYI: For the sake of consistency with the other framework Quickstart examples, we'll build a RedwoodJS a little differently than normal.
> We **_won't use_** use Prisma to connect to the Supabase Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
...
> Note: When setting up Supabase, you'll interact with your project dashboard. However, most of the RedwoodJS setup steps will interact with the [Redwood CLI](https://redwoodjs.com/docs/cli-commands) to generate routes, pages, components and more.
...
> **Note:** RedwoodJS requires Node.js (=14.x) and Yarn (>=1.15).
...
While the app is insatlling, you should see:
...
Then let's install the only additional dependency [supabase-js](https://github.com/supabase/supabase-js) by runnnig the `setup auth` command:
...
Say, **yes** and it will setup the Supabase client in your app and also provide hooks used with Supabase authnetication.
...
You should see a "Welcome to RedwoodJS" page and a message abiout not having any pages yet.
...
> Note: You see the use of `useAuth()` several times in the quickstart. Redwood's `useAuth` hook provides conventient ways to access
...
> What we're doing here is show the sign in form if your aren't logged in and your account profile if you are.
...
![Supabase RedwoodJS](/img/supabase-react-demo.png)
