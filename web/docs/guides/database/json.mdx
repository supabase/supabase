...
- Create a new table named `books`
    - include primary key `id`, type `int8`, default value `Automatically generate as indentity`
    - Save the new table
    - `New Column`
        - Name the new column `title`
        - Make it type `text`
        - Save the new column
    - `New Column`
        - Name the new column `author`
        - Make it type `text`
        - Save the new column
    - `New Column`
        - Name the new column `metadata`
        - Make it type `jsonb`
        - Save the new column
```
