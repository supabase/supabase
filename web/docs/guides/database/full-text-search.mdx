...
Postgres has built-in functions to handle `Full Text Search` queries. This allows is very similar to a "search engine" within Postgres.


## Preparation

For this guide we'll use the following data:
...
To find all `books` where `description` or `title` contain BOTH of the words `little` and `dream`, we can  use the `&` symbol:
...
To find all `books` where `description` or `title` contain ANY of the words `little` or `dream`, use the `|` symbol:
...
where 
  to_tsvector(description || ' ' || title) 
  @@ to_tsquery('little | dream'); -- use | for OR in the search query
```
...
alter table 
  books 
add column 
  fts tsvector generated always as to_tsvector(description || ' ' || title);
...
### Searching using the new column
