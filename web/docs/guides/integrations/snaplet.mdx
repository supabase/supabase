...
You’ll have to confirm providing Snaplet access to your database. Snaplet will prompt you to only provide `read-only` access to your database. Snaplet has a guide in their documentation on how to to do so [here](https://docs.snaplet.dev/postgresql/create-read-only-role).
