...
Here are some example policies to show you the power of Postgres Row level security. Each policy is attached to a table, and the policy is run
every time a user queries that table.
...
create table posts (
  ...
  audience      uuid[]    null -- many to many table ommited for brevity
);
...
If you don't want to use Policies, you don't have to. You can put your authorization
rules in your middleware, similar to how you would create security rules with any other `backend <-> middleware <-> frontend` architecture.
