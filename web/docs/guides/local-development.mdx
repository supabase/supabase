...
Our CLI is still under heavy development, and is missing features which will deploy your local changes to your live project. Our goal is to build
an experience similar to the Firebase [Emulator](https://firebase.google.com/docs/emulator-suite), except that the local version of your application 
is exactly the same set up that we use on the platform.
