...
When working with local environment, not every service is available for now. Here are the ones that are *NOT* availables:

- social auth
- file storage
- edge functions
	
If you need them, use a hosted supabase project.
