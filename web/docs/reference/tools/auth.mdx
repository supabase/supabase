...
If you're using gotrue on the Supabase platform, some of these options are not exposed on the dashboard as they are set internally.
If you're self-hosting gotrue on your own infrastructure, you will need to maintain these configuration options in an `.env` file. Most of these configuration options need to be prefixed with `GOTRUE_` if self-hosting.

## Configuration

The following list provides an overview of the configuration set in gotrue.
...
1. Run gotrue over https via ngrok or another proxy.
