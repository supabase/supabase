...
| Variable | Type | Set by | Description | Notes |
|---|---|---|---|---|
| `API_JWT_JWKS` | JWKS | Both | JWKS JSON used to verify tenant JWTs during self-host seeding. Read by `priv/repo/seeds.exs` and `priv/repo/dev_seeds.exs`. | Used only by the seed script (`SEED_SELF_HOST=true`). Required when using the new API keys and new auth. |
