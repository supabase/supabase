...
test.describe('Filter Bar', () => {
  ...
  test.describe('Keyboard Navigation - Freeform Input', () => {
    ...
    test('Tab navigates to next filter highlight', async ({ page, ref }) => {
      const tableName = `${tableNamePrefix}_tab_nav`
      ...
      try {
        ...
        // Highlight first filter using ArrowLeft
        const freeformInput = getFilterBarInput(page)
        await freeformInput.click()
        await page.keyboard.press('ArrowLeft')
        await page.keyboard.press('ArrowLeft')

        // First filter should be highlighted
        await expect(page.getByTestId('filter-condition-first_name')).toHaveAttribute(
          'data-highlighted',
          'true'
        )

        // Tab should move highlight to next filter
        await page.keyboard.press('Tab')

        await expect(page.getByTestId('filter-condition-first_name')).not.toHaveAttribute(
          'data-highlighted',
          'true'
        )
        await expect(page.getByTestId('filter-condition-last_name')).toHaveAttribute(
          'data-highlighted',
          'true'
        )
      } finally {
  ...
  test.describe('Keyboard Navigation - Dropdown Values', () => {
    ...
    test('Shift+Tab navigates to previous filter highlight', async ({ page, ref }) => {
      const tableName = `${tableNamePrefix}_shifttab_nav`
      ...
      try {
        ...
        // Highlight last filter using ArrowLeft
        const freeformInput = getFilterBarInput(page)
        await freeformInput.click()
        await page.keyboard.press('ArrowLeft')

        // Last filter should be highlighted
        await expect(page.getByTestId('filter-condition-last_name')).toHaveAttribute(
          'data-highlighted',
          'true'
        )

        // Shift+Tab should move highlight to previous filter
        await page.keyboard.press('Shift+Tab')

        await expect(page.getByTestId('filter-condition-last_name')).not.toHaveAttribute(
          'data-highlighted',
          'true'
        )
        await expect(page.getByTestId('filter-condition-first_name')).toHaveAttribute(
