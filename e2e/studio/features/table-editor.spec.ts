...
testRunner('table editor', () => {
  ...
  test('referencing record peek renders boolean values the same way as the main grid', async ({
    page,
    ref,
  }) => {
    const suffix = `${test.info().parallelIndex}_${test.info().repeatEachIndex}`
    const targetTable = `pw_fk_peek_bool_target_${suffix}`
    const sourceTable = `pw_fk_peek_bool_source_${suffix}`
    const popoverTitle = `Referencing record from public.${targetTable}:`
    ...
    await using _ = await withSetupCleanup(
      async () => {
        ...
        await query(`ALTER TABLE public.${targetTable} ENABLE ROW LEVEL SECURITY`)
        await query(`
          INSERT INTO ${targetTable} (name, is_active)
          VALUES ('target_true', true), ('target_false', false)
        `)
        ...
        await query(`ALTER TABLE public.${sourceTable} ENABLE ROW LEVEL SECURITY`)
        await query(`
          INSERT INTO ${sourceTable} (label, target_id)
          VALUES ('source_true', 1), ('source_false', 2)
        `)
      },
      async () => {
        await dropTable(sourceTable)
    ...
    await page.goto(toUrl(`/project/${ref}/editor?schema=public`))
    await page.getByRole('button', { name: `View ${sourceTable}`, exact: true }).click()
    await expect(page.getByRole('gridcell', { name: 'source_true' })).toBeVisible()
    await expect(page.getByRole('gridcell', { name: 'source_false' })).toBeVisible()

    const expectReferencingRecordPeek = async (
      sourceLabel: string,
      targetName: string,
      booleanValue: 'TRUE' | 'FALSE'
    ) => {
      const row = page.getByRole('row').filter({
        has: page.getByRole('gridcell', { name: sourceLabel, exact: true }),
      })

      await row.getByRole('button', { name: 'View referencing record' }).click()

      const popoverContent = page.locator('[data-radix-popper-content-wrapper]').last()
      await expect(popoverContent.getByText(popoverTitle)).toBeVisible()
      await expect(popoverContent.getByRole('gridcell', { name: targetName, exact: true })).toBeVisible()
      await expect(
        popoverContent.getByRole('gridcell', { name: booleanValue, exact: true })
      ).toBeVisible()
    ...
    await expectReferencingRecordPeek('source_true', 'target_true', 'TRUE')
    await expectReferencingRecordPeek('source_false', 'target_false', 'FALSE')
  })
})
