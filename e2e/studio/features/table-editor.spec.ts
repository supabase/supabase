...
const TABLE_NAME = 'pw-test-table'
...
test.describe('Table Editor', () => {
  ...
  test.beforeEach(async ({ browser, ref }) => {
    ...
    try {
      await deleteTestTable(page, TABLE_NAME)
    } catch (e) {
  ...
  test.afterEach(async () => {
    // Try to clean up the test table after each test
    try {
      await deleteTestTable(page, TABLE_NAME)
    } catch (e) {
      // Ignore if table does not exist
  ...
  test('should create a new table', async () => {
    await createTestTable(page, TABLE_NAME)
