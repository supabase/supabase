...
async function main() {
  ...
  // pnpm's `--` separator (from `pnpm run ... -- --workers=6`) lands after
  // whatever the package script already passed, so its position varies. A bare
  // `--` means nothing to `playwright test` — left in, it's read as a test-file
  // filter and the run exits with "No tests found".
