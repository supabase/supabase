...
| **Snapshots (2a)** | Two surfaces, deliberately kept in parallel to compare: (1) top-level cross-bucket **Snapshots** tab at `/storage/files/snapshots`, (2) per-bucket **Snapshots** tab inside a bucket view at `/storage/files/buckets/[bucketId]?view=snapshots` | `Snapshots/Snapshots.tsx` now takes an optional `bucketId` prop; when set it hides its bucket selector and locks to that bucket. `SnapshotsList.tsx`, `TakeSnapshotModal.tsx`, `RestoreSnapshotModal.tsx`; top-level page `pages/project/[ref]/storage/files/snapshots/index.tsx` (+ route); in-bucket view rendered from `pages/project/[ref]/storage/files/buckets/[bucketId].tsx` with a `NavMenu` (Files / Snapshots / Deleted files) driven by `?view=` nuqs state |
...
| **Deleted files (6a)** | Two surfaces, deliberately kept in parallel to compare: (1) top-level cross-bucket **Deleted files** tab at `/storage/files/trash`, (2) per-bucket **Deleted files** tab inside a bucket view at `/storage/files/buckets/[bucketId]?view=trash` | `Trash/Trash.tsx` now takes an optional `bucketId` prop; when set it hides its bucket selector and locks to that bucket. `TrashList.tsx`; top-level page `pages/project/[ref]/storage/files/trash/index.tsx` (+ route); in-bucket view rendered from the bucket page via the same `?view=` `NavMenu`. Internal naming ("Trash") kept for the component/route, only the displayed label changed |
