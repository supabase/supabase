...
Each room restricts the number of users authorized by applying RLS Policies to `public` schema tables you'll be creating and the auto-generated `realtime` schema tables.
...
And here one of the users does not have access because their RLS policies denied access:
![User denied access](./chat_unauthorized.png)
