...
- The declarative workflow requires the `pg-delta` diff engine, which is the default. Ensure `supabase/config.toml` does not opt out of it. `supabase init` writes the default explicitly:
  ```toml
  [experimental.pgdelta]
  enabled = true
  ```
- Do not set `enabled = false`. That switches the project to the legacy `migra` engine and disables the `db schema declarative` commands. If a project has already opted out, remove the opt-out, or pass the `--experimental` flag for a single command run
...
- If `config.toml` sets `[experimental.pgdelta] enabled = false`, the project uses the legacy `migra`-based workflow (`supabase stop`, then `supabase db diff -f <migration_name>`), which has additional limitations around view owners and grants, security invoker on views, `alter policy`, column privileges, comments, publication membership, roles, and domains. Remove the opt-out instead of following that flow
