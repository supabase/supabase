...
CREATE TABLE public.users (
  ...
  username text,
  list_id uuid NULL, -- can be selected by user - current list and updated
...
CREATE POLICY "Allow individual select access" on userlists FOR SELECT USING ( auth.uid() = users_id );
...
-- todo
-- try using count?
-- CREATE POLICY "Allow logged-in read access to shared list" on tasks FOR SELECT USING  ( select count(user_id) from (SELECT user_id from userlists where userlists.list_id=list_id ) > 0);
