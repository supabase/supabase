...
test.describe('Table Editor page', () => {
  ...
  test('should create a new table with one nullable field and one default field', async ({
    ...
    await expect(page.locator('div.view-lines')).toContainText(
      `CREATE  TABLE public.${tableName} (  id bigint GENERATED BY DEFAULT AS IDENTITY ,  created_at timestamp with time zone NULL DEFAULT now(),  \"defaultValueColumn\" smallint NULL DEFAULT '2'::smallint,  CONSTRAINT 7e55287894-0_pkey PRIMARY KEY (id)) TABLESPACE pg_default;`
    )
  })
