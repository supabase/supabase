      ...
      {!hideSidebar && !noContent && (
        <div
          ...
          className={[
            ...
            'hide-scrollbar w-64 overflow-auto border-r border-border',
          ].join(' ')}
        >
          {title && (
            <div className="mb-2">
              <div className="flex h-12 max-h-12 items-center border-b px-6 border-border">
...
const SectionWithHeaders = ({ section, subitems, subitemsParentKey }: SectionWithHeadersProps) => (
  <div key={section.heading} className="border-b py-5 px-6 border-border">
...
const SidebarLinkItem = ({
  ...
  return (
    <Link href={href || ''} className="block" target={isExternal ? '_blank' : '_self'}>
      <span className="group flex max-w-full cursor-pointer items-center space-x-2 border-border py-1 font-normal outline-none ring-foreground focus-visible:z-10 focus-visible:ring-1 group-hover:border-foreground-muted">
        {isExternal && (
