...
export const getRolesManagementPermissions = (
  ...
  availableRoles.forEach((role: string) => {
    ...
    const canAdd = checkPermissions(PermissionAction.CREATE, 'postgres.auth.subject_roles', {
      resource: { role_id: selectedRole!.id },
    })
    if (canAdd) rolesAddable.push(selectedRole.id)

    const canRemove = checkPermissions(PermissionAction.DELETE, 'postgres.auth.subject_roles', {
      resource: { role_id: selectedRole!.id },
    })
