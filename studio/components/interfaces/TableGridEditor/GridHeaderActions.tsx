...
import { Badge, Button, IconAlertCircle, IconCode, IconLock, Modal } from 'ui'
          ...
          <Link href={`/project/${projectRef}/auth/policies?search=${table.id}`}>
            <a>
              ...
              >
                {/* {!table.rls_enabled
                  ? 'RLS is not enabled'
                  : `${policies.length == 0 ? 'No' : policies.length} Auth polic${
                      policies.length > 1 || policies.length == 0 ? 'ies' : 'y'
                    }`} */}

                <div className="flex items-center gap-1">
                  <span>Auth Policies</span>({policies.length})
                </div>
